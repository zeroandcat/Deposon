...
| auth | `ark-[REDACTED]...e219` (runtime env, 永不入 prompt/JSON/disk) | 同 V6 |
...
- **key 永不入 prompt / JSON / disk**(`auth` 字段只截断 `ark-[REDACTED]...`)
...
| 4 | key 永不入 prompt/JSON/disk | ✅ 0 LLM 0 key; V2 阶段 1-3 auth 字段已 masked `ark-[REDACTED]...e219` |
