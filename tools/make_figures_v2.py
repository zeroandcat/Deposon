...
输出：figures/fig1_boundary_map_cn.png … fig5_poa_distribution_cn.png（300 dpi）
...
def fig1():
    ...
    ax.text((n_L - 1) / 2, 1.13, "族 L（真实语义图，先验臂主报区）",
            ha="center", fontsize=10, color=C_L)
    ax.text(n_L + (len(gids) - n_L - 1) / 2, 1.13,
            "族 S（合成结构图，先验臂不可得——预登记口径）",
            ha="center", fontsize=10, color=C_S)

    ax.bar(x - w / 2, fm, w, color=C_FIELD, label="场 field_mean", zorder=3)
    prior_x, prior_y = [], []
    ...
    ax.bar(prior_x, prior_y, w, color=C_PRIOR,
           label="先验 llm_prior（仅族 L）", zorder=3)
    ax.scatter(x, [named[g]["random"] for g in gids], marker="v", s=34,
               color=C_RANDOM, label="random（参考）", zorder=4)
    ax.scatter(x, [named[g]["degree"] for g in gids], marker="^", s=34,
               color=C_DEGREE, label="degree（参考）", zorder=4)

    # 胜负标记：每图在 {场, 先验?, random, degree} 中的第一名
    arms_of = lambda g: {"场": named[g]["field_mean"],
                         "random": named[g]["random"],
                         "degree": named[g]["degree"],
                         **({"先验": prior_named[g]} if g in prior_named else {})}
    for i, g in enumerate(gids):
        a = arms_of(g)
    ...
    ax.set_title("图 1：分工边界总览——22 图场-先验胜负地图（named Hits@3）",
                 fontsize=13)
    fig.tight_layout()
    fig.savefig(OUT / "fig1_boundary_map_cn.png", dpi=300)
    plt.close(fig)
...
def fig2():
    ...
    ax.scatter([], [], marker="o", s=72, color=C_S, label="族 S（合成结构图）")
    ax.scatter([], [], marker="s", s=72, color=C_L, label="族 L（真实语义图）")
    ax.scatter([], [], marker="o", s=90, facecolor="none",
               edgecolor="#B03030", linewidth=1.8,
               label=f"反转图（field<random，{len(rev)} 张）")
    ax.text(0.03, 0.97,
            f"符号检验：{st['n_pos']}+ / {st['n_neg']}- / {st['n_tie']} 平\n"
            f"p_exact = {st['p_exact']:.4f}（Holm 过）\n"
            f"反转 ≥3 张 ⇒ H_A_dead.triggered = true（H-A1 判死）",
            transform=ax.transAxes, va="top", fontsize=10,
            bbox=dict(boxstyle="round,pad=0.4", fc="#F7F4EF", ec="#CCCCCC"))
    ax.set_xlabel("random 臂 named Hits@3")
    ax.set_ylabel("场 field_mean 臂 named Hits@3")
    ax.set_xlim(-0.01, lim)
    ax.set_ylim(-0.01, lim)
    ax.set_aspect("equal")
    ax.grid(color="#E8E8E8", lw=0.6, zorder=0)
    ax.legend(loc="lower right", fontsize=9, framealpha=0.9)
    ax.set_title("图 2：H-A1 斩杀线与反转图分布（22 图符号检验散点）",
                 fontsize=13)
    fig.tight_layout()
    fig.savefig(OUT / "fig2_killsign_scatter_cn.png", dpi=300)
    plt.close(fig)
...
def fig3():
    ...
    co = reg["coefficients"]
    ax.text(0.985, 0.03,
            f"探索性回归 field_named ~ density + hub + sem（n={reg['n_observations']}）\n"
            f"R²={reg['r_squared']:.3f}；"
            f"β_hub=+{co['hub_concentration']['coefficient']:.2f}"
            f"（p={co['hub_concentration']['p_value']:.2e}）；"
            f"β_sem={co['real_semantics']['coefficient']:.2f}"
            f"（p={co['real_semantics']['p_value']:.4f}）",
            transform=ax.transAxes, va="bottom", ha="right", fontsize=9,
            bbox=dict(boxstyle="round,pad=0.4", fc="#F7F4EF", ec="#CCCCCC"))
    ax.set_yticks([0, 1])
    ax.set_yticklabels(["0（合成结构）", "1（真实语义）"])
    ax.set_ylim(-0.28, 1.28)
    ax.set_xlabel("hub_concentration（枢纽集中度）")
    ...
    ax.set_title("图 3：分工规律散点（hub_concentration × real_semantics，"
                 "着色 field_named）", fontsize=13)
    fig.tight_layout()
    fig.savefig(OUT / "fig3_division_scatter_cn.png", dpi=300)
    plt.close(fig)
...
def fig4():
    ...
    for ax, g in zip(axes.flat, graphs):
        ...
        ax.errorbar(a, h, yerr=hs, marker="o", ms=5, lw=1.6, color=C_FIELD,
                    capsize=3, label="命中率（dirichlet，5 seed 均值±std）",
                    zorder=3)
        ax.axhline(mf["hits"], ls="--", color=C_FIELD, lw=1.1,
                   label="mean-field 命中率（T=0）")
        ...
        ax2.errorbar(a, p, yerr=ps, marker="s", ms=5, lw=1.6, color=C_PHI,
                     capsize=3, label="终点 Φ（均值±std）", zorder=3)
        ax2.axhline(mf["phi"], ls=":", color=C_PHI, lw=1.1,
                    label="mean-field Φ（T=0）")
        ax2.set_ylabel("终点 Φ", color=C_PHI)
        ...
        ft = d["frontier_temps"]
        ax.set_title(f"{g}（前沿温度档 {len(ft)} 个"
                     f"{'：α=' + '、'.join(ft) if ft else ''}）", fontsize=10.5)
        ax.grid(color="#ECECEC", lw=0.6, zorder=0)
        ax.set_xlabel("dirichlet 浓度 α（等效温度：α→0 高温，α→∞ 低温）")
    handles, labels = axes.flat[0].get_legend_handles_labels()
    fig.legend(handles + [plt.Line2D([], [], marker="s", color=C_PHI, lw=1.6),
                          plt.Line2D([], [], ls=":", color=C_PHI, lw=1.1)],
               labels + ["终点 Φ（均值±std）", "mean-field Φ（T=0）"],
               loc="lower center", ncol=3, fontsize=9, frameon=False,
               bbox_to_anchor=(0.5, -0.005))
    fig.suptitle("图 4：GT-7 温度前沿逐图形态（命中率与 Φ 双轴；"
                 "verdict = mixed）", fontsize=13)
    fig.tight_layout(rect=(0, 0.045, 1, 1))
    fig.savefig(OUT / "fig4_gt7_frontier_cn.png", dpi=300)
    plt.close(fig)
...
def fig5():
    ...
    ax.text(15.5, fv["pass_threshold"] - 0.18,
            f"预登记通过线 {fv['pass_threshold']}", ha="center", fontsize=9,
            color="#555555")
    ax.axhline(fv["median_poa"], ls="-.", color="#8C7B3C", lw=1.2)
    ax.text(15.5, fv["median_poa"] + 0.05,
            f"冻结 17 有限值图中位 {fv['median_poa']:.3f}", ha="center",
            fontsize=9, color="#8C7B3C")
    ax.set_xticks(range(len(bars)))
    ax.set_xticklabels([g for g, _, _ in bars], rotation=55, ha="right",
                       fontsize=8.5)
    ax.set_ylabel("ECR = field_mean / max(random, degree)（named Hits@3；分布级口径，非经典 worst-case PoA）")
    ...
    ax.legend(handles, [
        "族 S（冻结 GT-4，17 有限值图之一）",
        "族 L（冻结 GT-4）",
        "红框：ECR<1（负协调，族 L 两图在冻结集）",
        f"ECR=∞（自利臂 named=0 而场>0，{fv['n_poa_inf']} 张单独计数）",
        "点纹：冻结外 2 张族 L 扩展图（同预登记公式现算，不计入中位数）",
    ], loc="upper right", fontsize=8.5, framealpha=0.92)
    ax.set_title("图 5：分布级协调比（ECR）分布——全 22 图条形（含 ECR<1 图与 ∞ 计数；非经典 worst-case PoA）",
                 fontsize=13)
    fig.tight_layout()
    fig.savefig(OUT / "fig5_poa_distribution_cn.png", dpi=300)
    plt.close(fig)
