...
### 4.6 题库效度：field_mean 52.5% 的口径

题库横向验证（deposon_v20_quiz_eval.json，40 题）：llm_prior 92.5%、field_mean
...
反向对应无噪声最好响应动态；物理能量的负值 Φ=−E（E 见 §3.1）是势函数候选。
「每个任务是独立玩家」的分解是建模选择而非唯一；Φ=−E 有解析依据（反向退火梯度
即 −∇E）。Sandholm（2010）群体最好响应流为该对应提供最近理论支点。

### 5.2 证据矩阵（六条，按判定强度三档标注）

判定列分三档：✅ 闭合（预登记判定通过）、🟡 受限一致（方向一致但有口径限定）、
❌ 弱一致或未决（鉴别力低或 inconclusive）。
...
1993）和确定性最好响应的经典分工一致；散射层的「温度」（噪声强度）由此获得博弈
...
信息量的非势分量。本分解是 Candogan 流分解的可操作类比（分解边效用向量，未沿
轨迹积分）。

### 5.4 审计的定量化：PoA 的口径限定

PoA 只作分布级报告：操作化定义 PoA=field_mean/max(自利臂)，自利臂集 {random,
degree}（预登记的 llm_prior 臂在族 S 不可得，分母只可能更小、PoA 只可能偏大，
...
仿射/可分前提未闭合；子集中位数与 Pigou 4/3 的数值接近仅为巧合，不主张实例级
...
本节限定的是划界推论（§4.5）的成立范围；主线结论（§5 的审计标量存在性与定量化）
的口径限定已在各节随行给出。场的成立域在 H-A1 判死后收缩为「相对平凡结构基线的稳健优势 + 高枢纽图局部优势」。
...
分工规律（§4.5）的适用范围需逐项限定：回归为 n=20 探索性且存在特征-设计循环性；
...
裁定、修订编年）集中收于本节与 paper/v2/REVISION_LOG_v2X.md（过程工件，非论文正文），正文叙事只陈述最终口径。
...
- **独立多角色评审与裁定史**：M1–M5 主线裁定与 m1–m7 细则闭合于
  ...
  退化口径的确立，§4.6/§4.7 的最终口径即由此而来）；20→22 图快照漂移归档于
  corrections E1。
...
20. Tramèr et al., NeurIPS 2020（自适应攻击）；21. Nasr et al., USENIX Sec
   [待核：年份需核实，应为 2025 或 in-press]；
