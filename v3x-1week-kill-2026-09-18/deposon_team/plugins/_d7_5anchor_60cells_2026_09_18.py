...
def verify_5_anchor_p_g_v01_5_anchors():
    """P-G V0.1 5 锚 SHA-12 实算(沿 P-G V0.1 worker 已落)"""
    p_g_v01_5_anchors = [
    ...
    results = []
    for name, expected in p_g_v01_5_anchors:
        # 沿 P-G V0 spec §2 预注册算法:
        # SHA-256(f'P_G_V0_PLACEHOLDER_{name}_2026_09_15')[0:12]
        seed = f'P_G_V0_PLACEHOLDER_{name}_2026_09_15'.encode()
        actual = hashlib.sha256(seed).hexdigest()[:12]
        if actual == expected:
            results.append(f'PASS: {name} SHA-12 = {actual}')
        else:
            results.append(f'FAIL: {name} SHA-12 = {actual} (expected {expected})')
    return results
