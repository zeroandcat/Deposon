...
| 鉴权 | `ark-de0b484e-...` (GB18030 读 LLM API.txt → runtime env,**永不入 prompt/JSON/disk**) |
| Proxy | **不设**(国内直连) |
...
| # | 铁律 | D 路径状态 | 证据 |
|---|---|---|---|
| 1 | key 从 GB18030 读 + env,不入 prompt/JSON/disk | ✅ | `auth: "ark-de0b484e-0889-46...e219"` 截断,JSON `auth` 字段已掩码;`ARK_CODING_PLAN_KEY` 仅 runtime env |
...
| 4 | key 永不入 prompt / JSON / disk | ✅ | JSON `auth` 字段截断 `ark-de0b484e-0889-46...e219`(只显示前 20 + 后 4);log 只显示截断版 |
