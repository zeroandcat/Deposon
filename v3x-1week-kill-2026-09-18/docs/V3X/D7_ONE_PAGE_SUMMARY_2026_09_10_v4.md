# V3.X 一周判死 D7 交付 (v4 版,2026-09-10)

致:王老师
自:Deposon 项目组(Mavis 执行线)

## 1. 4 KT 判死状态 + V3 v4 LLM 30 cells
...
## 2. 一周总览(V3 v4 vs V2 关键升级)

**4 PASS + 1 死 + 1 引用 PASS**,主线(KT-A1 机制侧预算 / KT-B1 守恒审计 / KT-D0 账指纹协议 / V3 v4 LLM 30 cells)成立;KT-C1 幂律死,主张终极降级为"两相结构存在但非独立标度律, 落入 2D Ising 普适类特例", 死线作为观察性证据归档。V3 v4 报告相对 V2 的 2 个升级: (1) V0.2 复审将 KT-B1 "0.0% 攻击者成功率" 升级为更精确的 **22.5% 主跑 / 21.3% reviewer-b 独立**(仍 < 50% PASS);(2) 🆕 V3 v4 30 cells LLM 边际验证从 V3 v3 的 15/30 FAIL 升级为 **24/30 = 80% PASS**(严格字面 V4.1-Flash 在 OpenRouter 真实可用, user PDF 信息准确, `max_tokens=1024` + `reasoning={max_tokens:0, exclude:true}` 把截断从 15 cells 降到 3 cells)。6 cells 失败归因: 3 截断(gsm8k_7/12 + strategyqa_9, max_tokens=1024 仍满)+ 3 语义判错(strategyqa_7/10/14, StrategyQA 反问/resemble/hypothetically 陷阱)。BPA 协议先导(激励相容 / 效用内生罚金骨架, 2 机制 × 4 任务族)作为附赠臂一并交付, 不阻塞主线(沿用 v3 提案 §7 "挂不上也是交付")。
...
- **V3 v4 30 cells 交付**:`docs/V3X/DEEPSEEK_V41_FLASH_30CELLS_V2_2026_09_10.md` (9.2KB) + `results/deposon_deepseek_v41_flash_30cells_v2_2026_09_10.json` (17.5KB)
...
- **完整 V3 v4 综合判死报告**:`docs/V3X/V3X_D7_V3_FINAL_REPORT_2026_09_10_v4.md` (25.3KB)

## 4. 后续选择(待您拍板)

- **路径 A**:4 PASS(KT-A1/B1/D0 + V3 v4 LLM 30 cells)进入 Phase 1 挂点深耕(2-3 月, 每月 5-10 分钟, 优先 KT-A1 博弈论转向 + 您 AAAI 2026 对接)
- **路径 B**:KT-C1 死线降级主张归档, 活线进入 Phase 1, KT-C1 作为"2D Ising 普适类特例"观察性证据保留
- **路径 C**:调方向到 P-F(新) IMMACULATE 风格可验证审计(差异化机会, 风险高, Phase 0 重启)
- **路径 D(V3 v4 新增)**:30 cells LLM 边际验证已 PASS; 可选 `max_tokens=2048` 重跑把 3 截断 cells 修到 27/30(不阻塞主线, 独立可执行, 3-4 min / ~$0.02)
- **路径 E(V3 v4 新增)**:V1 折中补收尾完整 → V2 真实 2 周工作量启动(1 周判死 → 6 月单论文的下一步, 等您拍板)
...
4. **双审 V0 草稿期**:本任务基于 V2 综合三版 + V0.2 复审 + V3 v4 30 cells 三源独立完成 ✓
5. **verifier 纪律**:每条数字均可在 frozen JSON + 5 锚 SHA-12 中重跑(H1=0.4350 / 22.5% 攻击 / R²=0.0007 / b CI / 24/30 = 80% / 根指纹 / SHA-12)✓
...
- 不复述实验细节(留给完整报告 `V3X_D7_V3_FINAL_REPORT_2026_09_10_v4`)✓
- 不在 D7 之外主动发摘要 ✓
- 不提 BOSS 测法技术细节(无 RBR/RM/Sinkhorn/β=1/8 等技术参数)✓
- 不提 V3 v4 LLM 30 cells 技术细节(无 `max_tokens=1024` / `reasoning.exclude` / `finish_reason` 等参数)✓
