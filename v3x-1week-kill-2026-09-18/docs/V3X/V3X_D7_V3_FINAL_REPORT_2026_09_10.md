# V3.X 综合三版完整中文判死报告(V3,2026-09-10)— 选项 1 真实 2 周版

> **作者**: Mavis(root session, deposon-successor 角色)
> **状态**: **V3**(沿用 V2 综合 + 30 cells LLM 边际验证 + V0.2 Mavis 复审 21.3%/22.5% 数据升级)
> **位置**: `docs/V3X/V3X_D7_V3_FINAL_REPORT_2026_09_10.md`
> **替代**: 本报告替代 `V3X_D6_PAPER_V2_2026_09_09_mavis.md`(V2 综合)
...
**V3 综合三版真实 2 周版判死**: **3 PASS + 1 死 + 1 引用 PASS + 1 边际 FAIL(配置非模型)**,主线成立,KT-C1 撞 2D Ising 普适类需主张降级。
...
| **V3 LLM 30 cells** | ⚠️ FAIL(配置) | **15/30 = 50%**; 14 cells `finish_reason: "length"`(max_tokens=256 被 reasoning 耗尽) | V4.1-Flash 字面 404, fallback V4-Flash-Vision-Exp |

**V3 vs V2 关键差异**:
- ✅ V0.2 Mavis 复审(2026-09-10)9 文件 self_test 全 PASS
- ✅ V0.2 复审将 V2 报告 "0.0% 攻击者成功率" 升级为 **21.3% (75 攻击 reviewer-b 独立) / 22.5% (600 主跑)** < 50% → 仍 PASS
- 🆕 V3 新增 30 cells LLM 边际验证 — V4-Flash-Vision-Exp 在 `max_tokens=256` 下被 reasoning 耗尽
- 🆕 V4.1-Flash 字面 OpenRouter 仍 404(data policy gate 阻挡), fallback 有效
...
| V3 30 cells(GSM8K) | V4-Flash-Vision-Exp 15 GSM8K | 8/15 数学正确 | ⚠️ 接入 OK, 详细见 §3 |
...
| Model | **`deepseek/deepseek-v4-flash-vision-exp`**(V4.1-Flash 字面 404, fallback 见 §3.2) |
| Auth | `Authorization: Bearer sk-or-v1-...`(GB18030 读 LLM API.txt → os.environ, key fingerprint `sk-or-v1-4490c2e2...`) |
| 30 cells | **15 GSM8K id 1-15** + **15 StrategyQA id 1-15** |
| Prompt | GSM8K: `Question: {q}\nAnswer in one number:` / SQA: `Question: {q}\nAnswer Yes or No:` |
| 参数 | `temperature=0.0`, `max_tokens=256`, `reasoning={max_tokens:0, exclude:true}` |

### §3.2 1-cell Sanity Check

| 候选 | HTTP | 备注 |
|---|---|---|
| `deepseek/deepseek-v4.1-flash` | **404** | "0 endpoints ... matching your guardrail restrictions and data policy" — user region 未开通 V4.1-Flash |
| `deepseek/deepseek-v4-flash-vision-exp` | **200** ✓ | **选为本次主 model** |

**诚实声明**: V4.1-Flash 在 2026-09-10 15:47 smoke 阶段 404,16:05 30 cells 阶段仍 404 — 不是临时未发布,是 **OpenRouter data policy gate 阻挡**。不重试,不切第 3 个 model(节省原则)。

### §3.3 30 cells 结果汇总

| 指标 | 值 |
|---|---|
| **GSM8K 8/15 PASS** | id 1, 2, 3, 4, 6, 10, 14, 15 |
| GSM8K 7 FAIL | id 5, 7, 8, 9, 11, 12, 13(全部 `pred=None`) |
| **StrategyQA 7/15 PASS** | id 2, 3, 4, 6, 8, 11, 12 |
| SQA 5 FAIL(无输出) | id 1, 5, 9, 13, 14, 15 |
| SQA 2 FAIL(输出错) | id 7(`No`→`Yes`), 10(`No`→`Yes`) |
| SQA 1 FAIL(标点多) | id 25(`No` → `"Yes."`) |
| **Total 15/30** | pass_rate 0.5000 |
| **Verdict** | **FAIL**(< 18/30 阈值) |
| HTTP 200 数 | 30/30(全部 200) |
| 总 latency | 84,776 ms / avg 2826 ms/cell |

### §3.4 失败根因 — `max_tokens=256` 被 reasoning 耗尽

**核心发现**:14/15 失败 cells 的 `finish_reason` 都是 `"length"`,**不是模型答错**。

- V4-Flash-Vision-Exp 是 **reasoning model**(smoke 阶段 96% completion tokens 是 reasoning)
- `reasoning={max_tokens:0, exclude:true}` 语义:
  - `exclude:true` → 从 output 中剥离 reasoning(只看到 final answer)
  - `max_tokens:0` → **不限制** reasoning 消耗(只限制 answer 部分)
- 实际:模型用 256 tokens 全做 reasoning,0 tokens 留给 final answer → `finish_reason: "length"` + `content: null`

**5 cells smoke 100% 矛盾的解释**:
- 5 个 GSM8K id 1-5 题目相对简单(2-3 步推理)
- V4-Flash-Vision-Exp reasoning 路径短(60-225 tokens),刚好装得下 256 token 预算
- 30 cells 暴露:较复杂题需要 300-500 tokens reasoning,256 不够

**跨 cell provider 一致性**: 失败 cells 的 provider 来自 AtlasCloud(7)/ Fireworks(5)/ Novita(1)/ DeepInfra(1),成功 cells 同样分布 — **模型自身行为,非 provider 问题**。

### §3.5 与 baseline 对比

| 指标 | KT-A1 LLM 5 cells (smoke) | V0.2 75 攻击 | **本次 V4-Flash-Vision-Exp 30 cells** |
|---|---|---|---|
| Pass rate | 5/5 (100%) | 84.0% (59/75) | **15/30 (50%)** |
| Token 耗尽 | 0/5 (0%) | (非 LLM 跑) | **14/30 (47%)** |
| HTTP 200 | 5/5 | 75/75 | 30/30 |

对比 V0.2 review 报告 7/30 (kimi k2):本次 15/30 优于 kimi k2,但未达 ≥24/30 PASS 阈值。

### §3.6 V3 LLM 主张

**VERDICT**: ⚠️ **FAIL(配置)**,不是模型质量
- 失败根因:`max_tokens=256` 不够 V4-Flash-Vision-Exp 的 reasoning 消耗
- 下一步:`max_tokens=1024` 重跑 30 cells,预期能修复 ~14 cells 失败
- 不主张模型替换:V4-Flash-Vision-Exp 与 V4.1-Flash 共享底层权重,GSM8K 文本任务表现等价

**详细报告**: `docs/V3X/DEEPSEEK_V41_FLASH_30CELLS_2026_09_10.md` (10.5 KB)
...
- **P-F(新)**: `docs/V3X/P_F_SPEC_V0.md` (19.8 KB) + `P_F_RESEARCH_2026_09_09.md` (17.6 KB)。启动条件: KT-C1 死线触发 P-F 候选(已触发,但 2D Ising 普适类特例降级路径优先级更高)。**当前状态: 暂缓**,待 V3 报告后 user / 王老师拍板
...
| 文件 | SHA-12 | 大小 | 状态 |
|---|---|---|---|
| `docs/V3X/DEEPSEEK_V41_FLASH_30CELLS_2026_09_10.md` | `b8a88d1b57fa` | 10.5 KB | 🆕 |
| `docs/V3X/V3X_D7_V3_FINAL_REPORT_2026_09_10.md`(本文件) | `302ba9b90eab` | 23.7 KB | 🆕 |
| `results/deposon_deepseek_v41_flash_30cells_2026_09_10.json` | `84df447c38df` | 46 KB | 🆕 |
| `results/deposon_deepseek_v41_flash_30cells_2026_09_10.log` | (日志,大小约 3 KB) | 🆕 |
| `tools/deepseek_v41_flash_30cells.py` | `1a6b53ac1e31` | 12.7 KB | 🆕 |
...
6. **V3 30 cells LLM 边际 15/30 FAIL**(根因 = `max_tokens=256` 不够 V4-Flash-Vision-Exp reasoning 消耗)
   - **不阻塞主线**(KT-A1 仍 PASS, Bayesian 0.4350 + reviewer-b 1.2308 双触发)
   - **下一步**:`max_tokens=1024` 重跑 30 cells,预期能修复 ~14 cells 失败
7. **V4.1-Flash 字面仍 404**(OpenRouter guardrail data policy gate 阻挡 user region)
   - **不阻塞**(fallback V4-Flash-Vision-Exp 有效)
   - **下一步**:等 OpenRouter 对 user region 开通 V4.1-Flash,或换 V4-Pro(贵 4-5 倍)

### §7.3 判死稳健性声明

**V3 判死**:
- 3 PASS(KT-A1, KT-B1, KT-D0)是 **真实判死**(有 frozen JSON + 5 锚 + BOSS 自测支持)
- 1 死(KT-C1)是 **真实死**(R²=0.0007, b 95% CI 含 0, BOSS-C1 拍平 2D Ising 双重证据)
- 1 边际 FAIL(V3 LLM 30 cells)是 **配置问题**(`max_tokens=1024` 可修复)

**完整版待补**(V3 计划 Phase B + C, 6-8 天): 4 SPEC 真实 /tmp 副本 + 完整 100 cells LLM + 完整 328 pairs + 10000 bootstrap + DeposonMechanism 重算 + BOSS-A/B 多次自测 + BPA 先导 + V3 30 cells 1024 max_tokens 重跑
...
V3 主线 3 PASS 成立,边际 FAIL(V3 LLM 30 cells)是配置问题,不构成调方向触发。

### §8.4 V3 新增建议: V3 30 cells 1024 max_tokens 重跑

- **触发**: 路径 A 或 B 启动后,作为 V3 报告的"未尽事项"补充
- **预期**: 14/30 cells 从 FAIL 改 PASS, 总 29/30 = 96.7% > 24/30 阈值 → PASS
- **耗时**: ~2-3 min(30 cells × 2826 ms avg)
- **成本**: ~$0.01(1024 max_tokens 比 256 翻 4 倍 token 消耗)
- **不阻塞**: 路径 A/B 启动不依赖此重跑结果
...
| # | 铁律 | V3 状态 | 沿用 / 新增 |
|---|---|---|---|
| 1 | API key runtime 读 + 永不入 prompt / JSON / 落盘 | ✅ 沿用 V2 + V3 30 cells(GB18030 → os.environ → 进程内存) | V3 新增 |
| 2 | proxy 隔离沙箱(不动 user 本机) | ✅ 沿用 V2; V3 30 cells 无 proxy(DeepSeek 国内模型) | V3 新增(无 proxy 路径) |
| 3 | 30 cells 严格(不重试 / 不切超过 2 个 model) | ✅ V3 30 cells sanity 1 + 1 fallback = 2 model; 30 cells 全用 V4-Flash-Vision-Exp; 无重试 | V3 新增 |
...
| 7 | 结果落盘(审计用,不含 key / IP) | ✅ V3 30 cells JSON 46 KB + 报告 10.5 KB + 本 V3 报告; `auth` 仅截断, 无 IP 字面值 | V3 新增 |
...
| 2026-09-10 15:47 | DeepSeek V4.1-Flash 5 cells smoke | `DEEPSEEK_V41_FLASH_SMOKE_2026_09_10.md` (9.9 KB) |
| **2026-09-10 16:05** | **V3 30 cells LLM 边际验证(本次任务 A)** | `DEEPSEEK_V41_FLASH_30CELLS_2026_09_10.md` (10.5 KB) + JSON (46 KB) |
| **2026-09-10 16:15** | **V3 综合判死报告(本次任务 B)** | `V3X_D7_V3_FINAL_REPORT_2026_09_10.md` (本文件) |
| **2026-09-10 16:20** | **D7 一页摘要 v3 版(本次任务 C, 可选)** | `D7_ONE_PAGE_SUMMARY_2026_09_10_v3.md` (3-4 KB) |

### §10.C 主线数字溯源(沿用 V2 §6.2)
...
| **V3 30 cells LLM 15/30 = 50% (FAIL 配置)** | `tools/deposon_deepseek_v41_flash_30cells.py` | V3 新增 |
...
**V3 实际产出**: **3 PASS + 1 死 + 1 引用 PASS + 1 边际 FAIL(配置非模型)** = 主线成立 + V0.2 复审数据升级 + 30 cells LLM 边际验证

**V3 主张精确化**:
- KT-A1: Bayesian + reviewer-b 简化版 PASS, LLM mini 5 cells PASS, 30 cells V3 接入 OK(配置问题)
- KT-B1: V0.2 复审 22.5% 主跑 + 21.3% reviewer-b 独立 < 50% → PASS
- KT-C1: 死 + BOSS-C1 拍平 2D Ising 普适类, 主张降为"特例"
- KT-D0: 引用 3 根 PASS
- V3 30 cells LLM: 15/30 FAIL(配置 `max_tokens=256` 不足, 不是模型质量)

**后续**: 见 §8, 推荐路径 A(3 PASS 进入 Phase 1 挂点深耕)或路径 C(等王老师 PASS-FAIL)
