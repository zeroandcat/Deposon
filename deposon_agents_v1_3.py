...
class KimiLLMBackend:
    ...
    DECOMPOSE_PROMPT = """你是数学应用题语义分解器。把题目分解为严格JSON（不要任何额外文字、不要markdown围栏）。
...
只输出JSON对象本身。"""
    ...
    def __init__(self, api_key: Optional[str] = None,
                 ...
                 max_tokens: int = 4000, enable_cache: bool = True):
...
class DeposonField:
    def __init__(self, feature_dim: int = 64,
        ...
        # v1.3.2: 共轭映射 (M2) —— 散射输入E_photon的来源
    ...
    def spawn_from_graph(self, brain_graph: Dict[str, Any], mode: str = 'unified') -> None:
        ...
        if mode == 'v1_blocking':
            ...
            # 高耦合消融: 类型绑定与degree修正之后, 所有节点 g_couple x3, g_aether 不变
            for d in self.deposons.values():
                d.g_couple *= 3.0
    ...
    def process_path(self, path: List[str], path_energy: float = 1.0) -> Dict[str, Any]:
        ...
        for idx, node_id in enumerate(path):
            ...
            per_node.append({
                ...
                'delta': abs(node_energy - (deposon.resonance_energy if deposon else 0.0))
            })
...
class BenchmarkEvaluator:
    ...
    def _compute_answer_from_path(self, graph: Dict, path: Optional[List[str]]):
        ...
        def fold_chain(ops_seq):
            ...
            for c in ops_seq:
                op = c['op']
                ops_idx = [o for o in c.get('operands', []) if 1 <= o <= len(number_values)]
                if acc is None:
                    if len(ops_idx) >= 2:
