...
OUT_PATH = os.path.join(RESULTS, "deposon_v17_fixed_sampler.json")
...
def main():
    ...
    for ei, (u, v) in enumerate(edges):
        ...
        pool = [j for j in range(N) if j != u and j != v and adj_obs[u, j] == 0]  # fixed_sampler: 负池显式排除金边 v（legacy 采样器可能把 v 抽进 take，实际少一个独特负样本）
    ...
    out = {"experiment": "deposon_v17_fixed_sampler", "spec_version": "v1.7.1-fixed-sampler",
           "config": config_dict(cfg), "protocol": {
               "same_as_v16_except_sampler": "同图/同种子70000+ei/top-3；负池排除金边 v（fixed_sampler），仅新增评分时刻归一融合与阴性对照",
