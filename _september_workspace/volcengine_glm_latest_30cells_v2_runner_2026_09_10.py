...
# [Trae 2026-09-18 修复 P1-1 代理泄漏] 原版仅清环境变量, Windows 注册表代理仍可被 urllib 采用 →
#   显式安装空 ProxyHandler 强制直连 (与 _worker_openrouter_rag_run / D05 runner 同款修复)
...
def extract_number(s):
    if s is None:
        return None
    s = str(s).strip()
    # [Trae 2026-09-18 修复 P1-2 千分位缺陷] 原版 r'-?\d+(?:\.\d+)?' 不含逗号, "$1,430" 被拆为
    #   1 / 430 取末位得 430 (假阴性); 补逗号并剥除
...
import os as _os_sc
assert _os_sc.path.basename(__file__) == 'volcengine_glm_latest_30cells_v2_runner_2026_09_10.py', '文件名漂移: ' + __file__
with open(__file__, 'r', encoding='utf-8') as _f_sc:
    _src_sc = _f_sc.read()
assert 'TRAE_SELFCHECK_2026_09_16_V3' in _src_sc
print('volcengine_glm_latest_30cells_v2_runner_2026_09_10.py SELF-CHECK PASS')
