...
def all_rounds(batch):
    rounds = []
    for r in range(1, 10):
        fp = f'results/{PREFIX}{batch}_r{r}_result.json'
        if os.path.exists(fp):
            rounds.append((r, fp))
    return rounds

cells_def = {
    "teacher_kimi":    [("batch1", "teacher", 2, 6)],  # batch1 r2-r5 (r6=distill)
    ...
    "distill_kimi":    [("batch1", "distill", 6, 7), ("batch6", "distill", 1, 7)],
...
with open('.tmp/_l14v3_aggregated_10cells_v4.json', 'w', encoding='utf-8') as f:
    json.dump(cells_data, f, ensure_ascii=False, indent=2)
sz = os.path.getsize('.tmp/_l14v3_aggregated_10cells_v4.json')
h = hashlib.sha256(open('.tmp/_l14v3_aggregated_10cells_v4.json','rb').read()).hexdigest()[:12]
print(f"\nSaved .tmp/_l14v3_aggregated_10cells_v4.json ({sz} B, SHA-12={h})")