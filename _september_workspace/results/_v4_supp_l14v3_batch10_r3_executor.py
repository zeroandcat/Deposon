# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch10_r3_executor.py
=====================================

V4 L14+ batch 10 · round 3 (reask=2) -- minimax 蒸馏侧第三棒 (独立计数接力棒)

派工依据:
  - 沿 L14+ 派工单 2026-09-26 16:55「minimax 蒸馏侧 round 3 (reask=2) 接力棒」
  - 沿 batch10 r2 executor (SHA-12 BB601B4F5ECB) + r2 result (SHA-12 400EE4D21FC9) 口径续跑
...
本棒范围:
  ...
  2. batch=10 (minimax distill) + round=3 (reask=2)
     - 模型 = mimo `mimo-v2.6-pro` (沿 r2 沿 mapping §1.1 minimax 拍板)
     - 端点 = https://token-plan-cn.xiaomimimo.com/v1/chat/completions (沿 r2)
     - 不走代理 (沿 r2 + L7 + add_T1 §1.6.2)
     - **endpoint_migrated=true** (沿 r2)
     - **lineage_discontinuity=true** (沿 r2)
     - **teacher_v3_anchor = "minimax-m3 (火山方舟已下线)"** (沿 r2)
  3. distill 侧独立计数:
     - filter = prompt_id 前缀 `minimax_t01_distill_`
     - 沿 r2 累加: predecessor r1+r2 同前缀 44 条 quadruples = 各 caption 2/5 successful
     - 本棒 22 caption × 1 call 后预期各 caption 3/5 successful (接力棒)
     - 目标 ≥5 successful/caption (远期)
  4. side=distill + teacher_label=minimax + 派生 metadata (endpoint_migrated/lineage_discontinuity/proxy_used/teacher_v3_anchor) (沿 r2)
  5. cumulative 监控: 起点 = r2 cumulative.post (362/9/353/2.49%); 本棒后 worst 31/384=8.07%; best 9/384=2.34%
  6. 进度判定块 `minimax_distill_side_round3_progress` (沿 r2 + batch8 r4 `glm2_distill_side_round4_progress` 接力棒命名)

与 batch10 r2 executor diff:
  1. ROUND=2 -> ROUND=3; REASK_R2=1 -> REASK_R3=2 (第 3 call/caption on distill side)
  2. prompt_id 后缀 _r2 -> _r3
  3. predecessor 链改: r2_executor BB601B4F5ECB + r2_result 400EE4D21FC9 + mapping 98A779D61C1E (dispatch 字面)
  4. baseline 切换: r1+r2 quadruples distill 过滤后 = 44 calls / 44 successful / 0 empty (各 caption 各 2 successful)
  5. 增量计数 -> distill 侧各 caption 由 2/5 顺利补至 3/5 (接力棒)
  6. is_round_2=True -> is_round_3=True
  7. 累计监控起点: r2 cumulative.post {362/9/353/2.49%} -> 本棒后 worst 31/384=8.07% / best 9/384=2.34%
  8. 判定块重命名: minimax_distill_side_round2_progress -> minimax_distill_side_round3_progress (沿 r2 接力棒命名)
  9. counting_scope_correction 改: r3_baseline = 2 successful/caption (沿 r1+r2), r3_progress (接力棒描述)
 10. per_caption 新增 round3_target/round3_met 字段
...
产物:
  - 写入: results/_v4_supp_l14v3_batch10_r3_result.json
    (schema = v4_l14v3_n26/1 + batch=10 + round=3)
  - 不覆盖 batch10 r2 result 400EE4D21FC9 或前棒 result

边界:
  - 不动任何既有件 (含 batch10 r2 双件 + batch10 r1 双件 + batch6 r5 双件 + batch8 r5 双件 + batch5 r1-r5 minimax teacher + batch1-8 全部 + prereg 05B975A86989 + captions 6a2656878745 + mapping 98A779D61C1E)
  - 跑不完拆段报断点
...
# ============================================================
# 端点配置 (沿 r2; model 沿模型映射留痕件 §1.1 minimax 拍板 mimo-v2.6-pro; 无代理)
# ============================================================
...
# 预算 (沿 r2; 1500s 容 22 caption calls + 串行 ≥2.5s)
...
REASK_R3 = 2  # round 3 = 第 3 call per caption (reask_idx = 2) -- 接力
TARGET_SUCCESSFUL_PER_CAPTION = 5  # distill 侧最终目标 (字面 ≥5)
ROUND3_TARGET_PER_CAPTION = 3  # 本棒 round 3 目标 = 3 (接力棒, 远期 ≥5)

# ============================================================
# 计数口径 (沿 r2 distill 侧独立计数; minimax 蒸馏侧独立域)
# ============================================================
...
def call_chat_mimo(
    ...
    """mimo 端点 chat 调用 (无代理; 沿 r2)."""
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
        "HTTP-Referer": "https://deposon.local/l14v3-batch10-r3",
        "X-Title": "deposon-l14v3-batch10-r3-minimax-distill",
    }
    body = {
        "model": MODEL_MINIMAX,
        "messages": messages,
...
# ============================================================
# 提示构造 (沿 r2 prompt 构造口径 = batch1 r6 reproduction_caliber anchor)
# ============================================================
PROMPT_SYSTEM = (
...
# ============================================================
# 单 call + 内层重试 (沿 r2; minimax 蒸馏侧 metadata 增 endpoint_migrated/lineage_discontinuity/proxy_used/teacher_v3_anchor)
# ============================================================
def run_one_quadruple(
    ...
    # minimax 蒸馏侧派生 metadata (沿 r2 + mapping §1.2 minimax 行)
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 10 round 3 | minimax distill side | reask=2 -- 接力棒")
    print("计划 calls = 22 (22 caption × 1 call)")
    print("计数口径: distill 侧独立 (prompt_id 前缀 minimax_t01_distill_) -- 沿 r2")
    print("distill 侧 baseline: 2 successful/caption (沿 r1+r2)")
    print("本棒 round 3 目标: 各 caption 补至 3/5 successful (接力棒, 远期 ≥5)")
    print("预算 ≤1500s watchdog")
    print("retry 修复沿 r2 (含 retry 触发的源 error_category tracking)")
    print("mimo 无代理 (沿 r2 + L7 + add_T1 §1.6.2)")
    print("annotations: endpoint_migrated=True, lineage_discontinuity=True (沿 r2)")
    print("=" * 60)
    ...
    # 3. 加载 r2 result (dispatch 字面 predecessor)
    with open(BATCH10_R2_RESULT_PATH, "r", encoding="utf-8") as f:
        r2 = json.load(f)

    # 4. minimax distill 侧独立计数 (沿 r2; 累加 r1+r2 quadruples via r2.cumulative)
    ...
    print(f"[DISTILL prior] total={cum_total_prior_distill} ok={cum_ok_prior_distill} "
          f"empty={cum_empty_prior_distill} (源 = r2 quadruples distill 前缀过滤; 各 caption 各 2 successful)")

    # 5. cumulative 监控沿 r2 cumulative 链路 (跨教师跨侧累计)
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = r2 aggregate.cumulative.post)")
    ...
    for i, item in enumerate(planned):
        ...
        target_met_round3 = "[OK] 接力 3/5" if cur_succ_distill >= ROUND3_TARGET_PER_CAPTION else "[..]"
        print(
            ...
            f"retries={rc} "
            f"{status} "
            f"[distill_succ={cur_succ_distill}/{TARGET_SUCCESSFUL_PER_CAPTION} {target_met_round3}] "
    ...
    per_caption_mixed_history_successful_calls = r3_mixed_history

    # 14. distill 侧 **进度判定** (沿 r2 + batch8 r4 `glm2_distill_side_round4_progress` 接力棒命名)
    minimax_distill_side_round3_progress = {
        "all_22_captions_met_target": met_target_count_distill == len(captions),
        "met_count_post_r3": met_round3_count_distill,
        "met_count_post_r3_cumulative_to_target": met_target_count_distill,
        ...
        "still_pending_round3": sorted([
            cid for cid, info in per_caption_distill_only_successful_calls.items() if not info["round3_met"]
        ]),
        "round3_progress_disposition": (
            "minimax 蒸馏侧 round 3 (reask=2) 接力棒完成 → batch10 distill side round 3 progress ✓ (22 caption 各 3/5 successful 接力态)"
            if (r3_total == len(captions) and r3_ok == len(captions))
            else f"minimax 蒸馏侧 round 3 接力棒完成 → batch10 distill side round 3 progress, r3_ok={r3_ok}/{len(captions)} (需后续 r4-r5 接力补足)"
        ),
        "rounds_to_reach_round3_target_planned": 3,
        "rounds_to_reach_target_planned": 5,
        "rounds_to_reach_target_actual": (
            "3 棒 (r1+r2+r3) = 22 caption × 3 calls = 66 distill 侧 calls "
            "(接力态, 远期目标 ≥5 需 2 more 棒接力补足)"
        ),
        "distill_side_target_met_at_round": (
            "r5 (expected)" if met_target_count_distill == len(captions) else "未达 (本棒为 r3 接力棒, 留 r4-r5 接力)"
        ),
    ...
    dispatch_target_progress_distill = {
        ...
        "target_round3": f">={ROUND3_TARGET_PER_CAPTION} successful calls/caption (round 3 接力目标)",
        "met_count_post_r3_round3": met_round3_count_distill,
        "met_count_post_r3_cumulative_to_target": met_target_count_distill,
        "total_captions": len(captions),
        "remaining_count": len(captions) - met_target_count_distill,
        "remaining_count_round3": len(captions) - met_round3_count_distill,
        "met_percentage_round3": round(met_round3_count_distill / len(captions) * 100, 2),
        "met_percentage_cumulative_to_target": round(met_target_count_distill / len(captions) * 100, 2),
        "still_pending_captions": sorted([
            cid for cid, info in per_caption_distill_only_successful_calls.items() if not info["met_target"]
        ]),
        "still_pending_captions_round3": sorted([
            cid for cid, info in per_caption_distill_only_successful_calls.items() if not info["round3_met"]
        ]),
        "is_progress_round": True,
    }

    # 16. counting_scope_correction 块 (沿 r2; 本棒特殊: 接力棒, 无 r1 混合口径问题)
    counting_scope_correction = {
        "r3_issue_disclosed": False,
        "r3_issue_description": (
            "本棒为 minimax 蒸馏侧 round 3 接力棒 (= batch10 round 3), predecessor r1+r2 同前缀 quadruples = 44 条;"
            "无 r3 混合口径 (本棒 predecessor r2 同前缀过滤后 = 22 条均 minimax distill side);"
            "无 kimi 蒸馏侧混合计数问题 (本棒沿 r2 minimax distill side 累加)"
        ),
        "r3_data_disposition": "本棒 = 接力棒, predecessor = r2 minimax distill side (沿 r2); 累加 r2 quadruples (r1+r2)",
        "r3_counting_domain": (
            f"distill 侧独立计数; filter 域 = prompt_id 前缀 '{DISTILL_SIDE_PROMPT_ID_PREFIX}'"
            " (沿 r2; minimax 蒸馏侧累加)"
        ),
        "r3_target": f"distill 侧远期目标 ≥{TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption",
        "r3_round3_target": f"本棒 round 3 接力目标 ≥{ROUND3_TARGET_PER_CAPTION} successful calls/caption",
        "r3_baseline_distill_only": (
            f"predecessor r1+r2 quadruples minimax distill 过滤后 = {cum_total_prior_distill} calls, "
            f"{cum_ok_prior_distill} successful, {cum_empty_prior_distill} empty (各 caption 各 2 successful)"
        ),
        "r3_post_distill_only": (
            f"r1+r2+r3 quadruples 后 = {sum(per_cap_total_distill.values())} calls, "
            f"{sum(per_cap_succ_distill.values())} successful, "
            f"{sum(per_cap_empty_distill.values())} empty (各 caption = 3/5 successful 接力态 expected)"
        ),
        "mixed_history_retention": (
            "per_caption_mixed_history_successful_calls 块 = {} 空字典 (minimax distill 起步后无 r1 混合域参照);"
            "该块保留作透明字段待后续 r4-r5 接力棒填充"
        ),
        "pending_pi_review": (
        ...
        "progress_disposition": (
            f"r3 接力棒 distill 侧独立计数基线 = {cum_total_prior_distill} calls 完成 (沿 r1+r2); "
            f"实测 r3 quadruples 后 = {sum(per_cap_total_distill.values())} calls; "
            f"目标 ≥{TARGET_SUCCESSFUL_PER_CAPTION} 远期未达 (接力棒实况, 留 r4-r5 接力补足)"
        ),
    }

    # 17. retry validation 块 (沿 r2)
    ...
    result = {
        ...
        "round": 3,
        "metadata": {
            "task": "L14V3_batch10_round3_minimax_distill_side_reask2_progress_round_independent_counting",
            ...
            "date": "2026-09-26",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 10 round 3 minimax distill side reask=2 接力棒",
            "is_progress_round": True,
            ...
            "spec_conformance": "PI 2026-09-26 同 agent 唤醒 (task_append) 接力棒 + 沿 r2 口径续跑 + minimax 模型映射沿 r2 (mimo-v2.6-pro) + retry 修复沿 r2 + 每 caption successful 计数终盘点 + K-N26-N2 累计监控 + distill 侧独立计数 (沿 r2 counting_scope_correction) + 进度判定块 minimax_distill_side_round3_progress (沿 r2 + batch8 r4 接力棒命名)",
            "teacher": TEACHER,
            "teacher_label": TEACHER_LABEL,
            "side": SIDE,
            "round_index": 3,
            "reask_idx_r3": REASK_R3,
            "round3_target_per_caption": ROUND3_TARGET_PER_CAPTION,
            ...
            "endpoint_selection_method": "沿 r2 executor BB601B4F5ECB 端点实测 = mimo mimo-v2.6-pro (无代理)",
            "model_id_sent": MODEL_MINIMAX,
            ...
            "model_id_inconsistency_honest_disclosure": r2["metadata"]["model_id_inconsistency_honest_disclosure"],
            "endpoint_migrated": True,
            "lineage_discontinuity": True,
            ...
            "proxy_settings": {
                ...
                "note": "mimo 端点 = 无代理 (沿 r2 + L7 + add_T1 §1.6.2); 显式清空代理 env (防御性)",
            },
            "key_discipline": {
            ...
            "tools_fault_param_correction": {
                ...
                "r2_status": "retry 修复 + tracking 改进沿 r2 (沿 BB601B4F5ECB)",
                "r3_status": "retry 修复沿 r2; r3 接力棒本棒无新参数修正",
                "rationale": (
                    "沿 r2 修复 + r2 tracking 改进; r3 接力棒仅按 22 caption × 1 call 续采 (reask_idx=2);"
            ...
            "diff_vs_r2_executor": [
                "1. ROUND=2 -> ROUND=3; REASK_R2=1 -> REASK_R3=2 (第 3 call/caption on distill side)",
                "2. prompt_id 后缀 _r2 -> _r3",
                "3. predecessor_sha12 链改: r2_executor BB601B4F5ECB + r2_result 400EE4D21FC9 + mapping 98A779D61C1E (dispatch 字面)",
                "4. predecessor_sha12 链 (扩展上下文): + r1_executor/result + batch5 r5 minimax teacher 双件 + batch6 r5 kimi distill 收官 (蒸馏侧收官先例)",
                "5. baseline 切换: r1+r2 quadruples distill 过滤后 = 44 calls / 44 successful / 0 empty (各 caption 各 2 successful)",
                "6. 增量计数 -> distill 侧各 caption 由 2/5 顺利补至 3/5 (接力棒)",
                "7. 累计监控起点: r2 cumulative.post {362/9/353/2.49%} -> 本棒后 worst 31/384=8.07% / best 9/384=2.34%",
                "8. 判定块重命名: minimax_distill_side_round2_progress -> minimax_distill_side_round3_progress (沿 r2 接力棒命名)",
                "9. counting_scope_correction 改: r3_baseline=2 successful/caption (沿 r1+r2), r3_progress (接力棒描述)",
                "10. metadata.is_progress_round=True (标记接力棒, 沿 r2)",
                "11. per_caption 新增 round3_target/round3_met 字段 (round 3 接力目标判定)",
                "12. dispatch_target_progress_distill 新增 target_round3/met_count_post_r3_round3/met_percentage_round3 (round 3 进度盘点)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "minimax_distill_side_round3_progress": minimax_distill_side_round3_progress,
        ...
        "aggregate": {
            ...
            "cumulative": {
                ...
                "cumulative_baseline_source": "batch10 r2 (minimax distill round 2 接力棒) aggregate.cumulative.post",
            },
        ...
        "empty_rate_trend": {
            "batch1_round1": {
                "calls": r2["empty_rate_trend"]["batch1_round1"]["calls"],
                "empty": r2["empty_rate_trend"]["batch1_round1"]["empty"],
                "rate": r2["empty_rate_trend"]["batch1_round1"]["rate"],
            },
            "batch1_round2": {
                "calls": r2["empty_rate_trend"]["batch1_round2"]["calls"],
                "empty": r2["empty_rate_trend"]["batch1_round2"]["empty"],
                "rate": r2["empty_rate_trend"]["batch1_round2"]["rate"],
            },
            "batch1_round3": {
                "calls": r2["empty_rate_trend"]["batch1_round3"]["calls"],
                "empty": r2["empty_rate_trend"]["batch1_round3"]["empty"],
                "rate": r2["empty_rate_trend"]["batch1_round3"]["rate"],
            },
            "batch1_round4": {
                "calls": r2["empty_rate_trend"]["batch1_round4"]["calls"],
                "empty": r2["empty_rate_trend"]["batch1_round4"]["empty"],
                "rate": r2["empty_rate_trend"]["batch1_round4"]["rate"],
            },
            "batch1_round5": {
                "calls": r2["empty_rate_trend"]["batch1_round5"]["calls"],
                "empty": r2["empty_rate_trend"]["batch1_round5"]["empty"],
                "rate": r2["empty_rate_trend"]["batch1_round5"]["rate"],
            },
            "batch6_round1": {
                "calls": r2["empty_rate_trend"]["batch6_round1"]["calls"],
                "empty": r2["empty_rate_trend"]["batch6_round1"]["empty"],
                "rate": r2["empty_rate_trend"]["batch6_round1"]["rate"],
            },
            "batch6_round2": {
            ...
            "batch6_round3": {
                "calls": r2["empty_rate_trend"]["batch6_round3"]["calls"],
                "empty": r2["empty_rate_trend"]["batch6_round3"]["empty"],
                "rate": r2["empty_rate_trend"]["batch6_round3"]["rate"],
            },
            "batch6_round4": {
                "calls": r2["empty_rate_trend"]["batch6_round4"]["calls"],
                "empty": r2["empty_rate_trend"]["batch6_round4"]["empty"],
                "rate": r2["empty_rate_trend"]["batch6_round4"]["rate"],
            },
            "batch6_round5": {
                "calls": r2["empty_rate_trend"]["batch6_round5"]["calls"],
                "empty": r2["empty_rate_trend"]["batch6_round5"]["empty"],
                "rate": r2["empty_rate_trend"]["batch6_round5"]["rate"],
            },
            "batch10_round1": {
                "calls": r2["empty_rate_trend"]["batch10_round1"]["calls"],
                "empty": r2["empty_rate_trend"]["batch10_round1"]["empty"],
                "rate": r2["empty_rate_trend"]["batch10_round1"]["rate"],
            },
            "batch10_round2": {
                "calls": r2["empty_rate_trend"]["batch10_round2"]["calls"],
                "empty": r2["empty_rate_trend"]["batch10_round2"]["empty"],
                "rate": r2["empty_rate_trend"]["batch10_round2"]["rate"],
            },
            "batch10_round3": {
                "calls": r3_total,
                "empty": r3_empty,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_3_n_distinct_sample_note": (
                    "round 3 接力棒第 3 call per caption 实测; minimax distill 侧独立计数 = 22 caption 各 3 successful "
                    "(沿 r1+r2+r3 quadruples, prompt_id 前缀 minimax_t01_distill_ filter 域, 前置 r1+r2 各 caption 各 2 successful);"
                    "**minimax 蒸馏侧 round 3 接力达成**: distill 侧 22/22 caption 3/5 successful 接力态;"
                    "远期 ≥5 目标需 2 more 棒接力 (r4-r5);"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": "本棒为 batch10 round 3 接力棒 — 后续 r4-r5 接力棒 (沿 task_append 同 agent 唤醒语义)",
            "note": (
                f"本棒 round 3 接力棒已完成（{r3_total}/{planned_total} calls）"
                + (f"; K-N26-N2 累计阈值触发: cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   if k_n26_n2_triggered else
                   f"; 撞 {WALL_TIME_BUDGET_S}s watchdog 提前停批 ({len(captions) - r3_ok} caption 未达标) 走 task_append 接力棒"
                   if wall_time_s > WALL_TIME_BUDGET_S and r3_ok < len(captions) else
                   f"; **minimax 蒸馏侧 22/22 caption 3/5 successful 接力** ✓"
                   if r3_ok == len(captions) else
                   f"; minimax 蒸馏侧未达标 {len(captions) - r3_ok} caption → 下一棒接力 (本棒为接力棒, 留 r4-r5 接力补足)")
            ),
        },
        "self_scan": {
    ...
    # 20. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch10_r3_result.json")
    ...
    print(f"  distill_side_met_round3_count (>=3 接力): {met_round3_count_distill}/{len(captions)}")
    print(f"  *** all_22_captions_met_target_distill (>=5 final): {minimax_distill_side_round3_progress['all_22_captions_met_target']} ***")
    print(f"  round3_progress_disposition: {minimax_distill_side_round3_progress['round3_progress_disposition']}")
    ...
    if minimax_distill_side_round3_progress["still_pending"]:
        print(f"  still_pending_distill (>=5 final 目标距, 22 captions): {len(minimax_distill_side_round3_progress['still_pending'])} captions")
    if minimax_distill_side_round3_progress["still_pending_round3"]:
        print(f"  still_pending_round3 (>=3 接力目标距): {minimax_distill_side_round3_progress['still_pending_round3']}")
    return 0
