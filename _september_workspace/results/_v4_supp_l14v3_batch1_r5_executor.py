# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch1_r5_executor.py
=====================================

V4 L14+ 首批 (batch 1) · round 5 收尾 — kimi 教师侧 round 5 + round 4 补缺
============================================================================

【派工依据】
----------------
- 沿 r4 `results/_v4_supp_l14v3_batch1_r4_executor.py`（SHA-12 `4f5269ba8f18`）+ r4 产物
  `results/_v4_supp_l14v3_batch1_r4_result.json`（SHA-12 `9c800874701a`）口径续跑
- 沿前棒 `60c78b6f0281` / `6e01255b80c9` / `2bdb5163400f` / `00c7b545c397` / `b48019494530` / `22eb01144062`
...
【本棒范围（沿 dispatch §1）】
----------------------------------
1. **round 4 补缺 15 件**（reask=3）：S1_n35 / S1_n45 / S1_n60 / S2 / S2_n20 / S2_n35 / S2_n45 / S2_n60 /
   S3 / S4 / S5 / S6 / S6_n20 / S6_n35 / S6_n60（沿 r4 result `missing_for_round_4_after_watchdog` 字面）
2. **round 5 续采**（reask=4）：22 caption × 第 5 call（达成 ≥5 calls/caption 收尾）
3. **目标**：本棒或下棒把 kimi 教师侧攒满 ≥5 successful calls/caption（reask=0..4 五轮）
4. **S6_n60 优先保证**（沿 dispatch §1）：目前 successful=2，本棒需 +1~2 calls
5. 计划 calls = 15 (round 4 makeup) + 22 (round 5) = 37 calls
6. 预算 ≤22 calls / ≤600s watchdog（沿 dispatch §2 + L7 实证 ≤275s 上限）；scheduling = round 4 makeup 15 件排最前
7. 跑不完如实报断点（沿 prereg §1.6.4 partial completion 协议）

【每 caption successful 计数盘点（沿 dispatch §6）】
---------------------------------------------------------
- 本棒增量计算：每 call 后实时更新 per_caption_successful_calls 字典
- 收尾盘点：result.json 顶层 `per_caption_successful_calls` 块
- 达标判定：≥5 successful calls/caption = 达标；< 5 = 未达标
- S6_n60 重点：单独 `s6_n60_status` 块
...
【retry 触发的源 error_category tracking 小改进（沿 dispatch §3 字面授权，可选）】
----------------------------------------------------------------------------------
r4 遗留 tracking 限制：retry 触发但最终成功的实例，源 error_category 未被记录（仅记录 retry 失败时的 category）。
本棒改进：在 run_one_quadruple 内累积 `retry_trigger_categories` 列表（每次 retry 触发时记录源 error_category）。
最后归入 result.json `retry_validation.r4_r5_combined_retry_trigger_categories` 字段。

【empty_rate 趋势监控 + K-N26-N2 累计阈值停采（沿 dispatch §4）】
--------------------------------------------------------------------
- 累计 = batch1 + r2 + r3 + r4 + r5 总 cumulative
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r4 产物）：
  - prior (b1+r2+r3+r4): total=74, empty=9, **rate=12.16%**
  - r5 worst case (22 EMPTY): cumulative = 31/96 = 32.29% (仍 < 50% 阈值)

【端点取舍（沿前棒实测）】
----------------------------
- teamo + tun + kimi-k3（沿前棒 `model_id_inconsistency_honest_disclosure` 字面）
- 不重新探活（沿 dispatch §5 「沿 r4 口径续跑」）

【铁律严守（沿前棒）】
------------------------
- R4 key 永不明文（无例外）
- V1–V3 只读不动 / R5 V4 frozen append-only / R6 P-G 不动 / R7 plugin spec 不动
- 派生 JSON 不合并（本棒 `_batch1_r5_*` 与前棒所有 _batch1_* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch1_r5_result.json`（schema = `v4_l14v3_n26/1` + batch=1 + round=5）
- 不覆盖 r4 result `9c800874701a` 或前棒 result（沿 dispatch §5）

【与 r4 executor diff（沿 dispatch §5 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. 新增参数：`ROUND = 5`
2. 新增调度：`planned_calls = [15 round 4 makeup reask=3] + [22 round 5 reask=4]` = 37 calls
3. 新增「每 caption successful calls 累计字典」跟踪（沿 dispatch §6）
4. 小改进：`retry_trigger_categories` tracking（沿 dispatch §3 字面授权）
5. 每 quadruple 增 `reask_idx` (3 or 4) 字段（沿 dispatch §1）
6. `per_caption_successful_calls` 顶层块：22 caption × {succ_count, target, need, met_5?}
7. `s6_n60_status` 块：重点跟踪 S6_n60 successful 进展
8. `dispatch_target_progress` 块：本棒对「≥5 successful calls/caption」目标的进展
9. `predecessor_sha12` 链新增 r4_executor `4f5269ba8f18` + r4_result `9c800874701a`

【边界】
----------
- 不动任何既有件（含 r4 executor `4f5269ba8f18` + r4 result `9c800874701a` + 前所有棒产物 + prereg `05B975A86989`）
- 跑不完拆段报断点
...
# 本棒调度
ROUND = 5
TARGET_SUCCESSFUL_PER_CAPTION = 5  # ≥5 successful calls/caption 收尾目标

# round 4 补缺 15 件（沿 r4 result `missing_for_round_4_after_watchdog` 字面）
ROUND_4_MAKEUP_CAPTION_IDS = [
    "S1_n35", "S1_n45", "S1_n60", "S2", "S2_n20", "S2_n35", "S2_n45", "S2_n60",
    "S3", "S4", "S5", "S6", "S6_n20", "S6_n35", "S6_n60",
]
ROUND_4_MAKEUP_REASK = 3

# round 5 全部 22 caption
ROUND_5_REASK = 4
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch1-r5",
        "X-Title": "deposon-l14v3-batch1-r5-kimi-teacher",
    }
    body = {
        "model": MODEL_KIMI_K3,
        "messages": messages,
...
# ============================================================
# 单 call + 内层重试 (沿 r4 retry bug 修复 + r5 retry 触发的源 error_category tracking)
# ============================================================
def run_one_quadruple(
    ...
    side: str,
    round_index: int,
    is_round_4_makeup: bool,
    is_round_5: bool,
    retry_trigger_categories_tracker: List[str],
) -> Dict[str, Any]:
    """retry_trigger_categories_tracker 是外部传入 list，每次 retry 触发时 append 源 error_category。"""
    ...
    while retries <= INNER_RETRY_MAX:
            ...
            if not is_empty_response(content):
                return {
                    ...
                    "per_call_metadata": {
                        ...
                        "is_round_4_makeup": is_round_4_makeup,
        ...
        if err_cat in NETWORK_ERROR_CATEGORIES_RETRY:
            # 网络异常 retry 路径（r4 修复）
            # r5 改进：tracking retry 触发的源 error_category
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 1 round 5 · kimi 教师侧收尾")
    print("计划 calls = 15 (round 4 makeup reask=3) + 22 (round 5 reask=4) = 37 calls")
    print("预算 ≤22 calls / ≤600s watchdog（沿 dispatch §2）")
    print("目标: 本棒或下棒把 kimi 教师侧攒满 ≥5 successful calls/caption")
    print("retry 修复沿 r4 + r5 retry 触发源 error_category tracking 小改进")
    print("=" * 60)
    ...
    # 3. 装载前棒 cumulative + 每 caption successful 计数（沿 dispatch §6）
    ...
    # 4. 调度 planned_calls（round 4 makeup 15 件排最前 + round 5 22 件续后）
    planned: List[Dict[str, Any]] = []
    for cap_id in ROUND_4_MAKEUP_CAPTION_IDS:
        planned.append({
            "caption_id": cap_id,
            "reask_idx": ROUND_4_MAKEUP_REASK,
            "is_round_4_makeup": True,
            "is_round_5": False,
            "round_index": 4,
            "phase": "round_4_makeup",
        })
    for c in captions:
        planned.append({
            "caption_id": c["id"],
            "reask_idx": ROUND_5_REASK,
            "is_round_4_makeup": False,
            "is_round_5": True,
            "round_index": 5,
            "phase": "round_5",
        })
    ...
    print(f"[PLANNED] total: {planned_total} ({len(ROUND_4_MAKEUP_CAPTION_IDS)} round 4 makeup + {len(captions)} round 5)")
    ...
    # r5 retry 触发的源 error_category tracking 小改进
    retry_trigger_categories_tracker: List[str] = []
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            is_round_4_makeup=item["is_round_4_makeup"],
            is_round_5=item["is_round_5"],
        ...
        if item["phase"] == "round_4_makeup":
            r5_round_4_makeup_count += 1
        elif item["phase"] == "round_5":
            r5_round_5_count += 1
        ...
        marker = (
            f"[r4-makeup]" if item["phase"] == "round_4_makeup"
            else f"[r{ROUND}]"
        )
        current_cum_rate_post = cum_empty / cum_total if cum_total > 0 else 0.0
        cur_succ = per_cap_succ[cap_id]
        target_met = "✓" if cur_succ >= TARGET_SUCCESSFUL_PER_CAPTION else "·"
        print(
    ...
    # 9. round 5 缺 caption
    ran_caption_ids_round_5 = set(
        q["per_call_metadata"]["caption_id"] for q in quadruples
        if q["per_call_metadata"].get("is_round_5")
    )
    missing_for_round_5 = sorted(
        c["id"] for c in captions if c["id"] not in ran_caption_ids_round_5
    )

    # 10. S6_n60 重点跟踪
    s6_n60_status = {
        "successful_count_post_r5": per_cap_succ["S6_n60"],
        "target": TARGET_SUCCESSFUL_PER_CAPTION,
        "met_target_post_r5": per_cap_succ["S6_n60"] >= TARGET_SUCCESSFUL_PER_CAPTION,
        "still_need": max(0, TARGET_SUCCESSFUL_PER_CAPTION - per_cap_succ["S6_n60"]),
        "in_round_4_makeup_list": "S6_n60" in ROUND_4_MAKEUP_CAPTION_IDS,
        "ran_in_round_4_makeup_this_batch": any(
            q["per_call_metadata"]["caption_id"] == "S6_n60" and q["per_call_metadata"]["is_round_4_makeup"]
            for q in quadruples
        ),
        "ran_in_round_5_this_batch": any(
            q["per_call_metadata"]["caption_id"] == "S6_n60" and q["per_call_metadata"]["is_round_5"]
            for q in quadruples
        ),
    }
    ...
    dispatch_target_progress = {
        ...
        "note": (
            "本棒后达标 caption 数；"
            "未达标 caption 留 worker 下一棒接力续采"
        ),
    }

    # 12. retry validation 块（r4 + r5 综合）
    ...
    retry_validation = {
        ...
        "delta_empty_rate_r5_vs_r3": round(empty_rate_r5 - (r3["aggregate"]["r3_empty_response_count"] / r3["aggregate"]["r3_total_calls"]), 4),
    }
    ...
    result = {
        ...
        "round": 5,
        "metadata": {
            "task": "L14V3_batch1_round5_kimi_teacher_side_finish",
            ...
            "date": "2026-09-24",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 1 round 5 收尾",
            "spec_conformance": "PI 2026-09-24 同 agent 唤醒 (task_append) 接力棒 + 沿 r4 口径续跑 + retry 修复沿 r4 + 每 caption successful 计数盘点 + K-N26-N2 累计监控",
            "teacher": "kimi",
            "side": "teacher",
            "round_index": 5,
            "target_successful_per_caption": TARGET_SUCCESSFUL_PER_CAPTION,
            "n_captions_total": len(captions),
            "n_round_4_makeup_planned": len(ROUND_4_MAKEUP_CAPTION_IDS),
            "n_round_4_makeup_actual": r5_round_4_makeup_count,
            "n_round_5_planned": len(captions),
            "n_round_5_actual": r5_round_5_count,
            ...
            "tools_fault_param_correction": {
                ...
                "r4_status": "retry bug 修复有效验证 (r4 result 9c800874701a)",
                "r5_status": "retry 修复沿 r4 + retry 触发的源 error_category tracking 小改进",
                "rationale": (
                    "沿 r4 修复；r5 tracking 改进 = 每次 retry 触发时记录源 error_category；"
            ...
            "diff_vs_r4_executor": [
                "1. 新增 ROUND=5 / TARGET_SUCCESSFUL_PER_CAPTION=5 参数化",
                "2. 新增调度: [15 round 4 makeup reask=3] + [22 round 5 reask=4] = 37 calls",
                "3. 每 caption successful 计数跟踪（沿 dispatch §6）",
                "4. r5 改进: retry_trigger_categories_tracker 列表（沿 dispatch §3 字面授权）",
                "5. prompt_id 增 _r4_makeup / _r5 后缀",
                "6. aggregate 增 cumulative (沿 r4) + per_caption_successful_counts (本棒新)",
                "7. per_caption_successful_calls 顶层块：22 caption × {succ_count, target, need, met_target} (沿 dispatch §6)",
                "8. s6_n60_status 顶层块：S6_n60 successful 进展 (沿 dispatch §1 优先保证)",
                "9. dispatch_target_progress 顶层块：本棒对「≥5 successful calls/caption」目标的进展 (沿 dispatch §6)",
                "10. retry_validation 顶层块（r4 + r5 综合；沿 r4 + r5 改进）",
                "11. predecessor_sha12 链新增 r4_executor/r4_result 全链",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "aggregate": {
            ...
            "r5_round_4_makeup_count": r5_round_4_makeup_count,
            "r5_round_5_count": r5_round_5_count,
            ...
            "missing_for_round_5_after_watchdog": missing_for_round_5,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_5_n_distinct_sample_note": (
                    "round 5 第 5 call 实测；与 r4/r3/r2/batch1 同 caption 历次 response_text 取并集 → "
                    "n_distinct 上限 ≥5 (理论上凑齐 ≥5 calls/caption 后非退化自证充分); "
                    "充分判定需 ≥5 successful calls/caption 收尾后由 verdict-keeper 统裁"
                ),
            },
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        "本棒未触发 K-N26-N2 累计阈值；跑至 600s watchdog 提前停批 / 全 37 calls 完成"
                    )
                ),
            },
        ...
        "breakpoint_status": {
            ...
            "round_4_makeup_actual": r5_round_4_makeup_count,
            "round_5_actual": r5_round_5_count,
            "missing_for_round_5_after_watchdog": missing_for_round_5,
            "checkpoint_file": None,
            "next_resume_via": "同 agent 唤醒 (task_append 续跑) — 沿 prereg §1.6.4",
            "note": (
                f"本棒 round 5 已完成"
                + (f"（round 4 makeup {r5_round_4_makeup_count}/{len(ROUND_4_MAKEUP_CAPTION_IDS)} + round 5 {r5_round_5_count}/22）")
                + (f"；K-N26-N2 累计阈值触发：cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50 at call {k_n26_n2_trigger_at_call_idx}, 立即停采 (沿 dispatch §4)"
                   if k_n26_n2_triggered else
                   f"；撞 600s watchdog 提前停批，剩余 round 5 待采 {len(missing_for_round_5)} caption 走 task_append 接力棒"
                   if missing_for_round_5 else
                   "；round 5 全 22 caption 已完成，未撞 watchdog")
            ),
        },
        "self_scan": {
    ...
    # 15. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch1_r5_result.json")
    ...
    print(f"    round 4 makeup: {r5_round_4_makeup_count}/{len(ROUND_4_MAKEUP_CAPTION_IDS)}")
    print(f"    round 5: {r5_round_5_count}/22")
    ...
    if missing_for_round_5:
        print(f"  missing_for_round_5_after_watchdog: {missing_for_round_5}")
    return 0
