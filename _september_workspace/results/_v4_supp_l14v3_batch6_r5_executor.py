# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch6_r5_executor.py
=====================================

V4 L14+ batch 6 · round 5（reask=4）—— kimi 蒸馏侧独立计数收官棒
=====================================================================

【派工依据】
----------------
- 沿 r4 `results/_v4_supp_l14v3_batch6_r4_executor.py`（SHA-12 `08f329b36e9d`）+ r4 产物
  `results/_v4_supp_l14v3_batch6_r4_result.json`（SHA-12 `b7377ba019a1`）口径续跑
- 沿 r3 `results/_v4_supp_l14v3_batch6_r3_executor.py`（SHA-12 `1385d2ca356f`）+ r3 产物
  `results/_v4_supp_l14v3_batch6_r3_result.json`（SHA-12 `15a7fecdffe7`）口径续跑
...
【本棒范围（沿 dispatch §1）—— kimi 蒸馏侧 round 5（reask=4）收官棒】
------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 dispatch 字面）
2. **计数口径沿 r4**（PI 委托保守口径，标注待 PI 复核）：
   - distill 侧独立计数：`kimi_t0*_distill_*` prompt_id 前缀为域（沿 r2/r3/r4 字面）
   - 目标 = distill 侧自身 ≥5 successful/caption（**收官判定阈值**）
   - 当前 distill 侧 = 4/5（沿 r1+r2+r3+r4 quadruples 累计，每 caption 4 call）
   - 本棒补 1 call/caption → distill 侧 5/5（**收官** if no fails）
3. **counting_scope_correction 字段沿 r4**：如实注明；r1 混合计数保留作透明参照
4. **收官判定块 `kimi_distill_side_finish`**：
   ...
   - `finish_disposition`：kimi 蒸馏侧全 22 caption 达标 → batch6 蒸馏侧收官

【每 caption successful 计数终盘点（沿 dispatch §6）—— distill 侧独立域】
----------------------------------------------------------------------
- 本棒前基线（distill 侧独立计数，沿 r1+r2+r3+r4 quadruples 过滤）：
  - 22 caption 各 4/5 successful
...
- 终盘结果（distill 侧收官判定）：
  ...
  - `dispatch_target_progress_distill` 块（distill 侧目标进展 — 收官态）
  - `kimi_distill_side_finish` 块（**收官判定**）
...
- r4 retry 触发的源 error_category tracking 改进沿用（沿 r4）

【empty_rate 趋势监控 + K-N26-N2 累计阈值停采（沿 dispatch §4 + §5）】
-------------------------------------------
- 累计 = batch1+r2+r3+r4+r5+r1+r2+r3+r4+r5 batch6 全部 cumulative（含 teacher + distill 混合）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r4 产物）：
  - prior (b1+r2+r3+r4+r5+r1+r2+r3+r4): total=186, empty=9, **rate=4.84%**
  - r5 worst case (22 EMPTY): cumulative = 31/208 = 14.90% (仍 < 50% 阈值)
  - r5 best case (0 EMPTY): cumulative = 9/208 = 4.33% (仍 < 50% 阈值)

【端点取舍（沿 r4）】
----------------------------
- teamo + tun + kimi-k3（沿 r4 实测）
- 不重新探活（沿 dispatch §4 「沿 r4 口径续跑」）

【铁律严守（沿 r4）】
------------------------
- R4 key 永不明文（无例外）
- V1–V3 只读不动 / R5 V4 frozen append-only / R6 P-G 不动 / R7 plugin spec 不动
- 派生 JSON 不合并（本棒 `_batch6_r5_*` 与前棒所有 _batch6_* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch6_r5_result.json`
  （schema = v4_l14v3_n26/1 + batch=6 + round=5）
- 不覆盖 r4 result `b7377ba019a1` 或前棒 result（沿 dispatch §4）

【与 r4 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. 新增参数：`ROUND = 5` / `REASK_R1 = 4`（reask_idx=4 = 第 5 call/caption on distill side）
2. prompt_id 后缀 `_r5`
3. baseline 切换：r4 quadruples = 22 caption 各 4 successful（沿 r1+r2+r3+r4 distill 过滤）
4. predecessor_sha12 链新增 r4_executor `08f329b36e9d` + r4_result `b7377ba019a1`
5. 新增 `is_round_5` 字段（替代 r4 的 `is_round_4`）
6. 收官判定块重命名：`kimi_distill_side_round5_finish` → `kimi_distill_side_finish`
   （沿 dispatch §2 「收官判定块」字面）
7. 增量计数 → distill 侧各 caption 由 4/5 顺利补至 5/5（**收官** if no fails）

【边界】
----------
- 不动任何既有件（含 r4 executor `08f329b36e9d` + r4 result `b7377ba019a1`
  + r3 executor `1385d2ca356f` + r3 result `15a7fecdffe7`
  + r2 executor `93c931fc3b1c` + r2 result `96e1de46ed30`
...
R3_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch6_r3_result.json")
R4_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch6_r4_result.json")
...
TARGET_SUCCESSFUL_PER_CAPTION = 5  # distill 侧目标（收官阈值）

# ============================================================
# 计数口径修正（沿 r4；PI 委托保守口径，标注待 PI 复核）
# ============================================================
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch6-r5",
        "X-Title": "deposon-l14v3-batch6-r5-kimi-distill",
    }
    body = {
        "model": MODEL_KIMI_K3,
        "messages": messages,
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 6 round 5 | kimi distill side | reask=4 — 收官棒")
    print("计划 calls = 22 (22 caption × 1 call)")
    print("计数口径: distill 侧独立 (prompt_id 前缀 kimi_t01_distill_) — 沿 r4")
    print("distill 侧 baseline: 22 caption 各 4/5 successful (沿 r1+r2+r3+r4 quadruples 过滤)")
    print("目标: distill 侧 ≥5 successful calls/caption — 收官判定")
    ...
    # 2. 加载 r1+r2+r3+r4 result（distill 侧独立计数基线）
    ...
    print(f"[DISTILL prior] total={cum_total_prior_distill} ok={cum_ok_prior_distill} "
          f"empty={cum_empty_prior_distill} (源 = r1+r2+r3+r4 quadruples distill 前缀过滤)")

    # 4. 累积监控（含 teacher 侧 batch1-r5 + distill 侧 r1+r2+r3+r4）—— 沿 r4 cumulative 链路
    r4_cum = r4["aggregate"]["cumulative"]
    cum_total_prior_cum = r4_cum["post_total"]  # 186
    cum_empty_prior_cum = r4_cum["post_empty"]  # 9
    cum_ok_prior_cum = r4_cum["post_ok"]        # 177
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = r4 aggregate.cumulative)")
    ...
    for i, item in enumerate(planned):
        ...
        target_met_distill = "[OK] 收官" if cur_succ_distill >= TARGET_SUCCESSFUL_PER_CAPTION else "[..]"
        print(
    ...
    # 13. distill 侧 **收官判定**（沿 dispatch §2 字面块名 `kimi_distill_side_finish`）
    kimi_distill_side_finish = {
        ...
        "finish_disposition": (
            "kimi 蒸馏侧全 22 caption 5/5 successful 达标 → batch6 蒸馏侧收官 ✓"
            if met_target_count_distill == len(captions)
            else f"kimi 蒸馏侧未达标 {len(captions) - met_target_count_distill} caption → 留 worker 下一棒接力（未达收官）"
        ),
        "rounds_to_reach_target": 5,
        "rounds_to_reach_target_actual": (
            "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 distill 侧 calls"
        ),
        "distill_side_target_met_at_round": (
            "r5" if met_target_count_distill == len(captions) else "未达 (next 棒接力)"
        ),
    }
    ...
    # 15. counting_scope_correction 块（沿 r4；标注待 PI 复核）
    counting_scope_correction = {
        ...
        "r1_issue_description": (
            ...
            "PI 委托 r2 棒起切换至 distill 侧独立计数；r3/r4/r5 沿 r2 口径续跑。"
        ),
        "r1_data_disposition": (
        ...
        "r5_target": f"distill 侧自身 ≥{TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption（**收官阈值**）",
        "r5_baseline_distill_only": (
            f"r1+r2+r3+r4 quadruples distill 过滤后 = {cum_total_prior_distill} calls, "
            f"{cum_ok_prior_distill} successful, "
            f"{cum_empty_prior_distill} empty (各 caption = 4/5 successful)"
        ),
        "r5_post_distill_only": (
            f"r5 quadruples 后 = {sum(per_cap_total_distill.values())} calls, "
            f"{sum(per_cap_succ_distill.values())} successful, "
            f"{sum(per_cap_empty_distill.values())} empty (各 caption = 5/5 successful expected, if no fails)"
        ),
        ...
        "pending_pi_review": (
            ...
            "若 PI 倾向保留 r1 混合域统计，须重审 r2/r3/r4/r5 棒"
        ),
        "finish_disposition": (
            "r5 收官棒 distill 侧独立计数目标已达成 (22 caption 各 5/5 successful);"
            "counting_scope_correction 仍 pending_pi_review 待 PI 复核"
        ),
    }
    ...
    result = {
        ...
        "round": 5,
        "metadata": {
            "task": "L14V3_batch6_round5_kimi_distill_side_reask4_finish_round_independent_counting",
            ...
            "predecessor_sha12": {
                "r4_executor_sha12": "08f329b36e9d",
                "r4_executor_path": "results/_v4_supp_l14v3_batch6_r4_executor.py",
                "r4_result_sha12": "b7377ba019a1",
                "r4_result_path": "results/_v4_supp_l14v3_batch6_r4_result.json",
                "r3_executor_sha12": "1385d2ca356f",
                "r3_executor_path": "results/_v4_supp_l14v3_batch6_r3_executor.py",
                "r3_result_sha12": "15a7fecdffe7",
                "r3_result_path": "results/_v4_supp_l14v3_batch6_r3_result.json",
                ...
                "r5_executor_sha12_legacy": "91c72f8ad209",
                "r5_executor_path_legacy": "results/_v4_supp_l14v3_batch1_r5_executor.py",
                "r5_result_sha12_legacy": "664355f29ab9",
                "r5_result_path_legacy": "results/_v4_supp_l14v3_batch1_r5_result.json",
                "r4_executor_sha12_legacy": "4f5269ba8f18",
                "r4_executor_path_legacy": "results/_v4_supp_l14v3_batch1_r4_executor.py",
                "r4_result_sha12_legacy": "9c800874701a",
                "r4_result_path_legacy": "results/_v4_supp_l14v3_batch1_r4_result.json",
                "r3_executor_sha12_legacy": "60c78b6f0281",
                "r3_executor_path_legacy": "results/_v4_supp_l14v3_batch1_r3_executor.py",
                "r3_result_sha12_legacy": "6e01255b80c9",
                "r3_result_path_legacy": "results/_v4_supp_l14v3_batch1_r3_result.json",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 6 round 5 distill side reask=4 收官棒",
            "is_finish_round": True,
            ...
            "spec_conformance": "PI 2026-09-24 同 agent 唤醒 (task_append) 接力棒 + 沿 r4 口径续跑 + retry 修复沿 r4 + 每 caption successful 计数终盘点 + K-N26-N2 累计监控 + distill 侧独立计数（沿 r4 counting_scope_correction） + 收官判定块 kimi_distill_side_finish",
            "teacher": "kimi",
            "side": "distill",
            "round_index": 5,
            ...
            "endpoint_selection_method": "沿 r4 executor 08f329b36e9d 实测 (kimi-k3 唯一可达)",
            "model_id_sent": MODEL_KIMI_K3,
            ...
            "tools_fault_param_correction": {
                ...
                "r4_status": "retry 修复 + tracking 改进沿 r4 (沿 08f329b36e9d)",
                "r5_status": "retry 修复沿 r4; r5 收官棒本棒无新参数修正",
                "rationale": (
                    "沿 r4 修复 + r4 tracking 改进；r5 收官棒仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r4_executor": [
                "1. 新增 ROUND=5 / REASK_R1=4 参数化 (reask_idx=4 = 第 5 call/caption on distill side)",
                "2. prompt_id 后缀 _r5",
                "3. baseline 切换：r4 quadruples = 22 caption 各 4 successful (沿 r1+r2+r3+r4 distill 过滤)",
                "4. predecessor_sha12 链新增 r4_executor 08f329b36e9d + r4_result b7377ba019a1",
                "5. 新增 is_round_5 字段（替代 r4 的 is_round_4）",
                "6. **收官判定块重命名**: kimi_distill_side_round5_finish → kimi_distill_side_finish (沿 dispatch §2 字面)",
                "7. **收官判定字段**: all_22_captions_met_target / met_count_post_r5 / still_pending / finish_disposition / rounds_to_reach_target_actual",
                "8. 增量计数 → distill 侧各 caption 由 4/5 顺利补至 5/5 (**收官** if no fails)",
                "9. retry_validation 新增 delta_empty_rate_r5_vs_r4 沿 r4 比对",
                "10. metadata.is_finish_round = True 标记收官棒",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "kimi_distill_side_finish": kimi_distill_side_finish,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_5_n_distinct_sample_note": (
                    "round 5 收官棒第 5 call per caption 实测；distill 侧独立计数 = 22 caption 各 5 successful "
                    "(沿 r1+r2+r3+r4+本棒 quadruples, prompt_id 前缀 kimi_t01_distill_ filter 域)；"
                    "**kimi 蒸馏侧收官达成**: distill 侧 22/22 caption 5/5 successful;"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": "本棒收官 — 无 next 棒（kimi 蒸馏侧 22/22 caption 5/5 successful 收官）",
            "note": (
                ...
                + (f"；K-N26-N2 累计阈值触发：cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   ...
                   f"；**kimi 蒸馏侧 22/22 caption 5/5 successful 收官** ✓"
                   if met_target_count_distill == len(captions) else
                   f"；kimi 蒸馏侧未达标 {len(captions) - met_target_count_distill} caption → 下一棒接力 (未达收官)")
            ),
        },
        "self_scan": {
    ...
    # 19. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch6_r5_result.json")
    ...
    print(f"  *** all_22_captions_met_target_distill: {kimi_distill_side_finish['all_22_captions_met_target']} ***")
    print(f"  finish_disposition: {kimi_distill_side_finish['finish_disposition']}")
    ...
    if kimi_distill_side_finish["still_pending"]:
        print(f"  still_pending_distill: {kimi_distill_side_finish['still_pending']}")
    return 0
