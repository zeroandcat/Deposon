# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch2_r4_executor.py
=====================================

V4 L14+ batch 2 · round 4 · GLM_1 教师侧 round 3 续采（reask=2）
================================================================
...
- 派工单（沿 2026-09-24 23:25 接力棒）：GLM_1 教师侧 round 4 (reask=2) 续采
- 拍板依据：ask_748d9242c7a6be3d83de63a0 (2026-09-24 23:08 显式)：GLM_1=glm-5.3 已在跑（沿 r2/r3）

【本棒范围（沿 dispatch）—— GLM_1 教师侧 round 3（reask=2）续采】
----------------------------------------------------------
...
---------------------------------------------------------
- 本棒前基线（沿 batch2 r3）：GLM_1 教师侧 = 44 calls / 0 empty / 44 ok / 0.0000 rate
...
--------------------------------------------------------------------
- 跨批累计 baseline = batch1 终态 119 + batch2 r2 (22) + batch2 r3 (22) → **163 calls / 9 empty / 5.52%**（沿 r3 result）
- 累计 = r3 baseline + batch2 r4（GLM_1 自身 r3 末 44/0/44/0.0000）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）

【端点取舍（沿 r3）】
----------------------------
- teamo + tun + glm-5.3（r3 已 smoke ok=200；本棒不再做完整 3-candidate probe，仅 1-call smoke）
...
- 派生 JSON 不合并（本棒 `_batch2_r4_*` 与 `_batch2_r3_*` / `_batch2_r2_*` / `_batch2_r1_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 + r1 + batch1 r6 + models_probe 全套不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch2_r4_result.json`（schema = `v4_l14v3_n26/1` + batch=2 + round=4）
- 不覆盖 r3 result `25251ACA6D79` 或 r2 result `8C125E257AF8` 或 batch1 r6 result `A4F851154551`
- predecessor_sha12 链：r3_executor/r3_result + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 4（升 r4 命名避开 r3 重叠）；BATCH = 2 不变
2. prompt_id 后缀 `_b2_r4`；t03 = teacher side round 3（取代 r3 的 t02 round 2 reask=1）
3. reask_idx = 2（本棒每位 caption 第 3 call；取代 r3 的 reask_idx=1）
4. phase_label：`b2_r4_need1`（取代 r3 的 `b2_r3_need1`）
5. round3_target = 3（取代 r3 的 round2_target=2）；is_round_2 = False（取代 r3 的 True）
6. smoke_call_only = True（沿 r3；不再做 3-candidate probe fallback）
7. predecessor_sha12 链：r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79`（新增；r2/r1/batch1_r6 全套沿 r3）
8. baseline prior：r3 终态 44 calls / 0 empty / 44 ok / 0.0000 rate（取代 r3 链的 22/0/22）
9. 跨批累计 baseline：r3 终态 163 calls / 9 empty / 5.52% rate（取代 r3 链的 141/9/6.38%）
10. 新增 `glm1_teacher_side_round3_progress` 块（本棒后 GLM_1 教师侧 round 3 完成度 ≥3 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 163/9/5.52%
12. spec_conformance 重写：GLM_1 教师侧 round 3 续采（reask=2），目标 ≥3 successful/caption
13. dispatch_target_progress 字段：target_round3 / still_pending_captions_round3
14. per_caption_successful_calls 字段新增 round3_target=3 / round3_met

【边界】
----------
- 不动任何既有件（含 r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `A4F851154551` + r1 executor `E52F930654D3` + r1 result `D245AE4CE385` + models_probe `3C9DC60B5071` + prereg `05B975A86989`）
- 跑不完拆段报断点
...
# 选定的 model_id（沿 r3 smoke 结论，不重探）
MODEL_GLM_1 = "glm-5.3"
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch2-r4",
        "X-Title": f"deposon-l14v3-batch2-r4-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r3；仅 1 call 确认端点 ok）
# ============================================================
def smoke_glm_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r3 smoke 结论 glm-5.3 ok=200 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b2_r4"
    prompt_id = f"glm1_t03_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 2 round 4 · GLM_1 教师侧 round 3（reask=2）续采")
    print("沿 r3 glm-5.3 smoke 结论 ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r3 (沿 batch1 r6)")
    print("=" * 60)
    ...
    # r3 终态 = 本棒前 GLM_1 自身累计基线
    glm1_prior_total = r3["aggregate"]["glm1_cumulative"]["post_total"]
    glm1_prior_empty = r3["aggregate"]["glm1_cumulative"]["post_empty"]
    glm1_prior_ok = r3["aggregate"]["glm1_cumulative"]["post_ok"]
    glm1_prior_rate = r3["aggregate"]["glm1_cumulative"]["post_rate"]
    print(f"[BASELINE r3 GLM_1] total={glm1_prior_total} ok={glm1_prior_ok} empty={glm1_prior_empty} rate={glm1_prior_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r3 smoke 结论 glm-5.3 ok=200）
    print("=" * 60)
    print("[SMOKE PHASE] glm-5.3 1-call smoke（沿 r3 smoke 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 4,
            "metadata": {
                "task": "L14V3_batch2_round4_GLM1_teacher_side_round3_reask2",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch2_r4_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b2_r4_need1"} for c in captions_sorted]
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=False,  # 语义：本棒 = GLM_1 教师侧 data collection round 3
    ...
    # 11. GLM_1 教师侧 round 3 进度块
    glm1_round3_progress = {
        "round3_target": ">=3 successful call/caption (round 3 reask=2)",
        "met_count_post_r4_round3": met_round3_count,
    ...
    # model_id_inconsistency_honest_disclosure（沿 r3 + r4 沿用）
    ...
    model_id_disclosure = (
        ...
        f"r2 1-call probe ok=200; r3 1-call smoke ok=200; r4 沿 r3 结论仅做 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                "PI 2026-09-24 batch2 GLM_1 教师侧 round 3 续采 (reask=2); "
                "沿 r3 glm-5.3 smoke 结论 (1-call smoke ok=200); r4 仅 1-call smoke; "
            ...
            "tools_fault_param_correction": {
                ...
                "r4_b2_status": "r4 仅 1-call smoke (沿 r3 结论)；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r3 glm-5.3 smoke 结论；r4 节省预算避免重复 probe；"
            ...
            "diff_vs_r3_executor": [
                "1. ROUND=4 (升 r4 命名)；BATCH=2 不变",
                "2. prompt_id 后缀 _b2_r4；t03_teacher (取代 r3 的 t02_teacher round 2 reask=1)",
                "3. reask_idx=2 (本棒每 caption 第 3 call；取代 r3 的 reask_idx=1)",
                "4. phase_label: b2_r4_need1 (取代 r3 的 b2_r3_need1)",
                "5. is_round_2=False (语义: GLM_1 教师侧 data collection round 3；取代 r3 的 is_round_2=True)",
                "6. smoke_call_only=True (沿 r3；不再做 3-candidate probe)",
                "7. predecessor_sha12 链: r3_executor 72FD2BB3838E + r3_result 25251ACA6D79 (新增；r2/r1/batch1_r6 全套沿 r3)",
                "8. baseline prior: r3 终态 44 calls / 0 empty / 44 ok (取代 r3 链的 22/0/22)",
                "9. 跨批累计 baseline: r3 终态 163 calls / 9 empty / 5.52% (取代 r3 链的 141/9/6.38%)",
                "10. 新增 glm1_teacher_side_round3_progress 块 (round 3 完成度 ≥3 successful/caption)",
                "11. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 163/9/5.52%",
                "12. spec_conformance 重写: round 3 续采 (reask=2)",
                "13. per_caption_successful_calls 字段新增 round3_target=3 / round3_met (取代 r3 的 round2_target=2 / round2_met)",
                "14. 调度注释: round 3 续采 (reask=2) 22 caption × 1 call",
                "15. http referer/title 字段更新 l14v3-batch2-r4 (取代 r3 的 r3)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "r4 沿 r3 glm-5.3 smoke 结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；r4 与 r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        ...
        "glm1_teacher_side_round3_progress": glm1_round3_progress,
        ...
        "empty_rate_trend": {
            ...
            "cross_batch_cumulative_post_r4": {
                ...
                "note": "r3 baseline 163/9/5.52% + r4 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round3_reask2_n_distinct_sample_note": (
                    f"round 3 reask=2 第 1 call 实测；本棒后 GLM_1 教师侧 round 3 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"r3 baseline GLM_1 = {glm1_prior_total} calls / {glm1_prior_empty} empty / {glm1_prior_rate:.4f}; "
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 3 (reask=2) 已完; 下棒可接力 GLM_1 教师侧 round 4 (>=2 more successful/caption)"
            ),
            "note": (
                f"本棒 round 3 (reask=2): actual={r4_total}/{planned_total} calls; "
                f"GLM_1 教师侧 round 3 完成度 = {met_round3_count}/{len(captions)} caption met round 3 目标 (≥3 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"GLM_1 model_id 沿 r3 smoke 选定 = {model_id} (1-call smoke 确认仍可用)"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
