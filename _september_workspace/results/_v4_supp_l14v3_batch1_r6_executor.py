# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch1_r6_executor.py
=====================================
...
6. 新增 `kimi_teacher_side_finish` 块：本棒后 kimi 教师侧是否达成全 22 caption 达标
...
# 本棒调度
ROUND = 6
REASK_R6 = 5  # round 6 第 6 call (reask_idx = 5)
...
PRIORITY_PLANNED_CALLS = [
    # S6_n60 × 2 (priority 1)
    {"caption_id": "S6_n60", "phase": "r6_s6_n60_call_1"},
    {"caption_id": "S6_n60", "phase": "r6_s6_n60_call_2"},
    # L_historical_causality × 2 (priority 2)
    {"caption_id": "L_historical_causality", "phase": "r6_l_historical_call_1"},
    {"caption_id": "L_historical_causality", "phase": "r6_l_historical_call_2"},
    # S2_n20 × 2 (priority 3)
    {"caption_id": "S2_n20", "phase": "r6_s2_n20_call_1"},
    {"caption_id": "S2_n20", "phase": "r6_s2_n20_call_2"},
    # need=1 其余 15 caption × 1 (alphabetical for determinism)
    {"caption_id": "L_biological_taxonomy", "phase": "r6_need1"},
    {"caption_id": "L_physics_concepts", "phase": "r6_need1"},
    {"caption_id": "S1_n35", "phase": "r6_need1"},
    {"caption_id": "S1_n45", "phase": "r6_need1"},
    {"caption_id": "S1_n60", "phase": "r6_need1"},
    {"caption_id": "S2", "phase": "r6_need1"},
    {"caption_id": "S2_n35", "phase": "r6_need1"},
    {"caption_id": "S2_n45", "phase": "r6_need1"},
    {"caption_id": "S2_n60", "phase": "r6_need1"},
    {"caption_id": "S3", "phase": "r6_need1"},
    {"caption_id": "S4", "phase": "r6_need1"},
    {"caption_id": "S5", "phase": "r6_need1"},
    {"caption_id": "S6", "phase": "r6_need1"},
    {"caption_id": "S6_n20", "phase": "r6_need1"},
    {"caption_id": "S6_n35", "phase": "r6_need1"},
]
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch1-r6",
        "X-Title": "deposon-l14v3-batch1-r6-kimi-teacher",
    }
    body = {
        "model": MODEL_KIMI_K3,
        "messages": messages,
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 1 round 6 · kimi 教师侧收官")
    ...
    planned = PRIORITY_PLANNED_CALLS
    ...
    retry_validation = {
        ...
        "delta_empty_rate_r6_vs_r5": round(empty_rate_r6 - r5["aggregate"]["r5_empty_response_rate"], 4),
        "delta_empty_rate_r6_vs_r3": round(empty_rate_r6 - (r3["aggregate"]["r3_empty_response_count"] / r3["aggregate"]["r3_total_calls"]), 4),
    }
    ...
    result = {
        ...
        "round": 6,
        "metadata": {
            "task": "L14V3_batch1_round6_kimi_teacher_side_finish",
            ...
            "date": "2026-09-24",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 1 round 6 收官",
            ...
            "diff_vs_r5_executor": [
                "1. 新增 ROUND=6 / REASK_R6=5 参数化",
                "2. 新增调度: PRIORITY_PLANNED_CALLS 优先级排序 (S6_n60 × 2 + L_historical_causality × 2 + S2_n20 × 2 + 15 need=1 × 1) = 21 calls",
                "3. 每 caption successful 计数跟踪（沿 r5）",
                "4. retry 触发源 error_category tracking 沿 r5",
                "5. prompt_id 后缀 _r6",
                "6. 新增 kimi_teacher_side_finish 块：本棒后 kimi 教师侧是否达成全 22 caption 达标 (沿 dispatch §1 'kimi 侧全 22 达标即收官')",
        ...
        "kimi_teacher_side_finish": kimi_teacher_side_finish,
        ...
        "breakpoint_status": {
            ...
            "note": (
                f"本棒 round 6 已完成（{r6_total}/{planned_total} calls）"
    ...
    # 14. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch1_r6_result.json")
    ...
    print(f"  quadruples: {r6_total} (ok={r6_ok} empty={r6_empty} fail={r6_fail})")
    print(f"  r6_empty_rate: {empty_rate_r6:.4f}")
    ...
    print(f"  all_22_captions_met_target: {kimi_teacher_side_finish['all_22_captions_met_target']}")
    print(f"  retry_bug_fix_applied_in_all_calls: {retry_validation['r6_retry_bug_fix_applied_in_all_calls']}")
    ...
    if kimi_teacher_side_finish["still_pending"]:
        print(f"  still_pending: {kimi_teacher_side_finish['still_pending']}")
    return 0
