# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch7_r1_executor.py
=====================================

V4 L14+ batch 7 · round 1 (reask=0) -- GLM_1 蒸馏侧第一棒 (独立计数起步棒)

派工依据:
  - 沿 L14+ 派工单 2026-09-25 02:09「GLM_1 蒸馏侧 round 1 (distill 侧开工) -- 夜间保守口径」
  - 沿 batch6 r5 executor (SHA-12 8a8babb911b8) + r5 result (SHA-12 e40424bf8abe) 口径续跑
...
本棒范围:
  ...
  2. batch=7 (GLM_1 distill) + round=1 (reask=0)
     - 模型 = teamo `glm-5.3` (沿模型映射 §1.1 GLM_1 拍板)
     - 端点 = https://api.teamorouter.cn/v1/chat/completions (沿 r5)
     - 走 tun 代理 (PI 2026-09-23 硬纪律)
  3. distill 侧独立计数:
     - filter = prompt_id 前缀 `glm1_t01_distill_`
     - 本棒为 GLM_1 distill 第一棒: predecessor r1-r4 同前缀 0 条 = 0/5 baseline
     - 目标 ≥5 successful/caption
     - 本棒后预期 22 caption 各 1/5 successful (起步棒)
  4. side=distill + teacher_label=GLM_1 字面字段
  5. cumulative 监控: 起点 = r5 post (208/9/4.33%); 本棒后 worst 31/230=13.48%; best 9/230=3.91%
  6. 起步判定块 `glm1_distill_side_round1_start` (all_22=False / met_count=22 expected / still_pending=22)

与 batch6 r5 executor diff:
  1. 模型切换 kimi-k3 -> glm-5.3
  2. teacher_label 新增 GLM_1 字面字段
  3. prompt_id 前缀 kimi_t01_distill_ -> glm1_t01_distill_
  4. _r5 -> _r1 后缀
  5. ROUND=5 -> ROUND=1; REASK_R1=4 -> REASK_R1=0
  6. predecessor 链改: r5_executor 8a8babb911b8 + r5_result e40424bf8abe + mapping 98A779D61C1E
  7. distill 侧 baseline = 0 (r1-r4 同前缀 0 条); r1 quadruples 增量 = 22
  8. per_caption 终盘点新增 met_target=1 起步档 (非 r5 的 met_target=5 收官档)
  9. cumulative 起点沿 r5 cumulative.post
 10. 判定块重命名: kimi_distill_side_finish -> glm1_distill_side_round1_start
 11. counting_scope_correction 改: r1_baseline=0/起步/起步棒描述 (无 r1 混合口径问题)
...
产物:
  - 写入: results/_v4_supp_l14v3_batch7_r1_result.json
    (schema = v4_l14v3_n26/1 + batch=7 + round=1)
  - 不覆盖 r5 result e40424bf8abe 或前棒 result

边界:
  - 不动任何既有件 (含 batch6 r5 双件 + r1-r5 全部 + prereg 05B975A86989 + captions 6a2656878745 + mapping 98A779D61C1E)
  - 跑不完拆段报断点
...
R5_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch6_r5_result.json")
...
# 预算 (沿 r5; 22 calls × 30s + 21×2.5s gap = ~712s; 留 1500s)
...
# ============================================================
# 计数口径 (沿 r5 distill 侧独立计数; GLM_1 蒸馏侧独立域)
# ============================================================
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch7-r1",
        "X-Title": "deposon-l14v3-batch7-r1-glm1-distill",
    }
    body = {
        "model": MODEL_GLM_5_3,
        "messages": messages,
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 7 round 1 | GLM_1 distill side | reask=0 -- 起步棒")
    print("计划 calls = 22 (22 caption × 1 call)")
    print("计数口径: distill 侧独立 (prompt_id 前缀 glm1_t01_distill_) -- 沿 r5")
    print("distill 侧 baseline: 0 successful/caption (本棒为 GLM_1 distill 第一棒)")
    ...
    # 2. 加载 r5 result (累积监控; 不是 GLM_1 distill predecessor)
    with open(R5_RESULT_PATH, "r", encoding="utf-8") as f:
        r5 = json.load(f)

    # 3. GLM_1 distill 侧独立计数 (本棒为起步棒, r1-r4 同前缀 0 条 = 0/5 baseline)
    ...
    print(f"[DISTILL prior] total={cum_total_prior_distill} ok={cum_ok_prior_distill} "
          f"empty={cum_empty_prior_distill} (本棒为 GLM_1 distill 起步棒, r1-r4 同前缀 0 条)")

    # 4. cumulative 监控沿 r5 cumulative 链路 (跨教师跨侧累计)
    r5_cum = r5["aggregate"]["cumulative"]
    cum_total_prior_cum = r5_cum["post_total"]    # 208
    cum_empty_prior_cum = r5_cum["post_empty"]    # 9
    cum_ok_prior_cum = r5_cum["post_ok"]          # 199
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = r5 aggregate.cumulative.post)")

    # 5. 没有 mixed history (本棒为 GLM_1 distill 第一棒, 无 r1 混合域参照)
    r1_mixed_history = {}
    ...
    # 12. mixed history 透明保留 (本棒 = {}; 无 r1 混合域参照)
    per_caption_mixed_history_successful_calls = r1_mixed_history

    # 13. distill 侧 **起步判定** (沿 dispatch §2 字面块名 `glm1_distill_side_round1_start`)
    glm1_distill_side_round1_start = {
        ...
        "start_disposition": (
            "GLM_1 蒸馏侧第一棒完成 → batch7 distill side 起步 ✓ (22 caption 各 1/5 successful 起步态)"
            if (r1_total == len(captions) and r1_ok == len(captions))
            else f"GLM_1 蒸馏侧第一棒完成 → batch7 distill side 起步, r1_ok={r1_ok}/{len(captions)} (需后续 r2-r4 接力补足)"
        ),
    ...
    # 15. counting_scope_correction 块 (沿 r5; 本棒特殊: 起步棒, 无 r1 混合口径问题)
    counting_scope_correction = {
        "r1_issue_disclosed": False,
        "r1_issue_description": (
            "本棒为 GLM_1 蒸馏侧第一棒 (= batch7 round 1), predecessor r1-r4 同前缀 quadruples = 0 条;"
            "无 r1 混合口径 (本棒 predecessor r1-r4 同前缀过滤后 = 0 条);"
            "无 kimi 蒸馏侧混合计数问题 (本棒为新教师 GLM_1 蒸馏侧独立开工)"
        ),
        ...
        "r1_baseline_distill_only": (
            f"predecessor r1-r4 quadruples GLM_1 distill 过滤后 = 0 calls, "
    ...
    result = {
        ...
        "round": 1,
        "metadata": {
            "task": "L14V3_batch7_round1_GLM_1_distill_side_reask0_start_round_independent_counting",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 7 round 1 GLM_1 distill side reask=0 起步棒",
            "is_start_round": True,
            ...
            "spec_conformance": "PI 2026-09-24 同 agent 唤醒 (task_append) 接力棒 + 沿 r5 口径续跑 + retry 修复沿 r5 + 每 caption successful 计数终盘点 + K-N26-N2 累计监控 + distill 侧独立计数 (沿 r5 counting_scope_correction) + 起步判定块 glm1_distill_side_round1_start",
            "teacher": TEACHER,
            "teacher_label": TEACHER_LABEL,
            "side": SIDE,
            "round_index": 1,
            ...
            "endpoint_selection_method": "沿模型映射留痕件 §1.1 GLM_1 拍板定案 (ask_748d9242 字面) + 沿 r5 executor 8a8babb911b8 端点实测 = teamo glm-5.3",
            "model_id_sent": MODEL_GLM_5_3,
            "model_id_sent_basis": "model_mapping 98A779D61C1E §1.1 GLM_1 列拍板定案 = teamo glm-5.3 (PI 2026-09-24 23:08 ask_748d9242 字面)",
            "model_id_inconsistency_honest_disclosure": r5["metadata"]["model_id_inconsistency_honest_disclosure"],
            "proxy_settings": {
                "http": PROXY_HTTP,
                "socks5": PROXY_SOCKS5,
            ...
            "tools_fault_param_correction": {
                ...
                "rationale": (
                    "沿 r5 修复 + r5 tracking 改进; r1 起步棒仅按 22 caption × 1 call 续采 (model 切换 kimi-k3 -> glm-5.3);"
            ...
            "diff_vs_r5_executor": [
                "1. 模型切换: kimi-k3 -> glm-5.3 (沿模型映射 98A779D61C1E §1.1 GLM_1 拍板)",
                "2. teacher_label 字段新增: per_call_metadata.teacher_label = 'GLM_1'",
                "3. prompt_id 前缀: kimi_t01_distill_ -> glm1_t01_distill_",
                "4. prompt_id 后缀 _r5 -> _r1",
                "5. ROUND=5 -> ROUND=1; REASK_R1=4 -> REASK_R1=0 (第 1 call/caption on distill side)",
                "6. predecessor_sha12 链改: batch6 r5_executor 8a8babb911b8 + r5_result e40424bf8abe + mapping 98A779D61C1E",
                "7. baseline 切换: r1-r4 GLM_1 distill quadruples = 0 (本棒为新教师起步棒)",
                "8. 增量计数 -> distill 侧各 caption 由 0/5 顺利补至 1/5 (起步棒)",
                "9. 累计监控起点: r5 cumulative.post {208/9/4.33%} -> 本棒后 worst 31/230=13.48% / best 9/230=3.91%",
                "10. 判定块重命名: kimi_distill_side_finish -> glm1_distill_side_round1_start",
                "11. counting_scope_correction 改: 本棒为新教师起步棒, 无 r1 混合口径问题",
                "12. metadata.teacher = 'GLM_1' (替代 r5 'kimi')",
                "13. metadata.is_start_round = True 标记起步棒",
                "14. per_call_metadata 新增 teacher_label 字段",
                "15. 端点选择依据: 模型映射留痕件 (PI 拍板 ask_748d9242) 字面而非 r5 端点复用",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "glm1_distill_side_round1_start": glm1_distill_side_round1_start,
        ...
        "empty_rate_trend": {
            "batch1_round1": {
                "calls": r5["empty_rate_trend"]["batch1_round1"]["calls"],
                "empty": r5["empty_rate_trend"]["batch1_round1"]["empty"],
                "rate": r5["empty_rate_trend"]["batch1_round1"]["rate"],
            },
            "batch1_round2": {
                "calls": r5["empty_rate_trend"]["batch1_round2"]["calls"],
                "empty": r5["empty_rate_trend"]["batch1_round2"]["empty"],
                "rate": r5["empty_rate_trend"]["batch1_round2"]["rate"],
            },
            "batch1_round3": {
                "calls": r5["empty_rate_trend"]["batch1_round3"]["calls"],
                "empty": r5["empty_rate_trend"]["batch1_round3"]["empty"],
                "rate": r5["empty_rate_trend"]["batch1_round3"]["rate"],
            },
            "batch1_round4": {
                "calls": r5["empty_rate_trend"]["batch1_round4"]["calls"],
                "empty": r5["empty_rate_trend"]["batch1_round4"]["empty"],
                "rate": r5["empty_rate_trend"]["batch1_round4"]["rate"],
            },
            "batch1_round5": {
                "calls": r5["empty_rate_trend"]["batch1_round5"]["calls"],
                "empty": r5["empty_rate_trend"]["batch1_round5"]["empty"],
                "rate": r5["empty_rate_trend"]["batch1_round5"]["rate"],
            },
            "batch6_round1": {
                "calls": r5["empty_rate_trend"]["batch6_round1"]["calls"],
                "empty": r5["empty_rate_trend"]["batch6_round1"]["empty"],
                "rate": r5["empty_rate_trend"]["batch6_round1"]["rate"],
            },
            "batch6_round2": {
                "calls": r5["empty_rate_trend"]["batch6_round2"]["calls"],
                "empty": r5["empty_rate_trend"]["batch6_round2"]["empty"],
                "rate": r5["empty_rate_trend"]["batch6_round2"]["rate"],
            },
            "batch6_round3": {
                "calls": r5["empty_rate_trend"]["batch6_round3"]["calls"],
                "empty": r5["empty_rate_trend"]["batch6_round3"]["empty"],
                "rate": r5["empty_rate_trend"]["batch6_round3"]["rate"],
            },
            "batch6_round4": {
                "calls": r5["empty_rate_trend"]["batch6_round4"]["calls"],
                "empty": r5["empty_rate_trend"]["batch6_round4"]["empty"],
                "rate": r5["empty_rate_trend"]["batch6_round4"]["rate"],
            },
            "batch6_round5": {
                "calls": r5["empty_rate_trend"]["batch6_round5"]["calls"],
                "empty": r5["empty_rate_trend"]["batch6_round5"]["empty"],
                "rate": r5["empty_rate_trend"]["batch6_round5"]["rate"],
            },
            "batch7_round1": {
                "calls": r1_total,
                "empty": r1_empty,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_1_n_distinct_sample_note": (
                    "round 1 起步棒第 1 call per caption 实测; GLM_1 distill 侧独立计数 = 22 caption 各 1 successful "
                    "(沿本棒 quadruples, prompt_id 前缀 glm1_t01_distill_ filter 域, 前置 predecessor r1-r4 同前缀 0 条);"
                    "**GLM_1 蒸馏侧起步达成**: distill 侧 22/22 caption 1/5 successful 起步态;"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": "本棒为 batch7 起步棒 — 后续 r2-r4 接力棒 (沿 task_append 同 agent 唤醒语义)",
            "note": (
                ...
                + (f"; K-N26-N2 累计阈值触发: cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   ...
                   f"; **GLM_1 蒸馏侧 22/22 caption 1/5 successful 起步** ✓"
                   if r1_ok == len(captions) else
                   f"; GLM_1 蒸馏侧未达标 {len(captions) - r1_ok} caption → 下一棒接力 (本棒为起步棒, 留 r2-r4 接力补足)")
            ),
        },
        "self_scan": {
    ...
    # 19. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch7_r1_result.json")
    ...
    print(f"  *** all_22_captions_met_target_distill: {glm1_distill_side_round1_start['all_22_captions_met_target']} ***")
    print(f"  start_disposition: {glm1_distill_side_round1_start['start_disposition']}")
    ...
    if glm1_distill_side_round1_start["still_pending"]:
        print(f"  still_pending_distill (≥5 目标距): {glm1_distill_side_round1_start['still_pending']}")
    return 0
