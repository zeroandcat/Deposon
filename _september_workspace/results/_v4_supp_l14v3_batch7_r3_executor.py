# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch7_r3_executor.py
=====================================

V4 L14+ batch 7 · round 3 (reask=2) -- GLM_1 蒸馏侧第三棒 (独立计数 2/5 -> 3/5)

派工依据:
  - 沿 L14+ 派工单 2026-09-25 02:38「GLM_1 蒸馏侧 round 3 (同 agent 唤醒保上下文, 沿 r2 口径续跑)」
  - 沿 batch7 r2 executor (SHA-12 35cda0c0f7da) + r2 result (SHA-12 924f12c4e7ab) 口径续跑
...
本棒范围:
  ...
  2. batch=7 (GLM_1 distill) + round=3 (reask=2)
     - 模型 = teamo `glm-5.3`, 端点走 tun
  3. distill 侧独立计数:
     - filter = prompt_id 前缀 `glm1_t01_distill_`
     - predecessor = r1+r2 quadruples (44 calls, 44 ok, 0 empty, 各 caption 各 2 successful)
     - 目标 ≥5 successful/caption (远期)
     - 本棒后预期 22 caption 各 3/5 successful
  4. side=distill + teacher_label=GLM_1 字面字段
  5. cumulative 监控起点: r2 post {252/9/0.0357}; 本棒后 worst 31/274=11.31% / best 9/274=3.28%
  6. 续跑判定块 `glm1_distill_side_round3_progress`

与 batch7 r2 executor diff:
  1. ROUND=2 -> ROUND=3; REASK_R1=1 -> REASK_R1=2
  2. prompt_id 后缀 _r2 -> _r3
  3. predecessor_sha12: r2_executor 35cda0c0f7da + r2_result 924f12c4e7ab (替代 r1 双件为直接 predecessor)
  4. baseline: r2 quadruples = 22 caption 各 2 successful (沿 r1+r2 quadruples filter glm1_t01_distill_ = 44 calls/44 ok/0 empty)
  5. 增量: distill 各 caption 由 2/5 补至 3/5
  6. cumulative 监控起点: r2 cumulative.post {252/9/0.0357} -> 本棒后 worst 31/274=11.31% / best 9/274=3.28%
  7. 判定块重命名: glm1_distill_side_round2_progress -> glm1_distill_side_round3_progress
  8. counting_scope_correction 改: r2_baseline=44 calls 2succ/0empty; r3 后预期 66 calls 3succ/0empty
  9. headers (HTTP-Referer / X-Title) 反映 batch7-r3
...
产物: results/_v4_supp_l14v3_batch7_r3_result.json
  (schema = v4_l14v3_n26/1 + batch=7 + round=3)
  不覆盖 r2 result 924f12c4e7ab 或前棒 result
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch7-r3",
        "X-Title": "deposon-l14v3-batch7-r3-glm1-distill",
    }
    body = {
        "model": MODEL_GLM_5_3,
        "messages": messages,
...
# ============================================================
# 提示构造 (沿 r1/r2 prompt 构造口径 = batch1 r6 reproduction_caliber)
# ============================================================
PROMPT_SYSTEM = (
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 7 round 3 | GLM_1 distill side | reask=2 -- 续跑棒 (2/5 -> 3/5)")
    print("planed calls = 22 (22 caption x 1 call)")
    print("counting caliber: distill independent (prompt_id prefix glm1_t01_distill_) -- along r2")
    print("distill baseline (post r1+r2): 2 successful/caption")
    print("goal: distill >=5 successful calls/caption (long-term)")
    print("budget <=1500s watchdog")
    print("retry fix along r2")
    print("=" * 60)
    ...
    # 2. 加载 r1 + r2 result (predecessor 双件 + cumulative 链路)
    with open(R1_RESULT_PATH, "r", encoding="utf-8") as f:
        r1 = json.load(f)
    with open(R2_RESULT_PATH, "r", encoding="utf-8") as f:
        r2 = json.load(f)

    # 3. GLM_1 distill 侧独立计数 (filter r1+r2 quadruples glm1_t01_distill_ 前缀)
    ...
    print(f"[DISTILL prior] total={cum_total_prior_distill} ok={cum_ok_prior_distill} "
          f"empty={cum_empty_prior_distill} (predecessor = r1+r2 quadruples GLM_1 distill prefix filter; expected 44/44/0 if all ok)")

    # 4. cumulative 监控沿 r2 cumulative 链路
    r2_cum = r2["aggregate"]["cumulative"]
    cum_total_prior_cum = r2_cum["post_total"]    # 252
    cum_empty_prior_cum = r2_cum["post_empty"]    # 9
    cum_ok_prior_cum = r2_cum["post_ok"]          # 243
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (source = r2 aggregate.cumulative.post)")

    # 5. mixed history 透明保留 (本棒 = {}; GLM_1 distill 为新教师, 无混合域)
    r1_mixed_history = {}
    ...
    for i, item in enumerate(planned):
        ...
        target_met_distill = "[OK] 续跑 3/5" if cur_succ_distill >= 3 else "[..]"
        print(
    ...
    # 13. distill 侧 **续跑判定**
    glm1_distill_side_round3_progress = {
        ...
        "progress_disposition": (
            "GLM_1 蒸馏侧第三棒完成 -> batch7 distill side 续跑 (22 caption 各 3/5 successful 续跑态)"
            if (r3_total == len(captions) and r3_ok == len(captions))
            else f"GLM_1 蒸馏侧第三棒完成 -> batch7 distill side 续跑, r3_ok={r3_ok}/{len(captions)} (需 r4-r5 接力补足)"
        ),
        "rounds_to_reach_target_planned": 5,
        "rounds_to_reach_target_actual": (
            "3 rounds (r1+r2+r3) = 22 caption x 3 calls = 66 distill side calls "
            "(续跑态, 目标 >=5 需 2 more rounds 接力)"
        ),
        "distill_side_target_met_at_round": (
            "r5 (expected)" if met_target_count_distill == len(captions) else "unreached (round 3 = 3 of 5, need 2 more rounds)"
        ),
        "is_progress_round": True,
        "round_progress_label": "r3 of 5 (3/5 -> 4/5 needed next)",
    }
    ...
    counting_scope_correction = {
        ...
        "r1_issue_description": (
            "本棒为 GLM_1 蒸馏侧 round 3 续跑棒; predecessor = r1+r2 quadruples (44 calls, 44 ok, 0 empty); "
            "GLM_1 distill side = brand new teacher side, 沿 r1+r2 计数干净"
        ),
        "r1_data_disposition": "r1+r2 quadruples 完整保留 (r1 SHA-12 3558a1d15867 + r2 SHA-12 924f12c4e7ab); 作为 distill 侧 predecessor 不视为废数据",
        "r3_counting_domain": (
            f"distill side independent counting; filter domain = prompt_id prefix '{DISTILL_SIDE_PROMPT_ID_PREFIX}'"
            " (along r1/r2; new teacher GLM_1 distill side)"
        ),
        "r3_target": f"distill side long-term goal >={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption",
        "r3_baseline_distill_only": (
            f"predecessor r1+r2 quadruples GLM_1 distill filter = {cum_total_prior_distill} calls, "
            f"{cum_ok_prior_distill} successful, "
            f"{cum_empty_prior_distill} empty (each caption = 2/5 successful, 沿 r1+r2 续跑)"
        ),
        "r3_post_distill_only": (
            f"r3 quadruples after = {sum(per_cap_total_distill.values())} calls, "
            f"{sum(per_cap_succ_distill.values())} successful, "
            f"{sum(per_cap_empty_distill.values())} empty (each caption = 3/5 successful 续跑态 expected)"
        ),
        "mixed_history_retention": (
            "per_caption_mixed_history_successful_calls block = empty dict (GLM_1 distill 为新教师); "
            "block 保留作透明字段待后续 r4-r5 接力棒填充"
        ),
        "pending_pi_review": (
        ...
        "progress_disposition": (
            f"r3 续跑棒 distill side baseline = {cum_total_prior_distill} calls 完成; "
            f"实测 r3 after = {sum(per_cap_total_distill.values())} calls; "
            f"goal >={TARGET_SUCCESSFUL_PER_CAPTION} long-term unreached (round 3 of 5 续跑实况)"
        ),
    }
    ...
    result = {
        ...
        "round": 3,
        "metadata": {
            "task": "L14V3_batch7_round3_GLM_1_distill_side_reask2_progress_round_independent_counting",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 truth-audit-only path batch 7 round 3 GLM_1 distill side reask=2 续跑棒",
            ...
            "spec_conformance": "PI 2026-09-24 同 agent 唤醒 (task_append) 接力棒 + 沿 r2 口径续跑 + retry 修复沿 r2 + 每 caption successful 计数终盘点 + K-N26-N2 累计监控 + distill 侧独立计数 (沿 r2) + 续跑判定块 glm1_distill_side_round3_progress",
            "teacher": TEACHER,
            "teacher_label": TEACHER_LABEL,
            "side": SIDE,
            "round_index": 3,
            ...
            "endpoint_selection_method": "along model mapping draft §1.1 GLM_1 拍板定案 (ask_748d9242 字面) + along r2 executor 35cda0c0f7da 端点实测 = teamo glm-5.3",
            "model_id_sent": MODEL_GLM_5_3,
            ...
            "tools_fault_param_correction": {
                ...
                "r2_status": "retry 修复 + tracking 改进沿 r2 (沿 35cda0c0f7da)",
                "r3_status": "retry 修复沿 r2; r3 续跑棒本棒无新参数修正",
                "rationale": (
                    "沿 r2 修复 + r2 tracking 改进; r3 续跑棒仅按 22 caption x 1 call 续采 (model 不变, 仅 reask_idx 1->2); "
            ...
            "diff_vs_r2_executor": [
                "1. ROUND=2 -> ROUND=3; REASK_R1=1 -> REASK_R1=2 (第 3 call/caption)",
                "2. prompt_id 后缀 _r2 -> _r3",
                "3. predecessor_sha12: r2_executor 35cda0c0f7da + r2_result 924f12c4e7ab (替代 r1 双件为直接 predecessor)",
                "4. baseline: r2 quadruples = 22 caption 各 2 successful (沿 r1+r2 filter glm1_t01_distill_ = 44 calls/44 ok/0 empty)",
                "5. 增量: distill 各 caption 由 2/5 补至 3/5",
                "6. cumulative 起点: r2 post {252/9/0.0357} -> 本棒后 worst 31/274=11.31% / best 9/274=3.28%",
                "7. 判定块重命名: glm1_distill_side_round2_progress -> glm1_distill_side_round3_progress",
                "8. counting_scope_correction 改: r2_baseline=44 calls 2succ/0empty; r3 后预期 66 calls 3succ/0empty",
                "9. headers 反映 batch7-r3",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "glm1_distill_side_round3_progress": glm1_distill_side_round3_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch7_round1": {
                "calls": r2["empty_rate_trend"]["batch7_round1"]["calls"],
                "empty": r2["empty_rate_trend"]["batch7_round1"]["empty"],
                "rate": r2["empty_rate_trend"]["batch7_round1"]["rate"],
            },
            "batch7_round2": {
                "calls": r2["empty_rate_trend"]["batch7_round2"]["calls"],
                "empty": r2["empty_rate_trend"]["batch7_round2"]["empty"],
                "rate": r2["empty_rate_trend"]["batch7_round2"]["rate"],
            },
            "batch7_round3": {
                "calls": r3_total,
                "empty": r3_empty,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_3_n_distinct_sample_note": (
                    "round 3 续跑棒第 3 call per caption 实测; GLM_1 distill side independent counting = 22 caption 各 3 successful "
                    "(沿 r1+r2+本棒 quadruples, prompt_id prefix glm1_t01_distill_ filter domain, predecessor r1+r2 = 44 calls 2succ/caption);"
                    "GLM_1 蒸馏侧续跑达成: distill side 22/22 caption 3/5 successful 续跑态 (2/5 -> 3/5);"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": "本棒为 batch7 第三棒 — 后续 r4-r5 接力棒 (沿 task_append 同 agent 唤醒语义)",
            "note": (
                f"本棒 round 3 续跑棒已完成（{r3_total}/{planned_total} calls）"
                + (f"; K-N26-N2 累计阈值触发" if k_n26_n2_triggered else
                   f"; hit {WALL_TIME_BUDGET_S}s watchdog 提前停批 ({len(captions) - r3_ok} caption 未达标)"
                   if wall_time_s > WALL_TIME_BUDGET_S and r3_ok < len(captions) else
                   f"; GLM_1 蒸馏侧 22/22 caption 3/5 successful 续跑 (2/5 -> 3/5)"
                   if r3_ok == len(captions) else
                   f"; GLM_1 蒸馏侧未达标 {len(captions) - r3_ok} caption -> 下一棒接力")
            ),
        },
        "self_scan": {
    ...
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch7_r3_result.json")
    ...
    print(f"  *** all_22_captions_met_target_distill: {glm1_distill_side_round3_progress['all_22_captions_met_target']} ***")
    print(f"  progress_disposition: {glm1_distill_side_round3_progress['progress_disposition']}")
    ...
    if glm1_distill_side_round3_progress["still_pending"]:
        print(f"  still_pending_distill (>=5 target distance): {glm1_distill_side_round3_progress['still_pending']}")
    return 0
