# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch3_r2_executor.py
====================================

V4 L14+ batch 3 · round 2 · GLM_2 教师侧 round 2（reask=1）
===========================================================

【派工依据】
----------------
- 沿 batch3 r1 executor `A4731B39343F` + r1 result `D24B84671C23` 口径锚（GLM_2 教师侧 round 1 reask=0）
...
- **glm2_corpus 口径拍板** (2026-09-24 23:42 ask_259658a4ffadd6f8aeb12f34 显式):
  **接受 system prompt 微调口径** = GLM_2 教师身份=语料标签+prompt 语境差异；近缘对分布差异有限
  如实入 N-26 构造域限制（不软化）；round1 的 22 calls 有效不作废；
  字段命名维持现状（三处冗余标注保留）

【本棒范围（沿 dispatch）—— GLM_2 教师侧 round 2（reask=1）】
----------------------------------------------------------
1. **沿用 glm-5.3**（r1 1-call smoke ok=200；本棒不再探活，仅 1-call smoke 防端点飘移）
...
---------------------------------------------------------
- 本棒前基线（沿 r1 末态）：GLM_2 教师侧 = 22 calls / 0 empty / 22 ok / 0.0% rate
- 跨批累计 baseline = r1 末态 **185 calls / 9 empty / 176 ok / 4.86%**（沿 r1 result）
...
【retry 修复逻辑沿 batch1 r6 + batch2 r2 + batch3 r1（沿 dispatch §3）】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = **185 calls / 9 empty / 4.86%**（沿 r1 末态，本棒前 GLM_2 r1 末态）
- GLM_2 自身 = 22 calls / 0 empty / 0.0% rate（r1 末态）
...
【端点取舍（沿 r1）】
----------------------------
- teamo + tun + glm-5.3（r1 已探活确认 ok=200；本棒不再做完整 3-candidate probe，仅 1-call smoke）
- model_id 字面 = glm-5.3
...
- 派生 JSON 不合并（本棒 `_batch3_r2_*` 与 `_batch3_r1_*` / `_batch2_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r1 executor `A4731B39343F` + r1 result `D24B84671C23` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 双件 + r1 探活件 + models_probe `3C9DC60B5071` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch3_r2_result.json`（schema = `v4_l14v3_n26/1` + batch=3 + round=2）
- 不覆盖 r1 result `D24B84671C23` 或 r3 result `25251ACA6D79` 或 r2 result `8C125E257AF8` 或 r1 探活 result `D245AE4CE385` 或 batch1 r6 result `A4F851154551`
- predecessor_sha12 链：r1_executor `A4731B39343F` + r1_result `D24B84671C23` + r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79` + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r1 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 2（升 r2 命名避开 r1 重叠）；BATCH = 3 不变
2. prompt_id 前缀 `glm2_t02_teacher_*`（取代 r1 的 `glm2_t01_teacher_*`）
   - t02 = teacher side round 2（取代 r1 的 t01 round 1）
   - 后缀 `_b3_r2`（取代 r1 的 `_b3_r1`）
3. REASK_R1 = 1（本棒每位 caption 第 2 call；取代 r1 的 reask_idx=0）
4. phase_label：`b3_r2_need1`（取代 r1 的 `b3_r1_need1`）
5. is_round_2 = True（语义：本棒为 GLM_2 教师侧 data collection round 2）
6. smoke_call_only = True（沿 r1）
7. predecessor_sha12 链：r1_executor `A4731B39343F` + r1_result `D24B84671C23`（取代 r1 链的 r3_executor/r3_result 仍保留）
8. baseline prior：GLM_2 prior = 22 calls / 0 empty / 22 ok / 0.0% rate（取代 r1 链的 0/0/0/0%）
9. 跨批累计 baseline：r1 末态 **185 calls / 9 empty / 4.86%**（取代 r1 链的 163/9/5.52%）
10. 新增 `glm2_teacher_side_round2_progress` 块（本棒后 GLM_2 教师侧 round 2 完成度 ≥2 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 185/9/4.86%
12. spec_conformance 重写：GLM_2 教师侧 round 2（reask=1），目标 ≥2 successful call
13. per_caption_successful_calls 字段新增 round2_target=2 / round2_met
14. 调度注释：GLM_2 教师侧 round 2（reask=1）22 caption × 1 call
15. glm2_corpus 口径拍板字段（沿 ask_259658a4ffadd6f8aeb12f34）：明示 system prompt 微调口径接受 + 近缘对分布差异如实入 N-26 构造域限制

【边界】
----------
- 不动任何既有件（含 r1 executor `A4731B39343F` + r1 result `D24B84671C23` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `A4F851154551` + r1 executor `E52F930654D3` + r1 result `D245AE4CE385` + models_probe `3C9DC60B5071` + prereg `05B975A86989`）
- 跑不完拆段报断点
...
TEACHER = "GLM_2"  # 沿 r1
TEACHER_LABEL = "GLM_2"  # 顶层字段沿 r1
MODEL_ID_SHARED_WITH = "GLM_1"  # 顶层字段沿 r1
SIDE = "teacher"
...
TARGET_PER_CAPTION_ROUND2 = 2  # 本棒 round 2 完成目标 ≥2 successful/caption
SMOKE_CALL_ONLY = True  # 沿 r1 smoke=glm-5.3 ok=200；本棒仅 1-call smoke 不再做 3-candidate probe

# 选定的 model_id（沿 r1 探活结论，不重探）
...
# glm2_corpus 口径拍板（沿 ask_259658a4ffadd6f8aeb12f34 2026-09-24 23:42 显式）
GLM2_CORPUS_CALIBRATION = {
    ...
    "rounds_covered": "round1 22 calls 有效不作废",
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch3-r2",
        "X-Title": f"deposon-l14v3-batch3-r2-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# 提示构造（沿 r1 + GLM_2 教师身份微调；口径拍板已落定）
# ============================================================
...
# ============================================================
# Smoke（沿 r1 probe 简化版；仅 1 call 确认端点 ok）
# ============================================================
def smoke_glm_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r1 probe 结论 glm-5.3 ok=200 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b3_r2"
    prompt_id = f"glm2_t02_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 3 round 2 · {TEACHER} 教师侧 round 2（reask=1）")
    print("沿 r1 glm-5.3 探活结论 ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r1 (沿 batch1 r6 + batch2 r2)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（GLM_2 r1 末态）
    with open(R1_RESULT_PATH_B3, "r", encoding="utf-8") as f:
        r1_b3 = json.load(f)
    glm2_prior_total = r1_b3["aggregate"]["glm2_cumulative"]["post_total"]
    glm2_prior_empty = r1_b3["aggregate"]["glm2_cumulative"]["post_empty"]
    glm2_prior_ok = r1_b3["aggregate"]["glm2_cumulative"]["post_ok"]
    glm2_prior_rate = r1_b3["aggregate"]["glm2_cumulative"]["post_rate"]
    print(f"[BASELINE GLM_2 r1] total={glm2_prior_total} ok={glm2_prior_ok} empty={glm2_prior_empty} rate={glm2_prior_rate:.4f}")

    # 跨批累计基线 = r1 末态（185 calls / 9 empty / 4.86%）
    cross_prior_total = r1_b3["aggregate"]["cross_batch_with_r3_baseline"]["cross_total"]
    cross_prior_empty = r1_b3["aggregate"]["cross_batch_with_r3_baseline"]["cross_empty"]
    cross_prior_ok = r1_b3["aggregate"]["cross_batch_with_r3_baseline"]["cross_ok"]
    cross_prior_rate = r1_b3["aggregate"]["cross_batch_with_r3_baseline"]["cross_rate"]
    print(f"[BASELINE cross batch (r1 末态)] total={cross_prior_total} ok={cross_prior_ok} empty={cross_prior_empty} rate={cross_prior_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r1 probe 结论 glm-5.3 ok=200）
    print("=" * 60)
    print(f"[SMOKE PHASE] {MODEL_GLM_2} 1-call smoke（沿 r1 probe 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 2,
            "metadata": {
                "task": f"L14V3_batch3_round2_{TEACHER}_teacher_side_round2_reask1",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch3_r2_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b3_r2_need1"} for c in captions_sorted]
    ...
    r1_per_cap = r1_b3.get("per_caption_successful_calls", {})
    ...
    for i, item in enumerate(planned):
        ...
        if r2_total > 0:
            current_batch_rate = r2_empty / r2_total
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=True,  # 语义：本棒 = GLM_2 教师侧 data collection round 2
        ...
        if r2_total > 0:
            batch_rate_post = r2_empty / r2_total
    ...
    # 11. GLM_2 教师侧 round 2 进度块
    glm2_round2_progress = {
    ...
    model_id_disclosure = (
        ...
        f"r2 沿 r1 结论仅做 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                f"PI 2026-09-24 batch3 GLM_2 教师侧 round 2 续采 (reask=1); "
                f"沿 r1 GLM_2 glm-5.3 同 model_id 复用结论 (1-call smoke ok=200); "
                ...
                f"目标 ≥2 successful/caption (round 2 完成)"
            ),
            "teacher": TEACHER,
            "teacher_label": TEACHER_LABEL,
            "model_id_shared_with": MODEL_ID_SHARED_WITH,
            ...
            "endpoint_selection_method": (
                "沿 r1 实测 (teamo 端点); r2 沿用 teamo 端点; "
            ...
            "tools_fault_param_correction": {
                ...
                "b3_r2_status": "b3_r2 GLM_2 沿 r1 结论仅做 1-call smoke；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r1 glm-5.3 探活结论；本棒节省预算避免重复 probe；"
            ...
            "diff_vs_r1_executor": [
                "1. ROUND=2 (升 r2 命名避开 r1 重叠)；BATCH=3 不变",
                "2. prompt_id 前缀 glm2_t02_teacher_* (取代 r1 的 glm2_t01_teacher_*)；后缀 _b3_r2 (取代 _b3_r1)",
                "3. REASK_R1=1 (本棒每 caption 第 2 call；取代 r1 的 reask_idx=0)",
                "4. phase_label: b3_r2_need1 (取代 r1 的 b3_r1_need1)",
                "5. is_round_2=True (语义: GLM_2 教师侧 data collection round 2)",
                "6. predecessor_sha12 链: r1_executor A4731B39343F + r1_result D24B84671C23 (取代 r1 链的 r3_executor/r3_result 仍保留)",
                "7. baseline prior: GLM_2 prior = 22 calls / 0 empty / 22 ok / 0.0% rate (取代 r1 链的 0/0/0/0%)",
                "8. 跨批累计 baseline: r1 末态 185 calls / 9 empty / 4.86% (取代 r1 链的 163/9/5.52%)",
                "9. 新增 glm2_teacher_side_round2_progress 块 (round 2 完成度 ≥2 successful/caption)",
                "10. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 185/9/4.86%",
                "11. spec_conformance 重写: GLM_2 教师侧 round 2 (reask=1)",
                "12. per_caption_successful_calls 字段新增 round2_target=2 / round2_met",
                "13. 调度注释: GLM_2 教师侧 round 2 (reask=1) 22 caption × 1 call",
                "14. glm2_corpus_calibration 字段新增 (沿 ask_259658a4ffadd6f8aeb12f34 口径拍板)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "b3_r2 沿 r1 glm-5.3 探活结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；b3_r2 与 r1/r3/r2/r1(b2)/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        "models_listing_summary_ref": {
            ...
            "note": (
                "b3_r2 不重探 /v1/models；仅引用 r2 探活清单 (sha12=3C9DC60B5071) "
        ...
        "glm2_teacher_side_round2_progress": glm2_round2_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch3_round2_round2_reask1": {
                ...
                "note": "b3_r2 GLM_2 教师侧 round 2 reask=1 22 caption × 1 call",
            },
            "batch3_round2_cumulative_glm2": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round2_reask1_n_distinct_sample_note": (
                    f"round 2 reask=1 第 1 call 实测；本棒后 GLM_2 教师侧 round 2 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"GLM_2 prior = {glm2_prior_total} calls / {glm2_prior_empty} empty / {glm2_prior_rate:.4f} (r1 末态); "
                    f"cross-batch rate = {cross_batch_rate:.4f}; "
                    f"baseline cross batch = {cross_prior_total} calls / {cross_prior_empty} empty / {cross_prior_rate:.4f} (r1 末态)"
                ),
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 2 (reask=1) 已完; 下棒可接力 GLM_2 教师侧 round 3 (>=3 more successful/caption)"
            ),
            "note": (
                f"本棒 round 2 (reask=1): actual={r2_total}/{planned_total} calls; "
                f"GLM_2 教师侧 round 2 完成度 = {met_round2_count}/{len(captions)} caption met round 2 目标 (≥2 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"GLM_2 model_id 沿 r1 探活选定 = {model_id} (与 GLM_1 同 model_id 不同教师；1-call smoke 确认仍可用)"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
