# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch8_r5_executor.py
=====================================

V4 L14+ batch 8 · round 5（reask=4）—— GLM_2 蒸馏侧 **收官棒**（沿 r4 口径全套）
==================================================================================

【派工依据】
----------------
- 沿 r4 `results/_v4_supp_l14v3_batch8_r4_executor.py`（SHA-12 `6b69ec8406d4`）+ r4 产物
  `results/_v4_supp_l14v3_batch8_r4_result.json`（SHA-12 `c7d5aa2a9b00`）口径续跑（GLM_2 蒸馏侧 round 4 22/22 caption ≥4 successful 接力达成）
- 沿 r3 `results/_v4_supp_l14v3_batch8_r3_executor.py`（SHA-12 `0f654d945f12`）+ r3 产物
  `results/_v4_supp_l14v3_batch8_r3_result.json`（SHA-12 `56faf9324fba`）口径续跑（GLM_2 蒸馏侧 round 3 22/22 caption ≥3 successful 接力达成）
- 沿 r2 `results/_v4_supp_l14v3_batch8_r2_executor.py`（SHA-12 `6a9bbc43c808`）+ r2 产物
  `results/_v4_supp_l14v3_batch8_r2_result.json`（SHA-12 `7b23996b5f4c`）口径续跑（GLM_2 蒸馏侧 round 2 22/22 caption ≥2 successful 续采达成）
...
- 沿 batch6 r5 `results/_v4_supp_l14v3_batch6_r5_executor.py`（SHA-12 `8a8babb911b8`）+ r5 产物
  `results/_v4_supp_l14v3_batch6_r5_result.json`（SHA-12 `e40424bf8abe`）口径续跑（kimi 蒸馏侧 22/22 caption 5/5 successful 收官）—— GLM_2 蒸馏侧收官沿此口径类比
...
【本棒范围（沿 dispatch）—— GLM_2 蒸馏侧 round 5 收官棒】
--------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 dispatch 字面）
2. **计数口径**：
   - **distill 侧独立计数**：`glm2_t01_distill_*` prompt_id 前缀为域（沿 r4 字面）
   - 目标 = GLM_2 蒸馏侧 round 5 ≥5 successful call/caption（**各 caption 由 4/5 顺利补至 5/5 —— 收官**）
   - 当前 GLM_2 蒸馏侧 = 22 caption 各 4 successful（沿 r1+r2+r3+r4 quadruples 过滤；r1+r2+r3+r4 88/88 全 OK）
   - 本棒补 1 call/caption → GLM_2 蒸馏侧各 caption 由 4 → 5 successful（**round 5 收官判定**）
3. **model 口径**（沿 r4）：
   - `model_id_sent` = `glm-5.3`（teamo 端点；与 GLM_1 现行采样复用）
   - `teacher_label` = `GLM_2`
   - `model_id_shared_with` = `GLM_1`（GLM_2 = 同 model 不同语料字面沿 mapping §1.2 派生字段）
4. **system prompt 沿用**（沿 r4 `PROMPT_SYSTEM` 字面不重写）：
...
5. **收官判定块 `glm2_distill_side_finish`**：
   ...
   - `finish_disposition`：GLM_2 蒸馏侧全 22 caption 5/5 successful 达标 → batch8 蒸馏侧收官 ✓
   - **收官棒**（沿 batch6 r5 收官口径类比；本棒 = GLM_2 蒸馏侧收官）

【每 caption successful 计数终盘点（沿 dispatch §6）—— GLM_2 蒸馏侧独立域】
----------------------------------------------------------------------
- 本棒前基线（GLM_2 蒸馏侧独立计数）：GLM_2 蒸馏侧 = 22 caption 各 4 successful（沿 r1+r2+r3+r4 quadruples 过滤；r1+r2+r3+r4 88/88 全 OK）
- 本棒增量计算：每 call 后实时更新 `per_cap_succ_glm2_distill` 字典
- 终盘结果（GLM_2 蒸馏侧独立计数）：
  - `per_caption_glm2_distill_only_successful_calls` 块（GLM_2 蒸馏侧独立计数）
  - `glm2_distill_side_finish` 块（**收官判定**）
  - `dispatch_target_progress_glm2_distill` 块（GLM_2 蒸馏侧目标进展 — 收官态）
...
-------------------------------------------
- 累计 = batch8 r4 终态 + batch8 r5 GLM_2 增量（GLM_2 蒸馏侧 round 5）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r4 产物）：
  - prior (b1+r2+r3+r4+r5+b6r1+r6r2+r6r3+r6r4+r6r5+b8r1+b8r2+b8r3+b8r4): total=296, empty=9, **rate=3.04%**
  - r5 worst case (22 EMPTY): cumulative = 31/318 = 9.75% (仍 < 50% 阈值)
  - r5 best case (0 EMPTY): cumulative = 9/318 = 2.83% (仍 < 50% 阈值)
...
- 派生 JSON 不合并（本棒 `_batch8_r5_*` 与前棒所有 _batch*_r* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch8_r5_result.json`
  （schema = v4_l14v3_n26/1 + batch=8 + round=5 + teacher=GLM_2）
- 不覆盖 batch8 r4 result `c7d5aa2a9b00` 或前棒 result（沿 dispatch §4）

【与 r4 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------------
1. **ROUND = 5 / REASK_R1 = 4** 参数化（r4 = 4/3；本棒 = 5/4；reask_idx=4 = 第 5 call/caption on GLM_2 distill side）
2. **prompt_id 后缀**：`_r4` → `_r5`
3. **baseline 切换**：
   - 跨批累计：batch8 r4 终态 cumulative (296/9/287/3.04%)
   - GLM_2 蒸馏侧独立：22 caption 各 4 successful（沿 r1+r2+r3+r4 quadruples 过滤）
4. **predecessor_sha12 链新增**：batch8 r4 executor `6b69ec8406d4` + r4 result `c7d5aa2a9b00`
5. **target 切换**：≥4 successful/caption (r4 接力阈值) → ≥5 successful/caption (r5 收官阈值)
6. **判定块命名**（**收官棒关键差异**）：
   - `glm2_distill_side_round4_progress` (r4 接力) → `glm2_distill_side_finish` (r5 收官；沿 batch6 r5 收官口径类比)
   - 字段 `met_target_round4` → `met_target`（统一为收官字段）
   - 字段 `still_pending_round4` → `still_pending`
   - 字段 `round4_progress_disposition` → `finish_disposition`
   - 字段 `rounds_to_reach_round4_target` → `rounds_to_reach_target`
   - 字段 `glm2_distill_side_round4_target_met_at_round` → `glm2_distill_side_target_met_at_round`
7. **is_finish_round 标记**：metadata.is_finish_round = True (本棒 = 收官棒；沿 batch6 r5 收官口径)
8. **mapping_sha12_discrepancy_note 沿用**（不动 mapping 文件）
9. **diff_vs_r4_executor 新块**（取代 r4 的 diff_vs_r3_executor）
10. **r5 增量计数 → GLM_2 蒸馏侧各 caption 由 4/5 顺利补至 5/5**（**收官** if no fails）
...
BATCH8_R2_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch8_r2_result.json")
BATCH8_R3_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch8_r3_result.json")
BATCH8_R4_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch8_r4_result.json")
...
TARGET_SUCCESSFUL_PER_CAPTION = 5  # GLM_2 蒸馏侧 round 5 收官阈值（≥5 successful calls/caption；各 caption 由 4 → 5 successful；收官）
TARGET_FINAL_PER_CAPTION = 5  # GLM_2 蒸馏侧最终目标阈值（5 successful calls/caption；本棒收官 = 达 final 阈值）
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 8 round 5 | GLM_2 distill side | reask=4 — **收官棒**")
    print(f"模型: glm-5.3 (teamo; GLM_2 复用 GLM_1 = 同 model 不同语料)")
    print(f"计划 calls = 22 (22 caption × 1 call)")
    print(f"计数口径: GLM_2 蒸馏侧独立 (prompt_id 前缀 glm2_t01_distill_) — 沿 r4")
    print(f"GLM_2 蒸馏侧 baseline: 22 caption 各 4 successful (沿 r1+r2+r3+r4 quadruples 过滤; r1+r2+r3+r4 88/88 全 OK)")
    print(f"目标: GLM_2 蒸馏侧 ≥5 successful calls/caption (round 5 收官阈值 = final)")
    print(f"收官判定块: glm2_distill_side_finish.all_22_captions_met_target (沿 batch6 r5 收官口径类比)")
    ...
    # 2. 加载 r1 + r2 + r3 + r4 + batch6 r5 + batch2 r2 result（用于 baseline + 锚链）
    with open(BATCH8_R1_RESULT_PATH, "r", encoding="utf-8") as f:
        b8r1 = json.load(f)
    with open(BATCH8_R2_RESULT_PATH, "r", encoding="utf-8") as f:
        b8r2 = json.load(f)
    with open(BATCH8_R3_RESULT_PATH, "r", encoding="utf-8") as f:
        b8r3 = json.load(f)
    with open(BATCH8_R4_RESULT_PATH, "r", encoding="utf-8") as f:
        b8r4 = json.load(f)
    ...
    # 3. GLM_2 蒸馏侧独立计数（filter by prompt_id 前缀）—— 本棒前 = r1+r2+r3+r4 quadruples
    ...
    for src in [b8r1, b8r2, b8r3, b8r4]:
    ...
    print(f"[GLM_2 DISTILL prior] total={cum_total_prior_glm2} ok={cum_ok_prior_glm2} "
          f"empty={cum_empty_prior_glm2} (源 = r1+r2+r3+r4 quadruples distill 前缀过滤; 各 caption 各 4 successful)")

    # 4. 跨批累计监控（沿 r4 cumulative 链路 + GLM_2 r5 增量）
    r4_cum = b8r4["aggregate"]["cumulative"]
    ...
    PLANNED_CALLS = [
        {"caption_id": c["id"], "phase": f"r5_glm2_distill_{c['id']}"}
        for c in captions
    ]
    ...
    for i, item in enumerate(planned):
        ...
        target_met_glm2_distill = "[OK] 接力" if cur_succ_glm2_distill >= TARGET_SUCCESSFUL_PER_CAPTION else "[..]"
        print(
    ...
    # 11. GLM_2 蒸馏侧 **收官判定**（沿 dispatch 字面块名 `glm2_distill_side_finish`；沿 batch6 r5 收官口径类比）
    glm2_distill_side_finish = {
        "all_22_captions_met_target": met_target_count_glm2_distill == len(captions),
        "met_count_post_r5": met_target_count_glm2_distill,
        "total_captions": len(captions),
        "still_pending": sorted([
            cid for cid, info in per_caption_glm2_distill_only_successful_calls.items() if not info["met_target"]
        ]),
        "finish_disposition": (
            f"GLM_2 蒸馏侧全 22 caption 5/5 successful 达标 → batch8 蒸馏侧收官 ✓"
            if met_target_count_glm2_distill == len(captions)
            else f"GLM_2 蒸馏侧未达标 {len(captions) - met_target_count_glm2_distill} caption → 留 worker 下一棒接力（未达收官）"
        ),
        "rounds_to_reach_target": 5,
        "rounds_to_reach_target_actual": (
            "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 GLM_2 蒸馏侧 calls"
        ),
        "glm2_distill_side_target_met_at_round": (
            "r5" if met_target_count_glm2_distill == len(captions) else "未达 (next 棒接力)"
        ),
    }

    # 12. dispatch §6 收尾盘点 —— GLM_2 蒸馏侧（收官态）
    dispatch_target_progress_glm2_distill = {
        "target": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (GLM_2 distill side only; 收官阈值 = final)",
        "target_final": f">={TARGET_FINAL_PER_CAPTION} successful calls/caption (GLM_2 distill side only; 最终收官阈值)",
        "met_count_post_r5": met_target_count_glm2_distill,
        "total_captions": len(captions),
        "remaining_count": len(captions) - met_target_count_glm2_distill,
        ...
        "met_percentage": round(met_target_count_glm2_distill / len(captions) * 100, 2),
    ...
    result = {
        ...
        "round": ROUND,
        "metadata": {
            "task": "L14V3_batch8_round5_GLM_2_distill_side_reask4_finish_round_independent_counting",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 8 round 5 GLM_2 distill side reask=4 收官棒",
            "is_finish_round": True,
            "is_round_5_finish_round": True,
            ...
            "prompt_construction_basis": {
                ...
                "reproduction_source_anchor_sha12": "6b69ec8406d4",
                "reproduction_source_anchor_path": "results/_v4_supp_l14v3_batch8_r4_executor.py",
                "reproduction_source_anchor_role": "r5 沿 r4 口径续跑 (GLM_2 蒸馏侧 22 caption × 5 calls 系列 round 5 收官)",
                "v3_distill_pipeline_style_inherited": (
                    "V3-style concept-graph distillation continuation prompt (沿 r4 executor 6b69ec8406d4 + batch6 r5 executor 8a8babb911b8);"
            ...
            "spec_conformance": (
                ...
                "retry 修复沿 batch6 r5 + 每 caption successful 计数终盘点 + "
                "K-N26-N2 累计监控 + GLM_2 蒸馏侧独立计数（沿 r1+r2+r3+r4 quadruples 过滤） + "
                "**收官判定块 glm2_distill_side_finish** (沿 batch6 r5 收官口径类比)"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            ...
            "tools_fault_param_correction": {
                ...
                "r2_status_anchor": "r2 沿 r1; r2 22/22 OK (1 retry, ssl_error)",
                "r3_status_anchor": "r3 沿 r2; r3 22/22 OK (0 retry)",
                "r4_status_anchor": "r4 沿 r3; r4 22/22 OK (0 retry)",
                "r5_status": "retry 修复沿 batch6 r5; r5 收官棒仅按 22 caption × 1 call 续采; hard fail 保持 fail path 立即返回",
                "rationale": (
                    "沿 batch6 r5 修复 + tracking 改进；r5 收官棒仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r4_executor": [
                "1. ROUND=5/REASK_R1=4 参数化 (r4 = 4/3; 本棒 = 5/4; reask_idx=4 = 第 5 call/caption on GLM_2 distill side)",
                "2. prompt_id 后缀 _r4 → _r5",
                "3. baseline 切换: 跨批累计 = r4 终态 cumulative (296/9/287/3.04%); GLM_2 蒸馏侧独立 = 22 caption 各 4 successful (沿 r1+r2+r3+r4 quadruples 过滤)",
                "4. predecessor_sha12 链新增 r4_executor 6b69ec8406d4 + r4_result c7d5aa2a9b00",
                "5. target 切换: ≥4 successful/caption (r4 接力阈值) → ≥5 successful/caption (r5 收官阈值 = final)",
                "6. 判定块命名: glm2_distill_side_round4_progress (r4 接力) → glm2_distill_side_finish (r5 收官; 沿 batch6 r5 收官口径类比); 字段 met_target_round4 → met_target; still_pending_round4 → still_pending; round4_progress_disposition → finish_disposition; rounds_to_reach_round4_target → rounds_to_reach_target; glm2_distill_side_round4_target_met_at_round → glm2_distill_side_target_met_at_round",
                "7. is_finish_round 标记: metadata.is_round_4_progress_round = True (r4) → is_finish_round = True (r5 = 收官棒)",
                "8. mapping_sha12_discrepancy_note 沿用 (不动 mapping 文件)",
                "9. diff_vs_r4_executor 新块 (取代 r4 的 diff_vs_r3_executor)",
                "10. **r5 增量计数 → GLM_2 蒸馏侧各 caption 由 4/5 顺利补至 5/5 收官** (r5 OK 全达标 if no fails)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "glm2_distill_side_finish": glm2_distill_side_finish,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_5_n_distinct_sample_note": (
                    "round 5 收官棒第 5 call per caption 实测；GLM_2 蒸馏侧独立计数 = "
                    f"22 caption 各 {per_cap_succ_glm2_distill[list(per_cap_succ_glm2_distill.keys())[0]]} successful (沿 r1+r2+r3+r4+r5 quadruples, prompt_id 前缀 glm2_t01_distill_ filter 域);"
                    f"**GLM_2 蒸馏侧 round 5 收官判定**: {met_target_count_glm2_distill}/{len(captions)} caption 达 5/5 successful;"
                    "GLM_2 蒸馏侧收官达成 (沿 batch6 r5 收官口径类比);"
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        "本棒未触发 K-N26-N2 累计阈值；跑至 1500s watchdog 提前停批 / 全 22 calls 完成 (round 5 收官态)"
                    )
                ),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "本棒收官 — GLM_2 蒸馏侧 22/22 caption 5/5 successful 收官；无 next 棒"
            ),
            "note": (
                ...
                + (f"；K-N26-N2 累计阈值触发：cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   if k_n26_n2_triggered else
                   f"；撞 {WALL_TIME_BUDGET_S}s watchdog 提前停批 ({len(captions) - met_target_count_glm2_distill} caption 未达 round 5 收官阈值) 走 task_append 接力棒"
                   if wall_time_s > WALL_TIME_BUDGET_S and met_target_count_glm2_distill < len(captions) else
                   f"；**GLM_2 蒸馏侧 round 5 收官达成** {met_target_count_glm2_distill}/{len(captions)} caption 5/5 successful ✓"
                   if met_target_count_glm2_distill == len(captions) else
                   f"；GLM_2 蒸馏侧未达标 {len(captions) - met_target_count_glm2_distill} caption → 下一棒接力 (未达收官)")
            ),
        },
        "self_scan": {
    ...
    # 17. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch8_r5_result.json")
    ...
    print(f"  glm2_distill_side_finish_met: {met_target_count_glm2_distill}/{len(captions)}")
    print(f"  *** all_22_captions_met_target: {glm2_distill_side_finish['all_22_captions_met_target']} ***")
    print(f"  finish_disposition: {glm2_distill_side_finish['finish_disposition']}")
    ...
    if glm2_distill_side_finish["still_pending"]:
        print(f"  still_pending: {glm2_distill_side_finish['still_pending']}")
    return 0
