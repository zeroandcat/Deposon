# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch2_r6_executor.py
=====================================

V4 L14+ batch 2 · round 6 · GLM_1 教师侧 round 5 收官棒（reask=4）
===================================================================

【派工依据】
----------------
- 沿 batch2 r5 executor `56B37DB28600` + r5 result `479F4119CF72` 口径（基础口径锚）
- 沿 batch2 r4 executor `D9FE7AA2D292` + r4 result `15B8DDC5BA9E` smoke 结论
- 沿 batch2 r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` smoke 结论
- 沿 batch2 r2 executor `E7418F47A130` + r2 result `8C125E257AF8` 1-call probe 结论
- 沿 batch1 r6 executor `5f02c7e0f094` + r6 result `A4F851154551` retry 修复口径 + 收官块惯例 (kimi_teacher_side_finish)
...
- 派工单（沿 2026-09-24 23:43 接力棒）：GLM_1 教师侧 r6 收官棒 (reask=4) 续采
- 拍板依据：ask_748d9242c7a6be3d83de63a0 (2026-09-24 23:08 显式)：GLM_1=glm-5.3 已在跑（沿 r2/r3/r4/r5）

【本棒范围（沿 dispatch）—— GLM_1 教师侧 round 5（reask=4）收官棒】
----------------------------------------------------------
1. **沿用 glm-5.3**（r5 1-call smoke ok=200；本棒不再探活，仅 1-call smoke 防端点飘移）
2. **22 caption × 1 call**（每位 caption 第 5 call；reask_idx=4；目标 5/5 successful/caption 收官）
3. **目标**：本棒后 22/22 caption 各 ≥5 successful call；**GLM_1 教师侧收官**
4. 预算 ≤22 caption calls + ≤1 smoke calls / ≤600s watchdog
5. 跑不完如实报断点

【每 caption successful 计数（沿 dispatch §6）】
---------------------------------------------------------
- 本棒前基线（沿 batch2 r5）：GLM_1 教师侧 = 88 calls / 0 empty / 88 ok / 0.0000 rate
- 本棒 round 5（reask=4）后：22/22 caption 各 ≥5 successful call（理论值；如端点飘移可能回落）
- 收官判定块：`glm1_teacher_side_finish.all_22_captions_met_target`（沿 batch1 r6 kimi_teacher_side_finish 惯例）
- 22/22 达 5/5 即报「GLM_1 教师侧收官」
- 终盘结果：result.json 顶层 `per_caption_successful_calls` 块 + `dispatch_target_progress` 块 + `glm1_teacher_side_finish` 块
...
--------------------------------------------------------------------
- 跨批累计 baseline = batch1 终态 119 + batch2 r2 (22) + batch2 r3 (22) + batch2 r4 (22) + batch2 r5 (22) → **207 calls / 9 empty / 4.35%**（沿 r5 result）
- 累计 = r5 baseline + batch2 r6（GLM_1 自身 r5 末 88/0/88/0.0000）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）

【端点取舍（沿 r5）】
----------------------------
- teamo + tun + glm-5.3（r5 已 smoke ok=200；本棒不再做完整 3-candidate probe，仅 1-call smoke）
...
- 派生 JSON 不合并（本棒 `_batch2_r6_*` 与 `_batch2_r5_*` / `_batch2_r4_*` / `_batch2_r3_*` / `_batch2_r2_*` / `_batch2_r1_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r5 executor `56B37DB28600` + r5 result `479F4119CF72` + r4 + r3 + r2 + r1 + batch1 r6 + models_probe 全套不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch2_r6_result.json`（schema = `v4_l14v3_n26/1` + batch=2 + round=6）
- 不覆盖 r5 result `479F4119CF72` 或 r4 result `15B8DDC5BA9E` 或 r3 result `25251ACA6D79` 或 r2 result `8C125E257AF8` 或 batch1 r6 result `A4F851154551`
- predecessor_sha12 链：r5_executor/r5_result + r4_executor/r4_result + r3_executor/r3_result + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r5 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 6（升 r6 命名避开 r5 重叠）；BATCH = 2 不变
2. prompt_id 后缀 `_b2_r6`；t05 = teacher side round 5（取代 r5 的 t04 round 4 reask=3）
3. reask_idx = 4（本棒每位 caption 第 5 call；取代 r5 的 reask_idx=3）
4. phase_label：`b2_r6_need1`（取代 r5 的 `b2_r5_need1`）
5. round5_target = 5（取代 r5 的 round4_target=4）；is_round_2 = False（沿 r5）
6. smoke_call_only = True（沿 r5；不再做 3-candidate probe fallback）
7. predecessor_sha12 链：r5_executor `56B37DB28600` + r5_result `479F4119CF72`（新增；r4/r3/r2/r1/batch1_r6 全套沿 r5）
8. baseline prior：r5 终态 88 calls / 0 empty / 88 ok / 0.0000 rate（取代 r5 链的 66/0/66）
9. 跨批累计 baseline：r5 终态 207 calls / 9 empty / 4.35% rate（取代 r5 链的 185/9/4.86%）
10. 新增 `glm1_teacher_side_round5_progress` 块（本棒后 GLM_1 教师侧 round 5 完成度 ≥5 successful/caption）
11. **新增 `glm1_teacher_side_finish` 块**（沿 batch1 r6 kimi_teacher_side_finish 惯例；22/22 达 5/5 即报 GLM_1 教师侧收官）
12. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 207/9/4.35%
13. spec_conformance 重写：GLM_1 教师侧 round 5 收官棒 (reask=4)，目标 ≥5 successful/caption 收官
14. dispatch_target_progress 字段：target_round5 / still_pending_captions_round5
15. per_caption_successful_calls 字段新增 round5_target=5 / round5_met
16. http referer/title 字段更新 l14v3-batch2-r6 (取代 r5 的 r5)
17. 注：本棒为 GLM_1 教师侧收官棒（target_successful_per_caption=5）

【边界】
----------
- 不动任何既有件（含 r5 executor `56B37DB28600` + r5 result `479F4119CF72` + r4 executor `D9FE7AA2D292` + r4 result `15B8DDC5BA9E` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `A4F851154551` + r1 executor `E52F930654D3` + r1 result `D245AE4CE385` + models_probe `3C9DC60B5071` + prereg `05B975A86989`）
- 跑不完拆段报断点
...
R3_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch2_r3_result.json")
R4_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch2_r4_result.json")
R5_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch2_r5_result.json")
...
KEY_INDEX_TEAMO_1BASED = 15  # 沿 r5 (line 15 = teamo key)
USE_PROXY = True
...
# 本棒调度（沿 dispatch）
BATCH = 2
ROUND = 6  # 升 r6 命名避开 r5 重叠
TEACHER = "GLM_1"
SIDE = "teacher"
REASK_R1 = 4  # 本棒 round 5 = 每 caption 第 5 call（reask_idx=4）= 收官
TARGET_SUCCESSFUL_PER_CAPTION = 5  # 收官目标
SMOKE_CALL_ONLY = True  # 沿 r5 smoke=glm-5.3 ok=200；本棒仅 1-call smoke 不再做 3-candidate probe

# 选定的 model_id（沿 r5 smoke 结论，不重探）
MODEL_GLM_1 = "glm-5.3"
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch2-r6",
        "X-Title": f"deposon-l14v3-batch2-r6-{TEACHER}-teacher-finish",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r5；仅 1 call 确认端点 ok）
# ============================================================
def smoke_glm_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r5 smoke 结论 glm-5.3 ok=200 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b2_r6"
    prompt_id = f"glm1_t05_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 2 round 6 · GLM_1 教师侧 round 5（reask=4）收官棒")
    print("沿 r5 glm-5.3 smoke 结论 ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    print("计划 calls = 22 caption × 1 call = 22 calls + 1 smoke call")
    print("目标: 收官 22/22 caption 各 ≥5 successful call (5/5 全部完成)")
    print("预算 ≤22 caption calls + 1 smoke call / ≤600s watchdog")
    print("retry 修复沿 r5 (沿 batch1 r6)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（batch1 r6 + batch2 r1 + batch2 r2 + batch2 r3 + batch2 r4 + batch2 r5）
    ...
    # r5 终态 = 本棒前 GLM_1 自身累计基线
    glm1_prior_total = r5["aggregate"]["glm1_cumulative"]["post_total"]
    glm1_prior_empty = r5["aggregate"]["glm1_cumulative"]["post_empty"]
    glm1_prior_ok = r5["aggregate"]["glm1_cumulative"]["post_ok"]
    glm1_prior_rate = r5["aggregate"]["glm1_cumulative"]["post_rate"]
    print(f"[BASELINE r5 GLM_1] total={glm1_prior_total} ok={glm1_prior_ok} empty={glm1_prior_empty} rate={glm1_prior_rate:.4f}")

    # 跨批累计基线 = r5 终态
    cross_prior_total = r5["aggregate"]["cross_batch_with_r4_baseline"]["cross_total"]
    cross_prior_empty = r5["aggregate"]["cross_batch_with_r4_baseline"]["cross_empty"]
    cross_prior_ok = r5["aggregate"]["cross_batch_with_r4_baseline"]["cross_ok"]
    cross_prior_rate = r5["aggregate"]["cross_batch_with_r4_baseline"]["cross_rate"]
    ...
    # 5. Smoke 1-call（沿 r5 smoke 结论 glm-5.3 ok=200）
    print("=" * 60)
    print("[SMOKE PHASE] glm-5.3 1-call smoke（沿 r5 smoke 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 6,
            "metadata": {
                "task": "L14V3_batch2_round6_GLM1_teacher_side_round5_reask4_finish",
                "prereg_sha12": "05B975A86989",
                "prereg_path": "results/_v4_supp_prereg_v02_add_L14V3_2026_09_24.md",
                "predecessor_sha12": {
                    "r5_executor_sha12": "56B37DB28600",
                    "r5_executor_path": "results/_v4_supp_l14v3_batch2_r5_executor.py",
                    "r5_result_sha12": "479F4119CF72",
                    "r5_result_path": "results/_v4_supp_l14v3_batch2_r5_result.json",
                    "r4_executor_sha12": "D9FE7AA2D292",
                    "r4_executor_path": "results/_v4_supp_l14v3_batch2_r4_executor.py",
                    "r4_result_sha12": "15B8DDC5BA9E",
                    "r4_result_path": "results/_v4_supp_l14v3_batch2_r4_result.json",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch2_r6_result.json")
    ...
    # 6. 调度：22 caption × 1 call (round 5 reask=4) 按 caption_id 字母序 = 收官
    captions_sorted = sorted(captions, key=lambda c: c["id"])
    planned = [{"caption_id": c["id"], "phase": "b2_r6_need1"} for c in captions_sorted]
    planned_total = len(planned)
    print(f"[PLANNED] total: {planned_total} (22 caption × 1 call) = 收官棒")
    ...
    # 从 r5 累计 per_caption_successful_calls 起步
    r5_per_cap = r5.get("per_caption_successful_calls", {})
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=False,  # 语义：本棒 = GLM_1 教师侧 data collection round 5 收官（沿 r5）
    ...
    # 跨批累计 = r5 baseline + GLM_1 本棒
    cross_batch_total = cross_prior_total + r6_total
    ...
    for cid in [c["id"] for c in captions]:
        ...
        if met_target:
            ...
            pending_target_cids.append(cid)
    ...
    # 11. GLM_1 教师侧 round 5 进度块（收官前最后进度）
    glm1_round5_progress = {
        "round5_target": ">=5 successful call/caption (round 5 reask=4) = 收官",
        "met_count_post_r6_round5": met_round5_count,
        ...
        "still_pending_captions_to_target": sorted(pending_target_cids),
    }

    # 12. **收官判定块**（沿 batch1 r6 kimi_teacher_side_finish 惯例）
    glm1_teacher_side_finish = {
        "all_22_captions_met_target": met_target_count == 22,
        "finish_disposition": (
            "GLM_1 教师侧全 22 caption 达标 → GLM_1 教师侧收官"
            if met_target_count == 22
            else f"GLM_1 教师侧 {met_target_count}/22 caption 达标；未收官（{len(pending_target_cids)} caption 待补）"
        ),
        "met_count_post_r6": met_target_count,
        "still_pending": sorted(pending_target_cids),
        "total_captions": 22,
        "round5_target": 5,
        "round5_met_count": met_round5_count,
        "round5_met_all_22": met_round5_count == 22,
        "is_round5_pass": met_round5_count == 22,
        "is_target_pass": met_target_count == 22,
        "convention_source": "沿 batch1 r6 kimi_teacher_side_finish 惯例 (sha12=A4F851154551)",
        "freeze_disposition": (
            "GLM_1 教师侧收官冻结" if met_target_count == 22 else "GLM_1 教师侧未收官冻结"
        ),
    }

    # 13. retry validation 块
    ...
    # model_id_inconsistency_honest_disclosure（沿 r5 + r6 沿用）
    ...
    model_id_disclosure = (
        ...
        f"r2 1-call probe ok=200; r3/r4/r5/r6 1-call smoke ok=200; r6 沿 r5 结论仅做 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                "PI 2026-09-24 batch2 GLM_1 教师侧 round 5 收官棒 (reask=4); "
                "沿 r5 glm-5.3 smoke 结论 (1-call smoke ok=200); r6 仅 1-call smoke; "
                "22 caption × 1 call = 22 calls (按 caption_id 字母序) = 收官; "
                "目标 ≥5 successful/caption 全 22/22 达标即 GLM_1 教师侧收官"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": 6,
            ...
            "tools_fault_param_correction": {
                ...
                "r3_b2_status": "r3 1-call smoke ok=200 确认 glm-5.3 仍可用",
                "r4_b2_status": "r4 1-call smoke ok=200 确认 glm-5.3 仍可用",
                "r5_b2_status": "r5 1-call smoke ok=200 确认 glm-5.3 仍可用",
                "r6_b2_status": "r6 仅 1-call smoke (沿 r5 结论)；收官棒；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r5 glm-5.3 smoke 结论；r6 节省预算避免重复 probe；"
                    "smoke 失败立即 abort 不重探；本棒为 GLM_1 教师侧收官棒"
                ),
            },
            ...
            "diff_vs_r5_executor": [
                "1. ROUND=6 (升 r6 命名)；BATCH=2 不变",
                "2. prompt_id 后缀 _b2_r6；t05_teacher (取代 r5 的 t04_teacher round 4 reask=3)",
                "3. reask_idx=4 (本棒每 caption 第 5 call；取代 r5 的 reask_idx=3)",
                "4. phase_label: b2_r6_need1 (取代 r5 的 b2_r5_need1)",
                "5. is_round_2=False (沿 r5; GLM_1 教师侧 data collection round 5 收官)",
                "6. smoke_call_only=True (沿 r5；不再做 3-candidate probe)",
                "7. predecessor_sha12 链: r5_executor 56B37DB28600 + r5_result 479F4119CF72 (新增；r4/r3/r2/r1/batch1_r6 全套沿 r5)",
                "8. baseline prior: r5 终态 88 calls / 0 empty / 88 ok (取代 r5 链的 66/0/66)",
                "9. 跨批累计 baseline: r5 终态 207 calls / 9 empty / 4.35% (取代 r5 链的 185/9/4.86%)",
                "10. 新增 glm1_teacher_side_round5_progress 块 (round 5 完成度 ≥5 successful/caption)",
                "11. **新增 glm1_teacher_side_finish 块 (沿 batch1 r6 kimi_teacher_side_finish 惯例)**",
                "12. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 207/9/4.35%",
                "13. spec_conformance 重写: round 5 收官棒 (reask=4)",
                "14. per_caption_successful_calls 字段新增 round5_target=5 / round5_met",
                "15. 调度注释: round 5 收官棒 (reask=4) 22 caption × 1 call",
                "16. http referer/title 字段更新 l14v3-batch2-r6 (取代 r5 的 r5)",
                "17. 注：本棒为 GLM_1 教师侧收官棒（target_successful_per_caption=5）；22/22 达 5/5 即 GLM_1 教师侧收官",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "r6 沿 r5 glm-5.3 smoke 结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；r6 与 r5/r4/r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并); "
                "本棒为 GLM_1 教师侧收官棒；收官判定块 glm1_teacher_side_finish 沿 batch1 r6 kimi_teacher_side_finish 惯例"
            ),
        },
        ...
        "glm1_teacher_side_round5_progress": glm1_round5_progress,
        "glm1_teacher_side_finish": glm1_teacher_side_finish,
        "dispatch_target_progress": {
            ...
            "still_pending_captions_cumulative_to_target": sorted(pending_target_cids),
            "finish_state": glm1_teacher_side_finish["finish_disposition"],
        },
        "aggregate": {
            ...
            "cross_batch_with_r5_baseline": {
                "r5_baseline_total": cross_prior_total,
                "r5_baseline_empty": cross_prior_empty,
                "r5_baseline_ok": cross_prior_ok,
                "r5_baseline_rate": cross_prior_rate,
            ...
            "r6_empty_response_rate_threshold_K_N26_N2_single_batch": 0.50,
            "r6_empty_response_above_threshold_single_batch": empty_rate_r6 > 0.50,
        },
        "empty_rate_trend": {
            ...
            "batch2_round2_round1_retry": {
                "calls": r2["aggregate"]["glm1_cumulative"]["post_total"],
                "empty": r2["aggregate"]["glm1_cumulative"]["post_empty"],
                "rate": r2["aggregate"]["glm1_cumulative"]["post_rate"],
            ...
            "batch2_round3_round2_reask1": {
                ...
                "rate": r3["aggregate"]["r3_empty_response_rate"],
                "note": "r3 round 2 reask=1 22 caption × 1 call; all OK",
            },
            "batch2_round4_round3_reask2": {
                "calls": r4["aggregate"]["r4_total_calls"],
                "empty": r4["aggregate"]["r4_empty_response_count"],
                "rate": r4["aggregate"]["r4_empty_response_rate"],
                "note": "r4 round 3 reask=2 22 caption × 1 call; all OK",
            },
            "batch2_round5_round4_reask3": {
                "calls": r5["aggregate"]["r5_total_calls"],
                "empty": r5["aggregate"]["r5_empty_response_count"],
                "rate": r5["aggregate"]["r5_empty_response_rate"],
                "note": "r5 round 4 reask=3 22 caption × 1 call; all OK",
            },
            "batch2_round6_round5_reask4_finish": {
                "calls": r6_total,
                "empty": r6_empty,
                "rate": round(empty_rate_r6, 4),
                "note": "r6 round 5 reask=4 22 caption × 1 call = GLM_1 教师侧收官棒",
            },
            "batch2_round6_cumulative_glm1": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
            ...
            "cross_batch_cumulative_post_r6": {
                ...
                "note": "r5 baseline 207/9/4.35% + r6 增量 = GLM_1 教师侧收官",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round5_reask4_n_distinct_sample_note": (
                    f"round 5 reask=4 收官棒 第 1 call 实测；本棒后 GLM_1 教师侧 round 5 = "
                    f"{met_round5_count}/{len(captions)} caption met round 5 目标 (≥5 successful/caption); "
                    f"GLM_1 教师侧累计 {cum_ok}/{cum_total} calls 全 OK; 收官判定 = glm1_teacher_side_finish"
                ),
            },
            "K_N26_N2_observation": {
                ...
                "single_batch_trigger_K_N26_N2_pass_False": empty_rate_r6 > 0.50,
                ...
                "monitor_disposition": (
                    ...
                    f"r5 baseline GLM_1 = {glm1_prior_total} calls / {glm1_prior_empty} empty / {glm1_prior_rate:.4f}; "
                    ...
                    f"GLM_1 教师侧 0.0000 empty_rate 收官"
                ),
                "cross_batch_with_r5_baseline_rate": round(cross_batch_rate, 4),
            },
            "verdict_pending": "5 教师批齐后由 verdict-keeper 统裁 (本棒 0 写 verdict; 本棒为 GLM_1 教师侧收官棒)",
        },
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "GLM_1 教师侧收官完成；本棒后 0 接力 (verdict-keeper 统裁; "
                "如本棒未达成 22/22 5/5 则需补棒)"
            ),
            "note": (
                f"本棒 round 5 (reask=4) 收官棒: actual={r6_total}/{planned_total} calls; "
                f"GLM_1 教师侧 round 5 完成度 = {met_round5_count}/{len(captions)} caption met round 5 目标 (≥5 successful/caption); "
                f"GLM_1 教师侧累计 ≥5 successful/caption = {met_target_count}/{len(captions)}; "
                f"stop_reason={stop_reason}; "
                f"收官判定 = {glm1_teacher_side_finish['finish_disposition']}; "
                f"GLM_1 model_id 沿 r5 smoke 选定 = {model_id} (1-call smoke 确认仍可用)"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
    ...
    print(f"[FINISH] {glm1_teacher_side_finish['finish_disposition']}")
