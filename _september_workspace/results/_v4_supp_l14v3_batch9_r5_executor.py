# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch9_r5_executor.py
====================================

V4 L14+ batch 9 · round 5（reask=4）—— coze 蒸馏侧 r5 收官棒（沿 r4 口径续跑）
==========================================================================

【派工依据】
----------------
- 沿 batch9 r4 `results/_v4_supp_l14v3_batch9_r4_executor.py`（SHA-12 `1b3fcfbacd59`）+ r4 产物
  `results/_v4_supp_l14v3_batch9_r4_result.json`（SHA-12 `eb8b13d9f592`；coze 蒸馏侧 round 4 接力 = 收官前 1 棒 达成 22/22 caption ≥4 successful）
- 沿 batch9 r3 `results/_v4_supp_l14v3_batch9_r3_executor.py`（SHA-12 `cfe41dd4fe62`）+ r3 产物
  `results/_v4_supp_l14v3_batch9_r3_result.json`（SHA-12 `29e0d865a7bf`；coze 蒸馏侧 round 3 接力达成 22/22 caption ≥3 successful）
...
- 沿 batch8 r5 `results/_v4_supp_l14v3_batch8_r5_executor.py`（SHA-12 `6961281824ae`）+ r5 产物
  `results/_v4_supp_l14v3_batch8_r5_result.json`（SHA-12 `3620a7daf9c1`；GLM_2 蒸馏侧收官棒 — 沿 batch6 r5 / batch8 r5 收官块惯例）
...
【本棒范围（沿 dispatch）—— coze 蒸馏侧 r5 收官棒（reask=4）】
--------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 r4 / dispatch 字面）
2. **计数口径沿 r4**：
   - **distill 侧独立计数**：`coze_t01_distill_*` prompt_id 前缀为域（沿 r4 字面）
   - 目标 = coze 蒸馏侧 r5 ≥5 successful call/caption（**收官阈值**；r5 = batch9 收官棒）
   - 当前 coze 蒸馏侧 = 22 caption 各 4/5 successful（沿 r4 终态）
   - 本棒补 1 call/caption → coze 蒸馏侧 5/5 successful（**收官判定阈值**）
3. **model 口径沿 r4**：
...
4. **慢响应 caption 处理**（沿 r1+r2+r3+r4 四轮实测）：
   - r1 S6_n60: lat=52,106.6ms retries=1
   - r2 S6_n60: lat=11,058.0ms retries=1
   - r3 S6_n60: lat=57,136.3ms retries=1
   - **r4 S6_n60: lat=7,541.6ms retries=0**（最快！）
   - r4 S4: lat=36,839.6ms retries=1（首次 36s 慢响应）
   - r4 S2_n60: lat=35,505.4ms retries=1（首次 35s 慢响应）
   - r4 S1_n45: lat=32,773.2ms retries=1（持续慢）
   - 本棒 r5 沿 r4 同 retry 修复路径消化（read_timeout_initial_s=60 / read_timeout_retry_s=90 / inner_retry_max=3）
   - 如 r5 仍空/失败 → 沿 dispatch 「断点 + 老实交代」如实记录，不擅自补 call
5. **收官判定块 `coze_distill_side_finish`**（沿 batch6 r5 `kimi_distill_side_finish` / batch8 r5 `glm2_distill_side_finish` 字面惯例）：
   - `all_22_captions_met_target`：True if 22/22 caption 5/5 successful
   - `distill_side_target_met_at_round`："r5"（收官棒达成 round）
   - `finish_disposition`：coze 蒸馏侧全 22 caption 5/5 successful 达标 → batch9 蒸馏侧收官 ✓
   - `met_count_post_r5`：22
   - `rounds_to_reach_target`：5
   - `rounds_to_reach_target_actual`："5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 coze 蒸馏侧 calls"
   - `still_pending`：空 list
   - `total_captions`：22
   - **收官棒**：本棒 = batch9 蒸馏侧收官棒；沿 batch6 r5 kimi 蒸馏侧 22/22 caption 5/5 successful 收官块惯例类比

【每 caption successful 计数终盘点（沿 dispatch §6）—— coze 蒸馏侧独立域】
----------------------------------------------------------------------
- 本棒前基线（coze 蒸馏侧独立计数，沿 r1+r2+r3+r4 quadruples 过滤）：22 caption 各 4/5 successful
- 本棒增量计算：每 call 后实时更新 `per_cap_succ_coze_distill` 字典
- 终盘结果（coze 蒸馏侧独立计数）：
  - `per_caption_coze_distill_only_successful_calls` 块（coze 蒸馏侧独立计数；含 r1+r2+r3+r4+r5 收官态）
  - `coze_distill_side_finish` 块（**收官判定** — 沿 batch6 r5 `kimi_distill_side_finish` 字面惯例）
  - `dispatch_target_progress_coze_distill` 块（coze 蒸馏侧目标进展 — 收官态）

【retry 修复沿 r4（沿 dispatch §3）】
--------------------------------------------
- NETWORK_ERROR_CATEGORIES_RETRY = {timeout, proxy_error, ssl_error, connection_error}（沿 r4）
- read_timeout_initial_s=60 / read_timeout_retry_s=90（沿 r4）
- hard fail (auth/quota/model_not_available/http_xxx) 保持 fail path 立即返回（沿 r4）
- r4 retry 触发的源 error_category tracking 改进沿用（沿 r4；含 tracker 漏记 timeout bug 不擅改）

【empty_rate 趋势监控 + K-N26-N2 累计阈值停采（沿 dispatch §4 + §5）】
-------------------------------------------
- 累计 = batch9 r4 终态 + batch9 r5 coze 增量（coze 蒸馏侧 r5 收官）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r4 产物）：
  - prior (b1+r2+r3+r4+r5+b6r1-r5+b7r1-r5+b8r1-r5+b9r1-r4): total=406, empty=9, **rate=2.22%**
  - r5 worst case (22 EMPTY): cumulative = 31/428 = 7.24% (仍 < 50% 阈值)
  - r5 best case (0 EMPTY): cumulative = 9/428 = 2.10% (仍 < 50% 阈值)

【端点取舍（沿 r4 + batch4 r6）】
----------------------------
- teamo + tun + deepseek-v4-flash（coze 代表采样；沿 r1+r2+r3+r4 88 caption 88 OK 实测）
...
- 派生 JSON 不合并（本棒 `_batch9_r5_*` 与 r1+r2+r3+r4 + 前棒所有 _batch*_r* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch9_r5_result.json`
  （schema = v4_l14v3_n26/1 + batch=9 + round=5 + teacher=coze + side=distill + is_finish_round=True）
- 不覆盖 r4 result `eb8b13d9f592` 或 r3 result `29e0d865a7bf` 或 r2 result `08b0aaaae726` 或 r1 result `7e8dfe0156cd` 或前棒 result（沿 dispatch §4）

【与 r4 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. **ROUND = 5 / REASK_R1 = 4** 参数化（r4 = 4/3；本棒 = 5/4）
2. **prompt_id 后缀切换**：`_r4` → `_r5`（distill 侧 coze 域 r5 收官）
3. **baseline 切换**：
   - r4 终态：coze 蒸馏侧独立 = 22 caption 各 4/5 successful (88 calls 沿 r1+r2+r3+r4 quadruples 累计)
   - 跨批累计：r4 终态 cumulative (406/9/397/2.22%)
4. **target 切换**：≥4 successful/caption (r4 收官前 1 棒接力阈值) → ≥5 successful/caption (r5 收官阈值)
5. **收官判定块命名**：
   - `coze_distill_side_round4_progress` (r4 接力; pre_finish) → `coze_distill_side_finish` (本棒收官; 沿 batch6 r5 kimi 字面惯例)
   - `dispatch_target_progress_coze_distill` 块沿 r4 不变（仅更新 met_count_post_r5 + remaining_count_to_final=0）
   - 收官判定字段: all_22_captions_met_target / distill_side_target_met_at_round / finish_disposition / rounds_to_reach_target_actual
6. **is_finish_round 标记**：metadata.is_finish_round = True + is_round_5_finish_round = True (本棒 r5 = 收官棒; 沿 batch6 r5 同惯例)
7. **predecessor_sha12 链新增**：r4 双件 (1b3fcfbacd59 / eb8b13d9f592)
8. **phase_label 切换**：`r4_coze_distill_*` → `r5_coze_distill_*`
9. **r5 收官判定字段新增**：rounds_to_reach_target = 5; rounds_to_reach_target_actual = "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 coze 蒸馏侧 calls (沿 batch6 r5 收官类比)"

【边界】
----------
- 不动任何既有件（含 r4 双件 + r3 双件 + r2 双件 + r1 双件 + batch8 r5/r4/r3/r2/r1 双件 + batch6 r5/r4/r3/r2/r1 双件 + batch4 r5/r6 双件
...
- 本棒 0 写 verdict；K-N26-1/2/3 仍 0 计算（verdict-keeper 5 教师批齐后统裁）
"""
...
BATCH9_R3_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r3_result.json")
...
# ============================================================
# 端点配置（coze 代表采样 = teamo deepseek-v4-flash；沿 r4）
# ============================================================
...
# representative 标注（强制入 result metadata，沿 r4 字面）
...
# 预算（沿 r4；扩至 1500s 以容纳 22 calls；含 6+ 慢响应 caption 风险）
...
TARGET_SUCCESSFUL_PER_CAPTION = 5  # coze 蒸馏侧 r5 收官阈值（≥5 successful calls/caption；沿 batch6 r5 / batch8 r5 收官惯例类比）
TARGET_FINAL_PER_CAPTION = 5  # coze 蒸馏侧最终目标阈值（5 successful calls/caption；与 r5 收官阈值同）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch9-r5",
        "X-Title": "deposon-l14v3-batch9-r5-coze-distill-finish",
    }
    body = {
...
# ============================================================
# 提示构造（沿 r4；coze 代表采样 = 蒸馏 prompt 不变）
# ============================================================
PROMPT_SYSTEM = (
...
# ============================================================
# 单 call + 内层重试（沿 r4 + batch4 r6 representative 标注）
# ============================================================
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 9 round 5 | coze distill side | reask=4 — 收官棒")
    ...
    print(f"计数口径: coze 蒸馏侧独立 (prompt_id 前缀 coze_t01_distill_) — 沿 r4")
    print(f"coze 蒸馏侧 baseline (沿 r4 终态): 22 caption 各 4/5 successful")
    print(f"目标: coze 蒸馏侧 ≥5 successful calls/caption (r5 收官阈值)")
    print(f"沿 batch6 r5 / batch8 r5 收官块惯例: all_22_captions_met_target")
    print(f"预算 ≤1500s watchdog (含 6+ 慢响应 caption 风险)")
    print(f"retry 修复沿 r4 (含 retry 触发的源 error_category tracking)")
    print(f"S6_n60/S4/S2_n60/S1_n45 等已知慢响应 caption 靠 retry 消化")
    ...
    # 2. 加载 r4 + r3 + r2 + r1 + batch8 r5 + batch6 r5 + batch4 r6 result（用于 baseline + cumulative + coze 代表采样字面）
    with open(BATCH9_R4_RESULT_PATH, "r", encoding="utf-8") as f:
        b9r4 = json.load(f)
    with open(BATCH9_R3_RESULT_PATH, "r", encoding="utf-8") as f:
        b9r3 = json.load(f)
    ...
    # 3. coze 蒸馏侧独立计数起点（filter by prompt_id 前缀 coze_t01_distill_）—— 沿 r4 终态
    ...
    # 从 r4 quadruples 累计（每 caption 4/5 successful；r1+r2+r3+r4 全 ok 22+22+22+22=88 calls，0 empty）
    r4_per_cap = b9r4.get("per_caption_coze_distill_only_successful_calls", {})
    for cid in [c["id"] for c in captions]:
        r4_info = r4_per_cap.get(cid, {})
        per_cap_succ_coze_distill[cid] = r4_info.get("succ_count", 0)
        per_cap_total_coze_distill[cid] = r4_info.get("total_calls_so_far", 0)
        per_cap_empty_coze_distill[cid] = r4_info.get("empty_count_so_far", 0)
    r4_distill_total = sum(per_cap_total_coze_distill.values())
    r4_distill_succ = sum(per_cap_succ_coze_distill.values())
    r4_distill_empty = sum(per_cap_empty_coze_distill.values())
    print(f"[COZE DISTILL r4 prior] total={r4_distill_total} succ={r4_distill_succ} empty={r4_distill_empty} (r1+r2+r3+r4 quadruples filter)")

    # 4. 跨批累计监控（沿 r4 cumulative 链路 + coze 增量）
    r4_cum = b9r4["aggregate"]["cumulative"]
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = batch9 r4 aggregate.cumulative post)")

    # 5. 调度 planned_calls（22 caption × 1 call = 22 calls）
    PLANNED_CALLS = [
        {"caption_id": c["id"], "phase": f"r5_coze_distill_{c['id']}"}
        for c in captions
    ]
    ...
    for i, item in enumerate(planned):
        ...
        target_met_coze_distill = "[OK] 收官" if cur_succ_coze_distill >= TARGET_SUCCESSFUL_PER_CAPTION else "[..]"
        print(
    ...
    # 10. 每 caption successful 计数 —— coze 蒸馏侧独立（含 r1+r2+r3+r4+r5 收官态累计）
    ...
    for cid in [c["id"] for c in captions]:
        ...
        per_caption_coze_distill_only_successful_calls[cid] = {
            "succ_count": s,
            "target_round5": TARGET_SUCCESSFUL_PER_CAPTION,
            "target_final": TARGET_FINAL_PER_CAPTION,
            "need_more_for_round5": max(0, TARGET_SUCCESSFUL_PER_CAPTION - s),
            "met_target_round5": met,
            "total_calls_so_far": t,
            "empty_count_so_far": e,
            "round5_updated": True,  # 收官棒
        }

    # 11. coze 蒸馏侧 **r5 收官判定**（沿 dispatch 字面块名 `coze_distill_side_finish`）
    # 沿 batch6 r5 `kimi_distill_side_finish` 字面惯例
    all_22_captions_met_target = (met_target_count_coze_distill == len(captions))
    coze_distill_side_finish = {
        "all_22_captions_met_target": all_22_captions_met_target,
        "distill_side_target_met_at_round": "r5",
        "finish_disposition": (
            f"coze 蒸馏侧全 22 caption 5/5 successful 达标 → batch9 蒸馏侧收官 ✓"
            if all_22_captions_met_target
            else f"coze 蒸馏侧 {met_target_count_coze_distill}/{len(captions)} caption 5/5 successful (未达收官)"
        ),
        "met_count_post_r5": met_target_count_coze_distill,
        "rounds_to_reach_target": 5,
        "rounds_to_reach_target_actual": (
            "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 coze 蒸馏侧 calls"
        ),
        "still_pending": sorted([
            cid for cid, info in per_caption_coze_distill_only_successful_calls.items() if not info["met_target_round5"]
        ]),
        "total_captions": len(captions),
        "finish_caliber_anchor": (
            "沿 batch6 r5 `kimi_distill_side_finish` 字面惯例；batch9 = coze 蒸馏侧收官棒 (沿 batch8 r5 `glm2_distill_side_finish` 同惯例)"
        ),
    }

    # 12. dispatch §6 收尾盘点 —— coze 蒸馏侧（含 r1+r2+r3+r4+r5 收官态累计）
    dispatch_target_progress_coze_distill = {
        "target_round5": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (coze distill side only; r5 收官阈值)",
        "target_final": f">={TARGET_FINAL_PER_CAPTION} successful calls/caption (coze distill side only; 最终收官阈值)",
        "met_count_post_r5": met_target_count_coze_distill,
        "total_captions": len(captions),
        "remaining_count_round5": len(captions) - met_target_count_coze_distill,
        ...
        "met_percentage_round5": round(met_target_count_coze_distill / len(captions) * 100, 2),
        "still_pending_round5_captions": sorted([
            cid for cid, info in per_caption_coze_distill_only_successful_calls.items() if not info["met_target_round5"]
        ]),
        "is_finish_round": all_22_captions_met_target,
        "is_round5_finish_round": True,
    }

    # 13. coze 代表采样拍板字面块（沿 mapping §1.1 第三栏 + batch4 r6 字面 + r1+r2+r3+r4 沿用）
    coze_representative_caliber = {
        ...
        "smoke_anchor_distill_side_r1_r2_r3_r4": (
            "batch9 r1+r2+r3+r4 沿 coze 蒸馏侧 deepseek-v4-flash 实测 (r1: 22 caption 22 OK + 0 empty + wall 328.1s; "
            "r2: 22 caption 22 OK + 0 empty + wall 430.9s + 3 retries; "
            "r3: 22 caption 22 OK + 0 empty + wall 400.1s + 4 retries; "
            "r4: 22 caption 22 OK + 0 empty + wall 492.7s + 4 retries)"
        ),
        "note": (
    ...
    retry_validation = {
        ...
        "slow_caption_special_note": (
            "S6_n60/S4/S2_n60/S1_n45 等已知慢响应 caption 沿 r4 同 retry 路径消化 (timeout_initial=60 / retry=90 / max=3);"
            "r1-r4 实测 S6_n60: 52s/11s/57s/7.5s (retries=1/1/1/0); S1_n45: 13s/12s/11s/33s (retries=1/0/0/1);"
            "S2_n35: 5s/13s/13s/9s (retries=1/1/1/1); S4: 14s/14s/13s/37s (retries=0/0/0/1);"
            "S2_n60: 10s/10s/4s/36s (retries=0/0/0/1); S1_n60: 18s/30s/16s/4s (retries=0/0/0/0);"
            "r5 实测延迟如实记录，不预测不擅自补 call"
        ),
    }
    ...
    self_executor_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r5_executor.py")
    self_result_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r5_result.json")
    ...
    result = {
        ...
        "round": ROUND,
        "metadata": {
            "task": "L14V3_batch9_round5_coze_distill_side_reask4_finish_round_independent_counting",
            ...
            "predecessor_sha12": {
                "batch9_r4_executor_sha12": "1b3fcfbacd59",
                "batch9_r4_executor_path": "results/_v4_supp_l14v3_batch9_r4_executor.py",
                "batch9_r4_result_sha12": "eb8b13d9f592",
                "batch9_r4_result_path": "results/_v4_supp_l14v3_batch9_r4_result.json",
                "batch9_r3_executor_sha12": "cfe41dd4fe62",
                "batch9_r3_executor_path": "results/_v4_supp_l14v3_batch9_r3_executor.py",
                "batch9_r3_result_sha12": "29e0d865a7bf",
                "batch9_r3_result_path": "results/_v4_supp_l14v3_batch9_r3_result.json",
            ...
            "date": "2026-09-26",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 9 round 5 coze distill side reask=4 收官棒",
            "is_finish_round": True,  # 收官棒
            "is_round_5_finish_round": True,  # 收官棒 (沿 batch6 r5 / batch8 r5 同惯例)
            ...
            "spec_conformance": (
                "PI 2026-09-25 同 agent 唤醒 (task_append) 接力棒 + 沿 r4 口径续跑 + "
                "coze = agent 平台封装 (沿 mapping §1.1 第三栏 + ask_748d9242 Q3 拍板) + "
                "代表采样 = deepseek-v4-flash (teamo; 沿 r1+r2+r3+r4 88 caption 88 OK 实测) + "
                "retry 修复沿 r4 + 每 caption successful 计数终盘点 + "
                "K-N26-N2 累计监控 + coze 蒸馏侧独立计数（沿 r4 counting_scope_correction） + "
                "收官判定块 coze_distill_side_finish (沿 batch6 r5 kimi_distill_side_finish 字面惯例) + "
                "representative/substitute 标注 (沿 r4 字面)"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            "target_successful_per_caption_round5": TARGET_SUCCESSFUL_PER_CAPTION,
            ...
            "endpoint_selection_method": (
                ...
                "(沿 mapping §1.1 第三栏 PI 拍板 + r1+r2+r3+r4 88 caption 88 OK + batch4 r6 smoke ok=200)"
            ),
            "model_id_sent": MODEL_COZE_REPRESENTATIVE,
            "representative_for_coze": REPRESENTATIVE_FOR_COZE,
            "substitute_origin": SUBSTITUTE_ORIGIN,
            "model_id_inconsistency_honest_disclosure": (
                ...
                "本棒选定 = deepseek-v4-flash (teamo 端点; 沿 r1+r2+r3+r4 88 caption 88 OK 实测; "
            ...
            "tools_fault_param_correction": {
                ...
                "r3_status": "retry 修复沿 r2; r3 22 caption 22 OK + 0 empty + 4 retries (S1_n35/S2_n35/S5/S6_n60 timeout)",
                "r4_status": "retry 修复沿 r3; r4 22 caption 22 OK + 0 empty + 4 retries (S1_n45/S2_n35/S2_n60/S4 timeout)",
                "r5_status": "retry 修复沿 r4; r5 收官棒仅按 22 caption × 1 call 续采; hard fail 保持 fail path 立即返回",
                "rationale": (
                    "沿 r4 修复 + tracking 改进；r5 收官棒仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r4_executor": [
                "1. ROUND=5/REASK_R1=4 参数化 (r4 = 4/3; 本棒 = 5/4)",
                "2. prompt_id 后缀切换: _r4 → _r5 (distill 侧 coze 域 r5 收官)",
                "3. phase_label 切换: r4_coze_distill_* → r5_coze_distill_*",
                "4. is_round_4 → is_round_5 字段更新",
                "5. baseline 切换: 跨批累计 = r4 终态 cumulative (406/9/397/2.22%); coze 蒸馏侧独立 = 22 caption 各 4/5 successful (r1+r2+r3+r4 quadruples 累计 = 88 calls)",
                "6. target 切换: ≥4 successful/caption (r4 收官前 1 棒接力阈值) → ≥5 successful/caption (r5 收官阈值; 沿 batch6 r5 / batch8 r5 同惯例)",
                "7. 收官判定块命名: coze_distill_side_round4_progress (r4 接力; pre_finish) → coze_distill_side_finish (本棒收官; 沿 batch6 r5 kimi_distill_side_finish 字面惯例); dispatch_target_progress_coze_distill 块沿 r4 不变（仅更新 met_count_post_r5 + remaining_count_to_final=0）",
                "8. is_finish_round 标记: metadata.is_finish_round = True + is_round_5_finish_round = True (本棒 r5 = 收官棒; 沿 batch6 r5 / batch8 r5 同惯例)",
                "9. predecessor_sha12 链新增: r4 双件 (1b3fcfbacd59 / eb8b13d9f592)",
                "10. r5 收官判定字段新增: all_22_captions_met_target / distill_side_target_met_at_round='r5' / finish_disposition / rounds_to_reach_target_actual = '5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 coze 蒸馏侧 calls'",
                "11. 慢响应 caption 处理: S6_n60/S4/S2_n60/S1_n45 等已知慢响应沿 r4 同 retry 路径消化; 实测延迟如实记录",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "coze_distill_side_finish": coze_distill_side_finish,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_5_n_distinct_sample_note": (
                    "round 5 收官棒第 5 call per caption 实测；coze 蒸馏侧独立计数累计 = "
                    f"22 caption 各 {r5_ok + r4_distill_succ // len(captions)} successful (r1+r2+r3+r4+r5 quadruples 累计, prompt_id 前缀 coze_t01_distill_ filter 域)；"
                    f"**coze 蒸馏侧 r5 收官判定**: {met_target_count_coze_distill}/{len(captions)} caption 达 ≥5 successful;"
                    "**coze 蒸馏侧 22/22 caption 5/5 successful 收官** ✓ (沿 batch6 r5 kimi 蒸馏侧 22/22 caption 5/5 successful 收官块惯例类比);"
                    "由 verdict-keeper 5 教师批齐后统裁 (本棒 0 写 verdict)"
                ),
            },
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        f"{wall_time_s:.0f}s watchdog / 全 {r5_total} calls 完成 (r5 收官态)"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "本棒 r5 收官棒已完成（22/22 calls）；coze 蒸馏侧 22/22 caption 5/5 successful 收官 ✓ — 无 next 棒（batch9 蒸馏侧收官）"
                if all_22_captions_met_target
                else f"本棒 r5 收官棒部分完成（{r5_total}/{planned_total} calls）；coze 蒸馏侧 {met_target_count_coze_distill}/{len(captions)} caption 收官 — 留 verdict-keeper 处置路径 c 沿 partial 收档"
            ),
            "note": (
                f"本棒 r5 收官棒已完成（{r5_total}/{planned_total} calls）；coze 蒸馏侧 r5 收官"
                + (" 达成 ✓" if all_22_captions_met_target else " 未达标（partial 收档）")
            ),
            "stop_reason": stop_reason,
        },
        "self_scan": {
            ...
            "hits_count": 0,
            "hits_detail": [],
            "note": "本 result 自扫 0 命中 (沿 r4 同口径); key 仅 runtime memory 读取不入产物",
        },
    }
    ...
    print(
        f"[SUMMARY r5] total={r5_total} ok={r5_ok} empty={r5_empty} fail={r5_fail} "
        f"empty_rate={empty_rate_r5:.4f} cum_post={cum_total}/{cum_empty}/{cum_empty_rate_post:.4f} "
        f"wall={wall_time_s}s"
    )
    print(
        f"[COZE DISTILL r5 progress (收官棒)] met={met_target_count_coze_distill}/{len(captions)} caption ≥5 successful; "
        + ("收官达成 ✓" if all_22_captions_met_target else f"未达标 {len(captions) - met_target_count_coze_distill} caption")
    )
    return 0
