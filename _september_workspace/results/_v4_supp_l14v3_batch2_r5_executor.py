# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch2_r5_executor.py
=====================================

V4 L14+ batch 2 · round 5 · GLM_1 教师侧 round 4 续采（reask=3）
================================================================

【派工依据】
----------------
- 沿 batch2 r4 executor `D9FE7AA2D292` + r4 result `15B8DDC5BA9E` 口径（基础口径锚）
...
- 派工单（沿 2026-09-24 23:33 接力棒）：GLM_1 教师侧 round 5 (reask=3) 续采
- 拍板依据：ask_748d9242c7a6be3d83de63a0 (2026-09-24 23:08 显式)：GLM_1=glm-5.3 已在跑（沿 r2/r3/r4）

【本棒范围（沿 dispatch）—— GLM_1 教师侧 round 4（reask=3）续采】
----------------------------------------------------------
...
3. **目标**：本棒后 22/22 caption 各 ≥4 successful call；下棒接力至 ≥5 successful/caption（GLM_1 教师侧收官棒）
4. 预算 ≤22 caption calls + ≤1 smoke calls / ≤600s watchdog
5. 跑不完如实报断点

【每 caption successful 计数（沿 dispatch §6）】
---------------------------------------------------------
- 本棒前基线（沿 batch2 r4）：GLM_1 教师侧 = 66 calls / 0 empty / 66 ok / 0.0000 rate
...
--------------------------------------------------------------------
- 跨批累计 baseline = batch1 终态 119 + batch2 r2 (22) + batch2 r3 (22) + batch2 r4 (22) → **185 calls / 9 empty / 4.86%**（沿 r4 result）
- 累计 = r4 baseline + batch2 r5（GLM_1 自身 r4 末 66/0/66/0.0000）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）

【端点取舍（沿 r4）】
----------------------------
- teamo + tun + glm-5.3（r4 已 smoke ok=200；本棒不再做完整 3-candidate probe，仅 1-call smoke）
...
- 派生 JSON 不合并（本棒 `_batch2_r5_*` 与 `_batch2_r4_*` / `_batch2_r3_*` / `_batch2_r2_*` / `_batch2_r1_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r4 executor `D9FE7AA2D292` + r4 result `15B8DDC5BA9E` + r3 + r2 + r1 + batch1 r6 + models_probe 全套不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch2_r5_result.json`（schema = `v4_l14v3_n26/1` + batch=2 + round=5）
- 不覆盖 r4 result `15B8DDC5BA9E` 或 r3 result `25251ACA6D79` 或 r2 result `8C125E257AF8` 或 batch1 r6 result `A4F851154551`
- predecessor_sha12 链：r4_executor/r4_result + r3_executor/r3_result + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r4 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 5（升 r5 命名避开 r4 重叠）；BATCH = 2 不变
2. prompt_id 后缀 `_b2_r5`；t04 = teacher side round 4（取代 r4 的 t03 round 3 reask=2）
3. reask_idx = 3（本棒每位 caption 第 4 call；取代 r4 的 reask_idx=2）
4. phase_label：`b2_r5_need1`（取代 r4 的 `b2_r4_need1`）
5. round4_target = 4（取代 r4 的 round3_target=3）；is_round_2 = False（沿 r4）
6. smoke_call_only = True（沿 r4；不再做 3-candidate probe fallback）
7. predecessor_sha12 链：r4_executor `D9FE7AA2D292` + r4_result `15B8DDC5BA9E`（新增；r3/r2/r1/batch1_r6 全套沿 r4）
8. baseline prior：r4 终态 66 calls / 0 empty / 66 ok / 0.0000 rate（取代 r4 链的 44/0/44）
9. 跨批累计 baseline：r4 终态 185 calls / 9 empty / 4.86% rate（取代 r4 链的 163/9/5.52%）
10. 新增 `glm1_teacher_side_round4_progress` 块（本棒后 GLM_1 教师侧 round 4 完成度 ≥4 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 185/9/4.86%
12. spec_conformance 重写：GLM_1 教师侧 round 4 续采（reask=3），目标 ≥4 successful/caption
13. dispatch_target_progress 字段：target_round4 / still_pending_captions_round4
14. per_caption_successful_calls 字段新增 round4_target=4 / round4_met
15. http referer/title 字段更新 l14v3-batch2-r5 (取代 r4 的 r4)
16. 注：本棒为 GLM_1 教师侧倒数第 2 棒（收官棒 = r6 再续 1 棒）；总目标 ≥5 successful/caption 仍需 r6 收官
...
# 选定的 model_id（沿 r4 smoke 结论，不重探）
MODEL_GLM_1 = "glm-5.3"
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch2-r5",
        "X-Title": f"deposon-l14v3-batch2-r5-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r4；仅 1 call 确认端点 ok）
# ============================================================
def smoke_glm_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r4 smoke 结论 glm-5.3 ok=200 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b2_r5"
    prompt_id = f"glm1_t04_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 2 round 5 · GLM_1 教师侧 round 4（reask=3）续采")
    print("沿 r4 glm-5.3 smoke 结论 ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r4 (沿 batch1 r6)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（batch1 r6 + batch2 r1 + batch2 r2 + batch2 r3 + batch2 r4）
    ...
    # r4 终态 = 本棒前 GLM_1 自身累计基线
    glm1_prior_total = r4["aggregate"]["glm1_cumulative"]["post_total"]
    glm1_prior_empty = r4["aggregate"]["glm1_cumulative"]["post_empty"]
    glm1_prior_ok = r4["aggregate"]["glm1_cumulative"]["post_ok"]
    glm1_prior_rate = r4["aggregate"]["glm1_cumulative"]["post_rate"]
    print(f"[BASELINE r4 GLM_1] total={glm1_prior_total} ok={glm1_prior_ok} empty={glm1_prior_empty} rate={glm1_prior_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r4 smoke 结论 glm-5.3 ok=200）
    print("=" * 60)
    print("[SMOKE PHASE] glm-5.3 1-call smoke（沿 r4 smoke 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 5,
            "metadata": {
                "task": "L14V3_batch2_round5_GLM1_teacher_side_round4_reask3",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch2_r5_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b2_r5_need1"} for c in captions_sorted]
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=False,  # 语义：本棒 = GLM_1 教师侧 data collection round 4（沿 r4）
    ...
    # 11. GLM_1 教师侧 round 4 进度块
    glm1_round4_progress = {
        "round4_target": ">=4 successful call/caption (round 4 reask=3)",
        "met_count_post_r5_round4": met_round4_count,
    ...
    # model_id_inconsistency_honest_disclosure（沿 r4 + r5 沿用）
    ...
    model_id_disclosure = (
        ...
        f"r2 1-call probe ok=200; r3 1-call smoke ok=200; r4 1-call smoke ok=200; r5 沿 r4 结论仅做 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                "PI 2026-09-24 batch2 GLM_1 教师侧 round 4 续采 (reask=3); "
                "沿 r4 glm-5.3 smoke 结论 (1-call smoke ok=200); r5 仅 1-call smoke; "
                "22 caption × 1 call = 22 calls (按 caption_id 字母序); "
                "目标 ≥4 successful/caption；下棒收官 r6 (reask=4) 达 ≥5 successful/caption"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": 5,
            ...
            "tools_fault_param_correction": {
                ...
                "r5_b2_status": "r5 仅 1-call smoke (沿 r4 结论)；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r4 glm-5.3 smoke 结论；r5 节省预算避免重复 probe；"
            ...
            "diff_vs_r4_executor": [
                "1. ROUND=5 (升 r5 命名)；BATCH=2 不变",
                "2. prompt_id 后缀 _b2_r5；t04_teacher (取代 r4 的 t03_teacher round 3 reask=2)",
                "3. reask_idx=3 (本棒每 caption 第 4 call；取代 r4 的 reask_idx=2)",
                "4. phase_label: b2_r5_need1 (取代 r4 的 b2_r4_need1)",
                "5. is_round_2=False (沿 r4; GLM_1 教师侧 data collection round 4)",
                "6. smoke_call_only=True (沿 r4；不再做 3-candidate probe)",
                "7. predecessor_sha12 链: r4_executor D9FE7AA2D292 + r4_result 15B8DDC5BA9E (新增；r3/r2/r1/batch1_r6 全套沿 r4)",
                "8. baseline prior: r4 终态 66 calls / 0 empty / 66 ok (取代 r4 链的 44/0/44)",
                "9. 跨批累计 baseline: r4 终态 185 calls / 9 empty / 4.86% (取代 r4 链的 163/9/5.52%)",
                "10. 新增 glm1_teacher_side_round4_progress 块 (round 4 完成度 ≥4 successful/caption)",
                "11. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 185/9/4.86%",
                "12. spec_conformance 重写: round 4 续采 (reask=3) + 注 r6 收官",
                "13. per_caption_successful_calls 字段新增 round4_target=4 / round4_met (取代 r4 的 round3_target=3 / round3_met)",
                "14. 调度注释: round 4 续采 (reask=3) 22 caption × 1 call",
                "15. http referer/title 字段更新 l14v3-batch2-r5 (取代 r4 的 r4)",
                "16. 注：本棒为 GLM_1 教师侧倒数第 2 棒；收官棒 = r6 (reask=4) 达 ≥5 successful/caption",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "r5 沿 r4 glm-5.3 smoke 结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；r5 与 r4/r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        ...
        "glm1_teacher_side_round4_progress": glm1_round4_progress,
        ...
        "empty_rate_trend": {
            ...
            "cross_batch_cumulative_post_r5": {
                ...
                "note": "r4 baseline 185/9/4.86% + r5 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round4_reask3_n_distinct_sample_note": (
                    f"round 4 reask=3 第 1 call 实测；本棒后 GLM_1 教师侧 round 4 = "
                    f"{met_round4_count}/{len(captions)} caption met round 4 目标; "
                    f"需后续 r6 收官棒达 ≥{TARGET_SUCCESSFUL_PER_CAPTION} successful/caption"
                ),
            },
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"r4 baseline GLM_1 = {glm1_prior_total} calls / {glm1_prior_empty} empty / {glm1_prior_rate:.4f}; "
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 4 (reask=3) 已完; 下棒收官 GLM_1 教师侧 round 5 (reask=4, >=1 more successful/caption) 达 ≥5/5"
            ),
            "note": (
                f"本棒 round 4 (reask=3): actual={r5_total}/{planned_total} calls; "
                f"GLM_1 教师侧 round 4 完成度 = {met_round4_count}/{len(captions)} caption met round 4 目标 (≥4 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"GLM_1 model_id 沿 r4 smoke 选定 = {model_id} (1-call smoke 确认仍可用)"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
