# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch4_r3_executor.py
====================================

V4 L14+ batch 4 · round 3 · coze 教师侧 round 3 续采（reask=2）
================================================================

【派工依据】
----------------
- 沿 batch4 r2 executor `7d8e12267790` + r2 result `6a9a362d9869` 口径（基础口径锚）
...
- 派工单 coze 教师批 4 round 3（接力棒，沿 r2 上下文唤醒）：22 caption × 1 call（reask=2）

【本棒范围（沿 dispatch）—— coze 教师侧 round 3（reask=2）续采】
------------------------------------------------------------
1. **沿用 teamo `deepseek-v4-flash` 代表采样**（r2 smoke ok=200 已确认；本棒仅 1-call smoke 防端点飘移）
2. **22 caption × 1 call**（每位 caption 第 3 call；reask_idx=2；目标 round 3 完成度 = 3/5 successful/caption）
3. **目标**：本棒后 22/22 caption 各 ≥3 successful call；下棒接力至 ≥5 successful/caption
...
---------------------------------------------------------
- 本棒前基线（沿 batch4 r2）：coze 教师侧 = 44 calls / 0 empty / 44 ok / 0.0000 rate
- 本棒 round 3（reask=2）后：22/22 caption 各 ≥3 successful call（理论值；如端点飘移可能回落）
- 终盘结果：result.json 顶层 `per_caption_successful_calls` 块 + `dispatch_target_progress` 块

【retry 修复逻辑沿 batch4 r2 + batch2 r3 + batch1 r6】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = batch1 119 + batch2 22+22+22 + batch4 r1 22 + batch4 r2 22
  → **229 calls / 9 empty / 3.93%**（沿 batch4 r2 result `6a9a362d9869`）
- coze_teacher_cumulative 自身从 r2 终态 44/0/44 起步
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 单棒 >50% 停采：r3 单棒 empty_rate > 50% 也立即停采（沿 dispatch）

【端点取舍（沿 r2 + r1 + r4 + r3 + r2）】
----------------------------
- teamo + tun + deepseek-v4-flash（沿 r2 smoke ok=200 + r2 models_probe 探活清单 sha12=`3c9dc60b5071` 已确认 44 models 含 deepseek-v4-flash）
...
- 派生 JSON 不合并（本棒 `_batch4_r3_*` 与 `_batch4_r2_*` / `_batch4_r1_*` / `_batch2_r*_` / `_batch1_*` 独立）
...
- 不覆盖既有件（r2 executor `7d8e12267790` + r2 result `6a9a362d9869` + r1 executor `e84bffd5dc7d` + r1 result `7d9fe9d319a2` + r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e` + r3 executor `72fd2bb3838e` + r3 result `25251aca6d79` + mapping `98a779d61c1e` + r2 models_probe `3c9dc60b5071` + prereg `05b975a86989` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch4_r3_result.json`（schema = `v4_l14v3_n26/1` + batch=4 + round=3）
- 不覆盖 r2 result `6a9a362d9869` 或 r1 result `7d9fe9d319a2` 或 r4 result `15b8ddc5ba9e` 或 r3 result `25251aca6d79` 或 batch1 r6 result `a4f851154551` 或 r1 result `d245ae4ce385`
- predecessor_sha12 链：mapping `98a779d61c1e` + r2 executor/result（最新接力锚）+ r1 executor/result + r4 executor/result + r3 executor/result（口径锚）+ r2 executor/result + models_probe + batch1 r6 executor/result + prereg `05b975a86989`

【与 r2 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 3（升 r3 命名）；BATCH = 4 不变
2. prompt_id 前缀 `coze_t03_teacher_*`（取代 r2 的 t02）+ 后缀 `_b4_r3`（取代 r2 的 `_b4_r2`）
3. reask_idx = 2（本棒每位 caption 第 3 call；取代 r2 的 reask_idx=1）
4. phase_label：`b4_r3_need2`（round 3 目标 = ≥3 successful/caption，need_more_to_target = 2）
5. is_round_3 = True（语义：本棒 = coze 教师侧 data collection round 3，区别于 r2 的 round 2）
6. smoke_call_only = True（沿 r2 smoke ok=200；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移）
7. predecessor_sha12 链：r2 executor `7d8e12267790` + r2 result `6a9a362d9869`（新增；取代 r2 链的 r1 双件仍保留作为历史累计 baseline）
8. baseline prior：r2 终态 44 calls / 0 empty / 44 ok / 0.0000 rate（取代 r2 链的 22/22/0/0.0）
9. 跨批累计 baseline：r2 终态 229 calls / 9 empty / 3.93% rate（取代 r2 链的 207/9/4.35%）
10. 新增 `coze_teacher_side_round3_progress` 块：本棒后 coze 教师侧 round 3 完成度（≥3 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 229/9/3.93%
12. spec_conformance 重写：coze 教师侧 round 3 续采（reask=2），目标 ≥3 successful/caption
13. model_id_inconsistency_honest_disclosure 字面继承 r1/r2（沿 mapping 拍板代表采样字面）
14. 派生 metadata 字段沿 r1/r2 不动（v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash）
15. per_caption_successful_calls 字段新增 round3_target=3 / round3_met
16. 调度注释：round 3 续采（reask=2）22 caption × 1 call

【边界】
----------
- 不动任何既有件（含 r2 executor `7d8e12267790` + r2 result `6a9a362d9869` + r1 executor `e84bffd5dc7d` + r1 result `7d9fe9d319a2` + r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e` + r3 executor `72fd2bb3838e` + r3 result `25251aca6d79` + r2 executor `e7418f47a130` + r2 result `8c125e257af8` + r1 executor `e52f930654d3` + r1 result `d245ae4ce385` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `a4f851154551` + models_probe `3c9dc60b5071` + mapping `98a779d61c1e` + prereg `05b975a86989`）
- 跑不完拆段报断点
...
KEY_INDEX_TEAMO_1BASED = 15  # 沿 r1/r2 (line 15 = teamo key)
USE_PROXY = True
...
TEACHER = "coze"
SIDE = "teacher"
REASK_R1 = 2  # 本棒 round 3 = 每 caption 第 3 call（reask_idx=2）
TARGET_SUCCESSFUL_PER_CAPTION = 5
SMOKE_CALL_ONLY = True  # 沿 r2 smoke ok=200；本棒仅 1-call smoke 防端点飘移

# 选定的 model_id（沿 mapping 拍板代表采样；r2 已 smoke ok=200）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch4-r3",
        "X-Title": f"deposon-l14v3-batch4-r3-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r2 简化版；仅 1 call 确认端点 ok）
# ============================================================
def smoke_coze_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r2 smoke ok=200 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b4_r3"
    prompt_id = f"coze_t03_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 4 round 3 · coze 教师侧 round 3（reask=2）续采")
    print("沿 r2 deepseek-v4-flash smoke ok=200; 本棒仅 1-call smoke 后跑 22 caption × 1 call")
    print("计划 calls = 22 caption × 1 call = 22 calls + 1 smoke call")
    print("目标: round 3 后 22/22 caption 各 ≥3 successful call")
    print("预算 ≤22 caption calls + 1 smoke call / ≤600s watchdog")
    print("retry 修复沿 r2 (沿 batch2 r3 + batch1 r6)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（b4 r2 终态作为 coze 自身 baseline；cross batch 沿 b4r2 终态）
    with open(B4_R2_RESULT_PATH, "r", encoding="utf-8") as f:
        b4_r2 = json.load(f)
    b4_r2_cross_total = b4_r2["aggregate"]["cross_batch_with_b4r1_baseline"]["cross_total"]
    b4_r2_cross_empty = b4_r2["aggregate"]["cross_batch_with_b4r1_baseline"]["cross_empty"]
    b4_r2_cross_ok = b4_r2["aggregate"]["cross_batch_with_b4r1_baseline"]["cross_ok"]
    b4_r2_cross_rate = b4_r2["aggregate"]["cross_batch_with_b4r1_baseline"]["cross_rate"]
    print(
        f"[BASELINE b4r2 cross batch] total={b4_r2_cross_total} "
        f"ok={b4_r2_cross_ok} empty={b4_r2_cross_empty} rate={b4_r2_cross_rate:.4f}"
    )

    coze_prior_total = b4_r2["aggregate"]["coze_cumulative"]["post_total"]
    coze_prior_ok = b4_r2["aggregate"]["coze_cumulative"]["post_ok"]
    coze_prior_empty = b4_r2["aggregate"]["coze_cumulative"]["post_empty"]
    coze_prior_rate = b4_r2["aggregate"]["coze_cumulative"]["post_rate"]
    print(
        f"[BASELINE b4r2 coze cumulative] total={coze_prior_total} "
    ...
    # 5. Smoke 1-call（deepseek-v4-flash 端点可达性确认；沿 r2 smoke ok=200 已知）
    print("=" * 60)
    print("[SMOKE PHASE] deepseek-v4-flash 1-call smoke（沿 r2 smoke ok=200 已知）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": ROUND,
            "metadata": {
                "task": "L14V3_batch4_round3_coze_teacher_side_round3_reask2",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch4_r3_result.json")
    ...
    # 6. 调度：22 caption × 1 call (round 3 reask=2) 按 caption_id 字母序
    captions_sorted = sorted(captions, key=lambda c: c["id"])
    planned = [{"caption_id": c["id"], "phase": "b4_r3_need2"} for c in captions_sorted]
    ...
    r2_per_cap = b4_r2.get("per_caption_successful_calls", {})
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_3=True,  # 语义：本棒 = coze 教师侧 data collection round 3
        ...
        target_met = "[OK]" if cur_succ >= 3 else "[..]"
        print(
    ...
    # 跨批累计 = b4r2 baseline + coze 本棒
    cross_batch_total = b4_r2_cross_total + r3_total
    cross_batch_empty = b4_r2_cross_empty + cum_empty - coze_prior_empty
    cross_batch_ok = b4_r2_cross_ok + cum_ok - coze_prior_ok
    ...
    for cid in [c["id"] for c in captions]:
        ...
        t = per_cap_total[cid]
        e = per_cap_empty[cid]
        met_r3 = s >= 3  # round 3 目标 = ≥3 successful/caption
    ...
    # 11. coze 教师侧 round 3 进度块
    coze_round3_progress = {
        "round3_target": ">=3 successful call/caption (round 3 reask=2)",
        "met_count_post_r3_round3": met_round3_count,
        "total_captions": len(captions),
        "remaining_count_round3": len(captions) - met_round3_count,
        "met_percentage_round3": round(met_round3_count / len(captions) * 100, 2),
        "still_pending_captions_round3": sorted([
            cid for cid, info in per_caption_successful_calls.items() if not info["round3_met"]
        ]),
    ...
    # model_id_inconsistency_honest_disclosure（沿 r1/r2 字面格式 + coze 不可考特别声明）
    ...
    model_id_disclosure = (
        ...
        f"r1 + r2 + r3 三棒累计 66 calls 0 empty 0 fail, model_returned 字面 = "
        f"deepseek-v4-flash-ga-260731 (主) + deepseek-v4-flash-0731 (次), 均 family 下实例; "
        f"r3 沿 r2 结论仅 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                "PI 2026-09-24 batch4 coze 教师侧 round 3 续采 (reask=2); "
                "沿 r2 deepseek-v4-flash smoke ok=200; r3 仅 1-call smoke; "
                "22 caption × 1 call = 22 calls (按 caption_id 字母序); "
                "目标 ≥3 successful/caption"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            "batch_index": BATCH,
            "round3_target_per_caption": 3,
            "round3_reask_idx": REASK_R1,
            ...
            "endpoint_selection_method": (
                ...
                "r3 沿 r2 smoke ok=200; r3 仅 1-call smoke 防端点飘移; "
            ...
            "tools_fault_param_correction": {
                ...
                "b4_r2_status": "r2 1-call smoke ok=200 + 22 caption × 1 call = 22 calls 全 OK; model_returned 字面 deepseek-v4-flash-ga-260731 (主) + deepseek-v4-flash-0731 (次)",
                "r3_b4_status": "r3 仅 1-call smoke (沿 r2 结论)；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r2 deepseek-v4-flash smoke ok=200; r3 节省预算避免重复 probe; "
            ...
            "diff_vs_r2_executor": [
                "1. ROUND=3 (升 r3 命名)；BATCH=4 不变",
                "2. prompt_id 前缀 coze_t03_teacher_* (取代 r2 的 t02)；后缀 _b4_r3 (取代 r2 的 _b4_r2)",
                "3. reask_idx=2 (本棒每 caption 第 3 call；取代 r2 的 reask_idx=1)",
                "4. phase_label: b4_r3_need2 (round 3 目标 = ≥3 successful/caption，need_more_to_target=2)",
                "5. is_round_3=True (语义: coze 教师侧 data collection round 3，区别于 r2 的 round 2)",
                "6. smoke_call_only=True (沿 r2 smoke ok=200；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移)",
                "7. predecessor_sha12 链: b4_r2_executor 7d8e12267790 + b4_r2_result 6a9a362d9869 (新增；取代 r2 链的 b4_r1/r4/r3/probe/batch1_r6/prereg 仍保留)",
                "8. baseline prior: r2 终态 44 calls / 0 empty / 44 ok / 0.0000 rate (取代 r2 链的 22/22/0/0.0)",
                "9. 跨批累计 baseline: r2 终态 229 calls / 9 empty / 3.93% rate (取代 r2 链的 207/9/4.35%)",
                "10. 新增 coze_teacher_side_round3_progress 块 (round 3 完成度 ≥3 successful/caption)",
                "11. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 229/9/3.93%",
                "12. spec_conformance 重写: coze 教师侧 round 3 续采 (reask=2)",
                "13. model_id_inconsistency_honest_disclosure 字面继承 r1/r2 (沿 mapping 拍板代表采样字面) + r1+r2+r3 累计补注",
                "14. 派生 metadata 字段沿 r1/r2 不动 (v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash)",
                "15. per_caption_successful_calls 字段新增 round3_target=3 / round3_met",
                "16. 调度注释: coze 教师侧 round 3 续采 (reask=2) 22 caption × 1 call",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "r3 沿 r2 deepseek-v4-flash 代表采样结论 (sha12=6a9a362d9869 实测 22 calls 全 OK); "
                "不再做 3-candidate probe fallback; 1-call smoke 失败立即 abort; "
                "r3 与 r2/r1/r4/r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并); "
                "coze 自身 baseline = 44/44/0/0.0 (沿 r2 终态); "
                "cross batch baseline = 229/220/9/3.93% (沿 r2 终态)"
            ),
        },
        ...
        "coze_teacher_side_round3_progress": coze_round3_progress,
        "dispatch_target_progress": {
            "target_round3": ">=3 successful calls/caption (round 3 reask=2)",
        ...
        "aggregate": {
            ...
            "cross_batch_with_b4r2_baseline": {
                "b4r2_baseline_total": b4_r2_cross_total,
                "b4r2_baseline_empty": b4_r2_cross_empty,
                "b4r2_baseline_ok": b4_r2_cross_ok,
                "b4r2_baseline_rate": b4_r2_cross_rate,
            ...
            "r3_empty_response_rate_threshold_K_N26_N2_single_batch": SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
            "r3_empty_response_above_threshold_single_batch": empty_rate_r3 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
        ...
        "empty_rate_trend": {
            ...
            "cross_batch_cumulative_post_r3": {
                ...
                "note": "b4r2 baseline 229/9/3.93% + r3 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round3_reask2_n_distinct_sample_note": (
                    f"round 3 reask=2 第 3 call 实测；本棒后 coze 教师侧 round 3 = "
                    f"{met_round3_count}/{len(captions)} caption met round 3 目标; "
            ...
            "K_N26_N2_observation": {
                ...
                "single_batch_trigger_K_N26_N2_pass_False": empty_rate_r3 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
                ...
                "monitor_disposition": (
                    ...
                    f"baseline cross batch = {b4_r2_cross_total} calls / {b4_r2_cross_empty} empty / {b4_r2_cross_rate:.4f}"
                ),
                "cross_batch_with_b4r2_baseline_rate": round(cross_batch_rate, 4),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 3 (reask=2) 已完; 下棒可接力 coze 教师侧 round 4 (>=4 more successful/caption)"
            ),
            "note": (
                f"本棒 round 3 (reask=2): actual={r3_total}/{planned_total} calls; "
                f"coze 教师侧 round 3 完成度 = {met_round3_count}/{len(captions)} caption met round 3 目标 (≥3 successful/caption); "
