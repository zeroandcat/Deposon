# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch3_r1_executor.py
====================================

V4 L14+ batch 3 · round 1 · GLM_2 教师侧 round 1（reask=0）
=========================================================

【派工依据】
----------------
- 沿 batch2 r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` 口径锚（GLM_1 教师侧 round 2 reask=1）
...
【本棒范围（沿 dispatch）—— GLM_2 教师侧 round 1（reask=0）】
----------------------------------------------------------
1. **沿用 glm-5.3**（r2 1-call probe ok=200；本棒不再探活，仅 1-call smoke 防端点飘移）
2. **22 caption × 1 call**（每位 caption 第 1 call；reask_idx=0；目标 1/5 successful/caption）
...
---------------------------------------------------------
- 本棒前基线（GLM_2 教师侧）= 0 calls / 0 empty / 0 ok / 0% rate（GLM_2 首棒）
- 跨批累计 baseline = GLM_1 r3 末态 **163 calls / 9 empty / 154 ok / 5.52%**（沿 r3 result）
...
【retry 修复逻辑沿 batch1 r6 + batch2 r2（沿 dispatch §3）】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = **163 calls / 9 empty / 5.52%**（沿 GLM_1 r3 result，本棒前 GLM_1 r3 末态）
- GLM_2 自身从 0 起步；单棒 empty_rate >0.50 立即停采报告（dispatch §5 字面）
...
- model_id 字面 = glm-5.3
  ...
  - GLM_2 与 GLM_1 不同教师身份 = 不同语料（V3 字面 same lineage）
...
- 派生 JSON 不合并（本棒 `_batch3_r1_*` 与 `_batch2_r3_*` / `_batch2_r2_*` / `_batch2_r1_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 双件 + r1 探活件 + models_probe `3C9DC60B5071` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch3_r1_result.json`（schema = `v4_l14v3_n26/1` + batch=3 + round=1）
- 不覆盖 r3 result `25251ACA6D79` 或 batch1 r6 result `A4F851154551` 或 r2 result `8C125E257AF8` 或 r1 result `D245AE4CE385`
- predecessor_sha12 链：r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79` + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 1（BATCH=3 不变；新批 batch=3 区分 GLM_2 与 GLM_1 同 model_id 不同教师）
2. prompt_id 前缀 `glm2_t01_teacher_*`（取代 r3 的 `glm1_t02_teacher_*`）
   - t01 = teacher side round 1（取代 r3 的 t02 round 2）
   - 后缀 `_b3_r1`（取代 r3 的 `_b2_r3`）
3. TEACHER = `GLM_2`（取代 r3 的 `GLM_1`）；新增 `teacher_label=GLM_2` 顶层字段
4. REASK_R1 = 0（本棒每位 caption 第 1 call；取代 r3 的 reask_idx=1）
5. phase_label：`b3_r1_need1`（取代 r3 的 `b2_r3_need1`）
6. is_round_2 = False（语义：本棒为 GLM_2 教师侧 data collection round 1）
7. smoke_call_only = True（沿 r3）
8. predecessor_sha12 链：r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79`（取代 r3 链的 r2_executor/r2_result 仍保留）
9. baseline prior：GLM_2 prior = 0 calls / 0 empty / 0 ok / 0% rate（取代 r3 链的 GLM_1 = 22/0/22）
11. 跨批累计 baseline：r3 终态 **163 calls / 9 empty / 5.52%**（取代 r3 链的 141/9/6.38%）
12. 新增 `glm2_teacher_side_round1_progress` 块（本棒后 GLM_2 教师侧 round 1 完成度 ≥1 successful/caption）
13. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 163/9/5.52%
14. spec_conformance 重写：GLM_2 教师侧 round 1（reask=0），目标 ≥1 successful call
15. model_id_shared_with = GLM_1 顶层字段（明示与 GLM_1 同 model_id 不同教师身份）
16. model_id_inconsistency_honest_disclosure 改写：明示 GLM_2 与 GLM_1 复用 glm-5.3 + ask_748d9242 映射拍板
...
# 本棒调度（沿 dispatch）
BATCH = 3  # 新批 batch=3 区分 GLM_2 与 GLM_1
ROUND = 1
TEACHER = "GLM_2"  # 取代 r3 的 GLM_1
TEACHER_LABEL = "GLM_2"  # 顶层字段明示
MODEL_ID_SHARED_WITH = "GLM_1"  # 顶层字段明示与 GLM_1 同 model_id 不同教师
SIDE = "teacher"
...
TARGET_PER_CAPTION_ROUND1 = 1  # 本棒 round 1 完成目标 ≥1 successful/caption
SMOKE_CALL_ONLY = True  # 沿 r3 probe=glm-5.3 ok=200；本棒仅 1-call smoke 不再做 3-candidate probe

# 选定的 model_id（沿 r2/r3 探活结论，不重探）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch3-r1",
        "X-Title": f"deposon-l14v3-batch3-r1-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# 提示构造（沿 r3 + GLM_2 教师身份微调）
# ============================================================
# GLM_2 系统提示：与 GLM_1 同 model_id 不同教师身份（沿 ask_748d9242 映射拍板）
# 不同语料 = GLM_2 教师身份的 system prompt 与 GLM_1 略异，凸显 GLM_2 作为独立教师
PROMPT_SYSTEM = (
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 3 round 1 · {TEACHER} 教师侧 round 1（reask=0）")
    ...
    print("retry 修复沿 r3 (沿 batch1 r6 + batch2 r2)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（GLM_2 首棒 + r3 末态跨批）
    # GLM_2 自身从 0 起步（首次采）
    glm2_prior_total = 0
    glm2_prior_empty = 0
    glm2_prior_ok = 0
    glm2_prior_rate = 0.0
    print(f"[BASELINE GLM_2] total={glm2_prior_total} ok={glm2_prior_ok} empty={glm2_prior_empty} rate={glm2_prior_rate:.4f} (GLM_2 首棒)")

    # 跨批累计基线 = GLM_1 r3 末态（163 calls / 9 empty / 5.52%）
    ...
    glm1_post_total = r3["aggregate"]["glm1_cumulative"]["post_total"]
    glm1_post_empty = r3["aggregate"]["glm1_cumulative"]["post_empty"]
    glm1_post_ok = r3["aggregate"]["glm1_cumulative"]["post_ok"]
    glm1_post_rate = r3["aggregate"]["glm1_cumulative"]["post_rate"]
    print(f"[BASELINE GLM_1 r3] total={glm1_post_total} ok={glm1_post_ok} empty={glm1_post_empty} rate={glm1_post_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r2/r3 probe 结论 glm-5.3 ok=200）
    print("=" * 60)
    print(f"[SMOKE PHASE] {MODEL_GLM_2} 1-call smoke（沿 r2/r3 probe 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 1,
            "metadata": {
                "task": f"L14V3_batch3_round1_{TEACHER}_teacher_side_round1_reask0",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch3_r1_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b3_r1_need1"} for c in captions_sorted]
    ...
    for i, item in enumerate(planned):
        ...
        # GLM_2 首棒：prior=0；首次判 = post-call 立即生效
        if cum_total > 0:
        ...
        if r1_total > 0:
            current_batch_rate = r1_empty / r1_total
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=False,  # 语义：本棒 = GLM_2 教师侧 data collection round 1
        ...
        if r1_total > 0:
            batch_rate_post = r1_empty / r1_total
    ...
    # 跨批累计 = GLM_1 r3 baseline + GLM_2 本棒
    cross_batch_total = cross_prior_total + r1_total
    cross_batch_empty = cross_prior_empty + cum_empty
    cross_batch_ok = cross_prior_ok + cum_ok
    ...
    # 11. GLM_2 教师侧 round 1 进度块
    glm2_round1_progress = {
    ...
    # model_id_inconsistency_honest_disclosure（GLM_2 与 GLM_1 复用 glm-5.3；ask_748d9242 映射拍板）
    ...
    model_id_disclosure = (
        ...
        f"r2 探活确认 glm-5.3 ok=200; r3 GLM_1 同 model_id 复用成功; 本棒 GLM_2 沿 r2/r3 结论仅做 1-call smoke 防端点飘移; "
        f"teacher_label = GLM_2 (顶层字段明示本棒教师身份); "
        f"model_id_shared_with = GLM_1 (顶层字段明示与 GLM_1 同 model_id 不同教师); "
        f"GLM_2 vs GLM_1 差异 = system prompt 标识 GLM_2 教师身份 (sibling lineage to GLM_1, same underlying model with distinct teacher corpus identity); "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                f"PI 2026-09-24 batch3 GLM_2 教师侧 round 1 (reask=0); "
                f"沿 r3 GLM_1 glm-5.3 同 model_id 复用结论 (1-call smoke ok=200); "
                ...
                f"目标 ≥1 successful/caption (round 1 完成)"
            ),
            "teacher": TEACHER,
            "teacher_label": TEACHER_LABEL,
            "model_id_shared_with": MODEL_ID_SHARED_WITH,
            ...
            "endpoint_selection_method": (
                "沿 r2/r3 实测 (teamo 端点); 本棒沿用 teamo 端点; "
            ...
            "tools_fault_param_correction": {
                ...
                "r3_b2_status": "r3 GLM_1 同 model_id 复用 glm-5.3 成功 (沿 r2 结论)",
                "b3_r1_status": "b3_r1 GLM_2 沿 r3 GLM_1 结论仅做 1-call smoke；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r2/r3 glm-5.3 探活结论；本棒节省预算避免重复 probe；"
            ...
            "diff_vs_r3_executor": [
                "1. BATCH=3 ROUND=1 (新批 batch=3 区分 GLM_2 与 GLM_1)",
                "2. TEACHER = GLM_2 (取代 r3 的 GLM_1)；新增顶层字段 teacher_label=GLM_2 + model_id_shared_with=GLM_1",
                "3. prompt_id 前缀 glm2_t01_teacher_* (取代 r3 的 glm1_t02_teacher_*)；后缀 _b3_r1 (取代 _b2_r3)",
                "4. REASK_R1=0 (本棒每 caption 第 1 call；取代 r3 的 reask_idx=1)",
                "5. phase_label: b3_r1_need1 (取代 r3 的 b2_r3_need1)",
                "6. is_round_2=False (语义: GLM_2 教师侧 data collection round 1)",
                "7. predecessor_sha12 链: r3_executor 72FD2BB3838E + r3_result 25251ACA6D79 (取代 r3 链的 r2_executor/r2_result 仍保留)",
                "8. baseline prior: GLM_2 prior = 0 calls / 0 empty / 0 ok / 0% rate (GLM_2 首棒；取代 r3 链的 GLM_1 = 22/0/22)",
                "9. 跨批累计 baseline: r3 终态 163 calls / 9 empty / 5.52% (取代 r3 链的 141/9/6.38%)",
                "10. 新增 glm2_teacher_side_round1_progress 块 (round 1 完成度 ≥1 successful/caption)",
                "11. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 163/9/5.52%",
                "12. spec_conformance 重写: GLM_2 round 1 (reask=0)",
                "13. per_caption_successful_calls 字段新增 round1_target=1 / round1_met",
                "14. 调度注释: GLM_2 round 1 (reask=0) 22 caption × 1 call",
                "15. GLM_2 vs GLM_1 差异 = system prompt 标识 GLM_2 教师身份 (sibling lineage to GLM_1)",
                "16. 单棒 empty_rate >50% 停采监控 (dispatch §5)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "b3_r1 沿 r2/r3 glm-5.3 探活结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；b3_r1 与 r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        "models_listing_summary_ref": {
            ...
            "note": (
                "b3_r1 不重探 /v1/models；仅引用 r2 探活清单 (sha12=3C9DC60B5071) "
        ...
        "glm2_teacher_side_round1_progress": glm2_round1_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch3_round1_round1_reask0": {
                ...
                "note": "b3_r1 GLM_2 教师侧 round 1 reask=0 22 caption × 1 call",
            },
            "batch3_round1_cumulative_glm2": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round1_reask0_n_distinct_sample_note": (
                    f"round 1 reask=0 第 1 call 实测；本棒后 GLM_2 教师侧 round 1 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"GLM_2 prior = {glm2_prior_total} calls / {glm2_prior_empty} empty / {glm2_prior_rate:.4f} (首棒); "
                    f"cross-batch rate = {cross_batch_rate:.4f}; "
                    f"baseline cross batch = {cross_prior_total} calls / {cross_prior_empty} empty / {cross_prior_rate:.4f} (r3 末态 GLM_1)"
                ),
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 1 (reask=0) 已完; 下棒可接力 GLM_2 教师侧 round 2 (>=1 more successful/caption)"
            ),
            "note": (
                f"本棒 round 1 (reask=0): actual={r1_total}/{planned_total} calls; "
                f"GLM_2 教师侧 round 1 完成度 = {met_round1_count}/{len(captions)} caption met round 1 目标 (≥1 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"GLM_2 model_id 沿 r2/r3 探活选定 = {model_id} (与 GLM_1 同 model_id 不同教师；1-call smoke 确认仍可用)"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
