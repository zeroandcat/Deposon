# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch8_r3_executor.py
=====================================

V4 L14+ batch 8 · round 3（reask=2）—— GLM_2 蒸馏侧 round 3 接力棒（沿 r2 口径全套）
==================================================================================
...
【本棒范围（沿 dispatch）—— GLM_2 蒸馏侧 round 3 接力棒】
--------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 dispatch 字面）
2. **计数口径**：
   - **distill 侧独立计数**：`glm2_t01_distill_*` prompt_id 前缀为域（沿 r2 字面）
   - 目标 = GLM_2 蒸馏侧 round 3 ≥3 successful call/caption（**各 caption 由 2/5 顺利补至 3/5**）
   - 当前 GLM_2 蒸馏侧 = 22 caption 各 2 successful（沿 r1+r2 quadruples 过滤；r1+r2 44/44 全 OK）
   - 本棒补 1 call/caption → GLM_2 蒸馏侧各 caption 由 2 → 3 successful（**round 3 接力判定**）
3. **model 口径**（沿 r2）：
   - `model_id_sent` = `glm-5.3`（teamo 端点；与 GLM_1 现行采样复用）
   - `teacher_label` = `GLM_2`
   - `model_id_shared_with` = `GLM_1`（GLM_2 = 同 model 不同语料字面沿 mapping §1.2 派生字段）
4. **system prompt 沿用**（沿 r2 `PROMPT_SYSTEM` 字面不重写）：
...
5. **round 3 接力判定块 `glm2_distill_side_round3_progress`**：
   - `met_count_post_r3`：本棒后达标 caption 数（预期 22）
   - `still_pending_round3`：未达标 caption 列表（预期空）
   - `round3_progress_disposition`：GLM_2 蒸馏侧 round 3 接力判定
   - **非收官棒**：本棒仅 round 3 接力（2→3）；后续 r4/r5 接力至 5/5 successful/caption（沿 batch6 r5 收官口径类比）

【每 caption successful 计数盘（沿 dispatch §6）—— GLM_2 蒸馏侧独立域】
----------------------------------------------------------------------
- 本棒前基线（GLM_2 蒸馏侧独立计数）：GLM_2 蒸馏侧 = 22 caption 各 2 successful（沿 r1+r2 quadruples 过滤；r1+r2 44/44 全 OK）
- 本棒增量计算：每 call 后实时更新 `per_cap_succ_glm2_distill` 字典
- 终盘结果（GLM_2 蒸馏侧独立计数）：
  - `per_caption_glm2_distill_only_successful_calls` 块（GLM_2 蒸馏侧独立计数）
  - `glm2_distill_side_round3_progress` 块（**round 3 接力判定**）
  - `dispatch_target_progress_glm2_distill` 块（GLM_2 蒸馏侧目标进展 — round 3 接力态）
...
-------------------------------------------
- 累计 = batch8 r2 终态 + batch8 r3 GLM_2 增量（GLM_2 蒸馏侧 round 3）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r2 产物）：
  - prior (b1+r2+r3+r4+r5+b6r1+r6r2+r6r3+r6r4+r6r5+b8r1+b8r2): total=252, empty=9, **rate=3.57%**
  - r3 worst case (22 EMPTY): cumulative = 31/274 = 11.31% (仍 < 50% 阈值)
  - r3 best case (0 EMPTY): cumulative = 9/274 = 3.28% (仍 < 50% 阈值)
...
- 派生 JSON 不合并（本棒 `_batch8_r3_*` 与前棒所有 _batch*_r* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch8_r3_result.json`
  （schema = v4_l14v3_n26/1 + batch=8 + round=3 + teacher=GLM_2）
- 不覆盖 batch8 r2 result `7b23996b5f4c` 或前棒 result（沿 dispatch §4）

【与 r2 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------------
1. **ROUND = 3 / REASK_R1 = 2** 参数化（r2 = 2/1；本棒 = 3/2；reask_idx=2 = 第 3 call/caption on GLM_2 distill side）
2. **prompt_id 后缀**：`_r2` → `_r3`
3. **baseline 切换**：
   - 跨批累计：batch8 r2 终态 cumulative (252/9/243/3.57%)
   - GLM_2 蒸馏侧独立：22 caption 各 2 successful（沿 r1+r2 quadruples 过滤）
4. **predecessor_sha12 链新增**：batch8 r2 executor `6a9bbc43c808` + r2 result `7b23996b5f4c`
5. **target 切换**：≥2 successful/caption (r2 续采阈值) → ≥3 successful/caption (r3 接力阈值)
6. **判定块命名**：
   - `glm2_distill_side_round2_progress` (r2 续采) → `glm2_distill_side_round3_progress` (r3 接力)
   - 字段 `met_target_round2` → `met_target_round3`
7. **is_round_3_progress_round 标记**：metadata.is_round_2_progress_round = True (r2) → is_round_3_progress_round = True (r3)
8. **mapping_sha12_discrepancy_note 沿用**（不动 mapping 文件）
9. **diff_vs_r2_executor 新块**（取代 r2 的 diff_vs_r1_executor）
10. **r3 增量计数 → GLM_2 蒸馏侧各 caption 由 2/5 顺利补至 3/5**（r3 OK 全达标 if no fails）
...
TARGET_SUCCESSFUL_PER_CAPTION = 3  # GLM_2 蒸馏侧 round 3 接力阈值（≥3 successful calls/caption；各 caption 由 2 → 3 successful）
TARGET_FINAL_PER_CAPTION = 5  # GLM_2 蒸馏侧最终目标阈值（5 successful calls/caption；沿 batch6 r5 收官口径类比；后续 r4-r5 接力）
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 8 round 3 | GLM_2 distill side | reask=2 — round 3 接力棒")
    print(f"模型: glm-5.3 (teamo; GLM_2 复用 GLM_1 = 同 model 不同语料)")
    print(f"计划 calls = 22 (22 caption × 1 call)")
    print(f"计数口径: GLM_2 蒸馏侧独立 (prompt_id 前缀 glm2_t01_distill_) — 沿 r2")
    print(f"GLM_2 蒸馏侧 baseline: 22 caption 各 2 successful (沿 r1+r2 quadruples 过滤; r1+r2 44/44 全 OK)")
    print(f"目标: GLM_2 蒸馏侧 ≥3 successful calls/caption (round 3 接力阈值)")
    print(f"最终目标: GLM_2 蒸馏侧 ≥5 successful calls/caption (后续 r4-r5 接力)")
    ...
    # 2. 加载 r1 + r2 + batch6 r5 + batch2 r2 result（用于 baseline + 锚链）
    ...
    # 3. GLM_2 蒸馏侧独立计数（filter by prompt_id 前缀）—— 本棒前 = r1+r2 quadruples
    ...
    print(f"[GLM_2 DISTILL prior] total={cum_total_prior_glm2} ok={cum_ok_prior_glm2} "
          f"empty={cum_empty_prior_glm2} (源 = r1+r2 quadruples distill 前缀过滤; 各 caption 各 2 successful)")

    # 4. 跨批累计监控（沿 r2 cumulative 链路 + GLM_2 r3 增量）
    r2_cum = b8r2["aggregate"]["cumulative"]
    ...
    PLANNED_CALLS = [
        {"caption_id": c["id"], "phase": f"r3_glm2_distill_{c['id']}"}
        for c in captions
    ]
    ...
    # 11. GLM_2 蒸馏侧 **round 3 接力判定**（沿 dispatch 字面块名 `glm2_distill_side_round3_progress`）
    glm2_distill_side_round3_progress = {
        "all_22_captions_met_target": met_target_count_glm2_distill == len(captions),
        "met_count_post_r3": met_target_count_glm2_distill,
        "total_captions": len(captions),
        "still_pending_round3": sorted([
            cid for cid, info in per_caption_glm2_distill_only_successful_calls.items() if not info["met_target_round3"]
        ]),
        "round3_progress_disposition": (
            f"GLM_2 蒸馏侧 round 3 接力达成 → 22/22 caption ≥3 successful (2→3 顺利) → 进入 round 4 接力（目标 ≥{TARGET_FINAL_PER_CAPTION} successful calls/caption 收官）"
            if met_target_count_glm2_distill == len(captions)
            else f"GLM_2 蒸馏侧 round 3 未达标 {len(captions) - met_target_count_glm2_distill} caption → 留 worker 下一棒接力（未达接力）"
        ),
        ...
        "glm2_distill_side_round3_target_met_at_round": (
            "r3" if met_target_count_glm2_distill == len(captions) else "未达 (next 棒接力)"
        ),
    }

    # 12. dispatch §6 收尾盘点 —— GLM_2 蒸馏侧
    dispatch_target_progress_glm2_distill = {
        "target_round3": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (GLM_2 distill side only; round 3 接力阈值)",
        "target_final": f">={TARGET_FINAL_PER_CAPTION} successful calls/caption (GLM_2 distill side only; 最终收官阈值)",
        "met_count_post_r3": met_target_count_glm2_distill,
        "total_captions": len(captions),
        "remaining_count_round3": len(captions) - met_target_count_glm2_distill,
        ...
        "met_percentage_round3": round(met_target_count_glm2_distill / len(captions) * 100, 2),
        "still_pending_round3_captions": sorted([
            cid for cid, info in per_caption_glm2_distill_only_successful_calls.items() if not info["met_target_round3"]
        ]),
        "is_round3_progress_round": True,
    }
    ...
    result = {
        ...
        "round": ROUND,
        "metadata": {
            "task": "L14V3_batch8_round3_GLM_2_distill_side_reask2_progress_round_independent_counting",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 8 round 3 GLM_2 distill side reask=2 接力棒",
            "is_finish_round": False,
            "is_round_3_progress_round": True,
            ...
            "prompt_construction_basis": {
                ...
                "reproduction_source_anchor_sha12": "6a9bbc43c808",
                "reproduction_source_anchor_path": "results/_v4_supp_l14v3_batch8_r2_executor.py",
                "reproduction_source_anchor_role": "r3 沿 r2 口径续跑 (GLM_2 蒸馏侧 22 caption × 5 calls 系列 round 3)",
                "v3_distill_pipeline_style_inherited": (
                    "V3-style concept-graph distillation continuation prompt (沿 r2 executor 6a9bbc43c808 + batch6 r5 executor 8a8babb911b8);"
            ...
            "spec_conformance": (
                ...
                "K-N26-N2 累计监控 + GLM_2 蒸馏侧独立计数（沿 r1+r2 quadruples 过滤） + "
                "round 3 接力判定块 glm2_distill_side_round3_progress"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            ...
            "tools_fault_param_correction": {
                ...
                "r3_status": "retry 修复沿 batch6 r5; r3 接力棒仅按 22 caption × 1 call 续采; hard fail 保持 fail path 立即返回",
                "rationale": (
                    "沿 batch6 r5 修复 + tracking 改进；r3 接力棒仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r2_executor": [
                "1. ROUND=3/REASK_R1=2 参数化 (r2 = 2/1; 本棒 = 3/2; reask_idx=2 = 第 3 call/caption on GLM_2 distill side)",
                "2. prompt_id 后缀 _r2 → _r3",
                "3. baseline 切换: 跨批累计 = r2 终态 cumulative (252/9/243/3.57%); GLM_2 蒸馏侧独立 = 22 caption 各 2 successful (沿 r1+r2 quadruples 过滤)",
                "4. predecessor_sha12 链新增 r2_executor 6a9bbc43c808 + r2_result 7b23996b5f4c",
                "5. target 切换: ≥2 successful/caption (r2 续采阈值) → ≥3 successful/caption (r3 接力阈值)",
                "6. 判定块命名: glm2_distill_side_round2_progress (r2 续采) → glm2_distill_side_round3_progress (r3 接力); 字段 met_target_round2 → met_target_round3",
                "7. is_round_3_progress_round 标记: metadata.is_round_2_progress_round = True (r2) → is_round_3_progress_round = True (r3)",
                "8. mapping_sha12_discrepancy_note 沿用 (不动 mapping 文件)",
                "9. diff_vs_r2_executor 新块 (取代 r2 的 diff_vs_r1_executor)",
                "10. r3 增量计数 → GLM_2 蒸馏侧各 caption 由 2/5 顺利补至 3/5 (r3 OK 全达标 if no fails)",
        ...
        "glm2_distill_side_round3_progress": glm2_distill_side_round3_progress,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_3_n_distinct_sample_note": (
                    "round 3 接力棒第 3 call per caption 实测；GLM_2 蒸馏侧独立计数 = "
                    f"22 caption 各 {per_cap_succ_glm2_distill[list(per_cap_succ_glm2_distill.keys())[0]]} successful (沿 r1+r2+r3 quadruples, prompt_id 前缀 glm2_t01_distill_ filter 域);"
                    f"**GLM_2 蒸馏侧 round 3 接力判定**: {met_target_count_glm2_distill}/{len(captions)} caption 达 ≥3 successful;"
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        "本棒未触发 K-N26-N2 累计阈值；跑至 1500s watchdog 提前停批 / 全 22 calls 完成 (round 3 接力态)"
                    )
                ),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "task_append 接力棒至 GLM_2 蒸馏侧 round 4 (>=2 more successful/caption); "
            ...
            "note": (
                ...
                + (f"；K-N26-N2 累计阈值触发：cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   if k_n26_n2_triggered else
                   f"；撞 {WALL_TIME_BUDGET_S}s watchdog 提前停批 ({len(captions) - met_target_count_glm2_distill} caption 未达 round 3 接力阈值) 走 task_append 接力棒"
                   if wall_time_s > WALL_TIME_BUDGET_S and met_target_count_glm2_distill < len(captions) else
                   f"；**GLM_2 蒸馏侧 round 3 接力达成** {met_target_count_glm2_distill}/{len(captions)} caption ≥3 successful ✓"
                   if met_target_count_glm2_distill == len(captions) else
                   f"；GLM_2 蒸馏侧 round 3 未达标 {len(captions) - met_target_count_glm2_distill} caption → 下一棒接力 (未达接力)")
            ),
        },
        "self_scan": {
    ...
    # 17. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch8_r3_result.json")
    ...
    print(f"  glm2_distill_side_round3_met: {met_target_count_glm2_distill}/{len(captions)}")
    print(f"  *** all_22_captions_met_round3_target: {glm2_distill_side_round3_progress['all_22_captions_met_target']} ***")
    print(f"  round3_progress_disposition: {glm2_distill_side_round3_progress['round3_progress_disposition']}")
    ...
    if glm2_distill_side_round3_progress["still_pending_round3"]:
        print(f"  still_pending_round3: {glm2_distill_side_round3_progress['still_pending_round3']}")
    return 0
