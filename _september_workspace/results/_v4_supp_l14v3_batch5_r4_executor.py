# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch5_r4_executor.py
=====================================

V4 L14+ batch 5 · round 4 · minimax 教师侧 round 4（reask=3）续采
==================================================================

【派工依据】
----------------
- 沿 batch5 r3 executor `027EF71C0668` + r3 result `8E2ACDACFA1E` 口径（基础口径锚，本棒同 agent 唤醒保上下文）
...
- 沿 batch1 r6 executor `5F02C7E0F094` + r6 result `A4F851154551` retry 修复口径（沿 r3 链记录值）
...
- 拍板入元数据沿 r3：ask_249ba8854ee343f7d9aa34de（2026-09-24 23:38）prompt_repro=accept_repro
  —— 蒸馏/采样数据效力 = V4 复现口径（非 V3 原文复刻）声明沿用

【本棒范围（沿 dispatch）—— minimax 教师侧 round 4（reask=3）续采】
------------------------------------------------------------------
1. **沿用 mimo-v2.6-pro**（沿 r3 1-call smoke ok=200；本棒沿 r3 结论不再做完整 probe）
...
4. **endpoint_migrated**: true（沿 r3）
5. **lineage_discontinuity**: true（沿 r3）
6. **prompt_repro**: accept_repro（沿 r3 拍板 ask_249ba8854ee343f7d9aa34de）
...
---------------------------------------------
- 本棒前基线（沿 r3 终态）：minimax 教师侧 = 66 calls / 0 empty / 66 ok / 0.0000 rate
...
【retry 修复逻辑沿 batch1 r6 + r3（沿 dispatch §3）】
-----------------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = r3 终态 229 calls / 9 empty / 220 ok / 3.93%
- 累计 = r3 cross-batch baseline + minimax 本棒（minimax 自身从 66 起步）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）

【端点取舍（沿 r3）】
------------------
- mimo + 无代理 + mimo-v2.6-pro（沿 r3 1-call smoke ok=200；r4 沿 r3 结论仅做 1-call smoke 防端点飘移）
...
- 派生 JSON 不合并（本棒 `_batch5_r4_*` 与 `_batch5_r3_*` / `_batch5_r2_*` / `_batch5_r1_*` / `_batch2_r3_*` / `_batch2_r2_*` / `_batch2_r1_*` / `_batch1_r6_*` 独立）
...
- **predecessor 命名沿 b5_* 前缀防冲突**（r3 修复先例：r1/r2/r3 → b5_r1/b5_r2/b5_r3 重命名；本棒新增 b5_r4）

【产物】
--------
- 写入：`results/_v4_supp_l14v3_batch5_r4_result.json`（schema = `v4_l14v3_n26/1` + batch=5 + round=4）
- 不覆盖 r3 result `8E2ACDACFA1E` 或 r3 executor `027EF71C0668` 或任何既有件
- predecessor_sha12 链：b5_r3_executor `027EF71C0668` + b5_r3_result `8E2ACDACFA1E` + b5_r2_executor `F97E46E3739E` + b5_r2_result `B0F76B5C3522` + b5_r1_executor `EF26ADF4669F` + b5_r1_result `120D45D1774F` + b2_r3_executor + b2_r3_result + b2_r2_executor + b2_r2_result + batch1_r6_executor + batch1_r6_result + b2_r1_executor + b2_r1_result + models_probe + mapping `98A779D61C1E` + prereg `05B975A86989`

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 4（升 r4 命名）；BATCH = 5 不变
2. prompt_id 后缀 `_b5_r4`；`minimax_t04_teacher_*`（取代 r3 的 `minimax_t03_teacher_*_b5_r3`）
3. reask_idx = 3（本棒每 caption 第 4 call；取代 r3 的 reask_idx=2）
4. phase_label：`b5_r4_need1`（取代 r3 的 `b5_r3_need1`）
5. is_round_4 = True（语义：本棒 = minimax 教师侧 data collection round 4；区别于 r3 的 is_round_3=True）
6. is_round_3 = False（语义：已过 round 3，进入 round 4；区别于 r3 的 is_round_3=True）
7. 新增 is_round_4 字段（取代 r3 的 is_round_3 在 _meta 标记）
8. smoke_call_only = True（沿 r3 结论；r4 仅 1-call smoke 防端点飘移；不再做 3-candidate probe fallback）
9. predecessor_sha12 链：b5_r3_executor `027EF71C0668` + b5_r3_result `8E2ACDACFA1E`（新增；取代 r3 链的 b5_r2_executor + b5_r2_result 仍保留）
10. baseline prior：r3 终态 minimax 66 calls / 0 empty / 66 ok / 0.0000 rate（取代 r3 链的 44/0/44）
11. 跨批累计 baseline：r3 终态 229 calls / 9 empty / 220 ok / 3.93%（取代 r3 链的 207/9/198/4.35%）
12. 新增 `minimax_teacher_side_round4_progress` 块（本棒后 minimax 教师侧 round 4 完成度 ≥4 successful/caption）
13. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 229/9/3.93%
14. spec_conformance 重写：minimax 教师侧 round 4 续采（reask=3）
15. per_caption_successful_calls 字段新增 round4_target=4 / round4_met
16. 调度注释：round 4 续采（reask=3）22 caption × 1 call
17. **predecessor 命名沿 b5_* 前缀防冲突**（沿 r3 修复先例）
...
- prompt_repro / data_validity 拍板沿 r3 入元数据；不动 N-26 主预登记 / K-N26-* 字面 / 既有阈值 / 既定结论
"""
...
TEACHER = "minimax"
SIDE = "teacher"
REASK_R3 = 3  # 本棒 round 4 = 每 caption 第 4 call（reask_idx=3）
TARGET_SUCCESSFUL_PER_CAPTION = 5
ROUND4_TARGET_PER_CAPTION = 4  # 本棒 round 4 目标（区别于 r3 的 3）
SMOKE_CALL_ONLY = True  # 沿 r3 结论；r4 仅 1-call smoke 防端点飘移
...
def call_chat_mimo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch5-r4",
        "X-Title": f"deposon-l14v3-batch5-r4-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
def smoke_mimo_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r3 结论 mimo-v2.6-pro ok=200；仅作端点可达性确认）。"""
    t0 = time.time()
...
# ============================================================
# 单 call + 内层重试（沿 r3 + is_round_4 扩展）
# ============================================================
def run_one_quadruple(
    ...
    suffix = "_b5_r4"
    prompt_id = f"minimax_t04_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 5 round 4 · minimax 教师侧 round 4（reask=3）续采")
    print("沿 r3 mimo-v2.6-pro 1-call smoke ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("prompt_repro=accept_repro (ask_249ba8854ee343f7d9aa34de 2026-09-24 23:38 沿 r3 拍板)")
    ...
    # minimax 自身前基线 = r3 终态（66 calls / 0 empty / 66 ok）
    minimax_prior_total = r3["aggregate"]["minimax_cumulative"]["post_total"]
    minimax_prior_empty = r3["aggregate"]["minimax_cumulative"]["post_empty"]
    minimax_prior_ok = r3["aggregate"]["minimax_cumulative"]["post_ok"]
    minimax_prior_rate = r3["aggregate"]["minimax_cumulative"]["post_rate"]
    print(f"[BASELINE minimax r3] total={minimax_prior_total} ok={minimax_prior_ok} empty={minimax_prior_empty} rate={minimax_prior_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r3 结论 mimo-v2.6-pro ok=200）
    print("=" * 60)
    print("[SMOKE PHASE] mimo-v2.6-pro 1-call smoke（沿 r3 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 4,
            "metadata": {
                "task": "L14V3_batch5_round4_minimax_teacher_side_round4_reask3",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch5_r4_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b5_r4_need1"} for c in captions_sorted]
    ...
    r3_per_cap = r3.get("per_caption_successful_calls", {})
    ...
    print(f"[PRIOR per_caption_succ r3 baseline] {dict(sorted(per_cap_succ.items()))}")
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            is_round_4=True,   # 语义：本棒 = minimax 教师侧 data collection round 4
    ...
    cross_batch_empty = cross_prior_empty + r4_empty
    cross_batch_ok = cross_prior_ok + r4_ok
    ...
    for cid in [c["id"] for c in captions]:
        ...
        t = per_cap_total[cid]
        e = per_cap_empty[cid]
        met_r4 = s >= ROUND4_TARGET_PER_CAPTION  # round 4 目标 = ≥4 successful/caption
    ...
    # 11. minimax 教师侧 round 4 进度块
    minimax_round4_progress = {
    ...
    # data_validity_disclosure（沿 r3 拍板 ask_249ba8854ee343f7d9aa34de）
    data_validity_disclosure = {
        ...
        "honest_disclosure": (
            "本棒 minimax 教师侧 round 4 (reask=3) 22 caption × 1 call 全部产出 "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-25",
            "spec_conformance": (
                "PI 2026-09-25 batch5 minimax 教师侧 round 4 续采 (reask=3); "
                "沿 r3 mimo-v2.6-pro 1-call smoke ok=200 结论；r4 仅 1-call smoke; "
            ...
            "endpoint_selection_method": (
                "沿 r3 实测 (mimo 端点); r4 沿用 mimo 端点; "
                "/v1/models 探活清单沿 r3 models_probe.json (sha12=3C9DC60B5071) 仅作 reference; "
            ...
            "tools_fault_param_correction": {
                ...
                "r3_b5_status": "r3 沿 r2 结论 mimo-v2.6-pro 1-call smoke ok=200",
                "r4_b5_status": "r4 沿 r3 结论仅做 1-call smoke 防端点飘移；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r3 mimo-v2.6-pro smoke 结论；r4 节省预算避免重复 probe；"
            ...
            "diff_vs_r3_executor": [
                "1. ROUND=4 (升 r4 命名)；BATCH=5 不变",
                "2. prompt_id 后缀 _b5_r4；t04_teacher (取代 r3 的 t03_teacher round 3 reask=2)",
                "3. reask_idx=3 (本棒每 caption 第 4 call；取代 r3 的 reask_idx=2)",
                "4. phase_label: b5_r4_need1 (取代 r3 的 b5_r3_need1)",
                "5. is_round_4=True (语义: minimax 教师侧 data collection round 4)",
                "6. is_round_3=False (语义: 已过 round 3)",
                "7. 新增 is_round_4 字段 (取代 r3 的 is_round_3=True)",
                "8. smoke_call_only=True (沿 r3 结论；不再做 3-candidate probe)",
                "9. predecessor_sha12 链: b5_r3_executor 027EF71C0668 + b5_r3_result 8E2ACDACFA1E (新增；r3 链的 b5_r2_executor + b5_r2_result 仍保留)",
                "10. baseline prior: r3 终态 minimax 66 calls / 0 empty / 66 ok / 0.0000 rate (取代 r3 链的 44/0/44)",
                "11. 跨批累计 baseline: r3 终态 229 calls / 9 empty / 220 ok / 3.93% (取代 r3 链的 207/9/198/4.35%)",
                "12. 新增 minimax_teacher_side_round4_progress 块 (round 4 完成度 ≥4 successful/caption)",
                "13. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 229/9/3.93%",
                "14. spec_conformance 重写: minimax 教师侧 round 4 续采 (reask=3)",
                "15. per_caption_successful_calls 字段新增 round4_target=4 / round4_met",
                "16. 调度注释: round 4 续采 (reask=3) 22 caption × 1 call",
            ...
            "iron_rules": {
                ...
                "is_round_4_disclosed": True,
            },
            "run_window_cst": now_iso,
            "stop_reason": stop_reason,
            "interruption_recovery_note": (
                "r4 沿 r3 mimo-v2.6-pro smoke 结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；r4 与 r3/r2/r1/b2_r3/b2_r2/b2_r1/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        "models_listing_summary_ref": {
            ...
            "note": (
                "r4 不重探 /v1/models；仅引用 r3 探活清单 (sha12=3C9DC60B5071) "
        ...
        "minimax_teacher_side_round4_progress": minimax_round4_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch5_round4_cumulative_minimax": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
            ...
            "cross_batch_cumulative_post_r4": {
                ...
                "note": "r3 baseline 229/9/3.93% + r4 minimax 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round4_reask3_n_distinct_sample_note": (
                    f"round 4 reask=3 第 4 call 实测；本棒后 minimax 教师侧 round 4 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"r3 baseline minimax = {minimax_prior_total} calls / {minimax_prior_empty} empty / {minimax_prior_rate:.4f}; "
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 4 (reask=3) 已完; 下棒可接力 minimax 教师侧 round 5 (>=1 more successful/caption, 目标 ≥5 cumulative/caption)"
            ),
            "note": (
                f"本棒 round 4 (reask=3): actual={r4_total}/{planned_total} calls; "
                f"minimax 教师侧 round 4 完成度 = {met_round4_count}/{len(captions)} caption met round 4 目标 (≥4 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"minimax model_id 沿 mapping §1.1 拍板 = {model_id} (1-call smoke 确认仍可用); "
                f"拍板 ask_249ba8854ee343f7d9aa34de prompt_repro=accept_repro 已沿 r3 入 metadata + data_validity_disclosure 块"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
