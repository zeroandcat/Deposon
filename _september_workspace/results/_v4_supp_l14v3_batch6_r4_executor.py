# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch6_r4_executor.py
=====================================

V4 L14+ batch 6 · round 4（reask=3）—— kimi 蒸馏侧独立计数第 4 轮
=====================================================================
...
【本棒范围（沿 dispatch §1）—— kimi 蒸馏侧 round 4（reask=3）】
------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 dispatch 字面）
2. **计数口径沿 r3**（PI 委托保守口径，标注待 PI 复核）：
   - distill 侧独立计数：`kimi_t0*_distill_*` prompt_id 前缀为域（沿 r2/r3 字面）
   - 目标 = distill 侧自身 ≥5 successful/caption
   - 当前 distill 侧 = 3/5（沿 r1+r2+r3 quadruples 累计，每 caption 3 call）
   - 本棒补 1 call/caption → distill 侧 4/5
3. **counting_scope_correction 字段沿 r3**：如实注明；r1 混合计数保留作透明参照

【每 caption successful 计数终盘点（沿 dispatch §6）—— distill 侧独立域】
----------------------------------------------------------------------
- 本棒前基线（distill 侧独立计数，沿 r1+r2+r3 quadruples 过滤）：
  - 22 caption 各 3/5 successful
...
- r3 retry 触发的源 error_category tracking 改进沿用（沿 r3）

【empty_rate 趋势监控 + K-N26-N2 累计阈值停采（沿 dispatch §4 + §5）】
-------------------------------------------
- 累计 = batch1+r2+r3+r4+r5+r1+r2+r3+r4 batch6 全部 cumulative（含 teacher + distill 混合）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r3 产物）：
  - prior (b1+r2+r3+r4+r5+r1+r2+r3): total=164, empty=9, **rate=5.49%**
  - r4 worst case (22 EMPTY): cumulative = 31/186 = 16.67% (仍 < 50% 阈值)
  - r4 best case (0 EMPTY): cumulative = 9/186 = 4.84% (仍 < 50% 阈值)

【端点取舍（沿 r3）】
----------------------------
- teamo + tun + kimi-k3（沿 r3 实测）
- 不重新探活（沿 dispatch §4 「沿 r3 口径续跑」）

【铁律严守（沿 r3）】
------------------------
- R4 key 永不明文（无例外）
- V1–V3 只读不动 / R5 V4 frozen append-only / R6 P-G 不动 / R7 plugin spec 不动
- 派生 JSON 不合并（本棒 `_batch6_r4_*` 与前棒所有 _batch6_* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch6_r4_result.json`
  （schema = v4_l14v3_n26/1 + batch=6 + round=4）
- 不覆盖 r3 result `15a7fecdffe7` 或前棒 result（沿 dispatch §4）

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. 新增参数：`ROUND = 4` / `REASK_R1 = 3`（reask_idx=3 = 第 4 call/caption on distill side）
2. prompt_id 后缀 `_r4`
3. baseline 切换：r3 quadruples = 22 caption 各 3 successful（沿 r1+r2+r3 distill 过滤）
4. predecessor_sha12 链新增 r3_executor `1385d2ca356f` + r3_result `15a7fecdffe7`
5. 新增 `is_round_4` 字段（替代 r3 的 `is_round_3`）
6. 增量计数 → distill 侧各 caption 由 3/5 顺利补至 4/5 (if no fails)

【边界】
----------
- 不动任何既有件（含 r3 executor `1385d2ca356f` + r3 result `15a7fecdffe7`
...
# ============================================================
# 计数口径修正（沿 r3；PI 委托保守口径，标注待 PI 复核）
# ============================================================
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch6-r4",
        "X-Title": "deposon-l14v3-batch6-r4-kimi-distill",
    }
    body = {
        "model": MODEL_KIMI_K3,
        "messages": messages,
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 6 round 4 | kimi distill side | reask=3")
    print("计划 calls = 22 (22 caption × 1 call)")
    print("计数口径: distill 侧独立 (prompt_id 前缀 kimi_t01_distill_) — 沿 r3")
    print("distill 侧 baseline: 22 caption 各 3/5 successful (沿 r1+r2+r3 quadruples 过滤)")
    ...
    # 2. 加载 r1 + r2 + r3 result（distill 侧独立计数基线）
    ...
    print(f"[DISTILL prior] total={cum_total_prior_distill} ok={cum_ok_prior_distill} "
          f"empty={cum_empty_prior_distill} (源 = r1+r2+r3 quadruples distill 前缀过滤)")

    # 4. 累积监控（含 teacher 侧 batch1-r5 + distill 侧 r1+r2+r3）—— 沿 r3 cumulative 链路
    r3_cum = r3["aggregate"]["cumulative"]
    cum_total_prior_cum = r3_cum["post_total"]  # 164
    cum_empty_prior_cum = r3_cum["post_empty"]  # 9
    cum_ok_prior_cum = r3_cum["post_ok"]        # 155
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = r3 aggregate.cumulative)")
    ...
    # 13. distill 侧收官判定
    kimi_distill_side_round4_finish = {
    ...
    # 15. counting_scope_correction 块（沿 r3；标注待 PI 复核）
    counting_scope_correction = {
        ...
        "r1_issue_description": (
            ...
            "PI 委托 r2 棒起切换至 distill 侧独立计数；r3/r4 沿 r2 口径续跑。"
        ),
        "r1_data_disposition": (
        ...
        "r4_counting_domain": (
            ...
            "（沿 r2/r3；后续所有 batch6 r* 棒沿此口径）"
        ),
        "r4_target": f"distill 侧自身 ≥{TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption",
        "r4_baseline_distill_only": (
            f"r1+r2+r3 quadruples distill 过滤后 = {cum_total_prior_distill} calls, "
            f"{cum_ok_prior_distill} successful, "
            f"{cum_empty_prior_distill} empty (各 caption = 3/5 successful)"
        ),
        "r4_post_distill_only": (
            f"r4 quadruples 后 = {sum(per_cap_total_distill.values())} calls, "
            f"{sum(per_cap_succ_distill.values())} successful, "
            f"{sum(per_cap_empty_distill.values())} empty (各 caption = 4/5 successful expected, if no fails)"
        ),
        ...
        "pending_pi_review": (
            ...
            "若 PI 倾向保留 r1 混合域统计，须重审 r2/r3/r4 棒"
        ),
    }
    ...
    result = {
        ...
        "round": 4,
        "metadata": {
            "task": "L14V3_batch6_round4_kimi_distill_side_reask3_independent_counting",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 6 round 4 distill side reask=3 独立计数",
            ...
            "spec_conformance": "PI 2026-09-24 同 agent 唤醒 (task_append) 接力棒 + 沿 r3 口径续跑 + retry 修复沿 r3 + 每 caption successful 计数终盘点 + K-N26-N2 累计监控 + distill 侧独立计数（沿 r3 counting_scope_correction）",
            "teacher": "kimi",
            "side": "distill",
            "round_index": 4,
            ...
            "endpoint_selection_method": "沿 r3 executor 1385d2ca356f 实测 (kimi-k3 唯一可达)",
            "model_id_sent": MODEL_KIMI_K3,
            ...
            "tools_fault_param_correction": {
                ...
                "r3_status": "retry 修复 + tracking 改进沿 r3 (沿 1385d2ca356f)",
                "r4_status": "retry 修复沿 r3; r4 本棒无新参数修正",
                "rationale": (
                    "沿 r3 修复 + r3 tracking 改进；r4 本棒仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r3_executor": [
                "1. 新增 ROUND=4 / REASK_R1=3 参数化 (reask_idx=3 = 第 4 call/caption on distill side)",
                "2. prompt_id 后缀 _r4",
                "3. baseline 切换：r3 quadruples = 22 caption 各 3 successful (沿 r1+r2+r3 distill 过滤)",
                "4. predecessor_sha12 链新增 r3_executor 1385d2ca356f + r3_result 15a7fecdffe7",
                "5. 新增 is_round_4 字段（替代 r3 的 is_round_3）",
                "6. 增量计数 → distill 侧各 caption 由 3/5 顺利补至 4/5 (if no fails)",
                "7. retry_validation 新增 delta_empty_rate_r4_vs_r3 沿 r3 比对",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "kimi_distill_side_round4_finish": kimi_distill_side_round4_finish,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_4_n_distinct_sample_note": (
                    "round 4 第 4 call per caption 实测；distill 侧独立计数 = 22 caption 各 4 successful "
                    "(沿 r1+r2+r3+本棒 quadruples, prompt_id 前缀 kimi_t01_distill_ filter 域)；"
                    "目标 distill 侧 ≥5 successful/caption → 后续 1 棒接力 (r5 22 caption × 1 call → 5/5 successful)"
                ),
            },
        ...
        "breakpoint_status": {
            ...
            "note": (
                f"本棒 round 4 已完成（{r4_total}/{planned_total} calls）"
                + (f"；K-N26-N2 累计阈值触发：cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   ...
                   f"；kimi 蒸馏侧 distill 侧独立计数：本棒后 22 caption 各 4/5 successful, 后续 1 棒接力 (r5 22 caption × 1 call → 5/5 successful 收官)")
            ),
        },
        "self_scan": {
    ...
    # 19. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch6_r4_result.json")
    ...
    print(f"  all_22_captions_met_target_distill: {kimi_distill_side_round4_finish['all_22_captions_met_target']}")
    ...
    if kimi_distill_side_round4_finish["still_pending"]:
        print(f"  still_pending_distill: {kimi_distill_side_round4_finish['still_pending']}")
    return 0
