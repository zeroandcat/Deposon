# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch3_r4_executor.py
====================================

V4 L14+ batch 3 · round 4 · GLM_2 教师侧 round 4（reask=3）
============================================================

【派工依据】
----------------
- 沿 batch3 r3 executor `1B3861943B7E` + r3 result `6E677FFCC880` 口径锚（GLM_2 教师侧 round 3 reask=2）
...
- **glm2_corpus 口径拍板** (2026-09-24 23:42 ask_259658a4ffadd6f8aeb12f34 显式):
  **接受 system prompt 微调口径** = GLM_2 教师身份=语料标签+prompt 语境差异；近缘对分布差异有限
  如实入 N-26 构造域限制（不软化）；round1+round2+round3 66 calls 有效不作废；
  字段命名维持现状（三处冗余标注保留）
- 接力节奏派工 (2026-09-24 23:55 显式): 延续 22×1 call/棒模式（已拍工程口径，不再请示）
- **夜间自主保守口径** (PI 2026-09-25 00:46 授权): 本棒先斩后奏，遇模糊取保守并如实标注待 PI 复核

【本棒范围（沿 dispatch）—— GLM_2 教师侧 round 4（reask=3）】
----------------------------------------------------------
1. **沿用 glm-5.3**（r3 1-call smoke ok=200；本棒不再探活，仅 1-call smoke 防端点飘移）
2. **22 caption × 1 call**（每位 caption 第 4 call；reask_idx=3；目标 4/5 successful/caption）
...
---------------------------------------------------------
- 本棒前基线（沿 r3 末态）：GLM_2 教师侧 = 66 calls / 0 empty / 66 ok / 0.0% rate
- 跨批累计 baseline = r3 末态 **229 calls / 9 empty / 220 ok / 3.93%**（沿 r3 result cross_batch_with_r2_baseline）
...
【retry 修复逻辑沿 batch1 r6 + batch2 r2 + batch3 r1/r2/r3（沿 dispatch §3）】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = **229 calls / 9 empty / 3.93%**（沿 r3 末态，本棒前 GLM_2 r3 末态）
- GLM_2 自身 = 66 calls / 0 empty / 0.0% rate（r3 末态）
...
【端点取舍（沿 r3）】
----------------------------
- teamo + tun + glm-5.3（r3 已探活确认 ok=200；本棒不再做完整 3-candidate probe，仅 1-call smoke）
- model_id 字面 = glm-5.3
...
- 派生 JSON 不合并（本棒 `_batch3_r4_*` 与 `_batch3_r3_*` / `_batch3_r2_*` / `_batch3_r1_*` / `_batch2_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r3 executor `1B3861943B7E` + r3 result `6E677FFCC880` + r2 executor `540C9369DD36` + r2 result `58E9C55A770D` + r1 executor `A4731B39343F` + r1 result `D24B84671C23` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 双件 + r1 探活件 + models_probe `3C9DC60B5071` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch3_r4_result.json`（schema = `v4_l14v3_n26/1` + batch=3 + round=4）
- 不覆盖 r3 result `6E677FFCC880` 或 r2 result `58E9C55A770D` 或 r1 result `D24B84671C23` 或 r3 result `25251ACA6D79` 或 r2 result `8C125E257AF8` 或 r1 探活 result `D245AE4CE385` 或 batch1 r6 result `A4F851154551`
- predecessor_sha12 链：r3_executor `1B3861943B7E` + r3_result `6E677FFCC880` + r2_executor `540C9369DD36` + r2_result `58E9C55A770D` + r1_executor `A4731B39343F` + r1_result `D24B84671C23` + r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79` + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 4（升 r4 命名避开 r3 重叠）；BATCH = 3 不变
2. prompt_id 前缀 `glm2_t04_teacher_*`（取代 r3 的 `glm2_t03_teacher_*`）
   - t04 = teacher side round 4（取代 r3 的 t03 round 3）
   - 后缀 `_b3_r4`（取代 r3 的 `_b3_r3`）
3. REASK_R1 = 3（本棒每位 caption 第 4 call；取代 r3 的 reask_idx=2）
4. phase_label：`b3_r4_need1`（取代 r3 的 `b3_r3_need1`）
5. is_round_2 = True（语义：本棒为 GLM_2 教师侧 data collection round 4；属 round ≥2 范畴）
6. smoke_call_only = True（沿 r3）
7. predecessor_sha12 链：r3_executor `1B3861943B7E` + r3_result `6E677FFCC880` + r2_executor `540C9369DD36` + r2_result `58E9C55A770D` + r1_executor `A4731B39343F` + r1_result `D24B84671C23` + r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79` + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe
8. baseline prior：GLM_2 prior = 66 calls / 0 empty / 66 ok / 0.0% rate（取代 r3 链的 44/0/44/0%）
9. 跨批累计 baseline：r3 末态 **229 calls / 9 empty / 3.93%**（取代 r3 链的 207/9/4.35%）
10. 新增 `glm2_teacher_side_round4_progress` 块（本棒后 GLM_2 教师侧 round 4 完成度 ≥4 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 229/9/3.93%
12. spec_conformance 重写：GLM_2 教师侧 round 4（reask=3），目标 ≥4 successful call
13. per_caption_successful_calls 字段新增 round4_target=4 / round4_met（取代 r3 的 round3_target=3 / round3_met）
14. 调度注释：GLM_2 教师侧 round 4（reask=3）22 caption × 1 call
15. glm2_corpus_calibration.rounds_covered 续展至「round1+round2+round3 66 calls 有效不作废」

【边界】
----------
- 不动任何既有件（含 r3 executor `1B3861943B7E` + r3 result `6E677FFCC880` + r2 executor `540C9369DD36` + r2 result `58E9C55A770D` + r1 executor `A4731B39343F` + r1 result `D24B84671C23` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `A4F851154551` + r1 executor `E52F930654D3` + r1 result `D245AE4CE385` + models_probe `3C9DC60B5071` + prereg `05B975A86989`）
- 跑不完拆段报断点
...
KEY_INDEX_TEAMO_1BASED = 15  # 沿 r3 (line 15 = teamo key)
USE_PROXY = True
...
# 本棒调度（沿 dispatch）
BATCH = 3  # batch=3 不变
ROUND = 4  # 升 r4 命名避开 r3 重叠
TEACHER = "GLM_2"  # 沿 r3
TEACHER_LABEL = "GLM_2"  # 顶层字段沿 r3
MODEL_ID_SHARED_WITH = "GLM_1"  # 顶层字段沿 r3
SIDE = "teacher"
REASK_R1 = 3  # 本棒 round 4 = 每 caption 第 4 call（reask_idx=3）
TARGET_SUCCESSFUL_PER_CAPTION = 5  # 最终目标 ≥5 successful/caption（沿 prereg §1.6）
TARGET_PER_CAPTION_ROUND4 = 4  # 本棒 round 4 完成目标 ≥4 successful/caption
SMOKE_CALL_ONLY = True  # 沿 r3 smoke=glm-5.3 ok=200；本棒仅 1-call smoke 不再做 3-candidate probe

# 选定的 model_id（沿 r3 探活结论，不重探）
...
GLM2_CORPUS_CALIBRATION = {
    ...
    "rounds_covered": "round1+round2+round3 66 calls 有效不作废",
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch3-r4",
        "X-Title": f"deposon-l14v3-batch3-r4-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# 提示构造（沿 r3 + GLM_2 教师身份微调；口径拍板已落定）
# ============================================================
...
# ============================================================
# Smoke（沿 r3 probe 简化版；仅 1 call 确认端点 ok）
# ============================================================
def smoke_glm_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r3 probe 结论 glm-5.3 ok=200 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b3_r4"
    prompt_id = f"glm2_t04_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 3 round 4 · {TEACHER} 教师侧 round 4（reask=3）")
    print("沿 r3 glm-5.3 探活结论 ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r3 (沿 batch1 r6 + batch2 r2 + batch3 r1/r2/r3)")
    ...
    # 3. 加载前置累计基线（GLM_2 r3 末态）
    with open(R3_RESULT_PATH_B3, "r", encoding="utf-8") as f:
        r3_b3 = json.load(f)
    glm2_prior_total = r3_b3["aggregate"]["glm2_cumulative"]["post_total"]
    glm2_prior_empty = r3_b3["aggregate"]["glm2_cumulative"]["post_empty"]
    glm2_prior_ok = r3_b3["aggregate"]["glm2_cumulative"]["post_ok"]
    glm2_prior_rate = r3_b3["aggregate"]["glm2_cumulative"]["post_rate"]
    print(f"[BASELINE GLM_2 r3] total={glm2_prior_total} ok={glm2_prior_ok} empty={glm2_prior_empty} rate={glm2_prior_rate:.4f}")

    # 跨批累计基线 = r3 末态（229 calls / 9 empty / 3.93%）
    cross_prior_total = r3_b3["aggregate"]["cross_batch_with_r2_baseline"]["cross_total"]
    cross_prior_empty = r3_b3["aggregate"]["cross_batch_with_r2_baseline"]["cross_empty"]
    cross_prior_ok = r3_b3["aggregate"]["cross_batch_with_r2_baseline"]["cross_ok"]
    cross_prior_rate = r3_b3["aggregate"]["cross_batch_with_r2_baseline"]["cross_rate"]
    print(f"[BASELINE cross batch (r3 末态)] total={cross_prior_total} ok={cross_prior_ok} empty={cross_prior_empty} rate={cross_prior_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r3 probe 结论 glm-5.3 ok=200）
    print("=" * 60)
    print(f"[SMOKE PHASE] {MODEL_GLM_2} 1-call smoke（沿 r3 probe 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 4,
            "metadata": {
                "task": f"L14V3_batch3_round4_{TEACHER}_teacher_side_round4_reask3",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch3_r4_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b3_r4_need1"} for c in captions_sorted]
    ...
    r3_per_cap = r3_b3.get("per_caption_successful_calls", {})
    ...
    for i, item in enumerate(planned):
        ...
        if r4_total > 0:
            current_batch_rate = r4_empty / r4_total
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=True,  # 语义：本棒 = GLM_2 教师侧 data collection round 4；属 round ≥2 范畴
        ...
        if r4_total > 0:
            batch_rate_post = r4_empty / r4_total
    ...
    # 跨批累计 = r3 baseline + GLM_2 本棒
    cross_batch_total = cross_prior_total + r4_total
    ...
    # 11. GLM_2 教师侧 round 4 进度块
    glm2_round4_progress = {
    ...
    model_id_disclosure = (
        ...
        f"r4 沿 r3 结论仅做 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-25",
            "spec_conformance": (
                f"PI 2026-09-25 batch3 GLM_2 教师侧 round 4 续采 (reask=3); "
                f"沿 r3 GLM_2 glm-5.3 同 model_id 复用结论 (1-call smoke ok=200); "
                ...
                f"目标 ≥4 successful/caption (round 4 完成); "
            ...
            "tools_fault_param_correction": {
                ...
                "b3_r4_status": "b3_r4 GLM_2 沿 r3 结论仅做 1-call smoke；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r3 glm-5.3 探活结论；本棒节省预算避免重复 probe；"
            ...
            "diff_vs_r3_executor": [
                "1. ROUND=4 (升 r4 命名避开 r3 重叠)；BATCH=3 不变",
                "2. prompt_id 前缀 glm2_t04_teacher_* (取代 r3 的 glm2_t03_teacher_*)；后缀 _b3_r4 (取代 _b3_r3)",
                "3. REASK_R1=3 (本棒每 caption 第 4 call；取代 r3 的 reask_idx=2)",
                "4. phase_label: b3_r4_need1 (取代 r3 的 b3_r3_need1)",
                "5. is_round_2=True (沿 r3；语义: GLM_2 教师侧 data collection round 4；属 round ≥2 范畴)",
                "6. predecessor_sha12 链: r3_executor 1B3861943B7E + r3_result 6E677FFCC880 + r2_executor 540C9369DD36 + r2_result 58E9C55A770D (取代 r3 链的 r2_executor/r2_result 仍保留)",
                "7. baseline prior: GLM_2 prior = 66 calls / 0 empty / 66 ok / 0.0% rate (取代 r3 链的 44/0/44/0%)",
                "8. 跨批累计 baseline: r3 末态 229 calls / 9 empty / 3.93% (取代 r3 链的 207/9/4.35%)",
                "9. 新增 glm2_teacher_side_round4_progress 块 (round 4 完成度 ≥4 successful/caption)",
                "10. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 229/9/3.93%",
                "11. spec_conformance 重写: GLM_2 教师侧 round 4 (reask=3)",
                "12. per_caption_successful_calls 字段新增 round4_target=4 / round4_met (取代 r3 的 round3_target=3 / round3_met)",
                "13. 调度注释: GLM_2 教师侧 round 4 (reask=3) 22 caption × 1 call",
                "14. glm2_corpus_calibration.rounds_covered 续展至 round1+round2+round3 66 calls 有效",
            ...
            "interruption_recovery_note": (
                "b3_r4 沿 r3 glm-5.3 探活结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；b3_r4 与 r3/r2/r1/r3/r2(b2)/r1(b2)/batch1_r6 派生 JSON 独立 (不合并); "
        ...
        "models_listing_summary_ref": {
            ...
            "note": (
                "b3_r4 不重探 /v1/models；仅引用 r2 探活清单 (sha12=3C9DC60B5071) "
        ...
        "glm2_teacher_side_round4_progress": glm2_round4_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch3_round4_round4_reask3": {
                ...
                "note": "b3_r4 GLM_2 教师侧 round 4 reask=3 22 caption × 1 call",
            },
            "batch3_round4_cumulative_glm2": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round4_reask3_n_distinct_sample_note": (
                    f"round 4 reask=3 第 1 call 实测；本棒后 GLM_2 教师侧 round 4 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"GLM_2 prior = {glm2_prior_total} calls / {glm2_prior_empty} empty / {glm2_prior_rate:.4f} (r3 末态); "
                    f"cross-batch rate = {cross_batch_rate:.4f}; "
                    f"baseline cross batch = {cross_prior_total} calls / {cross_prior_empty} empty / {cross_prior_rate:.4f} (r3 末态)"
                ),
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 4 (reask=3) 已完; 下棒可接力 GLM_2 教师侧 round 5 (>=1 more successful/caption)"
            ),
            "note": (
                f"本棒 round 4 (reask=3): actual={r4_total}/{planned_total} calls; "
                f"GLM_2 教师侧 round 4 完成度 = {met_round4_count}/{len(captions)} caption met round 4 目标 (≥4 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"GLM_2 model_id 沿 r3 探活选定 = {model_id} (与 GLM_1 同 model_id 不同教师；1-call smoke 确认仍可用); "
