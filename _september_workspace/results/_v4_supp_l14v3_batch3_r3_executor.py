# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch3_r3_executor.py
====================================

V4 L14+ batch 3 · round 3 · GLM_2 教师侧 round 3（reask=2）
===========================================================

【派工依据】
----------------
- 沿 batch3 r2 executor `540C9369DD36` + r2 result `58E9C55A770D` 口径锚（GLM_2 教师侧 round 2 reask=1）
...
- **glm2_corpus 口径拍板** (2026-09-24 23:42 ask_259658a4ffadd6f8aeb12f34 显式):
  **接受 system prompt 微调口径** = GLM_2 教师身份=语料标签+prompt 语境差异；近缘对分布差异有限
  如实入 N-26 构造域限制（不软化）；round1+round2 44 calls 有效不作废；
  字段命名维持现状（三处冗余标注保留）
- 接力节奏派工 (2026-09-24 23:55 显式): 延续 22×1 call/棒模式（已拍工程口径，不再请示）

【本棒范围（沿 dispatch）—— GLM_2 教师侧 round 3（reask=2）】
----------------------------------------------------------
1. **沿用 glm-5.3**（r2 1-call smoke ok=200；本棒不再探活，仅 1-call smoke 防端点飘移）
...
---------------------------------------------------------
- 本棒前基线（沿 r2 末态）：GLM_2 教师侧 = 44 calls / 0 empty / 44 ok / 0.0% rate
- 跨批累计 baseline = r2 末态 **207 calls / 9 empty / 198 ok / 4.35%**（沿 r2 result）
...
【retry 修复逻辑沿 batch1 r6 + batch2 r2 + batch3 r1/r2（沿 dispatch §3）】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = **207 calls / 9 empty / 4.35%**（沿 r2 末态，本棒前 GLM_2 r2 末态）
- GLM_2 自身 = 44 calls / 0 empty / 0.0% rate（r2 末态）
...
- 派生 JSON 不合并（本棒 `_batch3_r3_*` 与 `_batch3_r2_*` / `_batch3_r1_*` / `_batch2_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r2 executor `540C9369DD36` + r2 result `58E9C55A770D` + r1 executor `A4731B39343F` + r1 result `D24B84671C23` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 双件 + r1 探活件 + models_probe `3C9DC60B5071` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch3_r3_result.json`（schema = `v4_l14v3_n26/1` + batch=3 + round=3）
- 不覆盖 r2 result `58E9C55A770D` 或 r1 result `D24B84671C23` 或 r3 result `25251ACA6D79` 或 r2 result `8C125E257AF8` 或 r1 探活 result `D245AE4CE385` 或 batch1 r6 result `A4F851154551`
- predecessor_sha12 链：r2_executor `540C9369DD36` + r2_result `58E9C55A770D` + r1_executor `A4731B39343F` + r1_result `D24B84671C23` + r3_executor `72FD2BB3838E` + r3_result `25251ACA6D79` + r2_executor/r2_result + batch1_r6_executor/batch1_r6_result + r1_executor/r1_result + models_probe

【与 r2 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 3（升 r3 命名避开 r2 重叠）；BATCH = 3 不变
2. prompt_id 前缀 `glm2_t03_teacher_*`（取代 r2 的 `glm2_t02_teacher_*`）
   - t03 = teacher side round 3（取代 r2 的 t02 round 2）
   - 后缀 `_b3_r3`（取代 r2 的 `_b3_r2`）
3. REASK_R1 = 2（本棒每位 caption 第 3 call；取代 r2 的 reask_idx=1）
4. phase_label：`b3_r3_need1`（取代 r2 的 `b3_r2_need1`）
5. is_round_2 = True（语义：本棒为 GLM_2 教师侧 data collection round 3；沿 r2 = True 因属 round ≥2 范畴）
6. smoke_call_only = True（沿 r2）
7. predecessor_sha12 链：r2_executor `540C9369DD36` + r2_result `58E9C55A770D`（取代 r2 链的 r1_executor/r1_result 仍保留）
8. baseline prior：GLM_2 prior = 44 calls / 0 empty / 44 ok / 0.0% rate（取代 r2 链的 22/22/0/0%）
9. 跨批累计 baseline：r2 末态 **207 calls / 9 empty / 4.35%**（取代 r2 链的 185/9/4.86%）
10. 新增 `glm2_teacher_side_round3_progress` 块（本棒后 GLM_2 教师侧 round 3 完成度 ≥3 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 207/9/4.35%
12. spec_conformance 重写：GLM_2 教师侧 round 3（reask=2），目标 ≥3 successful call
13. per_caption_successful_calls 字段新增 round3_target=3 / round3_met
14. 调度注释：GLM_2 教师侧 round 3（reask=2）22 caption × 1 call
15. glm2_corpus_calibration 沿 r2（field_naming_status 字段续展 rounds_covered 至「round1+round2 44 calls 有效不作废」）

【边界】
----------
- 不动任何既有件（含 r2 executor `540C9369DD36` + r2 result `58E9C55A770D` + r1 executor `A4731B39343F` + r1 result `D24B84671C23` + r3 executor `72FD2BB3838E` + r3 result `25251ACA6D79` + r2 executor `E7418F47A130` + r2 result `8C125E257AF8` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `A4F851154551` + r1 executor `E52F930654D3` + r1 result `D245AE4CE385` + models_probe `3C9DC60B5071` + prereg `05B975A86989`）
- 跑不完拆段报断点
...
TEACHER = "GLM_2"  # 沿 r2
TEACHER_LABEL = "GLM_2"  # 顶层字段沿 r2
MODEL_ID_SHARED_WITH = "GLM_1"  # 顶层字段沿 r2
SIDE = "teacher"
...
TARGET_PER_CAPTION_ROUND3 = 3  # 本棒 round 3 完成目标 ≥3 successful/caption
SMOKE_CALL_ONLY = True  # 沿 r2 smoke=glm-5.3 ok=200；本棒仅 1-call smoke 不再做 3-candidate probe
...
GLM2_CORPUS_CALIBRATION = {
    ...
    "rounds_covered": "round1+round2 44 calls 有效不作废",
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch3-r3",
        "X-Title": f"deposon-l14v3-batch3-r3-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# 提示构造（沿 r2 + GLM_2 教师身份微调；口径拍板已落定）
# ============================================================
...
def run_one_quadruple(
    ...
    suffix = "_b3_r3"
    prompt_id = f"glm2_t03_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 3 round 3 · {TEACHER} 教师侧 round 3（reask=2）")
    ...
    print("retry 修复沿 r2 (沿 batch1 r6 + batch2 r2 + batch3 r1)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（GLM_2 r2 末态）
    with open(R2_RESULT_PATH_B3, "r", encoding="utf-8") as f:
        r2_b3 = json.load(f)
    glm2_prior_total = r2_b3["aggregate"]["glm2_cumulative"]["post_total"]
    glm2_prior_empty = r2_b3["aggregate"]["glm2_cumulative"]["post_empty"]
    glm2_prior_ok = r2_b3["aggregate"]["glm2_cumulative"]["post_ok"]
    glm2_prior_rate = r2_b3["aggregate"]["glm2_cumulative"]["post_rate"]
    print(f"[BASELINE GLM_2 r2] total={glm2_prior_total} ok={glm2_prior_ok} empty={glm2_prior_empty} rate={glm2_prior_rate:.4f}")

    # 跨批累计基线 = r2 末态（207 calls / 9 empty / 4.35%）
    cross_prior_total = r2_b3["aggregate"]["cross_batch_with_r1_baseline"]["cross_total"]
    cross_prior_empty = r2_b3["aggregate"]["cross_batch_with_r1_baseline"]["cross_empty"]
    cross_prior_ok = r2_b3["aggregate"]["cross_batch_with_r1_baseline"]["cross_ok"]
    cross_prior_rate = r2_b3["aggregate"]["cross_batch_with_r1_baseline"]["cross_rate"]
    print(f"[BASELINE cross batch (r2 末态)] total={cross_prior_total} ok={cross_prior_ok} empty={cross_prior_empty} rate={cross_prior_rate:.4f}")
    ...
    print("=" * 60)
    print(f"[SMOKE PHASE] {MODEL_GLM_2} 1-call smoke（沿 r2 probe 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 3,
            "metadata": {
                "task": f"L14V3_batch3_round3_{TEACHER}_teacher_side_round3_reask2",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch3_r3_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b3_r3_need1"} for c in captions_sorted]
    ...
    r2_per_cap = r2_b3.get("per_caption_successful_calls", {})
    ...
    for i, item in enumerate(planned):
        ...
        if r3_total > 0:
            current_batch_rate = r3_empty / r3_total
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=True,  # 语义：本棒 = GLM_2 教师侧 data collection round 3；属 round ≥2 范畴
        ...
        if r3_total > 0:
            batch_rate_post = r3_empty / r3_total
    ...
    # 11. GLM_2 教师侧 round 3 进度块
    glm2_round3_progress = {
    ...
    model_id_disclosure = (
        ...
        f"r3 沿 r2 结论仅做 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                f"PI 2026-09-24 batch3 GLM_2 教师侧 round 3 续采 (reask=2); "
                f"沿 r2 GLM_2 glm-5.3 同 model_id 复用结论 (1-call smoke ok=200); "
                ...
                f"目标 ≥3 successful/caption (round 3 完成); "
            ...
            "tools_fault_param_correction": {
                ...
                "rationale": (
                    "沿 r2 glm-5.3 探活结论；本棒节省预算避免重复 probe；"
            ...
            "diff_vs_r2_executor": [
                "1. ROUND=3 (升 r3 命名避开 r2 重叠)；BATCH=3 不变",
                "2. prompt_id 前缀 glm2_t03_teacher_* (取代 r2 的 glm2_t02_teacher_*)；后缀 _b3_r3 (取代 _b3_r2)",
                "3. REASK_R1=2 (本棒每 caption 第 3 call；取代 r2 的 reask_idx=1)",
                "4. phase_label: b3_r3_need1 (取代 r2 的 b3_r2_need1)",
                "5. is_round_2=True (沿 r2；语义: GLM_2 教师侧 data collection round 3；属 round ≥2 范畴)",
                "6. predecessor_sha12 链: r2_executor 540C9369DD36 + r2_result 58E9C55A770D (取代 r2 链的 r1_executor/r1_result 仍保留)",
                "7. baseline prior: GLM_2 prior = 44 calls / 0 empty / 44 ok / 0.0% rate (取代 r2 链的 22/22/0/0%)",
                "8. 跨批累计 baseline: r2 末态 207 calls / 9 empty / 4.35% (取代 r2 链的 185/9/4.86%)",
                "9. 新增 glm2_teacher_side_round3_progress 块 (round 3 完成度 ≥3 successful/caption)",
                "10. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 207/9/4.35%",
                "11. spec_conformance 重写: GLM_2 教师侧 round 3 (reask=2)",
                "12. per_caption_successful_calls 字段新增 round3_target=3 / round3_met",
                "13. 调度注释: GLM_2 教师侧 round 3 (reask=2) 22 caption × 1 call",
                "14. glm2_corpus_calibration.rounds_covered 续展至 round1+round2 44 calls 有效",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "b3_r3 沿 r2 glm-5.3 探活结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；b3_r3 与 r2/r1/r3/r2(b2)/r1(b2)/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        "models_listing_summary_ref": {
            ...
            "note": (
                "b3_r3 不重探 /v1/models；仅引用 r2 探活清单 (sha12=3C9DC60B5071) "
        ...
        "glm2_teacher_side_round3_progress": glm2_round3_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch3_round3_round3_reask2": {
                ...
                "note": "b3_r3 GLM_2 教师侧 round 3 reask=2 22 caption × 1 call",
            },
            "batch3_round3_cumulative_glm2": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round3_reask2_n_distinct_sample_note": (
                    f"round 3 reask=2 第 1 call 实测；本棒后 GLM_2 教师侧 round 3 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"GLM_2 prior = {glm2_prior_total} calls / {glm2_prior_empty} empty / {glm2_prior_rate:.4f} (r2 末态); "
                    f"cross-batch rate = {cross_batch_rate:.4f}; "
                    f"baseline cross batch = {cross_prior_total} calls / {cross_prior_empty} empty / {cross_prior_rate:.4f} (r2 末态)"
                ),
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 3 (reask=2) 已完; 下棒可接力 GLM_2 教师侧 round 4 (>=2 more successful/caption)"
            ),
            "note": (
                f"本棒 round 3 (reask=2): actual={r3_total}/{planned_total} calls; "
                f"GLM_2 教师侧 round 3 完成度 = {met_round3_count}/{len(captions)} caption met round 3 目标 (≥3 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"GLM_2 model_id 沿 r2 探活选定 = {model_id} (与 GLM_1 同 model_id 不同教师；1-call smoke 确认仍可用)"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
