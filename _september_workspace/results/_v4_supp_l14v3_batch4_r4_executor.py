# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch4_r4_executor.py
====================================

V4 L14+ batch 4 · round 4 · coze 教师侧 round 4 续采（reask=3）
================================================================

【派工依据】
----------------
- 沿 batch4 r3 executor `b5e1c3637405` + r3 result `ed5267df7d6f` 口径（基础口径锚）
...
- 派工单 coze 教师批 4 round 4（接力棒，沿 r3 上下文唤醒）：22 caption × 1 call（reask=3）

【本棒范围（沿 dispatch）—— coze 教师侧 round 4（reask=3）续采】
------------------------------------------------------------
1. **沿用 teamo `deepseek-v4-flash` 代表采样**（r3 smoke ok=200 已确认；r3 22 caption 全 OK；
   本棒仅 1-call smoke 防端点飘移）
2. **22 caption × 1 call**（每位 caption 第 4 call；reask_idx=3；目标 round 4 完成度 = 4/5 successful/caption）
3. **目标**：本棒后 22/22 caption 各 ≥4 successful call；下棒接力至 ≥5 successful/caption
...
- 背景：前棒 r4（不同派工）因 net::ERR_HTTP2_PING_FAILED 网络断连于勘察阶段中断（0 产物）
- 本棒（重派）保守处置：
  - 启动前必检 tun (127.0.0.1:1018) 端口可达；不通即停
  - 1-call smoke 失败立即 abort 不重探（沿 r3 口径）
  - 已落盘部分如实保留 + 报断点
  - 串行 ≥2.5s 不破；timeout/proxy/ssl/connection → 90s retry ≤3（沿 r3 + 修复版）
...
---------------------------------------------------------
- 本棒前基线（沿 batch4 r3）：coze 教师侧 = 66 calls / 0 empty / 66 ok / 0.0000 rate
- 本棒 round 4（reask=3）后：22/22 caption 各 ≥4 successful call（理论值；如端点飘移可能回落）
- 终盘结果：result.json 顶层 `per_caption_successful_calls` 块 + `dispatch_target_progress` 块

【retry 修复逻辑沿 batch4 r3 + batch4 r2 + batch2 r3 + batch1 r6】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline（沿 batch4 r3）：batch1 119 + batch2 22+22+22 + batch4 r1 22 + batch4 r2 22 + batch4 r3 22
  → **251 calls / 9 empty / 3.59%**（沿 batch4 r3 result `ed5267df7d6f`）
- coze_teacher_cumulative 自身从 r3 终态 66/0/66 起步
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 单棒 >50% 停采：r4 单棒 empty_rate > 50% 也立即停采（沿 dispatch）

【端点取舍（沿 r3 + r2 + r1 + r4 + r3 + r2）】
----------------------------
- teamo + tun + deepseek-v4-flash（沿 r3 smoke ok=200 + r3 22 caption 全 OK + r2 models_probe 探活清单 sha12=`3c9dc60b5071`
  已确认 44 models 含 deepseek-v4-flash）
- teamo 必走 tun 防封号（PI 2026-09-23 硬纪律）

【铁律严守（沿 r3/r2/r1/r4/r3/r2）】
------------------------
- R4 key 永不明文（无例外）；仅 runtime memory 读
- V1–V3 只读不动 / R5 V4 frozen append-only / R6 P-G 不动 / R7 plugin spec 不动
- 派生 JSON 不合并（本棒 `_batch4_r4_*` 与 `_batch4_r3_*` / `_batch4_r2_*` / `_batch4_r1_*` / `_batch2_r*_` / `_batch1_*` 独立）
...
- 不覆盖既有件（r3 executor `b5e1c3637405` + r3 result `ed5267df7d6f` + r2 executor `7d8e12267790` + r2 result `6a9a362d9869`
  + r1 executor `e84bffd5dc7d` + r1 result `7d9fe9d319a2` + r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e`
  + r3 executor `72fd2bb3838e` + r3 result `25251aca6d79` + r2 executor `e7418f47a130` + r2 result `8c125e257af8`
  + r1 executor `e52f930654d3` + r1 result `d245ae4ce385` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `a4f851154551`
  + mapping `98a779d61c1e` + r2 models_probe `3c9dc60b5071` + prereg `05b975a86989` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch4_r4_result.json`（schema = `v4_l14v3_n26/1` + batch=4 + round=4）
- 不覆盖 r3 result `ed5267df7d6f` 或 r2 result `6a9a362d9869` 或 r1 result `7d9fe9d319a2` 或 r4 result `15b8ddc5ba9e`
  或 r3 result `25251aca6d79` 或 batch1 r6 result `a4f851154551` 或 r1 result `d245ae4ce385`
- predecessor_sha12 链：mapping `98a779d61c1e` + r3 executor/result（最新接力锚）+ r2 executor/result
  + r1 executor/result + r4 executor/result + r3 executor/result + r2 executor/result + models_probe
  + batch1 r6 executor/result + prereg `05b975a86989`

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 4（升 r4 命名）；BATCH = 4 不变
2. prompt_id 前缀 `coze_t04_teacher_*`（取代 r3 的 t03）+ 后缀 `_b4_r4`（取代 r3 的 `_b4_r3`）
3. reask_idx = 3（本棒每位 caption 第 4 call；取代 r3 的 reask_idx=2）
4. phase_label：`b4_r4_need1`（round 4 目标 = ≥4 successful/caption，need_more_to_target = 1）
5. is_round_4 = True（语义：本棒 = coze 教师侧 data collection round 4，区别于 r3 的 round 3）
6. smoke_call_only = True（沿 r3 smoke ok=200 + r3 22 caption 全 OK；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移）
7. predecessor_sha12 链：r3 executor `b5e1c3637405` + r3 result `ed5267df7d6f`（新增；取代 r3 链的 r2 双件仍保留作为历史累计 baseline）
8. baseline prior：r3 终态 66 calls / 0 empty / 66 ok / 0.0000 rate（取代 r3 链的 44/44/0/0.0）
9. 跨批累计 baseline：r3 终态 251 calls / 9 empty / 3.59% rate（取代 r3 链的 229/9/3.93%）
10. 新增 `coze_teacher_side_round4_progress` 块：本棒后 coze 教师侧 round 4 完成度（≥4 successful/caption）
11. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 251/9/3.59%
12. spec_conformance 重写：coze 教师侧 round 4 续采（reask=3），目标 ≥4 successful/caption
13. model_id_inconsistency_honest_disclosure 字面继承 r1/r2/r3（沿 mapping 拍板代表采样字面）+ r1+r2+r3+r4 累计补注
14. 派生 metadata 字段沿 r1/r2/r3 不动（v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash）
15. per_caption_successful_calls 字段新增 round4_target=4 / round4_met
16. 调度注释：round 4 续采（reask=3）22 caption × 1 call
17. baseline 路径：B4_R3_RESULT_PATH（沿 r3 result `ed5267df7d6f`）取代 r3 链的 B4_R2_RESULT_PATH
18. 启动前必检 tun 端口（PI 2026-09-25 夜间保守口径；防 net::ERR_HTTP2_PING_FAILED 重演）

【边界】
----------
- 不动任何既有件（含 r3 executor `b5e1c3637405` + r3 result `ed5267df7d6f` + r2 executor `7d8e12267790`
  + r2 result `6a9a362d9869` + r1 executor `e84bffd5dc7d` + r1 result `7d9fe9d319a2` + r4 executor `d9fe7aa2d292`
  + r4 result `15b8ddc5ba9e` + r3 executor `72fd2bb3838e` + r3 result `25251aca6d79` + r2 executor `e7418f47a130`
  + r2 result `8c125e257af8` + r1 executor `e52f930654d3` + r1 result `d245ae4ce385` + batch1 r6 executor `5f02c7e0f094`
  + batch1 r6 result `a4f851154551` + models_probe `3c9dc60b5071` + mapping `98a779d61c1e` + prereg `05b975a86989`）
- 跑不完拆段报断点
...
KEY_INDEX_TEAMO_1BASED = 15  # 沿 r1/r2/r3 (line 15 = teamo key)
USE_PROXY = True
...
TEACHER = "coze"
SIDE = "teacher"
REASK_R1 = 3  # 本棒 round 4 = 每 caption 第 4 call（reask_idx=3）
TARGET_SUCCESSFUL_PER_CAPTION = 5
SMOKE_CALL_ONLY = True  # 沿 r3 smoke ok=200 + r3 22 caption 全 OK；本棒仅 1-call smoke 防端点飘移

# 选定的 model_id（沿 mapping 拍板代表采样；r3 已 smoke ok=200 + 22 caption 全 OK）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch4-r4",
        "X-Title": f"deposon-l14v3-batch4-r4-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r3 简化版；仅 1 call 确认端点 ok）
# ============================================================
def smoke_coze_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r3 smoke ok=200 + r3 22 caption 全 OK 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b4_r4"
    prompt_id = f"coze_t04_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 4 round 4 · coze 教师侧 round 4（reask=3）续采")
    print("沿 r3 deepseek-v4-flash smoke ok=200 + r3 22 caption 全 OK; 本棒仅 1-call smoke 后跑 22 caption × 1 call")
    print("计划 calls = 22 caption × 1 call = 22 calls + 1 smoke call")
    print("目标: round 4 后 22/22 caption 各 ≥4 successful call")
    print("预算 ≤22 caption calls + 1 smoke call / ≤600s watchdog")
    print("retry 修复沿 r3 (沿 batch2 r3 + batch1 r6)")
    ...
    # 3. 加载前置累计基线（b4 r3 终态作为 coze 自身 baseline；cross batch 沿 b4r3 终态）
    with open(B4_R3_RESULT_PATH, "r", encoding="utf-8") as f:
        b4_r3 = json.load(f)
    b4_r3_cross_total = b4_r3["aggregate"]["cross_batch_with_b4r2_baseline"]["cross_total"]
    b4_r3_cross_empty = b4_r3["aggregate"]["cross_batch_with_b4r2_baseline"]["cross_empty"]
    b4_r3_cross_ok = b4_r3["aggregate"]["cross_batch_with_b4r2_baseline"]["cross_ok"]
    b4_r3_cross_rate = b4_r3["aggregate"]["cross_batch_with_b4r2_baseline"]["cross_rate"]
    print(
        f"[BASELINE b4r3 cross batch] total={b4_r3_cross_total} "
        f"ok={b4_r3_cross_ok} empty={b4_r3_cross_empty} rate={b4_r3_cross_rate:.4f}"
    )

    coze_prior_total = b4_r3["aggregate"]["coze_cumulative"]["post_total"]
    coze_prior_ok = b4_r3["aggregate"]["coze_cumulative"]["post_ok"]
    coze_prior_empty = b4_r3["aggregate"]["coze_cumulative"]["post_empty"]
    coze_prior_rate = b4_r3["aggregate"]["coze_cumulative"]["post_rate"]
    print(
        f"[BASELINE b4r3 coze cumulative] total={coze_prior_total} "
    ...
    # 5. Smoke 1-call（deepseek-v4-flash 端点可达性确认；沿 r3 smoke ok=200 + r3 22 caption 全 OK 已知）
    print("=" * 60)
    print("[SMOKE PHASE] deepseek-v4-flash 1-call smoke（沿 r3 smoke ok=200 + r3 22 caption 全 OK 已知）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": ROUND,
            "metadata": {
                "task": "L14V3_batch4_round4_coze_teacher_side_round4_reask3",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch4_r4_result.json")
    ...
    # 6. 调度：22 caption × 1 call (round 4 reask=3) 按 caption_id 字母序
    captions_sorted = sorted(captions, key=lambda c: c["id"])
    planned = [{"caption_id": c["id"], "phase": "b4_r4_need1"} for c in captions_sorted]
    ...
    # 从 r3 累计 per_caption_successful_calls 起步
    r3_per_cap = b4_r3.get("per_caption_successful_calls", {})
    per_cap_succ = {cid: r3_per_cap.get(cid, {}).get("succ_count", 0) for cid in [c["id"] for c in captions]}
    per_cap_total = {cid: r3_per_cap.get(cid, {}).get("total_calls_so_far", 0) for cid in [c["id"] for c in captions]}
    per_cap_empty = {cid: r3_per_cap.get(cid, {}).get("empty_count_so_far", 0) for cid in [c["id"] for c in captions]}
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_4=True,  # 语义：本棒 = coze 教师侧 data collection round 4
        ...
        target_met = "[OK]" if cur_succ >= 4 else "[..]"
        print(
    ...
    # 跨批累计 = b4r3 baseline + coze 本棒
    cross_batch_total = b4_r3_cross_total + r4_total
    cross_batch_empty = b4_r3_cross_empty + cum_empty - coze_prior_empty
    cross_batch_ok = b4_r3_cross_ok + cum_ok - coze_prior_ok
    ...
    for cid in [c["id"] for c in captions]:
        ...
        t = per_cap_total[cid]
        e = per_cap_empty[cid]
        met_r4 = s >= 4  # round 4 目标 = ≥4 successful/caption
    ...
    # 11. coze 教师侧 round 4 进度块
    coze_round4_progress = {
        "round4_target": ">=4 successful call/caption (round 4 reask=3)",
        "met_count_post_r4_round4": met_round4_count,
        "total_captions": len(captions),
        "remaining_count_round4": len(captions) - met_round4_count,
        "met_percentage_round4": round(met_round4_count / len(captions) * 100, 2),
        "still_pending_captions_round4": sorted([
            cid for cid, info in per_caption_successful_calls.items() if not info["round4_met"]
        ]),
    ...
    # model_id_inconsistency_honest_disclosure（沿 r1/r2/r3 字面格式 + coze 不可考特别声明）
    ...
    model_id_disclosure = (
        ...
        f"r1 + r2 + r3 + r4 四棒累计 88 calls 0 empty 0 fail (沿 r3 终态 66 calls + 本棒增量), "
        f"model_returned 字面 = deepseek-v4-flash-ga-260731 (主) + deepseek-v4-flash-0731 (次), 均 family 下实例; "
        f"r4 沿 r3 结论仅 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-25",
            "spec_conformance": (
                "PI 2026-09-25 batch4 coze 教师侧 round 4 续采 (reask=3); "
                "沿 r3 deepseek-v4-flash smoke ok=200 + r3 22 caption 全 OK; r4 仅 1-call smoke; "
                "22 caption × 1 call = 22 calls (按 caption_id 字母序); "
                "目标 ≥4 successful/caption; "
            ...
            "round_index": ROUND,
            "batch_index": BATCH,
            "round4_target_per_caption": 4,
            "round4_reask_idx": REASK_R1,
            ...
            "endpoint_selection_method": (
                ...
                "r4 沿 r3 smoke ok=200 + r3 22 caption 全 OK; r4 仅 1-call smoke 防端点飘移; "
            ...
            "tools_fault_param_correction": {
                ...
                "b4_r3_status": "r3 1-call smoke ok=200 + 22 caption × 1 call = 22 calls 全 OK; model_returned 字面 deepseek-v4-flash-ga-260731 (主) + deepseek-v4-flash-0731 (次)",
                "r4_b4_status": "r4 仅 1-call smoke (沿 r3 结论)；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r3 deepseek-v4-flash smoke ok=200 + r3 22 caption 全 OK; r4 节省预算避免重复 probe; "
            ...
            "diff_vs_r3_executor": [
                "1. ROUND=4 (升 r4 命名)；BATCH=4 不变",
                "2. prompt_id 前缀 coze_t04_teacher_* (取代 r3 的 t03)；后缀 _b4_r4 (取代 r3 的 _b4_r3)",
                "3. reask_idx=3 (本棒每 caption 第 4 call；取代 r3 的 reask_idx=2)",
                "4. phase_label: b4_r4_need1 (round 4 目标 = ≥4 successful/caption，need_more_to_target=1)",
                "5. is_round_4=True (语义: coze 教师侧 data collection round 4，区别于 r3 的 round 3)",
                "6. smoke_call_only=True (沿 r3 smoke ok=200 + r3 22 caption 全 OK；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移)",
                "7. predecessor_sha12 链: b4_r3_executor b5e1c3637405 + b4_r3_result ed5267df7d6f (新增；取代 r3 链的 b4_r2/r1/r4/r3/probe/batch1_r6/prereg 仍保留)",
                "8. baseline prior: r3 终态 66 calls / 0 empty / 66 ok / 0.0000 rate (取代 r3 链的 44/44/0/0.0)",
                "9. 跨批累计 baseline: r3 终态 251 calls / 9 empty / 3.59% rate (取代 r3 链的 229/9/3.93%)",
                "10. 新增 coze_teacher_side_round4_progress 块 (round 4 完成度 ≥4 successful/caption)",
                "11. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 251/9/3.59%",
                "12. spec_conformance 重写: coze 教师侧 round 4 续采 (reask=3) + 夜间保守口径",
                "13. model_id_inconsistency_honest_disclosure 字面继承 r1/r2/r3 (沿 mapping 拍板代表采样字面) + r1+r2+r3+r4 累计补注",
                "14. 派生 metadata 字段沿 r1/r2/r3 不动 (v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash)",
                "15. per_caption_successful_calls 字段新增 round4_target=4 / round4_met",
                "16. 调度注释: coze 教师侧 round 4 续采 (reask=3) 22 caption × 1 call",
                "17. baseline 路径: B4_R3_RESULT_PATH (沿 r3 result ed5267df7d6f) 取代 r3 链的 B4_R2_RESULT_PATH",
                "18. 启动前必检 tun 端口 (PI 2026-09-25 00:46 夜间保守口径；防 net::ERR_HTTP2_PING_FAILED 重演)",
                "19. is_round_3 替换为 is_round_4 (语义升级)",
                "20. failure result 路径同步 r4: _v4_supp_l14v3_batch4_r4_result.json (取代 r3 链的 r3 result)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "r4 沿 r3 deepseek-v4-flash 代表采样结论 (sha12=ed5267df7d6f 实测 22 calls 全 OK); "
                "不再做 3-candidate probe fallback; 1-call smoke 失败立即 abort; "
                "启动前必检 tun 端口 (PI 2026-09-25 夜间保守口径; 防 net::ERR_HTTP2_PING_FAILED 重演); "
                "r4 与 r3/r2/r1/r4/r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并); "
                "coze 自身 baseline = 66/66/0/0.0 (沿 r3 终态); "
                "cross batch baseline = 251/242/9/3.59% (沿 r3 终态)"
            ),
        },
        "models_listing_summary_ref": {
            ...
            "note": (
                "r4 不重探 /v1/models; 仅引用 r2 探活清单 (sha12=3c9dc60b5071) "
        ...
        "coze_teacher_side_round4_progress": coze_round4_progress,
        "dispatch_target_progress": {
            "target_round4": ">=4 successful calls/caption (round 4 reask=3)",
            "target_cumulative": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (跨轮累计)",
            "met_count_post_r4_round4": met_round4_count,
            "met_count_post_r4_cumulative_to_target": met_target_count,
            "total_captions": len(captions),
            "remaining_count_round4": len(captions) - met_round4_count,
        ...
        "aggregate": {
            ...
            "cross_batch_with_b4r3_baseline": {
                "b4r3_baseline_total": b4_r3_cross_total,
                "b4r3_baseline_empty": b4_r3_cross_empty,
                "b4r3_baseline_ok": b4_r3_cross_ok,
                "b4r3_baseline_rate": b4_r3_cross_rate,
            ...
            "r4_empty_response_rate_threshold_K_N26_N2_single_batch": SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
            "r4_empty_response_above_threshold_single_batch": empty_rate_r4 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
        ...
        "empty_rate_trend": {
            ...
            "cross_batch_cumulative_post_r4": {
                ...
                "note": "b4r3 baseline 251/9/3.59% + r4 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round4_reask3_n_distinct_sample_note": (
                    f"round 4 reask=3 第 4 call 实测；本棒后 coze 教师侧 round 4 = "
                    f"{met_round4_count}/{len(captions)} caption met round 4 目标; "
            ...
            "K_N26_N2_observation": {
                ...
                "single_batch_trigger_K_N26_N2_pass_False": empty_rate_r4 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
                ...
                "monitor_disposition": (
                    ...
                    f"baseline cross batch = {b4_r3_cross_total} calls / {b4_r3_cross_empty} empty / {b4_r3_cross_rate:.4f}"
                ),
                "cross_batch_with_b4r3_baseline_rate": round(cross_batch_rate, 4),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 4 (reask=3) 已完; 下棒可接力 coze 教师侧 round 5 (>=5 more successful/caption)"
            ),
            "note": (
                f"本棒 round 4 (reask=3): actual={r4_total}/{planned_total} calls; "
                f"coze 教师侧 round 4 完成度 = {met_round4_count}/{len(captions)} caption met round 4 目标 (≥4 successful/caption); "
    ...
    print(f"[STATS] r4 ok={r4_ok} empty={r4_empty} fail={r4_fail} total={r4_total} rate={empty_rate_r4:.4f}")
