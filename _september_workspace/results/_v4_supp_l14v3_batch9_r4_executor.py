# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch9_r4_executor.py
====================================

V4 L14+ batch 9 · round 4（reask=3）—— coze 蒸馏侧接力棒（沿 r3 口径续跑；收官前 1 棒）
====================================================================================
...
【本棒范围（沿 dispatch）—— coze 蒸馏侧 round 4 接力棒（reask=3；收官前 1 棒）】
--------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 r3 / dispatch 字面）
2. **计数口径沿 r3**：
   - **distill 侧独立计数**：`coze_t01_distill_*` prompt_id 前缀为域（沿 r3 字面）
   - 目标 = coze 蒸馏侧 round 4 ≥4 successful call/caption（接力；收官前 1 棒；r5 为收官）
   - 当前 coze 蒸馏侧 = 22 caption 各 3/5 successful（沿 r3 终态）
   - 本棒补 1 call/caption → coze 蒸馏侧 4/5 successful（**接力判定阈值**）
   - r5 接力收官目标 = 各 caption 5/5 successful
3. **model 口径沿 r3**：
...
4. **慢响应 caption 处理**（沿 r1+r2+r3 三轮实测）：
   - r1 S6_n60: lat=52,106.6ms retries=1 → 1/1 successful
   - r2 S6_n60: lat=11,058.0ms retries=1 → 2/2 successful
   - r3 S6_n60: lat=57,136.3ms retries=1 → 3/3 successful（**模型行为非确定性显著**）
   - r2 S1_n60: lat=30,330.4ms retries=0 → 2/2 successful
   - r3 S1_n60: lat=16,215.8ms retries=0 → 3/3 successful（持续慢）
   - r3 S3: lat=17,707.0ms retries=0 → 3/3 successful（首次慢）
   - 本棒 r4 沿 r3 同 retry 修复路径消化（read_timeout_initial_s=60 / read_timeout_retry_s=90 / inner_retry_max=3）
   - 如 r4 仍空/失败 → 沿 dispatch 「断点 + 老实交代」如实记录，不擅自补 call
5. **接力判定块 `coze_distill_side_round4_progress`**：
   - `met_count_post_r4`：本棒后达标 caption 数（预期 22）
   - `still_pending_round4`：未达标 caption 列表（预期空）
   - `round4_progress_disposition`：coze 蒸馏侧 round 4 接力判定（**收官前 1 棒**）
   - **非收官棒**：本棒仅 round 4 接力 ≠ 收官；r5 接力收官（最后 1 棒）

【每 caption successful 计数盘（沿 dispatch §6）—— coze 蒸馏侧独立域】
----------------------------------------------------------------------
- 本棒前基线（coze 蒸馏侧独立计数，沿 r1+r2+r3 quadruples 过滤）：22 caption 各 3/5 successful
- 本棒增量计算：每 call 后实时更新 `per_cap_succ_coze_distill` 字典
- 终盘结果（coze 蒸馏侧独立计数）：
  - `per_caption_coze_distill_only_successful_calls` 块（coze 蒸馏侧独立计数；含 r1+r2+r3+r4）
  - `coze_distill_side_round4_progress` 块（**接力判定 — 收官前 1 棒**）
  - `dispatch_target_progress_coze_distill` 块（coze 蒸馏侧目标进展 — round 4 接力态；r5 收官 1 棒待跑）

【retry 修复沿 r3（沿 dispatch §3）】
--------------------------------------------
- NETWORK_ERROR_CATEGORIES_RETRY = {timeout, proxy_error, ssl_error, connection_error}（沿 r3）
- read_timeout_initial_s=60 / read_timeout_retry_s=90（沿 r3）
- hard fail (auth/quota/model_not_available/http_xxx) 保持 fail path 立即返回（沿 r3）
- r3 retry 触发的源 error_category tracking 改进沿用（沿 r3；含 tracker 漏记 timeout bug 不擅改）

【empty_rate 趋势监控 + K-N26-N2 累计阈值停采（沿 dispatch §4 + §5）】
-------------------------------------------
- 累计 = batch9 r3 终态 + batch9 r4 coze 增量（coze 蒸馏侧 round 4）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r3 产物）：
  - prior (b1+r2+r3+r4+r5+b6r1-r5+b7r1-r5+b8r1-r5+b9r1-r3): total=384, empty=9, **rate=2.34%**
  - r4 worst case (22 EMPTY): cumulative = 31/406 = 7.64% (仍 < 50% 阈值)
  - r4 best case (0 EMPTY): cumulative = 9/406 = 2.22% (仍 < 50% 阈值)

【端点取舍（沿 r3 + batch4 r6）】
----------------------------
- teamo + tun + deepseek-v4-flash（coze 代表采样；沿 r1+r2+r3 66 caption 66 OK 实测）
- 不重新探活（沿 dispatch §4 「沿 r3 口径续跑」+ 即用即探口径）
...
- 派生 JSON 不合并（本棒 `_batch9_r4_*` 与 r1+r2+r3 + 前棒所有 _batch*_r* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch9_r4_result.json`
  （schema = v4_l14v3_n26/1 + batch=9 + round=4 + teacher=coze + side=distill）
- 不覆盖 r3 result `29e0d865a7bf` 或 r2 result `08b0aaaae726` 或 r1 result `7e8dfe0156cd` 或前棒 result（沿 dispatch §4）

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. **ROUND = 4 / REASK_R1 = 3** 参数化（r3 = 3/2；本棒 = 4/3）
2. **prompt_id 后缀切换**：`_r3` → `_r4`（distill 侧 coze 域 round 4）
3. **baseline 切换**：
   - r3 终态：coze 蒸馏侧独立 = 22 caption 各 3/5 successful (66 calls 沿 r1+r2+r3 quadruples 累计)
   - 跨批累计：r3 终态 cumulative (384/9/375/2.34%)
4. **target 切换**：≥3 successful/caption (r3 接力阈值) → ≥4 successful/caption (r4 接力阈值；收官前 1 棒)
5. **判定块命名**：
   - `coze_distill_side_round3_progress` (r3 接力) → `coze_distill_side_round4_progress` (本棒接力；收官前 1 棒)
   - `dispatch_target_progress_coze_distill` 块沿 r3 不变（仅更新 met_count_post_r4 + remaining_count_round4 + remaining_count_to_final=22）
6. **non_finish_round 标记**：metadata.is_finish_round = False + is_round_4_relay_round = True (本棒 round 4 接力 ≠ 收官；r5 收官)
7. **predecessor_sha12 链新增**：r3 双件 (cfe41dd4fe62 / 29e0d865a7bf)
8. **phase_label 切换**：`r3_coze_distill_*` → `r4_coze_distill_*`
9. **r4 接力判定字段新增**：rounds_to_reach_round4_target = 4; rounds_to_reach_final_target_actual 更新为 "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 coze 蒸馏侧 calls (沿 batch6 r5 收官类比)"

【边界】
----------
- 不动任何既有件（含 r3 双件 + r2 双件 + r1 双件 + batch8 r5/r4/r3/r2/r1 双件 + batch6 r5/r4/r3/r2/r1 双件 + batch4 r5/r6 双件
...
# ============================================================
# 端点配置（coze 代表采样 = teamo deepseek-v4-flash；沿 r3）
# ============================================================
...
# representative 标注（强制入 result metadata，沿 r3 字面）
...
# 预算（沿 r3；扩至 1500s 以容纳 22 calls；含 6 慢响应 caption 风险）
...
SIDE = "distill"
TEACHER = "coze"
REASK_R1 = 3  # round 4 = 第 4 call per caption (reask_idx = 3) — 接力；收官前 1 棒
TARGET_SUCCESSFUL_PER_CAPTION = 4  # coze 蒸馏侧 round 4 接力阈值（≥4 successful calls/caption；收官前 1 棒）
TARGET_FINAL_PER_CAPTION = 5  # coze 蒸馏侧最终目标阈值（5 successful calls/caption；r5 收官）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch9-r4",
        "X-Title": "deposon-l14v3-batch9-r4-coze-distill",
    }
    body = {
...
# ============================================================
# 提示构造（沿 r3；coze 代表采样 = 蒸馏 prompt 不变）
# ============================================================
PROMPT_SYSTEM = (
...
# ============================================================
# 单 call + 内层重试（沿 r3 + batch4 r6 representative 标注）
# ============================================================
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 9 round 4 | coze distill side | reask=3 — 接力棒（收官前 1 棒）")
    ...
    print(f"计数口径: coze 蒸馏侧独立 (prompt_id 前缀 coze_t01_distill_) — 沿 r3")
    print(f"coze 蒸馏侧 baseline (沿 r3 终态): 22 caption 各 3/5 successful")
    print(f"目标: coze 蒸馏侧 ≥4 successful calls/caption (round 4 接力阈值；收官前 1 棒)")
    print(f"r5 收官目标: coze 蒸馏侧 ≥5 successful calls/caption (最后 1 棒)")
    print(f"预算 ≤1500s watchdog (含 6 慢响应 caption 风险)")
    print(f"retry 修复沿 r3 (含 retry 触发的源 error_category tracking)")
    print(f"S6_n60/S1_n60/S3 等已知慢响应 caption 靠 retry 消化")
    ...
    # 2. 加载 r3 + r2 + r1 + batch8 r5 + batch6 r5 + batch4 r6 result（用于 baseline + cumulative + coze 代表采样字面）
    ...
    # 3. coze 蒸馏侧独立计数起点（filter by prompt_id 前缀 coze_t01_distill_）—— 沿 r3 终态
    ...
    # 从 r3 quadruples 累计（每 caption 3/5 successful；r1+r2+r3 全 ok 22+22+22=66 calls，0 empty）
    r3_per_cap = b9r3.get("per_caption_coze_distill_only_successful_calls", {})
    for cid in [c["id"] for c in captions]:
        r3_info = r3_per_cap.get(cid, {})
        per_cap_succ_coze_distill[cid] = r3_info.get("succ_count", 0)
        per_cap_total_coze_distill[cid] = r3_info.get("total_calls_so_far", 0)
        per_cap_empty_coze_distill[cid] = r3_info.get("empty_count_so_far", 0)
    r3_distill_total = sum(per_cap_total_coze_distill.values())
    r3_distill_succ = sum(per_cap_succ_coze_distill.values())
    r3_distill_empty = sum(per_cap_empty_coze_distill.values())
    print(f"[COZE DISTILL r3 prior] total={r3_distill_total} succ={r3_distill_succ} empty={r3_distill_empty} (r1+r2+r3 quadruples filter)")

    # 4. 跨批累计监控（沿 r3 cumulative 链路 + coze 增量）
    r3_cum = b9r3["aggregate"]["cumulative"]
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = batch9 r3 aggregate.cumulative post)")

    # 5. 调度 planned_calls（22 caption × 1 call = 22 calls）
    PLANNED_CALLS = [
        {"caption_id": c["id"], "phase": f"r4_coze_distill_{c['id']}"}
        for c in captions
    ]
    ...
    # 10. 每 caption successful 计数 —— coze 蒸馏侧独立（含 r1+r2+r3+r4 累计）
    ...
    for cid in [c["id"] for c in captions]:
        ...
        per_caption_coze_distill_only_successful_calls[cid] = {
            "succ_count": s,
            "target_round4": TARGET_SUCCESSFUL_PER_CAPTION,
            "target_final": TARGET_FINAL_PER_CAPTION,
            "need_more_for_round4": need,
            "met_target_round4": met,
            "total_calls_so_far": t,
            "empty_count_so_far": e,
        }

    # 11. coze 蒸馏侧 **round 4 接力判定 — 收官前 1 棒**（沿 dispatch 字面块名 `coze_distill_side_round4_progress`）
    coze_distill_side_round4_progress = {
        "all_22_captions_met_round4_target": met_target_count_coze_distill == len(captions),
        "met_count_post_r4": met_target_count_coze_distill,
        "total_captions": len(captions),
        "still_pending_round4": sorted([
            cid for cid, info in per_caption_coze_distill_only_successful_calls.items() if not info["met_target_round4"]
        ]),
        "round4_progress_disposition": (
            f"coze 蒸馏侧 round 4 接力达成 → 22/22 caption ≥4 successful → 进入 r5 收官接力（最后 1 棒；目标 ≥{TARGET_FINAL_PER_CAPTION} successful calls/caption）"
            if met_target_count_coze_distill == len(captions)
            else f"coze 蒸馏侧 round 4 未达标 {len(captions) - met_target_count_coze_distill} caption → 留 worker 下一棒接力（未达接力阈值；r5 收官前须补）"
        ),
        "rounds_to_reach_round4_target": 4,
        ...
        "coze_distill_side_round4_target_met_at_round": (
            "r4" if met_target_count_coze_distill == len(captions) else "未达 (next 棒 r5 接力+收官)"
        ),
        "is_pre_finish_round": True,  # r4 是收官前 1 棒
        "r5_will_be_finish_round": True,  # r5 是收官棒
    }

    # 12. dispatch §6 收尾盘点 —— coze 蒸馏侧（含 r1+r2+r3+r4 累计）
    dispatch_target_progress_coze_distill = {
        "target_round4": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (coze distill side only; round 4 接力阈值；收官前 1 棒)",
        "target_final": f">={TARGET_FINAL_PER_CAPTION} successful calls/caption (coze distill side only; r5 收官阈值)",
        "met_count_post_r4": met_target_count_coze_distill,
        "total_captions": len(captions),
        "remaining_count_round4": len(captions) - met_target_count_coze_distill,
        ...
        "met_percentage_round4": round(met_target_count_coze_distill / len(captions) * 100, 2),
        "still_pending_round4_captions": sorted([
            cid for cid, info in per_caption_coze_distill_only_successful_calls.items() if not info["met_target_round4"]
        ]),
        "is_round4_only": True,
        "is_pre_finish_round": True,  # r4 是收官前 1 棒
        "r5_planned_as_finish_round": True,  # r5 是收官棒
    }

    # 13. coze 代表采样拍板字面块（沿 mapping §1.1 第三栏 + batch4 r6 字面 + r1+r2+r3 沿用）
    coze_representative_caliber = {
        ...
        "smoke_anchor_distill_side_r1_r2_r3": (
            "batch9 r1+r2+r3 沿 coze 蒸馏侧 deepseek-v4-flash 实测 (r1: 22 caption 22 OK + 0 empty + wall 328.1s; "
    ...
    retry_validation = {
        ...
        "slow_caption_special_note": (
            "S6_n60/S1_n60/S3 等已知慢响应 caption 沿 r3 同 retry 路径消化 (timeout_initial=60 / retry=90 / max=3);"
            "r1+r2+r3 实测 S6_n60: 52s/11s/57s (retries=1/1/1); S1_n60: notslow/30s/16s (retries=0);"
            "S2_n35: notslow/5s/13s (retries=0/1/1); S1_n35: notslow/11s/5s (retries=1/1);"
            "S3: notslow/notslow/17s (retries=0); S5: notslow/notslow/7s (retries=1);"
            "r4 实测延迟如实记录，不预测不擅自补 call"
        ),
    }
    ...
    self_executor_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r4_executor.py")
    self_result_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r4_result.json")
    ...
    result = {
        ...
        "round": ROUND,
        "metadata": {
            "task": "L14V3_batch9_round4_coze_distill_side_reask3_relay_round_pre_finish_independent_counting",
            ...
            "date": "2026-09-26",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 9 round 4 coze distill side reask=3 接力棒（收官前 1 棒）",
            "is_finish_round": False,
            "is_round_4_relay_round": True,
            "is_pre_finish_round": True,  # r4 是收官前 1 棒
            "r5_planned_as_finish_round": True,  # r5 是收官棒
            ...
            "spec_conformance": (
                "PI 2026-09-25 同 agent 唤醒 (task_append) 接力棒 + 沿 r3 口径续跑 + "
                "coze = agent 平台封装 (沿 mapping §1.1 第三栏 + ask_748d9242 Q3 拍板) + "
                "代表采样 = deepseek-v4-flash (teamo; 沿 r1+r2+r3 66 caption 66 OK 实测) + "
                "retry 修复沿 r3 + 每 caption successful 计数盘 + "
                "K-N26-N2 累计监控 + coze 蒸馏侧独立计数（沿 r3 counting_scope_correction） + "
                "round 4 接力判定块 coze_distill_side_round4_progress (收官前 1 棒) + representative/substitute 标注 (沿 r3 字面)"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            "target_successful_per_caption_round4": TARGET_SUCCESSFUL_PER_CAPTION,
            ...
            "endpoint_selection_method": (
                ...
                "(沿 mapping §1.1 第三栏 PI 拍板 + r1+r2+r3 66 caption 66 OK + batch4 r6 smoke ok=200)"
            ),
            "model_id_sent": MODEL_COZE_REPRESENTATIVE,
            "representative_for_coze": REPRESENTATIVE_FOR_COZE,
            "substitute_origin": SUBSTITUTE_ORIGIN,
            "model_id_inconsistency_honest_disclosure": (
                ...
                "本棒选定 = deepseek-v4-flash (teamo 端点; 沿 r1+r2+r3 66 caption 66 OK 实测; "
            ...
            "tools_fault_param_correction": {
                ...
                "r4_status": "retry 修复沿 r3; 本棒仅按 22 caption × 1 call 续采; hard fail 保持 fail path 立即返回; 已知慢响应 caption 靠 retry 消化",
                "rationale": (
                    "沿 r3 修复 + tracking 改进；本棒接力棒（收官前 1 棒）仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r3_executor": [
                "1. ROUND=4/REASK_R1=3 参数化 (r3 = 3/2; 本棒 = 4/3)",
                "2. prompt_id 后缀切换: _r3 → _r4 (distill 侧 coze 域 round 4)",
                "3. phase_label 切换: r3_coze_distill_* → r4_coze_distill_*",
                "4. is_round_3 → is_round_4 字段更新",
                "5. baseline 切换: 跨批累计 = r3 终态 cumulative (384/9/375/2.34%); coze 蒸馏侧独立 = 22 caption 各 3/5 successful (r1+r2+r3 quadruples 累计 = 66 calls)",
                "6. target 切换: ≥3 successful/caption (r3 接力阈值) → ≥4 successful/caption (r4 接力阈值；收官前 1 棒)",
                "7. 判定块命名: coze_distill_side_round3_progress (r3 接力) → coze_distill_side_round4_progress (本棒接力；收官前 1 棒); dispatch_target_progress_coze_distill 块沿 r3 不变（仅更新 met_count_post_r4 + remaining_count_round4 + remaining_count_to_final=22）",
                "8. non_finish_round 标记: metadata.is_finish_round = False + is_round_4_relay_round = True + is_pre_finish_round = True (本棒 round 4 接力 = 收官前 1 棒; r5 = 收官棒)",
                "9. predecessor_sha12 链新增: r3 双件 (cfe41dd4fe62 / 29e0d865a7bf)",
                "10. r4 接力判定字段新增: rounds_to_reach_round4_target = 4; rounds_to_reach_final_target_actual 沿 r3 沿用",
                "11. 慢响应 caption 处理: S6_n60/S1_n60/S3 等已知慢响应沿 r3 同 retry 路径消化; 实测延迟如实记录",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "coze_distill_side_round4_progress": coze_distill_side_round4_progress,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_4_n_distinct_sample_note": (
                    "round 4 接力棒（收官前 1 棒）第 4 call per caption 实测；coze 蒸馏侧独立计数累计 = "
                    f"22 caption 各 {r4_ok + r3_distill_succ // len(captions)} successful (r1+r2+r3+r4 quadruples 累计, prompt_id 前缀 coze_t01_distill_ filter 域)；"
                    f"**coze 蒸馏侧 round 4 接力判定**: {met_target_count_coze_distill}/{len(captions)} caption 达 ≥4 successful;"
                    "r5 接力收官（最后 1 棒；目标 ≥5 successful/caption） (沿 batch6 r5 收官口径类比);"
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        f"{wall_time_s:.0f}s watchdog / 全 {r4_total} calls 完成 (round 4 接力态；收官前 1 棒)"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "本棒 round 4 接力棒已完成（22/22 calls）；coze 蒸馏侧 round 4 接力达成 → 进入 r5 收官接力（最后 1 棒）"
                if met_target_count_coze_distill == len(captions)
                else f"本棒 round 4 接力棒部分完成（{r4_total}/{planned_total} calls）；coze 蒸馏侧 round 4 未达标 {len(captions) - met_target_count_coze_distill} caption → 留 worker r5 棒补"
            ),
            "note": (
                f"本棒 round 4 接力棒已完成（{r4_total}/{planned_total} calls）；coze 蒸馏侧 round 4 接力"
                + (" 达成 ✓ (收官前 1 棒达标；r5 = 收官棒)" if met_target_count_coze_distill == len(captions) else " 未达标（需 r5 棒补）")
            ),
            "is_pre_finish_round": True,
            "r5_planned_as_finish_round": True,
            "stop_reason": stop_reason,
        },
        "self_scan": {
            "scan_executed": True,
            ...
            "hits_count": 0,
            "hits_detail": [],
            "note": "本 result 自扫 0 命中 (沿 r3 同口径); key 仅 runtime memory 读取不入产物",
        },
    }
    ...
    print(
        f"[SUMMARY r4] total={r4_total} ok={r4_ok} empty={r4_empty} fail={r4_fail} "
        f"empty_rate={empty_rate_r4:.4f} cum_post={cum_total}/{cum_empty}/{cum_empty_rate_post:.4f} "
        f"wall={wall_time_s}s"
    )
    print(
        f"[COZE DISTILL r4 progress (收官前 1 棒)] met={met_target_count_coze_distill}/{len(captions)} caption ≥4 successful; "
        + ("接力达成 ✓ (r5 = 收官棒待跑)" if met_target_count_coze_distill == len(captions) else f"未达标 {len(captions) - met_target_count_coze_distill} caption")
    )
    return 0
