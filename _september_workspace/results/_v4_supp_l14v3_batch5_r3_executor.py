# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch5_r3_executor.py
=====================================

V4 L14+ batch 5 · round 3 · minimax 教师侧 round 3（reask=2）续采
==================================================================

【派工依据】
----------------
- 沿 batch5 r2 executor `F97E46E3739E` + r2 result `B0F76B5C3522` 口径（基础口径锚，本棒同 agent 唤醒保上下文）
...
- 拍板入元数据沿 r2：ask_249ba8854ee343f7d9aa34de（2026-09-24 23:38）prompt_repro=accept_repro
  —— 蒸馏/采样数据效力 = V4 复现口径（非 V3 原文复刻）声明沿用

【本棒范围（沿 dispatch）—— minimax 教师侧 round 3（reask=2）续采】
------------------------------------------------------------------
1. **沿用 mimo-v2.6-pro**（沿 r2 1-call smoke ok=200, model_returned=mimo-v2.6-pro；本棒沿 r2 结论不再做完整 probe）
2. **22 caption × 1 call**（每位 caption 第 3 call；reask_idx=2；目标 3/5 successful/caption）
3. **目标**：本棒后 22/22 caption 各 ≥3 successful call；下棒接力至 ≥5 successful/caption
4. **endpoint_migrated**: true（沿 r2）
5. **lineage_discontinuity**: true（沿 r2）
6. **prompt_repro**: accept_repro（沿 r2 拍板 ask_249ba8854ee343f7d9aa34de）
...
---------------------------------------------
- 本棒前基线（沿 r2 终态）：minimax 教师侧 = 44 calls / 0 empty / 44 ok / 0.0000 rate
...
--------------------------------------------------------------------
- 跨批累计 baseline = r2 终态 207 calls / 9 empty / 198 ok / 4.35%
- 累计 = r2 cross-batch baseline + minimax 本棒（minimax 自身从 44 起步）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）

【端点取舍（沿 r2）】
------------------
- mimo + 无代理 + mimo-v2.6-pro（沿 r2 1-call smoke ok=200；r3 沿 r2 结论仅做 1-call smoke 防端点飘移）
...
- 派生 JSON 不合并（本棒 `_batch5_r3_*` 与 `_batch5_r2_*` / `_batch5_r1_*` / `_batch2_r3_*` / `_batch2_r2_*` / `_batch2_r1_*` / `_batch1_r6_*` 独立）
...
- 不覆盖既有件（r2 executor `F97E46E3739E` + r2 result `B0F76B5C3522` + r1 executor `EF26ADF4669F` + r1 result `120D45D1774F` + r3 executor + r3 result + r2 executor + r2 result + batch1 r6 executor + batch1 r6 result + r1 executor + r1 result + models_probe + mapping `98A779D61C1E` + prereg `05B975A86989` 不动）
- **predecessor 命名沿 b5_* 前缀防冲突**（r2 修复先例：r1/r2/r3 → b2_r1/b2_r2/b2_r3 重命名；本棒新增 b5_r1/b5_r2；b5_r3 为本棒 result；chain 全程命名空间无歧义）

【产物】
--------
- 写入：`results/_v4_supp_l14v3_batch5_r3_result.json`（schema = `v4_l14v3_n26/1` + batch=5 + round=3）
- 不覆盖 r2 result `B0F76B5C3522` 或 r2 executor `F97E46E3739E` 或任何既有件
- predecessor_sha12 链：b5_r2_executor `F97E46E3739E` + b5_r2_result `B0F76B5C3522` + b5_r1_executor `EF26ADF4669F` + b5_r1_result `120D45D1774F` + b2_r3_executor + b2_r3_result + b2_r2_executor + b2_r2_result + batch1_r6_executor + batch1_r6_result + b2_r1_executor + b2_r1_result + models_probe + mapping `98A779D61C1E` + prereg `05B975A86989`

【与 r2 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 3（升 r3 命名）；BATCH = 5 不变
2. prompt_id 后缀 `_b5_r3`；`minimax_t03_teacher_*`（取代 r2 的 `minimax_t02_teacher_*_b5_r2`）
3. reask_idx = 2（本棒每 caption 第 3 call；取代 r2 的 reask_idx=1）
4. phase_label：`b5_r3_need1`（取代 r2 的 `b5_r2_need1`）
5. is_round_3 = True（语义：本棒 = minimax 教师侧 data collection round 3；区别于 r2 的 round 2 reask=1）
6. is_round_2 = False（语义：已过 round 2，进入 round 3；区别于 r2 的 is_round_2=True）
7. smoke_call_only = True（沿 r2 结论；r3 仅 1-call smoke 防端点飘移；不再做 3-candidate probe fallback）
8. predecessor_sha12 链：b5_r2_executor `F97E46E3739E` + b5_r2_result `B0F76B5C3522`（新增；取代 r2 链的 b5_r1_executor + b5_r1_result 仍保留）
9. baseline prior：r2 终态 minimax 44 calls / 0 empty / 44 ok / 0.0000 rate（取代 r2 链的 22/0/22）
10. 跨批累计 baseline：r2 终态 207 calls / 9 empty / 198 ok / 4.35%（取代 r2 链的 185/9/4.86%）
11. 新增 `minimax_teacher_side_round3_progress` 块（本棒后 minimax 教师侧 round 3 完成度 ≥3 successful/caption）
12. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 207/9/4.35%
13. spec_conformance 重写：minimax 教师侧 round 3 续采（reask=2）
14. per_caption_successful_calls 字段新增 round3_target=3 / round3_met
15. 调度注释：round 3 续采（reask=2）22 caption × 1 call
16. **predecessor 命名沿 b5_* 前缀防冲突**（沿 r2 修复先例）

【边界】
--------
- 不动任何既有件（含 r2 executor `F97E46E3739E` + r2 result `B0F76B5C3522` + r1 executor `EF26ADF4669F` + r1 result `120D45D1774F` + b2_r3 executor + b2_r3 result + b2_r2 executor + b2_r2 result + batch1 r6 executor + batch1 r6 result + b2_r1 executor + b2_r1 result + models_probe + mapping + prereg）
- 跑不完拆段报断点
- 0 触动 V1–V3 资产 / R5 V4 frozen / R6 P-G / R7 plugin spec
- mimo 不走 tun（沿 L7 实证 + add_T1 §1.6.2）
- prompt_repro / data_validity 拍板沿 r2 入元数据；不动 N-26 主预登记 / K-N26-* 字面 / 既有阈值 / 既定结论
"""
...
TEACHER = "minimax"
SIDE = "teacher"
REASK_R2 = 2  # 本棒 round 3 = 每 caption 第 3 call（reask_idx=2）
TARGET_SUCCESSFUL_PER_CAPTION = 5
SMOKE_CALL_ONLY = True  # 沿 r2 结论；r3 仅 1-call smoke 防端点飘移
...
def call_chat_mimo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch5-r3",
        "X-Title": f"deposon-l14v3-batch5-r3-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r2 简化版；仅 1 call 确认 mimo 端点 ok）
# ============================================================
def smoke_mimo_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r2 结论 mimo-v2.6-pro ok=200；仅作端点可达性确认）。"""
    t0 = time.time()
...
# ============================================================
# 单 call + 内层重试（沿 r2 + is_round_3 扩展）
# ============================================================
def run_one_quadruple(
    ...
    suffix = "_b5_r3"
    prompt_id = f"minimax_t03_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 5 round 3 · minimax 教师侧 round 3（reask=2）续采")
    print("沿 r2 mimo-v2.6-pro 1-call smoke ok=200；本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r2 (沿 batch1 r6)；mimo 端点无代理 (沿 L7 + add_T1 §1.6.2)")
    print("endpoint_migrated=true / lineage_discontinuity=true")
    print("prompt_repro=accept_repro (ask_249ba8854ee343f7d9aa34de 2026-09-24 23:38 沿 r2 拍板)")
    ...
    # r2 终态 = 本棒前 cross-batch 累计基线
    cross_prior_total = r2["aggregate"]["cross_batch_with_r1_baseline"]["cross_total"]
    cross_prior_empty = r2["aggregate"]["cross_batch_with_r1_baseline"]["cross_empty"]
    cross_prior_ok = r2["aggregate"]["cross_batch_with_r1_baseline"]["cross_ok"]
    cross_prior_rate = r2["aggregate"]["cross_batch_with_r1_baseline"]["cross_rate"]
    print(f"[BASELINE cross batch r2] total={cross_prior_total} ok={cross_prior_ok} empty={cross_prior_empty} rate={cross_prior_rate:.4f}")

    # minimax 自身前基线 = r2 终态（44 calls / 0 empty / 44 ok）
    minimax_prior_total = r2["aggregate"]["minimax_cumulative"]["post_total"]
    minimax_prior_empty = r2["aggregate"]["minimax_cumulative"]["post_empty"]
    minimax_prior_ok = r2["aggregate"]["minimax_cumulative"]["post_ok"]
    minimax_prior_rate = r2["aggregate"]["minimax_cumulative"]["post_rate"]
    print(f"[BASELINE minimax r2] total={minimax_prior_total} ok={minimax_prior_ok} empty={minimax_prior_empty} rate={minimax_prior_rate:.4f}")
    ...
    # 5. Smoke 1-call（沿 r2 结论 mimo-v2.6-pro ok=200）
    print("=" * 60)
    print("[SMOKE PHASE] mimo-v2.6-pro 1-call smoke（沿 r2 结论）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": 3,
            "metadata": {
                "task": "L14V3_batch5_round3_minimax_teacher_side_round3_reask2",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch5_r3_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b5_r3_need1"} for c in captions_sorted]
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_2=False,  # 语义：已过 round 2
            is_round_3=True,   # 语义：本棒 = minimax 教师侧 data collection round 3
    ...
    # 跨批累计 = r2 cross-batch baseline + minimax 本棒
    cross_batch_total = cross_prior_total + r3_total
    cross_batch_empty = cross_prior_empty + r3_empty
    cross_batch_ok = cross_prior_ok + r3_ok
    ...
    # 11. minimax 教师侧 round 3 进度块
    minimax_round3_progress = {
    ...
    # data_validity_disclosure（沿 r2 拍板 ask_249ba8854ee343f7d9aa34de）
    data_validity_disclosure = {
        ...
        "honest_disclosure": (
            "本棒 minimax 教师侧 round 3 (reask=2) 22 caption × 1 call 全部产出 "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                "PI 2026-09-24 batch5 minimax 教师侧 round 3 续采 (reask=2); "
                "沿 r2 mimo-v2.6-pro 1-call smoke ok=200 结论；r3 仅 1-call smoke; "
            ...
            "tools_fault_param_correction": {
                ...
                "r2_b5_status": "r2 沿 r1 结论 mimo-v2.6-pro 1-call smoke ok=200",
                "r3_b5_status": "r3 沿 r2 结论仅做 1-call smoke 防端点飘移；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r2 mimo-v2.6-pro smoke 结论；r3 节省预算避免重复 probe；"
            ...
            "diff_vs_r2_executor": [
                "1. ROUND=3 (升 r3 命名)；BATCH=5 不变",
                "2. prompt_id 后缀 _b5_r3；t03_teacher (取代 r2 的 t02_teacher round 2 reask=1)",
                "3. reask_idx=2 (本棒每 caption 第 3 call；取代 r2 的 reask_idx=1)",
                "4. phase_label: b5_r3_need1 (取代 r2 的 b5_r2_need1)",
                "5. is_round_3=True (语义: minimax 教师侧 data collection round 3)",
                "6. is_round_2=False (语义: 已过 round 2)",
                "7. smoke_call_only=True (沿 r2 结论；不再做 3-candidate probe)",
                "8. predecessor_sha12 链: b5_r2_executor F97E46E3739E + b5_r2_result B0F76B5C3522 (新增；r2 链的 b5_r1_executor + b5_r1_result 仍保留)",
                "9. baseline prior: r2 终态 minimax 44 calls / 0 empty / 44 ok / 0.0000 rate (取代 r2 链的 22/0/22)",
                "10. 跨批累计 baseline: r2 终态 207 calls / 9 empty / 198 ok / 4.35% (取代 r2 链的 185/9/4.86%)",
                "11. 新增 minimax_teacher_side_round3_progress 块 (round 3 完成度 ≥3 successful/caption)",
                "12. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 207/9/4.35%",
                "13. spec_conformance 重写: minimax 教师侧 round 3 续采 (reask=2)",
                "14. per_caption_successful_calls 字段新增 round3_target=3 / round3_met",
                "15. 调度注释: round 3 续采 (reask=2) 22 caption × 1 call",
                "16. predecessor 命名沿 b5_* 前缀防冲突 (沿 r2 修复先例)",
                "17. 新增 is_round_3 字段 (取代 r2 的 is_round_2=True)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
                ...
                "is_round_3_disclosed": True,
            },
            "run_window_cst": now_iso,
            "stop_reason": stop_reason,
            "interruption_recovery_note": (
                "r3 沿 r2 mimo-v2.6-pro smoke 结论；不再做 3-candidate probe fallback；"
                "1-call smoke 失败立即 abort；r3 与 r2/r1/b2_r3/b2_r2/b2_r1/batch1_r6 派生 JSON 独立 (不合并)"
            ),
        },
        ...
        "minimax_teacher_side_round3_progress": minimax_round3_progress,
        ...
        "empty_rate_trend": {
            ...
            "batch5_round3_cumulative_minimax": {
                "calls": cum_total,
                "empty": cum_empty,
                "ok": cum_ok,
            ...
            "cross_batch_cumulative_post_r3": {
                ...
                "note": "r2 baseline 207/9/4.35% + r3 minimax 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round3_reask2_n_distinct_sample_note": (
                    f"round 3 reask=2 第 3 call 实测；本棒后 minimax 教师侧 round 3 = "
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    f"r2 baseline minimax = {minimax_prior_total} calls / {minimax_prior_empty} empty / {minimax_prior_rate:.4f}; "
                    f"r2 baseline cross batch = {cross_prior_total} calls / {cross_prior_empty} empty / {cross_prior_rate:.4f}; "
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 3 (reask=2) 已完; 下棒可接力 minimax 教师侧 round 4 (>=2 more successful/caption, 目标 ≥5 cumulative/caption)"
            ),
            "note": (
                f"本棒 round 3 (reask=2): actual={r3_total}/{planned_total} calls; "
                f"minimax 教师侧 round 3 完成度 = {met_round3_count}/{len(captions)} caption met round 3 目标 (≥3 successful/caption); "
                f"stop_reason={stop_reason}; "
                f"minimax model_id 沿 mapping §1.1 拍板 = {model_id} (1-call smoke 确认仍可用); "
                f"新拍板 ask_249ba8854ee343f7d9aa34de prompt_repro=accept_repro 已沿 r2 入 metadata + data_validity_disclosure 块"
            ),
            "planned_total": planned_total,
            "stop_reason": stop_reason,
        },
