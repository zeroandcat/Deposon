# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch4_r5_executor.py
====================================

V4 L14+ batch 4 · round 5 · coze 教师侧 round 5（reask=4）**收官**
================================================================

【派工依据】
----------------
- 沿 batch4 r4 executor `184186902A9F` + r4 result `41E0BA55E205` 口径（基础口径锚）
...
- 沿 batch2 r2 executor `e7418f47a130` + r2 result `8c125e257af8` 历史口径锚
- 沿 /v1/models 探活 `_v4_supp_l14v3_batch2_r2_models_probe.json` (sha12=`3c9dc60b5071`)
  确认 `deepseek-v4-flash` 在 44 models 清单内（V4 现行可考）
- 派工单 coze 教师批 4 round 5（接力棒，沿 r4 上下文唤醒）：22 caption × 1 call（reask=4）**收官**

【本棒范围（沿 dispatch）—— coze 教师侧 round 5（reask=4）收官】
------------------------------------------------------------
1. **沿用 teamo `deepseek-v4-flash` 代表采样**（r4 smoke ok=200 + r4 22 caption 全 OK 已知）
2. **22 caption × 1 call**（每位 caption 第 5 call；reask_idx=4；目标 round 5 完成度 = 5/5 successful/caption **收官**）
3. **目标**：本棒后 22/22 caption 各 ≥5 successful call；coze 教师侧 target 5/5 收官
4. 预算 ≤22 caption calls + ≤1 smoke calls / ≤600s watchdog
5. 跑不完如实报断点

【夜间保守口径 / 断连处置】
--------------------------------
- PI 2026-09-25 00:46 拍板「今晚保守口径下先斩后奏」
- 背景：r4 前棒因 net::ERR_HTTP2_PING_FAILED 网络断连于勘察阶段中断（0 产物），r4 重派已验证 tun 预检有效
- 本棒 r5 沿 r4 保守处置：
  - 启动前必检 tun (127.0.0.1:1018) 端口可达；不通即停
  - 1-call smoke 失败立即 abort 不重探（沿 r4 口径）
  - 已落盘部分如实保留 + 报断点
  - 串行 ≥2.5s 不破；timeout/proxy/ssl/connection → 90s retry ≤3（沿 r4 + 修复版）
  - **建议沿用 `python -u > log 2>&1` 防 300s 自动截杀**（沿 r4 同会话验证）
...
---------------------------------------------------------
- 本棒前基线（沿 batch4 r4）：coze 教师侧 = 88 calls / 0 empty / 88 ok / 0.0000 rate
- 本棒 round 5（reask=4）后：22/22 caption 各 ≥5 successful call **收官**（理论值；如端点飘移可能回落）
- 终盘结果：result.json 顶层 `per_caption_successful_calls` 块 + `dispatch_target_progress` 块
- 收官判定块 `coze_teacher_side_finish`（沿 batch1 r6 惯例）

【retry 修复逻辑沿 batch4 r4 + batch4 r3 + batch2 r3 + batch1 r6】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline（沿 batch4 r4）：batch1 119 + batch2 22+22+22 + batch4 r1 22 + batch4 r2 22 + batch4 r3 22 + batch4 r4 22
  → **273 calls / 9 empty / 3.30%**（沿 batch4 r4 result `41e0ba55e205`）
- coze_teacher_cumulative 自身从 r4 终态 88/0/88 起步
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 单棒 >50% 停采：r5 单棒 empty_rate > 50% 也立即停采（沿 dispatch）

【端点取舍（沿 r4 + r3 + r2 + r1 + r4 + r3 + r2）】
----------------------------
- teamo + tun + deepseek-v4-flash（沿 r4 smoke ok=200 + r4 22 caption 全 OK + r2 models_probe 探活清单 sha12=`3c9dc60b5071`
  已确认 44 models 含 deepseek-v4-flash）
- teamo 必走 tun 防封号（PI 2026-09-23 硬纪律）

【铁律严守（沿 r4/r3/r2/r1/r4/r3/r2）】
------------------------
- R4 key 永不明文（无例外）；仅 runtime memory 读
- V1–V3 只读不动 / R5 V4 frozen append-only / R6 P-G 不动 / R7 plugin spec 不动
- 派生 JSON 不合并（本棒 `_batch4_r5_*` 与 `_batch4_r4_*` / `_batch4_r3_*` / `_batch4_r2_*` / `_batch4_r1_*` / `_batch2_r*_` / `_batch1_*` 独立）
...
- 不覆盖既有件（r4 executor `184186902A9F` + r4 result `41e0ba55e205` + r3 executor `b5e1c3637405` + r3 result `ed5267df7d6f`
  + r2 executor `7d8e12267790` + r2 result `6a9a362d9869` + r1 executor `e84bffd5dc7d` + r1 result `7d9fe9d319a2`
  + r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e` + r3 executor `72fd2bb3838e` + r3 result `25251aca6d79`
  + r2 executor `e7418f47a130` + r2 result `8c125e257af8` + r1 executor `e52f930654d3` + r1 result `d245ae4ce385`
  + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `a4f851154551` + mapping `98a779d61c1e`
  + r2 models_probe `3c9dc60b5071` + prereg `05b975a86989` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch4_r5_result.json`（schema = `v4_l14v3_n26/1` + batch=4 + round=5）
- 不覆盖 r4 result `41e0ba55e205` 或 r3 result `ed5267df7d6f` 或 r2 result `6a9a362d9869`
  或 r1 result `7d9fe9d319a2` 或 r4 result `15b8ddc5ba9e` 或 r3 result `25251aca6d79`
  或 batch1 r6 result `a4f851154551` 或 r1 result `d245ae4ce385`
- predecessor_sha12 链：mapping `98a779d61c1e` + r4 executor/result（最新接力锚）+ r3 executor/result
  + r2 executor/result + r1 executor/result + r4 executor/result + r3 executor/result + r2 executor/result
  + models_probe + batch1 r6 executor/result + prereg `05b975a86989`

【与 r4 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 5（升 r5 命名）；BATCH = 4 不变
2. prompt_id 前缀 `coze_t05_teacher_*`（取代 r4 的 t04）+ 后缀 `_b4_r5`（取代 r4 的 `_b4_r4`）
3. reask_idx = 4（本棒每位 caption 第 5 call；取代 r4 的 reask_idx=3）
4. phase_label：`b4_r5_finish`（round 5 目标 = ≥5 successful/caption，need_more_to_target = 0；**收官**）
5. is_round_5 = True（语义：本棒 = coze 教师侧 data collection round 5，区别于 r4 的 round 4）
6. smoke_call_only = True（沿 r4 smoke ok=200 + r4 22 caption 全 OK；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移）
7. predecessor_sha12 链：r4 executor `184186902A9F` + r4 result `41e0ba55E205`（新增；取代 r4 链的 r3 双件仍保留作为历史累计 baseline）
8. baseline prior：r4 终态 88 calls / 0 empty / 88 ok / 0.0000 rate（取代 r4 链的 66/66/0/0.0）
9. 跨批累计 baseline：r4 终态 273 calls / 9 empty / 3.30% rate（取代 r4 链的 251/9/3.59%）
10. 新增 `coze_teacher_side_round5_progress` 块：本棒后 coze 教师侧 round 5 完成度（≥5 successful/caption 收官）
11. **新增 `coze_teacher_side_finish` 收官判定块（沿 batch1 r6 惯例）**：
    - `all_22_captions_met_target` = 22/22 时 True；用于下游 verdict-keeper 触发判死线
    - `total_captions` = 22
    - `met_target_count` = 满足 ≥5 successful/caption 的 caption 数
    - `met_target_percentage` = 收官百分比
    - `rounds_used` = [r1, r2, r3, r4, r5]
    - `cumulative_calls_total` = 收官后 coze 自身累计 calls
    - `cumulative_empty_rate` = 收官后 coze 自身累计 empty_rate
    - `finish_status` = "complete" / "partial"（本棒若端点飘移未达 22/22 则记 partial）
12. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline 更新为 273/9/3.30%
13. spec_conformance 重写：coze 教师侧 round 5 **收官**（reask=4），目标 ≥5 successful/caption 收官
14. model_id_inconsistency_honest_disclosure 字面继承 r1/r2/r3/r4（沿 mapping 拍板代表采样字面）+ r1+r2+r3+r4+r5 累计补注
15. 派生 metadata 字段沿 r1/r2/r3/r4 不动（v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash）
16. per_caption_successful_calls 字段新增 round5_target=5 / round5_met
17. 调度注释：round 5 收官（reask=4）22 caption × 1 call
18. baseline 路径：B4_R4_RESULT_PATH（沿 r4 result `41e0ba55e205`）取代 r4 链的 B4_R3_RESULT_PATH
19. 启动前必检 tun 端口（PI 2026-09-25 夜间保守口径；防 net::ERR_HTTP2_PING_FAILED 重演）
20. is_round_4 替换为 is_round_5（语义升级）
21. failure result 路径同步 r5：_v4_supp_l14v3_batch4_r5_result.json（取代 r4 链的 r4 result）

【边界】
----------
- 不动任何既有件（含 r4 executor `184186902A9F` + r4 result `41e0ba55e205` + r3 executor `b5e1c3637405`
  + r3 result `ed5267df7d6f` + r2 executor `7d8e12267790` + r2 result `6a9a362d9869` + r1 executor `e84bffd5dc7d`
  + r1 result `7d9fe9d319a2` + r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e` + r3 executor `72fd2bb3838e`
  + r3 result `25251aca6d79` + r2 executor `e7418f47a130` + r2 result `8c125e257af8` + r1 executor `e52f930654d3`
  + r1 result `d245ae4ce385` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `a4f851154551`
...
KEY_INDEX_TEAMO_1BASED = 15  # 沿 r1/r2/r3/r4 (line 15 = teamo key)
USE_PROXY = True
...
TEACHER = "coze"
SIDE = "teacher"
REASK_R1 = 4  # 本棒 round 5 = 每 caption 第 5 call（reask_idx=4）**收官**
TARGET_SUCCESSFUL_PER_CAPTION = 5
SMOKE_CALL_ONLY = True  # 沿 r4 smoke ok=200 + r4 22 caption 全 OK；本棒仅 1-call smoke 防端点飘移

# 选定的 model_id（沿 mapping 拍板代表采样；r4 已 smoke ok=200 + 22 caption 全 OK）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch4-r5",
        "X-Title": f"deposon-l14v3-batch4-r5-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r4 简化版；仅 1 call 确认端点 ok）
# ============================================================
def smoke_coze_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r4 smoke ok=200 + r4 22 caption 全 OK 已知）；仅作端点可达性确认。"""
    t0 = time.time()
...
def run_one_quadruple(
    ...
    suffix = "_b4_r5"
    prompt_id = f"coze_t05_{side}_{caption_id}{suffix}"
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 4 round 5 · coze 教师侧 round 5（reask=4）收官")
    print("沿 r4 deepseek-v4-flash smoke ok=200 + r4 22 caption 全 OK; 本棒仅 1-call smoke 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r4 (沿 batch2 r3 + batch1 r6)")
    ...
    # 3. 加载前置累计基线（b4 r4 终态作为 coze 自身 baseline；cross batch 沿 b4r4 终态）
    with open(B4_R4_RESULT_PATH, "r", encoding="utf-8") as f:
        b4_r4 = json.load(f)
    b4_r4_cross_total = b4_r4["aggregate"]["cross_batch_with_b4r3_baseline"]["cross_total"]
    b4_r4_cross_empty = b4_r4["aggregate"]["cross_batch_with_b4r3_baseline"]["cross_empty"]
    b4_r4_cross_ok = b4_r4["aggregate"]["cross_batch_with_b4r3_baseline"]["cross_ok"]
    b4_r4_cross_rate = b4_r4["aggregate"]["cross_batch_with_b4r3_baseline"]["cross_rate"]
    print(
        f"[BASELINE b4r4 cross batch] total={b4_r4_cross_total} "
        f"ok={b4_r4_cross_ok} empty={b4_r4_cross_empty} rate={b4_r4_cross_rate:.4f}"
    )

    coze_prior_total = b4_r4["aggregate"]["coze_cumulative"]["post_total"]
    coze_prior_ok = b4_r4["aggregate"]["coze_cumulative"]["post_ok"]
    coze_prior_empty = b4_r4["aggregate"]["coze_cumulative"]["post_empty"]
    coze_prior_rate = b4_r4["aggregate"]["coze_cumulative"]["post_rate"]
    print(
        f"[BASELINE b4r4 coze cumulative] total={coze_prior_total} "
    ...
    # 5. Smoke 1-call（deepseek-v4-flash 端点可达性确认；沿 r4 smoke ok=200 + r4 22 caption 全 OK 已知）
    print("=" * 60)
    print("[SMOKE PHASE] deepseek-v4-flash 1-call smoke（沿 r4 smoke ok=200 + r4 22 caption 全 OK 已知）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": ROUND,
            "metadata": {
                "task": "L14V3_batch4_round5_coze_teacher_side_round5_reask4_finish",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch4_r5_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b4_r5_finish"} for c in captions_sorted]
    ...
    r4_per_cap = b4_r4.get("per_caption_successful_calls", {})
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_5=True,  # 语义：本棒 = coze 教师侧 data collection round 5 收官
    ...
    # 跨批累计 = b4r4 baseline + coze 本棒
    cross_batch_total = b4_r4_cross_total + r5_total
    cross_batch_empty = b4_r4_cross_empty + cum_empty - coze_prior_empty
    cross_batch_ok = b4_r4_cross_ok + cum_ok - coze_prior_ok
    ...
    for cid in [c["id"] for c in captions]:
        ...
        t = per_cap_total[cid]
        e = per_cap_empty[cid]
        met_r5 = s >= 5  # round 5 目标 = ≥5 successful/caption 收官
        met_target = s >= TARGET_SUCCESSFUL_PER_CAPTION  # TARGET = 5
        if met_r5:
            met_round5_count += 1
        if met_target:
            met_target_count += 1
    ...
    # 11. coze 教师侧 round 5 进度块
    coze_round5_progress = {
        "round5_target": ">=5 successful call/caption (round 5 reask=4 收官)",
    ...
    # 11b. **新增 coze_teacher_side_finish 收官判定块（沿 batch1 r6 惯例）**
    ...
    coze_teacher_side_finish = {
        ...
        "note": (
            f"沿 batch1 r6 惯例收官判定块；本棒 round 5 (reask=4) 完成后 "
    ...
    # model_id_inconsistency_honest_disclosure（沿 r1/r2/r3/r4 字面格式 + coze 不可考特别声明）
    ...
    model_id_disclosure = (
        ...
        f"r1 + r2 + r3 + r4 + r5 五棒累计 110 calls 0 empty 0 fail (沿 r4 终态 88 calls + 本棒增量), "
        f"model_returned 字面 = deepseek-v4-flash-ga-260731 (主) + deepseek-v4-flash-0731 (次), 均 family 下实例; "
        f"r5 沿 r4 结论仅 1-call smoke 防端点飘移; "
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-25",
            "spec_conformance": (
                "PI 2026-09-25 batch4 coze 教师侧 round 5 收官 (reask=4); "
                "沿 r4 deepseek-v4-flash smoke ok=200 + r4 22 caption 全 OK; r5 仅 1-call smoke; "
            ...
            "endpoint_selection_method": (
                ...
                "r5 沿 r4 smoke ok=200 + r4 22 caption 全 OK; r5 仅 1-call smoke 防端点飘移; "
            ...
            "tools_fault_param_correction": {
                ...
                "b4_r4_status": "r4 1-call smoke ok=200 + 22 caption × 1 call = 22 calls 全 OK; model_returned 字面 deepseek-v4-flash-ga-260731 (主) + deepseek-v4-flash-0731 (次)",
                "r5_b4_status": "r5 仅 1-call smoke (沿 r4 结论)；未做 3-candidate probe fallback",
                "rationale": (
                    "沿 r4 deepseek-v4-flash smoke ok=200 + r4 22 caption 全 OK; r5 节省预算避免重复 probe; "
            ...
            "diff_vs_r4_executor": [
                "1. ROUND=5 (升 r5 命名 收官)；BATCH=4 不变",
                "2. prompt_id 前缀 coze_t05_teacher_* (取代 r4 的 t04)；后缀 _b4_r5 (取代 r4 的 _b4_r4)",
                "3. reask_idx=4 (本棒每 caption 第 5 call；取代 r4 的 reask_idx=3)",
                "4. phase_label: b4_r5_finish (round 5 目标 = ≥5 successful/caption 收官，need_more_to_target=0)",
                "5. is_round_5=True (语义: coze 教师侧 data collection round 5 收官，区别于 r4 的 round 4)",
                "6. smoke_call_only=True (沿 r4 smoke ok=200 + r4 22 caption 全 OK；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移)",
                "7. predecessor_sha12 链: b4_r4_executor 184186902A9F + b4_r4_result 41e0ba55E205 (新增；取代 r4 链的 b4_r3/r2/r1/r4/r3/probe/batch1_r6/prereg 仍保留)",
                "8. baseline prior: r4 终态 88 calls / 0 empty / 88 ok / 0.0000 rate (取代 r4 链的 66/66/0/0.0)",
                "9. 跨批累计 baseline: r4 终态 273 calls / 9 empty / 3.30% rate (取代 r4 链的 251/9/3.59%)",
                "10. 新增 coze_teacher_side_round5_progress 块 (round 5 完成度 ≥5 successful/caption 收官)",
                "11. **新增 coze_teacher_side_finish 收官判定块 (沿 batch1 r6 惯例)** — all_22_captions_met_target / finish_status / cumulative_calls_total / cumulative_empty_rate / rounds_used / finish_verdict_trigger_for_verdict_keeper",
                "12. K-N26-N2 触发阈值 0.50 不变；监控 baseline 更新为 273/9/3.30%",
                "13. spec_conformance 重写: coze 教师侧 round 5 收官 (reask=4) + 夜间保守口径 + python -u 建议",
                "14. model_id_inconsistency_honest_disclosure 字面继承 r1/r2/r3/r4 (沿 mapping 拍板代表采样字面) + r1+r2+r3+r4+r5 累计补注",
                "15. 派生 metadata 字段沿 r1/r2/r3/r4 不动 (v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash)",
                "16. per_caption_successful_calls 字段新增 round5_target=5 / round5_met",
                "17. 调度注释: coze 教师侧 round 5 收官 (reask=4) 22 caption × 1 call",
                "18. baseline 路径: B4_R4_RESULT_PATH (沿 r4 result 41e0ba55e205) 取代 r4 链的 B4_R3_RESULT_PATH",
                "19. 启动前必检 tun 端口 (PI 2026-09-25 00:46 夜间保守口径；防 net::ERR_HTTP2_PING_FAILED 重演)",
                "20. is_round_4 替换为 is_round_5 (语义升级)",
                "21. failure result 路径同步 r5: _v4_supp_l14v3_batch4_r5_result.json (取代 r4 链的 r4 result)",
                "22. ROUNDS_USED = [r1, r2, r3, r4, r5] (收官判定准备)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
                ...
                "finish_block_present": True,
            },
            "run_window_cst": now_iso,
            "stop_reason": stop_reason,
            "interruption_recovery_note": (
                "r5 沿 r4 deepseek-v4-flash 代表采样结论 (sha12=41e0ba55e205 实测 22 calls 全 OK); "
                "不再做 3-candidate probe fallback; 1-call smoke 失败立即 abort; "
                "启动前必检 tun 端口 (PI 2026-09-25 夜间保守口径; 防 net::ERR_HTTP2_PING_FAILED 重演); "
                "r5 与 r4/r3/r2/r1/r4/r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并); "
                "coze 自身 baseline = 88/88/0/0.0 (沿 r4 终态); "
                "cross batch baseline = 273/264/9/3.30% (沿 r4 终态); "
                "本棒为 coze 教师侧 round 5 收官棒; 收官判定见 coze_teacher_side_finish 块"
            ),
        },
        "models_listing_summary_ref": {
            ...
            "note": (
                "r5 不重探 /v1/models; 仅引用 r2 探活清单 (sha12=3c9dc60b5071) "
        ...
        "coze_teacher_side_round5_progress": coze_round5_progress,
        ...
        "aggregate": {
            ...
            "cross_batch_with_b4r4_baseline": {
                "b4r4_baseline_total": b4_r4_cross_total,
                "b4r4_baseline_empty": b4_r4_cross_empty,
                "b4r4_baseline_ok": b4_r4_cross_ok,
                "b4r4_baseline_rate": b4_r4_cross_rate,
            ...
            "r5_empty_response_rate_threshold_K_N26_N2_single_batch": SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
            "r5_empty_response_above_threshold_single_batch": empty_rate_r5 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
            "single_batch_triggered_this_batch": single_batch_triggered,
        },
        "empty_rate_trend": {
            ...
            "cross_batch_cumulative_post_r2": {
                ...
                "ok": 220,
                "rate": round(9 / 229, 4),
                "note": "r2 终态 cross batch baseline (kimi + GLM_1 + coze r1+r2)",
            },
            "batch4_round3_cumulative_coze": {
                "calls": 66,
                "empty": 0,
                "ok": 66,
                "rate": 0.0,
                "note": "r3 终态 coze 自身累计",
            },
            "cross_batch_cumulative_post_r3": {
                ...
                "ok": 242,
                "rate": round(9 / 251, 4),
                "note": "r3 终态 cross batch baseline (kimi + GLM_1 + coze r1+r2+r3)",
            },
            "batch4_round4_cumulative_coze": {
                "calls": 88,
                "empty": 0,
                "ok": 88,
                "rate": 0.0,
                "note": "r4 终态 coze 自身累计",
            },
            "cross_batch_cumulative_post_r4": {
                "calls": 273,
                "empty": 9,
                "ok": 264,
                "rate": round(9 / 273, 4),
                "note": "r4 终态 cross batch baseline (kimi + GLM_1 + coze r1+r2+r3+r4)",
            },
            ...
            "cross_batch_cumulative_post_r5": {
                ...
                "note": "b4r4 baseline 273/9/3.30% + r5 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round5_reask4_n_distinct_sample_note": (
                    f"round 5 reask=4 第 5 call 实测；本棒后 coze 教师侧 round 5 = "
                    f"{met_round5_count}/{len(captions)} caption met round 5 目标 (收官)；"
                    f"r1+r2+r3+r4+r5 五棒累计 110 calls 收官"
                ),
            },
            "K_N26_N2_observation": {
                ...
                "single_batch_trigger_K_N26_N2_pass_False": empty_rate_r5 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
                ...
                "monitor_disposition": (
                    ...
                    f"baseline cross batch = {b4_r4_cross_total} calls / {b4_r4_cross_empty} empty / {b4_r4_cross_rate:.4f}"
                ),
                "cross_batch_with_b4r4_baseline_rate": round(cross_batch_rate, 4),
            },
            "verdict_pending": "5 教师批齐后由 verdict-keeper 统裁 (本棒 0 写 verdict; 收官判定见 coze_teacher_side_finish 块)",
        },
        "breakpoint_status": {
            ...
            "note": (
                ...
                f"coze 教师侧 round 5 完成度 = {met_round5_count}/{len(captions)} caption met round 5 目标 (≥5 successful/caption 收官); "
