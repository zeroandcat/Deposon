# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch9_r2_executor.py
====================================

V4 L14+ batch 9 · round 2（reask=1）—— coze 蒸馏侧接力棒（沿 r1 口径续跑）
==========================================================================
...
【本棒范围（沿 dispatch）—— coze 蒸馏侧 round 2 接力棒（reask=1）】
--------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 r1 / dispatch 字面）
2. **计数口径沿 r1**：
   - **distill 侧独立计数**：`coze_t01_distill_*` prompt_id 前缀为域（沿 r1 字面）
   - 目标 = coze 蒸馏侧 round 2 ≥2 successful call/caption（接力；非开工非收官）
   - 当前 coze 蒸馏侧 = 22 caption 各 1/5 successful（沿 r1 终态）
   - 本棒补 1 call/caption → coze 蒸馏侧 2/5 successful（**接力判定阈值**）
3. **model 口径沿 r1**：
...
4. **S6_n60 已知慢响应处理**（沿 r1 实测）：
   - r1 S6_n60 lat=52,106.6ms（~52s），retries=1，仍 OK → 1/1 successful
   - 本棒 r2 沿 r1 同 retry 修复路径消化（read_timeout_initial_s=60 / read_timeout_retry_s=90 / inner_retry_max=3）
   - 如 r2 S6_n60 仍空/失败 → 沿 dispatch 「断点 + 老实交代」如实记录，不擅自补 call
5. **接力判定块 `coze_distill_side_round2_progress`**：
   - `met_count_post_r2`：本棒后达标 caption 数（预期 22）
   - `still_pending_round2`：未达标 caption 列表（预期空）
   - `round2_progress_disposition`：coze 蒸馏侧 round 2 接力判定
   - **非收官棒**：本棒仅 round 2 接力 ≠ 收官；后续 r3/r4/r5 接力至 5/5 successful/caption

【每 caption successful 计数盘（沿 dispatch §6）—— coze 蒸馏侧独立域】
----------------------------------------------------------------------
- 本棒前基线（coze 蒸馏侧独立计数，沿 r1 quadruples 过滤）：22 caption 各 1/5 successful
- 本棒增量计算：每 call 后实时更新 `per_cap_succ_coze_distill` 字典
- 终盘结果（coze 蒸馏侧独立计数）：
  - `per_caption_coze_distill_only_successful_calls` 块（coze 蒸馏侧独立计数；含 r1 + r2）
  - `coze_distill_side_round2_progress` 块（**接力判定**）
  - `dispatch_target_progress_coze_distill` 块（coze 蒸馏侧目标进展 — round 2 接力态）
...
-------------------------------------------
- 累计 = batch9 r1 终态 + batch9 r2 coze 增量（coze 蒸馏侧 round 2）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 r1 产物）：
  - prior (b1+r2+r3+r4+r5+b6r1-r5+b7r1-r5+b8r1-r5+b9r1): total=340, empty=9, **rate=2.65%**
  - r2 worst case (22 EMPTY): cumulative = 31/362 = 8.56% (仍 < 50% 阈值)
  - r2 best case (0 EMPTY): cumulative = 9/362 = 2.49% (仍 < 50% 阈值)

【端点取舍（沿 r1 + batch4 r6）】
----------------------------
- teamo + tun + deepseek-v4-flash（coze 代表采样；沿 r1 22 caption 22 OK 实测）
...
- 派生 JSON 不合并（本棒 `_batch9_r2_*` 与 r1 + 前棒所有 _batch*_r* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch9_r2_result.json`
  （schema = v4_l14v3_n26/1 + batch=9 + round=2 + teacher=coze + side=distill）
- 不覆盖 r1 result `7e8dfe0156cd` 或前棒 result（沿 dispatch §4）

【与 r1 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. **ROUND = 2 / REASK_R1 = 1** 参数化（r1 = 1/0；本棒 = 2/1）
2. **prompt_id 后缀切换**：`_r1` → `_r2`（distill 侧 coze 域 round 2）
3. **baseline 切换**：
   - r1 终态：coze 蒸馏侧独立 = 22 caption 各 1/5 successful (88 calls 沿 r1 quadruples 累计 = 22 calls)
   - 跨批累计：r1 终态 cumulative (340/9/331/2.65%)
4. **target 切换**：≥1 successful/caption (r1 开工阈值) → ≥2 successful/caption (r2 接力阈值)
5. **判定块命名**：
   - `coze_distill_side_round1_progress` (r1 开工) → `coze_distill_side_round2_progress` (本棒接力)
   - `dispatch_target_progress_coze_distill` 块沿 r1 不变（仅更新 met_count_post_r2 + remaining_count_round2）
6. **non_finish_round 标记**：metadata.is_finish_round = False + is_round_2 = True (本棒 round 2 接力 ≠ 收官)
7. **predecessor_sha12 链新增**：r1 双件 (e78ca71bd077 / 7e8dfe0156cd)
8. **phase_label 切换**：`r1_coze_distill_*` → `r2_coze_distill_*`
9. **r2 接力判定字段新增**：rounds_to_reach_round2_target = 2; rounds_to_reach_final_target_actual 更新为 "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 coze 蒸馏侧 calls (沿 batch6 r5 收官类比)"

【边界】
----------
- 不动任何既有件（含 r1 双件 + batch8 r5/r4/r3/r2/r1 双件 + batch6 r5/r4/r3/r2/r1 双件 + batch4 r5/r6 双件
...
# ============================================================
# 端点配置（coze 代表采样 = teamo deepseek-v4-flash；沿 r1）
# ============================================================
...
# representative 标注（强制入 result metadata，沿 r1 字面）
...
SIDE = "distill"
TEACHER = "coze"
REASK_R1 = 1  # round 2 = 第 2 call per caption (reask_idx = 1) — 接力
TARGET_SUCCESSFUL_PER_CAPTION = 2  # coze 蒸馏侧 round 2 接力阈值（≥2 successful calls/caption）
TARGET_FINAL_PER_CAPTION = 5  # coze 蒸馏侧最终目标阈值（5 successful calls/caption；沿 batch6 r5 收官口径类比；后续 r3-r5 接力）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch9-r2",
        "X-Title": "deposon-l14v3-batch9-r2-coze-distill",
    }
    body = {
...
# ============================================================
# 提示构造（沿 r1；coze 代表采样 = 蒸馏 prompt 不变）
# ============================================================
PROMPT_SYSTEM = (
...
# ============================================================
# 单 call + 内层重试（沿 r1 + batch4 r6 representative 标注）
# ============================================================
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 9 round 2 | coze distill side | reask=1 — 接力棒")
    ...
    print(f"计数口径: coze 蒸馏侧独立 (prompt_id 前缀 coze_t01_distill_) — 沿 r1")
    print(f"coze 蒸馏侧 baseline (沿 r1 终态): 22 caption 各 1/5 successful")
    print(f"目标: coze 蒸馏侧 ≥2 successful calls/caption (round 2 接力阈值)")
    print(f"最终目标: coze 蒸馏侧 ≥5 successful calls/caption (后续 r3-r5 接力)")
    print(f"预算 ≤1500s watchdog")
    print(f"retry 修复沿 r1 (含 retry 触发的源 error_category tracking)")
    print(f"S6_n60 已知慢响应 (r1 lat=52s) 靠 retry 消化 (timeout_initial=60 / retry=90 / max=3)")
    ...
    # 2. 加载 r1 + batch8 r5 + batch6 r5 + batch4 r6 result（用于 baseline + cumulative + coze 代表采样字面）
    ...
    # 3. coze 蒸馏侧独立计数起点（filter by prompt_id 前缀 coze_t01_distill_）—— 沿 r1 终态
    ...
    # 从 r1 quadruples 累计（每 caption 1/5 successful；r1 全 ok 22/22，0 empty）
    r1_per_cap = b9r1.get("per_caption_coze_distill_only_successful_calls", {})
    for cid in [c["id"] for c in captions]:
        r1_info = r1_per_cap.get(cid, {})
        per_cap_succ_coze_distill[cid] = r1_info.get("succ_count", 0)
        per_cap_total_coze_distill[cid] = r1_info.get("total_calls_so_far", 0)
        per_cap_empty_coze_distill[cid] = r1_info.get("empty_count_so_far", 0)
    r1_distill_total = sum(per_cap_total_coze_distill.values())
    r1_distill_succ = sum(per_cap_succ_coze_distill.values())
    r1_distill_empty = sum(per_cap_empty_coze_distill.values())
    print(f"[COZE DISTILL r1 prior] total={r1_distill_total} succ={r1_distill_succ} empty={r1_distill_empty} (r1 quadruples filter)")

    # 4. 跨批累计监控（沿 r1 cumulative 链路 + coze 增量）
    r1_cum = b9r1["aggregate"]["cumulative"]
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = batch9 r1 aggregate.cumulative post)")

    # 5. 调度 planned_calls（22 caption × 1 call = 22 calls）
    PLANNED_CALLS = [
        {"caption_id": c["id"], "phase": f"r2_coze_distill_{c['id']}"}
        for c in captions
    ]
    ...
    # 10. 每 caption successful 计数 —— coze 蒸馏侧独立（含 r1 + r2 累计）
    ...
    # 11. coze 蒸馏侧 **round 2 接力判定**（沿 dispatch 字面块名 `coze_distill_side_round2_progress`）
    coze_distill_side_round2_progress = {
        "all_22_captions_met_round2_target": met_target_count_coze_distill == len(captions),
        "met_count_post_r2": met_target_count_coze_distill,
        "total_captions": len(captions),
        "still_pending_round2": sorted([
            cid for cid, info in per_caption_coze_distill_only_successful_calls.items() if not info["met_target_round2"]
        ]),
        "round2_progress_disposition": (
            f"coze 蒸馏侧 round 2 接力达成 → 22/22 caption ≥2 successful → 进入 round 3 接力（目标 ≥{TARGET_FINAL_PER_CAPTION} successful calls/caption 收官）"
            if met_target_count_coze_distill == len(captions)
            else f"coze 蒸馏侧 round 2 未达标 {len(captions) - met_target_count_coze_distill} caption → 留 worker 下一棒接力（未达接力阈值）"
        ),
        ...
        "coze_distill_side_round2_target_met_at_round": (
            "r2" if met_target_count_coze_distill == len(captions) else "未达 (next 棒接力)"
        ),
    }

    # 12. dispatch §6 收尾盘点 —— coze 蒸馏侧（含 r1 + r2 累计）
    dispatch_target_progress_coze_distill = {
        "target_round2": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (coze distill side only; round 2 接力阈值)",
        "target_final": f">={TARGET_FINAL_PER_CAPTION} successful calls/caption (coze distill side only; 最终收官阈值)",
        "met_count_post_r2": met_target_count_coze_distill,
        "total_captions": len(captions),
        "remaining_count_round2": len(captions) - met_target_count_coze_distill,
        ...
        "met_percentage_round2": round(met_target_count_coze_distill / len(captions) * 100, 2),
    ...
    # 13. coze 代表采样拍板字面块（沿 mapping §1.1 第三栏 + batch4 r6 字面 + r1 沿用）
    coze_representative_caliber = {
        ...
        "smoke_anchor_distill_side_r1": (
            "batch9 r1 沿 coze 蒸馏侧 deepseek-v4-flash 实测 (22 caption 22 OK + 0 empty + wall 328.1s)"
        ),
        "note": (
    ...
    retry_validation = {
        ...
        "s6_n60_special_note": (
            "S6_n60 已知慢响应 (r1 lat=52106.6ms); 本棒沿 r1 retry 路径消化 (timeout_initial=60 / retry=90 / max=3)"
            "；若 r2 仍空则如实记录不补 call"
        ),
    }
    ...
    self_executor_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r2_executor.py")
    self_result_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch9_r2_result.json")
    ...
    result = {
        ...
        "round": ROUND,
        "metadata": {
            "task": "L14V3_batch9_round2_coze_distill_side_reask1_relay_round_independent_counting",
            ...
            "date": "2026-09-26",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 9 round 2 coze distill side reask=1 接力棒",
            "is_finish_round": False,
            "is_round_2_relay_round": True,
            ...
            "spec_conformance": (
                ...
                "代表采样 = deepseek-v4-flash (teamo; 沿 r1 22 caption 22 OK 实测) + "
                "retry 修复沿 r1 + 每 caption successful 计数盘 + "
                "K-N26-N2 累计监控 + coze 蒸馏侧独立计数（沿 r1 counting_scope_correction） + "
                "round 2 接力判定块 coze_distill_side_round2_progress + representative/substitute 标注 (沿 r1 字面)"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            ...
            "endpoint_selection_method": (
                ...
                "(沿 mapping §1.1 第三栏 PI 拍板 + r1 22 caption 22 OK + batch4 r6 smoke ok=200)"
            ),
            "model_id_sent": MODEL_COZE_REPRESENTATIVE,
            "representative_for_coze": REPRESENTATIVE_FOR_COZE,
            "substitute_origin": SUBSTITUTE_ORIGIN,
            "model_id_inconsistency_honest_disclosure": (
                ...
                "本棒选定 = deepseek-v4-flash (teamo 端点; 沿 r1 22 caption 22 OK 实测; "
            ...
            "tools_fault_param_correction": {
                ...
                "r2_status": "retry 修复沿 r1; 本棒仅按 22 caption × 1 call 续采; hard fail 保持 fail path 立即返回; S6_n60 已知慢响应靠 retry 消化",
                "rationale": (
                    "沿 r1 修复 + tracking 改进；本棒接力棒仅按 22 caption × 1 call 续采；"
            ...
            "diff_vs_r1_executor": [
                "1. ROUND=2/REASK_R1=1 参数化 (r1 = 1/0; 本棒 = 2/1)",
                "2. prompt_id 后缀切换: _r1 → _r2 (distill 侧 coze 域 round 2)",
                "3. phase_label 切换: r1_coze_distill_* → r2_coze_distill_*",
                "4. is_round_1 → is_round_2 字段更新",
                "5. baseline 切换: 跨批累计 = r1 终态 cumulative (340/9/331/2.65%); coze 蒸馏侧独立 = 22 caption 各 1/5 successful (r1 quadruples 累计)",
                "6. target 切换: ≥1 successful/caption (r1 开工阈值) → ≥2 successful/caption (r2 接力阈值)",
                "7. 判定块命名: coze_distill_side_round1_progress (r1 开工) → coze_distill_side_round2_progress (本棒接力); dispatch_target_progress_coze_distill 块沿 r1 不变（仅更新 met_count_post_r2 + remaining_count_round2）",
                "8. non_finish_round 标记: metadata.is_finish_round = False + is_round_2_relay_round = True (本棒 round 2 接力 ≠ 收官)",
                "9. predecessor_sha12 链新增: r1 双件 (e78ca71bd077 / 7e8dfe0156cd)",
                "10. r2 接力判定字段新增: rounds_to_reach_round2_target = 2; rounds_to_reach_final_target_actual 沿 r1 沿用",
                "11. S6_n60 已知慢响应处理: r1 lat=52106.6ms retries=1; 本棒靠 retry 消化; 仍空则如实记录不补 call",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
        ...
        "coze_distill_side_round2_progress": coze_distill_side_round2_progress,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_2_n_distinct_sample_note": (
                    "round 2 接力棒第 2 call per caption 实测；coze 蒸馏侧独立计数累计 = "
                    f"22 caption 各 {r2_ok + r1_distill_succ // len(captions)} successful (r1+r2 quadruples 累计, prompt_id 前缀 coze_t01_distill_ filter 域)；"
                    f"**coze 蒸馏侧 round 2 接力判定**: {met_target_count_coze_distill}/{len(captions)} caption 达 ≥2 successful;"
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        f"{wall_time_s:.0f}s watchdog / 全 {r2_total} calls 完成 (round 2 接力态)"
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "本棒 round 2 接力棒已完成（22/22 calls）；coze 蒸馏侧 round 2 接力达成 → 进入 round 3 接力"
                if met_target_count_coze_distill == len(captions)
                else f"本棒 round 2 接力棒部分完成（{r2_total}/{planned_total} calls）；coze 蒸馏侧 round 2 未达标 {len(captions) - met_target_count_coze_distill} caption → 留 worker 下一棒接力"
            ),
            "note": (
                f"本棒 round 2 接力棒已完成（{r2_total}/{planned_total} calls）；coze 蒸馏侧 round 2 接力"
        ...
        "self_scan": {
            ...
            "hits_count": 0,
            "hits_detail": [],
            "note": "本 result 自扫 0 命中 (沿 r1 同口径); key 仅 runtime memory 读取不入产物",
        },
    }
    ...
    print(
        f"[SUMMARY r2] total={r2_total} ok={r2_ok} empty={r2_empty} fail={r2_fail} "
        f"empty_rate={empty_rate_r2:.4f} cum_post={cum_total}/{cum_empty}/{cum_empty_rate_post:.4f} "
        f"wall={wall_time_s}s"
    )
    print(
        f"[COZE DISTILL r2 progress] met={met_target_count_coze_distill}/{len(captions)} caption ≥2 successful; "
