# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch4_r1_executor.py
====================================

V4 L14+ batch 4 · round 1 · coze 教师侧 round 1 代表采样（reask=0）
===================================================================
...
- 派工单 coze 教师批 4 round 1：22 caption × 1 call（reask=0）

【本棒范围（沿 dispatch）—— coze 教师侧 round 1（reask=0）首采】
------------------------------------------------------------
1. **沿用 teamo `deepseek-v4-flash` 代表采样**（coze 底层 model 不可考；mapping 件拍板）
2. **22 caption × 1 call**（每位 caption 第 1 call；reask_idx=0；目标 round 1 完成度 = 1/5 successful/caption）
...
---------------------------------------------------------
- 本棒前基线（coze 教师侧）：**0 calls / 0 empty / 0 ok**（coze 教师首批采，无任何前置 baseline）
...
【retry 修复逻辑沿 batch2 r3 + batch1 r6】
--------------------------------------------
...
--------------------------------------------------------------------
- 跨批累计 baseline = batch1 终态 119 + batch2 r2 = 22 + batch2 r3 = 22 + batch2 r4 = 22
  → **185 calls / 9 empty / 4.86%**（沿 batch2 r4 result `15b8ddc5ba9e`）
- coze_teacher_cumulative 自身从 0 起步
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 单棒 >50% 停采：r1 单棒 empty_rate > 50% 也立即停采（沿 dispatch）

【端点取舍（沿 r2 + r3 + r4）】
----------------------------
- teamo + tun + deepseek-v4-flash（沿 r2 models_probe 探活清单 sha12=`3c9dc60b5071` 已确认 44 models 含 deepseek-v4-flash）
...
- 派生 JSON 不合并（本棒 `_batch4_r1_*` 与 `_batch2_r1-4_*` / `_batch1_*` 独立）
...
- 不覆盖既有件（r3 executor `72fd2bb3838e` + r3 result `25251aca6d79` + r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e` + mapping `98a779d61c1e` + r2 models_probe `3c9dc60b5071` + prereg `05b975a86989` 不动）

【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch4_r1_result.json`（schema = `v4_l14v3_n26/1` + batch=4 + round=1）
- 不覆盖 r4 result `15b8ddc5ba9e` 或 r3 result `25251aca6d79` 或 batch1 r6 result `a4f851154551` 或 r1 result `d245ae4ce385`
- predecessor_sha12 链：mapping `98a779d61c1e` + r4 executor/result + r3 executor/result（口径锚）+ r2 executor/result + models_probe + batch1 r6 executor/result + prereg `05b975a86989`

【与 r3 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------
1. ROUND = 1 / BATCH = 4（coze 教师首批；升 b4 命名避开 b2 连续续采重叠）
2. prompt_id 前缀 `coze_t01_teacher_*` + 后缀 `_b4_r1`；t01 = teacher side round 1（首次采）
3. TEACHER = "coze"（取代 r3/r4 的 GLM_1）
4. reask_idx = 0（本棒每位 caption 第 1 call；取代 r3 的 reask_idx=1 / r4 的 reask_idx=2）
5. phase_label：`b4_r1_need4`（round 1 目标 = ≥1 successful/caption，need_more_to_target = 4）
6. is_round_1 = True（语义：本棒 = coze 教师侧 data collection round 1）
7. smoke_call_only = True（沿 r2 probe=r3 smoke ok=200 + r4 同模式；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移）
8. MODEL_COZE = "deepseek-v4-flash"（取代 r3/r4 的 glm-5.3；代表采样沿 mapping 拍板）
9. metadata 新增 `representative_for_coze: true` + `substitute_origin: "coze_agent_platform_underlying_model_not_documented"`（强制标注）
10. predecessor_sha12 链：mapping `98a779d61c1e`（coze 拍板源） + b2r4 双件 + b2r3 双件（口径锚）+ b2r2 + models_probe + batch1 r6 + prereg
11. baseline prior：coze 自身 0/0/0 + cross batch 185/9/4.86%（沿 b2r4 终态）
12. 新增 `coze_teacher_side_round1_progress` 块：本棒后 coze 教师侧 round 1 完成度（≥1 successful/caption）
13. K-N26-N2 触发阈值 = 0.50 不变；监控 baseline coze 自身 0 起步 / cross batch 185/9/4.86%
14. spec_conformance 重写：coze 教师侧 round 1 首采（reask=0），目标 ≥1 successful/caption；模型为 `deepseek-v4-flash` 代表采样
15. model_id_inconsistency_honest_disclosure 重写：coze 底层 model 不可考 + PI 拍板代表采样字面
16. 派生 metadata 字段新增 `v3_anchor_api_name: "coze"` / `v3_anchor_9backbone_name: null`（coze 不在 9-backbone）/ `v4_current_model_id_sent: "deepseek-v4-flash"`

【边界】
----------
- 不动任何既有件（含 r4 executor `d9fe7aa2d292` + r4 result `15b8ddc5ba9e` + r3 executor `72fd2bb3838e` + r3 result `25251aca6d79` + r2 executor `e7418f47a130` + r2 result `8c125e257af8` + r1 executor `e52f930654d3` + r1 result `d245ae4ce385` + batch1 r6 executor `5f02c7e0f094` + batch1 r6 result `a4f851154551` + models_probe `3c9dc60b5071` + mapping `98a779d61c1e` + prereg `05b975a86989`）
- 跑不完拆段报断点
...
KEY_INDEX_TEAMO_1BASED = 15  # 沿 r2/r3/r4 (line 15 = teamo key)
USE_PROXY = True
...
# 工具失灵族参数修正（沿 r2/r3/r4）
READ_TIMEOUT_INITIAL_S = 60
READ_TIMEOUT_RETRY_S = 90
INNER_RETRY_MAX = 3

# 网络异常 retry 类别（沿 r2/r3/r4 + dispatch §1 字面）
NETWORK_ERROR_CATEGORIES_RETRY = {"timeout", "proxy_error", "ssl_error", "connection_error"}
...
TEACHER = "coze"
SIDE = "teacher"
REASK_R1 = 0  # 本棒 round 1 = 每 caption 第 1 call（reask_idx=0）
TARGET_SUCCESSFUL_PER_CAPTION = 5
SMOKE_CALL_ONLY = True  # 沿 r2/r3/r4 同模式；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移

# 选定的 model_id（沿 mapping 拍板代表采样）
...
def call_chat_teamo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch4-r1",
        "X-Title": f"deposon-l14v3-batch4-r1-{TEACHER}-teacher",
    }
    body = {
        "model": model_id,
        "messages": messages,
...
# ============================================================
# Smoke（沿 r2/r3/r4 简化版；仅 1 call 确认端点 ok）
# ============================================================
def smoke_coze_model(api_key: str, model_id: str) -> Dict[str, Any]:
    """1-call smoke（沿 r2/r3/r4 probe 简化模式）；确认 deepseek-v4-flash 代表采样端点可达性。"""
    t0 = time.time()
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 4 round 1 · coze 教师侧 round 1（reask=0）代表采样首采")
    print("沿 model mapping 98a779d61c1e 拍板: deepseek-v4-flash 代表采样; coze 底层 model 不可考")
    print("本棒 smoke 1-call 后跑 22 caption × 1 call")
    ...
    print("retry 修复沿 r2/r3/r4 (沿 batch1 r6)")
    print("=" * 60)
    ...
    # 3. 加载前置累计基线（batch2 r4 终态作为 cross batch baseline；coze 自身 0 起步）
    with open(B2_R4_RESULT_PATH, "r", encoding="utf-8") as f:
        b2_r4 = json.load(f)
    b2_r4_cross_total = b2_r4["aggregate"]["cross_batch_with_r3_baseline"]["cross_total"]
    b2_r4_cross_empty = b2_r4["aggregate"]["cross_batch_with_r3_baseline"]["cross_empty"]
    b2_r4_cross_ok = b2_r4["aggregate"]["cross_batch_with_r3_baseline"]["cross_ok"]
    b2_r4_cross_rate = b2_r4["aggregate"]["cross_batch_with_r3_baseline"]["cross_rate"]
    print(
        f"[BASELINE b2r4 cross batch] total={b2_r4_cross_total} "
        f"ok={b2_r4_cross_ok} empty={b2_r4_cross_empty} rate={b2_r4_cross_rate:.4f}"
    )

    glm1_cum_post_total = b2_r4["aggregate"]["glm1_cumulative"]["post_total"]
    glm1_cum_post_ok = b2_r4["aggregate"]["glm1_cumulative"]["post_ok"]
    glm1_cum_post_empty = b2_r4["aggregate"]["glm1_cumulative"]["post_empty"]
    glm1_cum_post_rate = b2_r4["aggregate"]["glm1_cumulative"]["post_rate"]
    print(
        f"[BASELINE b2r4 GLM_1 cumulative] total={glm1_cum_post_total} "
        f"ok={glm1_cum_post_ok} empty={glm1_cum_post_empty} rate={glm1_cum_post_rate:.4f}"
    )
    ...
    coze_prior_ok = 0
    coze_prior_rate = 0.0
    print(f"[BASELINE coze_teacher] total={coze_prior_total} ok={coze_prior_ok} empty={coze_prior_empty} rate={coze_prior_rate:.4f} (coze 教师首批采)")
    ...
    # 5. Smoke 1-call（deepseek-v4-flash 端点可达性确认）
    print("=" * 60)
    print("[SMOKE PHASE] deepseek-v4-flash 1-call smoke（沿 r2 models_probe 确认在 44 models 清单）")
    print("=" * 60)
    ...
    if not (smoke_rec["ok"] and smoke_rec["status_code"] == 200 and smoke_rec["model_returned"].strip() != ""):
        ...
        fail_result = {
            ...
            "round": ROUND,
            "metadata": {
                "task": "L14V3_batch4_round1_coze_teacher_side_round1_reask0_representative_sampling",
        ...
        out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch4_r1_result.json")
    ...
    planned = [{"caption_id": c["id"], "phase": "b4_r1_need4"} for c in captions_sorted]
    ...
    for i, item in enumerate(planned):
        ...
        rec = run_one_quadruple(
            ...
            side="teacher",
            round_index=ROUND,
            is_round_1=True,  # 语义：本棒 = coze 教师侧 data collection round 1（首批采）
    ...
    # 跨批累计 = b2r4 baseline + coze 本棒
    cross_batch_total = b2_r4_cross_total + r1_total
    cross_batch_empty = b2_r4_cross_empty + cum_empty - coze_prior_empty
    cross_batch_ok = b2_r4_cross_ok + cum_ok - coze_prior_ok
    ...
    # 11. coze 教师侧 round 1 进度块
    coze_round1_progress = {
    ...
    # model_id_inconsistency_honest_disclosure（沿 r3 字面格式 + coze 不可考特别声明）
    ...
    result = {
        ...
        "metadata": {
            ...
            "date": "2026-09-24",
            "spec_conformance": (
                "PI 2026-09-24 batch4 coze 教师侧 round 1 首采 (reask=0); "
                "沿 mapping 98a779d61c1e 拍板 deepseek-v4-flash 代表采样; "
            ...
            "endpoint_selection_method": (
                ...
                "r1 仅 1-call smoke 防端点飘移; 44 models 清单字面含 deepseek-v4-flash"
            ),
            "model_id_sent": model_id,
            ...
            "tools_fault_param_correction": {
                ...
                "r2_b2_status": "r2 1-call probe ok=200 选定 glm-5.3 (kimi/GLM_1/GLM_2 沿用)",
                "r3_b2_status": "r3 仅 1-call smoke (沿 r2 结论)；未做 3-candidate probe fallback",
                "r4_b2_status": "r4 仅 1-call smoke (沿 r3 结论)；GLM_1 round 3 续采 ok=200",
                "r1_b4_status": "r1 仅 1-call smoke (沿 r2 models_probe 44 models 清单确认 deepseek-v4-flash 在清单); 未做 3-candidate probe fallback; 沿 mapping 拍板代表采样",
                "rationale": (
                    "沿 r2 models_probe sha12=3c9dc60b5071 已确认 deepseek-v4-flash 在 teamo 44 models 清单; "
                    "r1 节省预算避免重复 probe; smoke 失败立即 abort 不重探"
                ),
            },
            ...
            "diff_vs_r3_executor": [
                ...
                "2. prompt_id 前缀 coze_t01_teacher_* + 后缀 _b4_r1；t01 = teacher side round 1 (首次采)",
                "3. TEACHER = coze (取代 r3/r4 的 GLM_1)",
                "4. reask_idx=0 (本棒每 caption 第 1 call；取代 r3 的 reask_idx=1 / r4 的 reask_idx=2)",
                "5. phase_label: b4_r1_need4 (round 1 目标 = ≥1 successful/caption，need_more_to_target=4)",
                "6. is_round_1=True (语义: coze 教师侧 data collection round 1 首批采)",
                "7. smoke_call_only=True (沿 r3/r4 同模式；本棒对 deepseek-v4-flash 1-call smoke 防端点飘移)",
                "8. MODEL_COZE_REPRESENTATIVE = deepseek-v4-flash (取代 r3/r4 的 glm-5.3；代表采样沿 mapping 拍板)",
                "9. metadata 新增 representative_for_coze: true + substitute_origin: coze_agent_platform_underlying_model_not_documented (强制标注)",
                "10. predecessor_sha12 链: mapping 98a779d61c1e (coze 拍板源) + b2r4 双件 + b2r3 双件 (口径锚) + b2r2 + models_probe + batch1 r6 + prereg",
                "11. baseline prior: coze 自身 0/0/0 (首批采) + cross batch 185/9/4.86% (沿 b2r4 终态)",
                "12. 新增 coze_teacher_side_round1_progress 块 (round 1 完成度 ≥1 successful/caption)",
                "13. K-N26-N2 触发阈值 0.50 不变；监控 baseline coze 自身 0 起步 / cross batch 185/9/4.86%",
                "14. 新增单棒 >50% 停采监控 (single_batch_empty_rate_threshold=0.50)",
                "15. spec_conformance 重写: coze 教师侧 round 1 首采 (reask=0)，代表采样 = deepseek-v4-flash",
                "16. model_id_inconsistency_honest_disclosure 重写: coze 底层 model 不可考 + PI 拍板代表采样字面",
                "17. 派生 metadata 字段新增 v3_anchor_api_name=coze / v3_anchor_9backbone_name=null / v4_current_model_id_sent=deepseek-v4-flash / v4_representative_not_identity=true",
                "18. 新增 mapping_decision_basis 块: mapping document sha12 + ask_id + ask_other_verbatim + ask_decision",
                "19. per_caption_successful_calls 字段新增 round1_target=1 / round1_met",
                "20. 调度注释: coze 教师侧 round 1 首采 (reask=0) 22 caption × 1 call",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
            ...
            "interruption_recovery_note": (
                "r1 沿 mapping 98a779d61c1e deepseek-v4-flash 代表采样结论; "
                "不再做 3-candidate probe fallback; 1-call smoke 失败立即 abort; "
                "r1 与 r4/r3/r2/r1/batch1_r6 派生 JSON 独立 (不合并); "
                "coze 自身 baseline = 0/0/0 (首批采，无任何前置 GLM_1/kimi 同 lineage 数据)"
            ),
        },
        ...
        "coze_teacher_side_round1_progress": coze_round1_progress,
        ...
        "aggregate": {
            ...
            "cross_batch_with_b2r4_baseline": {
                "b2r4_baseline_total": b2_r4_cross_total,
                "b2r4_baseline_empty": b2_r4_cross_empty,
                "b2r4_baseline_ok": b2_r4_cross_ok,
                "b2r4_baseline_rate": b2_r4_cross_rate,
            ...
            "r1_empty_response_rate_threshold_K_N26_N2_single_batch": SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
            "r1_empty_response_above_threshold_single_batch": empty_rate_r1 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
        ...
        "empty_rate_trend": {
            ...
            "cross_batch_cumulative_post_r1": {
                ...
                "note": "b2r4 baseline 185/9/4.86% + r1 增量",
            },
        },
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round1_reask0_n_distinct_sample_note": (
                    f"round 1 reask=0 第 1 call 实测；本棒后 coze 教师侧 round 1 = "
            ...
            "K_N26_N2_observation": {
                ...
                "single_batch_trigger_K_N26_N2_pass_False": empty_rate_r1 > SINGLE_BATCH_EMPTY_RATE_THRESHOLD,
                ...
                "monitor_disposition": (
                    ...
                    f"baseline cross batch = {b2_r4_cross_total} calls / {b2_r4_cross_empty} empty / {b2_r4_cross_rate:.4f}"
                ),
                "cross_batch_with_b2r4_baseline_rate": round(cross_batch_rate, 4),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                ...
                "本棒 round 1 (reask=0) 已完; 下棒可接力 coze 教师侧 round 2 (>=2 successful/caption)"
            ),
            "note": (
                f"本棒 round 1 (reask=0): actual={r1_total}/{planned_total} calls; "
                f"coze 教师侧 round 1 完成度 = {met_round1_count}/{len(captions)} caption met round 1 目标 (≥1 successful/caption); "
