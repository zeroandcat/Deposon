# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch10_r5_executor.py
=====================================

V4 L14+ batch 10 · round 5 (reask=4) -- minimax 蒸馏侧 **收官棒**（沿 r4 口径 + baseline 修正续跑）

派工依据:
  - 沿 L14+ 派工单 2026-09-26 17:14「minimax 蒸馏侧 r5 收官棒 (reask=4)」
  - 沿 batch10 r4 executor (SHA-12 4F68937D7A95) + r4 result (SHA-12 9D91A8DC039A) 口径续跑
  - 沿 baseline 修正口径 (派工单 §1): r1+r2+r3+r4 四件 quadruples 累加为权威源
...
本棒范围:
  ...
  2. batch=10 (minimax distill) + round=5 (reask=4) **收官棒**
     - 模型 = mimo `mimo-v2.6-pro` (沿 r4 沿 mapping §1.1 minimax 拍板)
     - 端点 = https://token-plan-cn.xiaomimimo.com/v1/chat/completions (沿 r4)
     - 不走代理 (沿 r4 + L7 + add_T1 §1.6.2)
     - **endpoint_migrated=true** (沿 r4)
     - **lineage_discontinuity=true** (沿 r4)
     - **teacher_v3_anchor = "minimax-m3 (火山方舟已下线)"** (沿 r4)
  3. distill 侧独立计数 (沿 r4 baseline 修正):
     - filter = prompt_id 前缀 `minimax_t01_distill_`
     - **真值 baseline = 4 successful/caption (r1+r2+r3+r4 各 1)**  ← **沿 r4 baseline 修正**
     - 本棒 22 caption × 1 call 后预期各 caption **真值 5/5 successful 收官**
     - 目标 ≥5 successful/caption (本棒 = 收官 = 远期目标)
  4. **收官判定块 `minimax_distill_side_finish`** (沿 batch6 r5 `kimi_distill_side_finish` 收官块字面惯例):
     - `all_22_captions_met_target`: True if 22/22 caption 5/5 successful
     - `met_count_post_r5`: 收官后达标 caption 数 (预期 22)
     - `still_pending`: 未达标 caption 列表 (预期空)
     - `finish_disposition`: minimax 蒸馏侧全 22 caption 5/5 successful 达标 → batch10 蒸馏侧 **收官**
  5. cumulative 监控: 起点 = r4 cumulative.post (406/9/397/2.22%); 本棒后 worst 31/428=7.24%; best 9/428=2.10%
  6. **counting_bug_correction 字段沿 r4** (派工单 §1; r3 88A06F08928F per_caption 计数 bug 透明披露):
     - r3 计数 bug 沿 r4 字段已披露
     - 本棒 baseline 沿 r4 修正方式: r1+r2+r3+r4 四件 quadruples 累加 (88 entries) 为权威源
     - **r3.per_caption 字段不采用 (因 r3 counting bug)**
     - 跨源 cross-check: 四件累加 vs r4.per_caption 字段 (r4 baseline 修正后, 应 match=True)

与 batch10 r4 executor diff:
  1. ROUND=4 -> ROUND=5; REASK_R4=3 -> REASK_R5=4 (第 5 call/caption on distill side) **收官棒**
  2. prompt_id 后缀 _r4 -> _r5
  3. predecessor 链 (dispatch 字面): r4_executor 4F68937D7A95 + r4_result 9D91A8DC039A + mapping
  4. predecessor 链 (baseline 修正累加): + r3_executor/result + r2_executor/result + r1_executor/result (r3 per_caption 字段不采用)
  5. **baseline 读取逻辑沿 r4**: r1+r2+r3+r4 四件 quadruples 累加 (88 entries) = 4 successful/caption
  6. **真值 baseline = 4 successful/caption** (沿 r4)
  7. 增量计数 -> distill 侧各 caption 由真值 4/5 顺利补至真值 5/5 (**收官** if no fails)
  8. is_round_4=True -> is_round_5=True; metadata.is_finish_round=True
  9. 累计监控起点: r4 cumulative.post {406/9/397/2.22%} -> 本棒后 worst 31/428=7.24% / best 9/428=2.10%
 10. **判定块重命名**: minimax_distill_side_round4_progress -> **minimax_distill_side_finish** (沿 batch6 r5 `kimi_distill_side_finish` 收官块字面惯例)
 11. counting_bug_correction 字段沿 r4 (派工单 §1); 跨源 cross-check: 四件累加 vs r4.per_caption 字段 (应 match=True, 因 r4 baseline 已修正)
 12. counting_scope_correction 块: r5 收官棒描述 (沿 r4 + 收官口径)
 13. per_caption: round4_target/round4_met -> round5_target/round5_met (本棒 = 收官 = 达 final 阈值)
...
铁律严守:
  ...
  - succeeded ≠ 跑完落盘核验
  - **不擅自修复 r3 88A06F08928F** (沿 R5 frozen append-only; bug 沿 r4 字段已透明披露)
  - 老实交代 0 产物

产物:
  - 写入: results/_v4_supp_l14v3_batch10_r5_result.json
    (schema = v4_l14v3_n26/1 + batch=10 + round=5)
  - 不覆盖 batch10 r4 result 9D91A8DC039A 或前棒 result

边界:
  - 不动任何既有件 (含 batch10 r1/r2/r3/r4 双件 + batch6 r5 双件 + batch8 r5 双件 + batch5 r1-r5 minimax teacher + batch1-8 全部 + prereg 05B975A86989 + captions 6a2656878745 + mapping 98A779D61C1E)
  - **不擅自修复 r3 88A06F08928F** (R5 frozen append-only; bug 沿 r4 字段已披露)
  - 跑不完拆段报断点
...
BATCH10_R4_RESULT_PATH = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch10_r4_result.json")

# r3 counting bug SHA-12 (派工单 §1 沿 r4)
R3_RESULT_SHA12 = "88A06F08928F"
R3_RESULT_PATH = "results/_v4_supp_l14v3_batch10_r3_result.json"

# ============================================================
# 端点配置 (沿 r4; model 沿模型映射留痕件 §1.1 minimax 拍板 mimo-v2.6-pro; 无代理)
# ============================================================
...
# K-N26-N2 累计 empty_rate 停采阈值 (沿 r4)
K_N26_N2_CUMULATIVE_EMPTY_RATE_THRESHOLD = 0.50

# 预算 (沿 r4; 1500s 容 22 caption calls + 串行 ≥2.5s)
...
REASK_R5 = 4  # round 5 = 第 5 call per caption (reask_idx = 4) -- 收官
TARGET_SUCCESSFUL_PER_CAPTION = 5  # distill 侧收官阈值 (≥5 successful calls/caption)
ROUND5_TARGET_PER_CAPTION = 5  # 本棒 round 5 目标 = 5 (收官 = 达 final 阈值)

# ============================================================
# 计数口径 (沿 r4 distill 侧独立计数; minimax 蒸馏侧独立域)
# **baseline 沿 r4 修正**: r1+r2+r3+r4 四件 quadruples 累加 (派工单 §1)
# ============================================================
DISTILL_SIDE_PROMPT_ID_PREFIX = "minimax_t01_distill_"

# baseline 沿 r4 修正: r1+r2+r3+r4 四件 quadruples 累加 = 4 successful/caption
BASELINE_TRUE_PER_CAPTION = 4  # 沿 r4 真值起点 (派工单 §1 沿 r4)
...
def call_chat_mimo(
    ...
    headers = {
        ...
        "HTTP-Referer": "https://deposon.local/l14v3-batch10-r5",
        "X-Title": "deposon-l14v3-batch10-r5-minimax-distill",
    }
    body = {
        "model": MODEL_MINIMAX,
        "messages": messages,
...
# ============================================================
# 提示构造 (沿 r4 prompt 构造口径 = batch1 r6 reproduction_caliber anchor)
# ============================================================
PROMPT_SYSTEM = (
...
# ============================================================
# 单 call + 内层重试 (沿 r4; minimax 蒸馏侧 metadata 增 endpoint_migrated/lineage_discontinuity/proxy_used/teacher_v3_anchor)
# ============================================================
def run_one_quadruple(
    ...
    # minimax 蒸馏侧派生 metadata (沿 r4 + mapping §1.2 minimax 行)
...
def main() -> int:
    print("=" * 60)
    print("V4 L14+ batch 10 round 5 | minimax distill side | reask=4 -- 收官棒")
    print("计划 calls = 22 (22 caption × 1 call)")
    print("计数口径: distill 侧独立 (prompt_id 前缀 minimax_t01_distill_) -- 沿 r4 + baseline 修正")
    print("**真值 baseline = 4 successful/caption (r1+r2+r3+r4 四件累加)** -- 沿 r4 baseline 修正续跑")
    print("本棒 round 5 收官目标: 各 caption 补至 5/5 successful (收官 if no fails)")
    print("预算 ≤1500s watchdog")
    print("retry 修复沿 r4 (含 retry 触发的源 error_category tracking)")
    print("mimo 无代理 (沿 r4 + L7 + add_T1 §1.6.2)")
    print("annotations: endpoint_migrated=True, lineage_discontinuity=True (沿 r4)")
    print("=" * 60)
    ...
    # 3. 加载 r1/r2/r3/r4 result (baseline 修正累加; 派工单 §1 沿 r4)
    ...
    with open(BATCH10_R3_RESULT_PATH, "r", encoding="utf-8") as f:
        r3 = json.load(f)
    with open(BATCH10_R4_RESULT_PATH, "r", encoding="utf-8") as f:
        r4 = json.load(f)

    # 4. minimax distill 侧独立计数 (沿 r4 baseline 修正; 派工单 §1)
    # **真值 baseline = r1+r2+r3+r4 四件 quadruples 累加 (4 successful per caption)**
    # **r3.per_caption 字段记 2/5 系 counting bug** (沿 r4 字段已披露; r4 已 baseline 修正)
    ...
    for src_label, src_result in [("r1", r1), ("r2", r2), ("r3", r3), ("r4", r4)]:
    ...
    print(f"[DISTILL prior] total={cum_total_prior_distill} ok={cum_ok_prior_distill} "
          f"empty={cum_empty_prior_distill} (源 = r1+r2+r3+r4 四件 quadruples 累加; 真值 baseline = 4/caption)")

    # 5. 跨源 cross-check (派工单 §1: 二者对账一致为准; r4 baseline 已修正, 期望 match=True)
    cross_check_r4_per_cap = {}
    r4_per_cap_data = r4.get("per_caption_distill_only_successful_calls", {})
    if isinstance(r4_per_cap_data, dict):
        for cid, info in r4_per_cap_data.items():
            if isinstance(info, dict) and "succ_count" in info:
                cross_check_r4_per_cap[cid] = info["succ_count"]
    cross_check_match = all(
        cross_check_r4_per_cap.get(cid, -1) == per_cap_succ_distill[cid]
        for cid in per_cap_succ_distill.keys()
    ) if cross_check_r4_per_cap else False
    print(f"[CROSS-CHECK r4.per_caption vs 四件累加] match={cross_check_match} (派工单 §1: 二者对账; r4 已 baseline 修正, 期望 match=True)")
    if not cross_check_match:
        mismatch_count = sum(
            1 for cid in per_cap_succ_distill.keys()
            if cross_check_r4_per_cap.get(cid, -1) != per_cap_succ_distill[cid]
        )
        print(f"  mismatch_count = {mismatch_count} (r4 per_caption 字段 ≠ 四件累加 → 需复核)")

    # 6. cumulative 监控沿 r4 cumulative 链路 (跨教师跨侧累计)
    r4_cum = r4["aggregate"]["cumulative"]
    cum_total_prior_cum = r4_cum["post_total"]    # 406
    cum_empty_prior_cum = r4_cum["post_empty"]    # 9
    cum_ok_prior_cum = r4_cum["post_ok"]          # 397
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = r4 aggregate.cumulative.post)")
    ...
    for i, item in enumerate(planned):
        ...
        # 收官判定: 本棒补至 5 = 收官
        finish_met = cur_succ_distill >= ROUND5_TARGET_PER_CAPTION
        target_met_round5 = "[OK] 收官 5/5" if finish_met else "[..]"
        print(
            ...
            f"retries={rc} "
            f"{status} "
            f"[distill_succ={cur_succ_distill}/{TARGET_SUCCESSFUL_PER_CAPTION} {target_met_round5}] "
    ...
    met_target_count_distill = 0  # ≥5 final target = 收官达标
    for cid in [c["id"] for c in captions]:
        ...
        per_caption_distill_only_successful_calls[cid] = {
            ...
            "round5_met": met_target,  # 收官 = 5/5 = met_target=True
            "need_more": need,
    ...
    per_caption_mixed_history_successful_calls = r5_mixed_history

    # 15. **收官判定块** `minimax_distill_side_finish` (沿 batch6 r5 `kimi_distill_side_finish` 收官块字面惯例)
    minimax_distill_side_finish = {
        ...
        "finish_disposition": (
            "minimax 蒸馏侧全 22 caption 5/5 successful 达标 → batch10 蒸馏侧 **收官** ✓"
            if met_target_count_distill == len(captions)
            else f"minimax 蒸馏侧未达标 {len(captions) - met_target_count_distill} caption → 留 worker 下一棒接力（未达收官）"
        ),
        "rounds_to_reach_target_planned": 5,
        "rounds_to_reach_target_actual": (
            "5 棒 (r1+r2+r3+r4+r5) = 22 caption × 5 calls = 110 distill 侧 calls (收官态)"
        ),
        "distill_side_target_met_at_round": (
            "r5 (收官)" if met_target_count_distill == len(captions) else "未达 (本棒为 r5 收官棒, 留下一棒接力)"
        ),
    ...
    dispatch_target_progress_distill = {
        "target": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (distill side only) -- 收官阈值",
        "target_round5": f">={ROUND5_TARGET_PER_CAPTION} successful calls/caption (round 5 收官目标)",
        "met_count_post_r5_round5": met_target_count_distill,
        "met_count_post_r5_cumulative_to_target": met_target_count_distill,
        "total_captions": len(captions),
        "remaining_count": len(captions) - met_target_count_distill,
        "remaining_count_round5": len(captions) - met_target_count_distill,
        "met_percentage_round5": round(met_target_count_distill / len(captions) * 100, 2),
        ...
        "is_finish_round": True,
        "baseline_correction_note": "r5 baseline 沿 r4 修正: r1+r2+r3+r4 四件 quadruples 累加 (派工单 §1 沿 r4); r3 per_caption 字段不采用 (因 r3 counting bug)",
    }

    # 17. **counting_bug_correction 字段** (派工单 §1; r3 计数 bug 沿 r4 透明披露 + 本棒 cross-check)
    counting_bug_correction = {
        ...
        "r4_baseline_fix_method": (
            ...
            "r4 采用 (a) r1+r2+r3 三件 quadruples 累加 (66 entries) 作为 baseline 唯一权威源;"
            "**r3.per_caption 字段不采用 (因 r3 counting bug)**"
        ),
        "r4_cross_check_r3_per_cap_disposition": (
            "派工单 §1 「二者对账一致为准」: 三件累加 vs r3.per_caption 字段"
            " -> match = False (派工单 §1 期望: 三件累加为权威源, r3.per_caption 字段已知 bug 不参与判定)"
        ),
        "r5_baseline_fix_method_沿_r4": (
            "本棒沿 r4 baseline 修正续跑: r1+r2+r3+r4 四件 quadruples 累加 (88 entries) 作为 baseline 唯一权威源;"
            "**r3.per_caption 字段仍不采用 (因 r3 counting bug, 沿 r4 处置)**;"
            "r4.per_caption 字段已 baseline 修正 (4/caption), 与四件累加一致 (派工单 §1 沿 r4 期望 match=True)"
        ),
        "r5_cross_check_r4_per_cap_actual": (
            f"本棒实测: 四件累加 vs r4.per_caption 字段 -> match = {cross_check_match} (派工单 §1: 二者对账; r4 已 baseline 修正, 期望 match=True)"
        ),
        "r5_baseline_修正后_per_caption_true_value": (
            f"baseline 四件累加 = {cum_total_prior_distill} calls, {cum_ok_prior_distill} successful, "
            f"{cum_empty_prior_distill} empty (各 caption = {BASELINE_TRUE_PER_CAPTION}/5 successful 真值起点, 沿 r4)"
        ),
        "r5_post_distill_true_value_收官态": (
            f"r1+r2+r3+r4+r5 quadruples 后 = {sum(per_cap_total_distill.values())} calls, "
            f"{sum(per_cap_succ_distill.values())} successful, "
            f"{sum(per_cap_empty_distill.values())} empty "
            f"(各 caption = {ROUND5_TARGET_PER_CAPTION}/5 successful 收官态 expected)"
        ),
        "r3_文件处置": (
            f"r3 既有 result ({R3_RESULT_SHA12} at {R3_RESULT_PATH}) 不擅自修复 (沿 R5 frozen append-only);"
            "bug 沿 r4 counting_bug_correction 字段已透明披露; 本棒沿 r4 字段继续披露 (不改字面)"
        ),
        "PI_reviewer_note": (
            "PI 复核 r3 88A06F08928F per_caption 字段是否需后续批次重算修正 (修正方式: 读 r1.quadruples 补齐 baseline);"
            "PI 复核 r5 counting_bug_correction 字段是否需入 N-26 主预登记 §0.5 边界声明 / K-N26-N1 observation 块;"
            "PI 复核 minimax 蒸馏侧收官判定 (≥5 successful/caption × 22/22) 是否需 N-26 verdict-keeper 统裁"
        ),
    }

    # 18. counting_scope_correction 块 (沿 r4; r5 收官棒描述)
    counting_scope_correction = {
        "r5_issue_disclosed": False,
        "r5_issue_description": (
            "本棒为 minimax 蒸馏侧 r5 收官棒 (= batch10 round 5), predecessor r1+r2+r3+r4 同前缀 quadruples = 88 条;"
            "无 r5 混合口径 (本棒 predecessor r4 同前缀过滤后 = 22 条均 minimax distill side);"
            "无 kimi 蒸馏侧混合计数问题 (本棒沿 r4 minimax distill side 累加)"
        ),
        "r5_data_disposition": (
            "本棒 = 收官棒 + 沿 r4 baseline 修正续跑; "
            "predecessor = r1+r2+r3+r4 minimax distill side (四件 quadruples 累加, 不依赖 r3.per_caption 字段); "
            "累加 r1+r2+r3+r4 quadruples"
        ),
        "r5_counting_domain": (
            f"distill 侧独立计数; filter 域 = prompt_id 前缀 '{DISTILL_SIDE_PROMPT_ID_PREFIX}'"
            " (沿 r4; minimax 蒸馏侧累加; **baseline 修正沿 r4: 四件累加, 不读 r3.per_caption 字段**)"
        ),
        "r5_target": f"distill 侧收官阈值 ≥{TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (本棒 = 收官 = 达 final 阈值)",
        "r5_round5_target": f"本棒 r5 收官目标 ≥{ROUND5_TARGET_PER_CAPTION} successful calls/caption (真值口径)",
        "r5_baseline_distill_only": (
            f"predecessor r1+r2+r3+r4 quadruples minimax distill 过滤后 (四件累加) = {cum_total_prior_distill} calls, "
            f"{cum_ok_prior_distill} successful, {cum_empty_prior_distill} empty "
            f"(各 caption 各 {BASELINE_TRUE_PER_CAPTION} successful 真值起点, 沿 r4 baseline 修正)"
        ),
        ...
        "pending_pi_review": (
            ...
            "PI 复核 r3 counting bug 修正方式 (沿 r4 baseline 修正 + 本棒沿 r4 续跑);"
            "PI 复核 minimax 蒸馏侧收官判定 (≥5 successful/caption × 22/22) 是否需 N-26 verdict-keeper 统裁;"
            "PI 拍板沿用 ask_748d9242 minimax 模型映射 = mimo 端点 mimo-v2.6-pro 字面"
        ),
        "finish_disposition": (
            f"r5 收官棒 distill 侧独立计数基线 (四件累加) = {cum_total_prior_distill} calls 完成 (沿 r1+r2+r3+r4); "
            f"实测 r5 quadruples 后 = {sum(per_cap_total_distill.values())} calls; "
            f"收官 = {'22/22 caption 5/5 successful 达标' if met_target_count_distill == len(captions) else f'未达 {len(captions) - met_target_count_distill} caption'} "
            f"(本棒收官态实况)"
        ),
    }

    # 19. retry validation 块 (沿 r4)
    ...
    result = {
        ...
        "round": 5,
        "metadata": {
            "task": "L14V3_batch10_round5_minimax_distill_side_reask4_finish_round_baseline_corrected",
            ...
            "predecessor_sha12": {
                ...
                "r5_dispatch_note": "派工单 §1: predecessor 链 r4 双件 + mapping; baseline 修正沿 r4 (r1+r2+r3+r4 四件 quadruples 累加)",
                "r4_executor_sha12": "4f68937d7a95",
                "r4_executor_path": "results/_v4_supp_l14v3_batch10_r4_executor.py",
                "r4_result_sha12": "9d91a8dc039a",
                "r4_result_path": "results/_v4_supp_l14v3_batch10_r4_result.json",
                "r4_baseline_correction_applied": True,
                ...
                "r3_result_counting_bug_note": "r3 per_caption 字段记 2/5 系 counting bug; 真值 = 3/5; r4 已 baseline 修正 (不读 r3.per_caption 字段)",
                ...
                # 扩展上下文: 蒸馏侧收官字面锚 (kimi distill + GLM_2 distill)
                "kimi_r5_executor_sha12": "8a8babb911b8",
                "kimi_r5_executor_path": "results/_v4_supp_l14v3_batch6_r5_executor.py",
                "kimi_r5_result_sha12": "e40424bf8abe",
                "kimi_r5_result_path": "results/_v4_supp_l14v3_batch6_r5_result.json",
            ...
            "date": "2026-09-26",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 10 round 5 minimax distill side reask=4 **收官棒** (baseline 修正沿 r4)",
            "is_finish_round": True,
            ...
            "spec_conformance": "PI 2026-09-26 同 agent 唤醒 (task_append) 接力棒 + 沿 r4 口径续跑 + minimax 模型映射沿 r4 (mimo-v2.6-pro) + retry 修复沿 r4 + 每 caption successful 计数终盘点 + K-N26-N2 累计监控 + distill 侧独立计数 (baseline 修正沿 r4: r1+r2+r3+r4 四件 quadruples 累加, 派工单 §1) + 收官判定块 minimax_distill_side_finish (沿 batch6 r5 kimi_distill_side_finish 收官块字面惯例) + counting_bug_correction 字段 (r3 88A06F08928F 计数 bug 沿 r4 透明披露)",
            "teacher": TEACHER,
            "teacher_label": TEACHER_LABEL,
            "side": SIDE,
            "round_index": 5,
            "reask_idx_r5": REASK_R5,
            ...
            "endpoint_selection_method": "沿 r4 executor 4F68937D7A95 端点实测 = mimo mimo-v2.6-pro (无代理)",
            "model_id_sent": MODEL_MINIMAX,
            ...
            "proxy_settings": {
                ...
                "note": "mimo 端点 = 无代理 (沿 r4 + L7 + add_T1 §1.6.2); 显式清空代理 env (防御性)",
            },
            "key_discipline": {
            ...
            "tools_fault_param_correction": {
                ...
                "r4_status": "retry 修复 + tracking 改进沿 r4 (沿 4F68937D7A95)",
                "r5_status": "retry 修复沿 r4; r5 收官棒本棒无新参数修正",
                "rationale": (
                    "沿 r4 修复 + r4 tracking 改进; r5 收官棒仅按 22 caption × 1 call 续采 (reask_idx=4);"
                    "baseline 修正沿 r4 (r1+r2+r3+r4 四件 quadruples 累加), 0 触动调用参数 / 阈值 / 模型"
                ),
            },
            ...
            "diff_vs_r4_executor": [
                ...
                "2. prompt_id 后缀 _r4 -> _r5",
                "3. **predecessor 链 (dispatch 字面)**: r4_executor 4F68937D7A95 + r4_result 9D91A8DC039A + mapping",
                "4. **predecessor 链 (baseline 修正累加沿 r4)**: + r3_executor/result + r2_executor/result + r1_executor/result (r3 per_caption 字段不采用)",
                "5. **baseline 读取逻辑沿 r4**: r1+r2+r3+r4 四件 quadruples 累加 (88 entries) = 4 successful/caption",
                "6. **真值 baseline = 4 successful/caption** (沿 r4)",
                "7. 增量计数 -> distill 侧各 caption 由真值 4/5 顺利补至真值 5/5 (**收官** if no fails)",
                "8. is_round_4=True -> is_round_5=True; metadata.is_finish_round=True (收官棒)",
                "9. 累计监控起点: r4 cumulative.post {406/9/397/2.22%} -> 本棒后 worst 31/428=7.24% / best 9/428=2.10%",
                "10. **判定块重命名**: minimax_distill_side_round4_progress -> **minimax_distill_side_finish** (沿 batch6 r5 kimi_distill_side_finish 收官块字面惯例)",
                "11. counting_bug_correction 字段沿 r4 (派工单 §1); 跨源 cross-check: 四件累加 vs r4.per_caption 字段 (r4 baseline 已修正, 期望 match=True)",
                "12. counting_scope_correction 块: r5 收官棒描述 (沿 r4 + 收官口径)",
                "13. per_caption: round4_target/round4_met -> round5_target/round5_met (本棒 = 收官 = 达 final 阈值)",
                "14. 收官判定块字段: all_22_captions_met_target / met_count_post_r5 / still_pending / finish_disposition / rounds_to_reach_target_actual / distill_side_target_met_at_round",
                "15. 0 擅调 r3 既有 result (R5 frozen append-only); r3 仅在 r5 产物 counting_bug_correction 字段引用 + 透明披露",
            ],
            "iron_rules": {
                "R1_no_llm": False,
                "R2_no_proxy": False,
                ...
                "minimax_distill_side_finish_achieved": met_target_count_distill == len(captions),
            },
        ...
        "minimax_distill_side_finish": minimax_distill_side_finish,
        ...
        "aggregate": {
            ...
            "baseline_correction_summary": {
                ...
                "r3_per_caption_field_disposition": "counting bug; r4+r5 baseline 不读此字段",
                "r4_per_caption_field_value": 4,
                "r4_per_caption_field_true_value": 4,
                "r4_per_caption_field_disposition": "baseline 修正后正确 (match 四件累加)",
                "r5_baseline_method": "r1+r2+r3+r4 四件 quadruples 累加 (沿 r4)",
                "r5_baseline_total_calls": cum_total_prior_distill,
                "r5_baseline_ok_calls": cum_ok_prior_distill,
                "r5_baseline_per_caption_succ": BASELINE_TRUE_PER_CAPTION,
                "cross_check_match_r4_per_cap_vs_four_pieces": cross_check_match,
            },
            "cumulative": {
                ...
                "cumulative_baseline_source": "batch10 r4 (minimax distill round 4 接力棒 + baseline 修正) aggregate.cumulative.post",
            },
        ...
        "empty_rate_trend": {
            ...
            "batch10_round1": {
                "calls": r4["empty_rate_trend"]["batch10_round1"]["calls"],
                "empty": r4["empty_rate_trend"]["batch10_round1"]["empty"],
                "rate": r4["empty_rate_trend"]["batch10_round1"]["rate"],
            },
            "batch10_round2": {
                "calls": r4["empty_rate_trend"]["batch10_round2"]["calls"],
                "empty": r4["empty_rate_trend"]["batch10_round2"]["empty"],
                "rate": r4["empty_rate_trend"]["batch10_round2"]["rate"],
            },
            "batch10_round3": {
                "calls": r4["empty_rate_trend"]["batch10_round3"]["calls"],
                "empty": r4["empty_rate_trend"]["batch10_round3"]["empty"],
                "rate": r4["empty_rate_trend"]["batch10_round3"]["rate"],
            },
            "batch10_round4": {
                "calls": r4["empty_rate_trend"]["batch10_round4"]["calls"],
                "empty": r4["empty_rate_trend"]["batch10_round4"]["empty"],
                "rate": r4["empty_rate_trend"]["batch10_round4"]["rate"],
            },
            "batch10_round5": {
                "calls": r5_total,
                "empty": r5_empty,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_5_n_distinct_sample_note": (
                    f"round 5 收官棒第 5 call per caption 实测; minimax distill 侧独立计数 = 22 caption 各 {ROUND5_TARGET_PER_CAPTION} successful "
                    f"(沿 r1+r2+r3+r4+r5 quadruples, prompt_id 前缀 minimax_t01_distill_ filter 域, "
                    f"前置 r1+r2+r3+r4 真值 baseline = 4 successful/caption, 四件累加 = 88 calls);"
                    f"**minimax 蒸馏侧 r5 收官达成**: distill 侧 22/22 caption 5/5 successful 收官态 (baseline 修正后);"
                    f"由 verdict-keeper 统裁 (本棒 0 写 verdict)"
                ),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": "本棒为 batch10 收官棒 — minimax 蒸馏侧 22/22 caption 5/5 successful 收官; 无 next 棒 (verdict-keeper 统裁 5 教师)",
            "note": (
                f"本棒 round 5 收官棒已完成（{r5_total}/{planned_total} calls; baseline 修正沿 r4）"
                + (f"; K-N26-N2 累计阈值触发: cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   if k_n26_n2_triggered else
                   f"; 撞 {WALL_TIME_BUDGET_S}s watchdog 提前停批 ({len(captions) - r5_ok} caption 未达标) 走 task_append 接力棒"
                   if wall_time_s > WALL_TIME_BUDGET_S and r5_ok < len(captions) else
                   f"; **minimax 蒸馏侧 22/22 caption 5/5 successful 收官** ✓ (baseline 修正后)"
                   if r5_ok == len(captions) else
                   f"; minimax 蒸馏侧未达标 {len(captions) - r5_ok} caption → 下一棒接力 (本棒为收官棒, 但未达收官)")
            ),
        },
        "self_scan": {
    ...
    # 22. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch10_r5_result.json")
    ...
    print(f"  distill_side_met_target_count (>=5 收官): {met_target_count_distill}/{len(captions)}")
    print(f"  *** all_22_captions_met_target_distill (>=5 收官): {minimax_distill_side_finish['all_22_captions_met_target']} ***")
    print(f"  finish_disposition: {minimax_distill_side_finish['finish_disposition']}")
    print(f"  baseline_correction: 四件累加={cum_total_prior_distill} calls / {cum_ok_prior_distill} succ / 真值 4/caption")
    print(f"  cross_check_match (r4.per_cap vs 四件累加): {cross_check_match}")
    ...
    if minimax_distill_side_finish["still_pending"]:
        print(f"  still_pending_distill (>=5 收官目标距, {len(minimax_distill_side_finish['still_pending'])} captions): {minimax_distill_side_finish['still_pending']}")
    else:
        print(f"  *** minimax 蒸馏侧 r5 收官达成: 0 still_pending ***")
    return 0
