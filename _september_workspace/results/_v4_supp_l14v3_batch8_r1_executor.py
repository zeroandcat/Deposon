# -*- coding: utf-8 -*-
"""
_v4_supp_l14v3_batch8_r1_executor.py
=====================================

V4 L14+ batch 8 · round 1（reask=0）—— GLM_2 蒸馏侧开工棒（GLM_2 复用同 model 不同语料）
==================================================================================
...
【本棒范围（沿 dispatch）—— GLM_2 蒸馏侧 round 1 开工棒】
--------------------------------------------------------------------
1. **22 caption × 1 call** = 22 calls（沿 dispatch 字面）
2. **计数口径**：
   - **distill 侧独立计数**：`glm2_t01_distill_*` prompt_id 前缀为域（沿 batch6 r5 字面）
   - 目标 = GLM_2 蒸馏侧 round 1 ≥1 successful call/caption（开工棒；非收官）
   - 当前 GLM_2 蒸馏侧 = 0 calls（GLM_2 蒸馏侧 round 1 首次采）
   - 本棒补 1 call/caption → GLM_2 蒸馏侧 1/1 successful（**开工判定阈值**）
3. **model 口径**：
   - `model_id_sent` = `glm-5.3`（teamo 端点；与 GLM_1 现行采样复用）
   - `teacher_label` = `GLM_2`
   - `model_id_shared_with` = `GLM_1`（GLM_2 = 同 model 不同语料字面沿 mapping §1.2 派生字段）
4. **system prompt 微调口径标注沿用**（沿 batch6 r5 `PROMPT_SYSTEM` 字面不重写）：
...
5. **开工判定块 `glm2_distill_side_round1_progress`**：
   - `met_count_post_r1`：本棒后达标 caption 数（预期 22）
   - `still_pending`：未达标 caption 列表（预期空）
   - `round1_progress_disposition`：GLM_2 蒸馏侧 round 1 开工判定
...
----------------------------------------------------------------------
- 本棒前基线（GLM_2 蒸馏侧独立计数）：GLM_2 蒸馏侧 = 0 calls（GLM_2 蒸馏侧 round 1 首次采）
- 本棒增量计算：每 call 后实时更新 `per_cap_succ_glm2_distill` 字典
- 终盘结果（GLM_2 蒸馏侧独立计数）：
  - `per_caption_glm2_distill_only_successful_calls` 块（GLM_2 蒸馏侧独立计数）
  - `glm2_distill_side_round1_progress` 块（**开工判定**）
  - `dispatch_target_progress_glm2_distill` 块（GLM_2 蒸馏侧目标进展 — 开工态）
...
-------------------------------------------
- 累计 = batch6 r5 终态 + batch8 r1 GLM_2 增量（GLM_2 蒸馏侧 round 1）
- 触发 K-N26-N2 pass=False 条件：cumulative_empty_rate > 0.50 → 立即停采报告（不硬跑）
- 实测累计基线（沿 batch6 r5 产物）：
  - prior (b1+r2+r3+r4+r5+b6r1+r6r2+r6r3+r6r4+r6r5): total=208, empty=9, **rate=4.33%**
  - r1 worst case (22 EMPTY): cumulative = 31/230 = 13.48% (仍 < 50% 阈值)
  - r1 best case (0 EMPTY): cumulative = 9/230 = 3.91% (仍 < 50% 阈值)
...
- 派生 JSON 不合并（本棒 `_batch8_r1_*` 与前棒所有 _batch*_r* 同级独立）
...
【产物】
----------
- 写入：`results/_v4_supp_l14v3_batch8_r1_result.json`
  （schema = v4_l14v3_n26/1 + batch=8 + round=1 + teacher=GLM_2）
- 不覆盖 batch6 r5 result `e40424bf8abe` 或前棒 result（沿 dispatch §4）

【与 batch6 r5 executor diff（沿 dispatch §4 「若需改参数则新件 + 注明 diff」）】
-------------------------------------------------------------------------------
1. **BATCH = 8 / ROUND = 1 / REASK_R1 = 0** 参数化（batch6 r5 = 6/5/4；本棒 = 8/1/0）
2. **prompt_id 前缀切换**：`kimi_t01_distill_` → `glm2_t01_distill_`（distill 侧 GLM_2 域）
3. **teacher 切换**：kimi → GLM_2（side=distill；teacher_label=GLM_2）
4. **model 切换**：kimi-k3 → glm-5.3（GLM_2 复用 GLM_1；model_id_shared_with=GLM_1）
5. **GLM_2 派生字段新增**（沿 mapping §1.2 字面）：
...
6. **system prompt 沿用**：复用 GLM_1 现行 batch6 r5 `PROMPT_SYSTEM` 字面（不重写）
7. **predecessor_sha12 链**：batch6 r5 双件 + batch2 r2 GLM_1 双件 + mapping + prereg + captions
8. **baseline 切换**：
   - 跨批累计：batch6 r5 终态 cumulative (208/9/199/4.33%)
   - GLM_2 蒸馏侧独立：0 calls（GLM_2 蒸馏侧 round 1 首次采）
9. **target 切换**：≥5 successful/caption (batch6 r5 收官阈值) → ≥1 successful/caption (round 1 开工阈值)
10. **判定块命名**：
    - `kimi_distill_side_finish` (batch6 r5 收官) → `glm2_distill_side_round1_progress` (本棒开工)
    - `dispatch_target_progress_distill` → `dispatch_target_progress_glm2_distill`
...
TARGET_SUCCESSFUL_PER_CAPTION = 1  # GLM_2 蒸馏侧 round 1 开工阈值（≥1 successful calls/caption）
TARGET_FINAL_PER_CAPTION = 5  # GLM_2 蒸馏侧最终目标阈值（5 successful calls/caption；沿 batch6 r5 收官口径类比；后续 r2-r5 接力）
...
def main() -> int:
    print("=" * 60)
    print(f"V4 L14+ batch 8 round 1 | GLM_2 distill side | reask=0 — 开工棒")
    print(f"模型: glm-5.3 (teamo; GLM_2 复用 GLM_1 = 同 model 不同语料)")
    print(f"计划 calls = 22 (22 caption × 1 call)")
    print(f"计数口径: GLM_2 蒸馏侧独立 (prompt_id 前缀 glm2_t01_distill_) — 沿 batch6 r5")
    print(f"GLM_2 蒸馏侧 baseline: 0 calls (GLM_2 蒸馏侧 round 1 首次采)")
    print(f"目标: GLM_2 蒸馏侧 ≥1 successful calls/caption (round 1 开工阈值)")
    print(f"最终目标: GLM_2 蒸馏侧 ≥5 successful calls/caption (后续 r2-r5 接力)")
    ...
    # 注：batch6 r5 = kimi 蒸馏侧，无 GLM_2 distill quadruples；GLM_2 蒸馏侧起始 = 0
    print(f"[GLM_2 DISTILL prior] total=0 ok=0 empty=0 (GLM_2 蒸馏侧 round 1 首次采)")

    # 4. 跨批累计监控（沿 batch6 r5 cumulative 链路 + GLM_2 增量）
    b6r5_cum = b6r5["aggregate"]["cumulative"]
    cum_total_prior_cum = b6r5_cum["post_total"]  # 208
    cum_empty_prior_cum = b6r5_cum["post_empty"]  # 9
    cum_ok_prior_cum = b6r5_cum["post_ok"]        # 199
    ...
    print(f"[CUMULATIVE prior] total={cum_total_prior_cum} ok={cum_ok_prior_cum} "
          f"empty={cum_empty_prior_cum} rate={cum_rate_prior:.4f} (源 = batch6 r5 aggregate.cumulative post)")

    # 5. 调度 planned_calls（22 caption × 1 call = 22 calls）
    PLANNED_CALLS = [
        {"caption_id": c["id"], "phase": f"r1_glm2_distill_{c['id']}"}
        for c in captions
    ]
    ...
    for i, item in enumerate(planned):
        ...
        target_met_glm2_distill = "[OK] 开工" if cur_succ_glm2_distill >= TARGET_SUCCESSFUL_PER_CAPTION else "[..]"
        print(
    ...
    # 11. GLM_2 蒸馏侧 **开工判定**（沿 dispatch 字面块名 `glm2_distill_side_round1_progress`）
    glm2_distill_side_round1_progress = {
        "all_22_captions_met_round1_target": met_target_count_glm2_distill == len(captions),
        "met_count_post_r1": met_target_count_glm2_distill,
        "total_captions": len(captions),
        "still_pending_round1": sorted([
            cid for cid, info in per_caption_glm2_distill_only_successful_calls.items() if not info["met_target_round1"]
        ]),
        "round1_progress_disposition": (
            f"GLM_2 蒸馏侧 round 1 开工达成 → 22/22 caption ≥1 successful → 进入 round 2 接力（目标 ≥{TARGET_FINAL_PER_CAPTION} successful calls/caption 收官）"
            if met_target_count_glm2_distill == len(captions)
            else f"GLM_2 蒸馏侧 round 1 未达标 {len(captions) - met_target_count_glm2_distill} caption → 留 worker 下一棒接力（未达开工）"
        ),
        ...
        "glm2_distill_side_round1_target_met_at_round": (
            "r1" if met_target_count_glm2_distill == len(captions) else "未达 (next 棒接力)"
        ),
    }

    # 12. dispatch §6 收尾盘点 —— GLM_2 蒸馏侧
    dispatch_target_progress_glm2_distill = {
        "target_round1": f">={TARGET_SUCCESSFUL_PER_CAPTION} successful calls/caption (GLM_2 distill side only; round 1 开工阈值)",
        "target_final": f">={TARGET_FINAL_PER_CAPTION} successful calls/caption (GLM_2 distill side only; 最终收官阈值)",
        "met_count_post_r1": met_target_count_glm2_distill,
        "total_captions": len(captions),
        "remaining_count_round1": len(captions) - met_target_count_glm2_distill,
        ...
        "met_percentage_round1": round(met_target_count_glm2_distill / len(captions) * 100, 2),
    ...
    result = {
        ...
        "round": ROUND,
        "metadata": {
            "task": "L14V3_batch8_round1_GLM_2_distill_side_reask0_start_round_independent_counting",
            ...
            "date": "2026-09-25",
            "freeze_day": "V4 L14+ N-26 真审唯一路径 batch 8 round 1 GLM_2 distill side reask=0 开工棒",
            ...
            "prompt_construction_basis": {
                ...
                "reproduction_source_anchor_role": "GLM_2_复用同_distill_prompt（GLM_2 = GLM_1 同 model 不同语料；蒸馏 prompt 不变）",
            ...
            "spec_conformance": (
                ...
                "K-N26-N2 累计监控 + GLM_2 蒸馏侧独立计数（沿 batch6 r5 counting_scope_correction） + "
                "开工判定块 glm2_distill_side_round1_progress"
            ),
            "teacher": TEACHER,
            "side": SIDE,
            "round_index": ROUND,
            ...
            "endpoint_selection_method": (
                "GLM_2 复用 GLM_1 现行采样 = teamo glm-5.3 (沿 batch2 r2 5be9dfe83a27 实测 22 calls + "
            ...
            "diff_vs_batch6_r5_executor": [
                ...
                "3. teacher 切换: kimi → GLM_2 (side=distill; teacher_label=GLM_2)",
                "4. model 切换: kimi-k3 → glm-5.3 (GLM_2 复用 GLM_1; model_id_shared_with=GLM_1)",
                "5. GLM_2 派生字段新增 (沿 mapping §1.2 字面): v3_anchor_api_name='GLM_2' / v3_anchor_9backbone_name='glm-5.3-flash' / v4_current_model_id_sent='glm-5.3（复用 GLM_1）' / v4_current_model_returned='glm-5.3' / reuse_from_teacher='GLM_1'",
                "6. system prompt 沿用: 复用 GLM_1 现行 batch6 r5 PROMPT_SYSTEM 字面（不重写）",
                "7. predecessor_sha12 链: batch6 r5 双件 + batch2 r2 GLM_1 双件 + mapping + prereg + captions",
                "8. baseline 切换: 跨批累计 = batch6 r5 终态 cumulative (208/9/199/4.33%); GLM_2 蒸馏侧独立 = 0 calls (GLM_2 蒸馏侧 round 1 首次采)",
                "9. target 切换: ≥5 successful/caption (batch6 r5 收官阈值) → ≥1 successful/caption (round 1 开工阈值)",
                "10. 判定块命名: kimi_distill_side_finish (batch6 r5 收官) → glm2_distill_side_round1_progress (本棒开工); dispatch_target_progress_distill → dispatch_target_progress_glm2_distill",
                "11. non_finish_round 标记: metadata.is_finish_round = False (本棒仅 round 1 开工 ≠ 收官)",
            ],
            "iron_rules": {
                "R1_no_llm": False,
        ...
        "glm2_distill_side_round1_progress": glm2_distill_side_round1_progress,
        ...
        "K_N26_observability_only": {
            ...
            "K_N26_N1_observation": {
                "round_1_n_distinct_sample_note": (
                    "round 1 开工棒第 1 call per caption 实测；GLM_2 蒸馏侧独立计数 = "
                    f"22 caption 各 {r1_ok} successful (沿本棒 quadruples, prompt_id 前缀 glm2_t01_distill_ filter 域)；"
                    f"**GLM_2 蒸馏侧 round 1 开工判定**: {met_target_count_glm2_distill}/{len(captions)} caption 达 ≥1 successful;"
            ...
            "K_N26_N2_observation": {
                ...
                "monitor_disposition": (
                    ...
                    + (
                        ...
                        "本棒未触发 K-N26-N2 累计阈值；跑至 1500s watchdog 提前停批 / 全 22 calls 完成 (开工态)"
                    )
                ),
            },
        ...
        "breakpoint_status": {
            ...
            "next_resume_via": (
                "task_append 接力棒至 GLM_2 蒸馏侧 round 2 (>=4 more successful/caption); "
                "沿 prereg §1.6.4 同 agent 唤醒"
            ),
            "note": (
                f"本棒 round 1 开工棒已完成（{r1_total}/{planned_total} calls）"
                + (f"；K-N26-N2 累计阈值触发：cum_rate={round(k_n26_n2_trigger_cum_rate,4) if k_n26_n2_trigger_cum_rate else None} > 0.50, 立即停采 (沿 dispatch §4)"
                   if k_n26_n2_triggered else
                   f"；撞 {WALL_TIME_BUDGET_S}s watchdog 提前停批 ({len(captions) - met_target_count_glm2_distill} caption 未达 round 1 开工阈值) 走 task_append 接力棒"
                   if wall_time_s > WALL_TIME_BUDGET_S and met_target_count_glm2_distill < len(captions) else
                   f"；**GLM_2 蒸馏侧 round 1 开工达成** {met_target_count_glm2_distill}/{len(captions)} caption ≥1 successful ✓"
                   if met_target_count_glm2_distill == len(captions) else
                   f"；GLM_2 蒸馏侧 round 1 未达标 {len(captions) - met_target_count_glm2_distill} caption → 下一棒接力 (未达开工)")
            ),
        },
        "self_scan": {
    ...
    # 17. 落盘
    out_path = os.path.join(RESULTS_DIR, "_v4_supp_l14v3_batch8_r1_result.json")
    ...
    print(f"  glm2_distill_side_round1_met: {met_target_count_glm2_distill}/{len(captions)}")
    print(f"  *** all_22_captions_met_round1_target: {glm2_distill_side_round1_progress['all_22_captions_met_round1_target']} ***")
    print(f"  round1_progress_disposition: {glm2_distill_side_round1_progress['round1_progress_disposition']}")
    ...
    if glm2_distill_side_round1_progress["still_pending_round1"]:
        print(f"  still_pending_round1: {glm2_distill_side_round1_progress['still_pending_round1']}")
    return 0
