...
| codex（D1 review） | `11-12-20-778-asset_20260921-111220-778_48387412b109_7ca46e76-v4_d1_review_codex_2026_09_20.md` | `48387412B109` | `48387412B109` | 4,506 / 46 |
| codex（brainstorm，14 条） | `11-12-20-782-asset_20260921-111220-782_0ff6c042b845_6d74a672-v4_distillation_brainstorm_reply_codex_2026_09_21.md` | `0FF6C042B845` | `0FF6C042B845` | 9,048 / 39 |
| 归属未定（主件） | `11-12-20-756-asset_20260921-111220-756_c78cfb42e33d_e371e458-_v4_distillation_theme_reply_2026_09_20_v1.0.md` | `C78CFB42E33D` | `C78CFB42E33D` | 18,841 / 89 |
| 归属未定（addendum） | `11-12-20-761-asset_20260921-111220-761_8295c3201f20_ee5ad31d-_v4_distillation_theme_reply_addendum_2026_09_20_v1.0.md` | `8295C3201F20` | `8295C3201F20` | 14,153 / 91 |
...
上述 16 个文件全部用 `ReadAllLines`（UTF-8）与 `Get-Content -LiteralPath ... | Measure-Object -Line` 双口径独立核验；`ReadAllLines` 计数一律大于 `Measure-Object` 计数（差值约为每文件末尾缺省换行符一行）。本稿引用行号时一律按 `ReadAllLines` 口径。

### §0.5 已知锚与本轮自测的差异

| 文件 | PI 派工单行数 | 本轮行数 | 差异原因 |
|---|---|---|---|
| `_v4_experiment_invitation_2026_09_20.md` | 436 | 436 | 一致 |

其余 15 件的 SHA-12 / 字节数 / 行数均与派工单 §1 与 §2 完全一致。`Mavis v1.1` 与 `Mavis v1.2` 的字节差 2 是派工单未自测部分；本轮实测 v1.1 = 36,824 / v1.2 = 36,826，差 2 字节，v1.1 → v1.2 有微小正文修订。

### §0.6 归属未定的处理

派工单 §3 第 7 条禁止将附件侧 `756` 与 `761` 两件擅自归到任何厂商名下；`756` 在磁盘 L3 自报 `From: an invited reader team (multi-agent)`、`761` 在磁盘 L3 同样自报 `From: the same invited reader team (multi-agent)`。本稿将两件并称"an invited reader team (multi-agent)"，并列入下文的"归属未定"分节，不做厂商归并。

### §0.7 本稿（输出文件）自身锚的处置

本稿 §0.1–§0.6 给出的 SHA-12 / 字节 / 行数是**输入侧 16 文件**的实算；本稿自身的锚仅在本会话最终回报中给出（避免 self-referential hash 反复自修改的循环）。本稿在以下事实下保留输入侧自测 + 不试图自洽输出侧锚的策略：

- 本稿不对外引用任何 verdict token（PASS / FAIL / PARTIAL / GRAY / STRONG_PASS / DEAD / TRIGGERED / NOISE 等）；凡上述 token 出现，皆附 `path:SHA-12:line` 引文标注。
- 本稿行数双口径核验已记录在 §0.4；本稿不重新复算自身行数（理由同上）。
...
- `an invited reader team (multi-agent)` 主件 `assets/11-12-20-756-asset...C78CFB42E33D:18` 同一条。
- `豆包工作` 主件 `assets/11-12-20-747-asset...19C97B5CFF3C:11` 同一条。

九件独立同向；这是本主题中观察人最多的单一批评。本稿在第 §0.5 标注与已知锚一致。
...
- `an invited reader team (multi-agent)` 主件 `assets/11-12-20-756-asset...C78CFB42E33D:19` 同向。
...
- `an invited reader team (multi-agent)` 主件 `assets/11-12-20-756-asset...C78CFB42E33D:19` 与 `...C78CFB42E33D:27` 同向。
...
- `an invited reader team (multi-agent)` 主件 `assets/11-12-20-756-asset...C78CFB42E33D:20` 同向。
...
- `an invited reader team (multi-agent)` 主件 `assets/11-12-20-756-asset...C78CFB42E33D:21` 实测 byte-identical under full SHA-256。
...
- **第三方立场**（`an invited reader team (multi-agent)` 主件 `D36`）： `assets/11-12-20-756-asset...C78CFB42E33D:29` 接受 §2.1 的"minimax 唯一含 outputs"事实，但提出把"输出"重新读作"字节级指纹"——artifact-level fingerprinting，[re-fit hypothesis]。
...
- **枢纽**（`an invited reader team (multi-agent)` 主件 Closing(a)）： `assets/11-12-20-756-asset...C78CFB42E33D:87` S-36 / S-17 / S-37 是"一个问题三张脸"，GLM_1 混淆矩阵是唯一可重算的假阳结构，是 S-37 的具体锚。
...
- **支持用（"作 prior value"）**（`an invited reader team (multi-agent)` 主件 S-02/S-08/S-30 段）： `assets/11-12-20-756-asset...C78CFB42E33D:81` 三风险前置：object mismatch / order isomorphism / tier mismatch，honest residual 是 "curvature invariants as features"。
...
- **三组 × 5 = 15 解读**（`Mavis v1.2`、`trae work v1.2`、`an invited reader team (multi-agent)` 主件、`豆包工作` 主件、`workbuddy part2`）：多源同向，但具体数字 15 一致。
...
- **S-32 反驳**（`an invited reader team (multi-agent)` 主件 S-32/S-35 段）： `assets/11-12-20-756-asset...C78CFB42E33D:79` "bit strings copyable given access；at finite precision 'exact copy' undefined；the residue reduces to cost claims"。
- **S-09 不一致**（`an invited reader team (multi-agent)` 主件 S-09/S-14 段）： `assets/11-12-20-756-asset...C78CFB42E33D:75` on-disk analyst 判 S-09 不产；methodology analyst 判其理论核心 live。文档 surface 这一分歧，未掩盖。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-04+S-01 `assets/11-12-20-756-asset...C78CFB42E33D:33` "S-01 obstacle is access, not physics — weights are bit strings, copyable given access"。[`pure speculation`]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-02/S-08/S-30 段 `assets/11-12-20-756-asset...C78CFB42E33D:81` 见 §2.4。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-04+S-01 `assets/11-12-20-756-asset...C78CFB42E33D:33` "R1-series, arXiv:2501.12948" 把"hardest to copy" 读弱为"hardest to copy *without leaving a trace*"。[`pure speculation`，memory-recalled]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-06 段 `assets/11-12-20-756-asset...C78CFB42E33D:35` "the record is a single-backbone per-cell correctness rate; no refusal-event field appears"。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-09/S-14 段 `assets/11-12-20-756-asset...C78CFB42E33D:75` §2.7 同上。`codex brainstorm` `assets/11-12-20-782-asset...0FF6C042B845:23–25` temperature is service-side, not model-side。
  - `an invited reader team (multi-agent)` addendum `assets/...8295C3201F20:57–62` "frozen decoding zero": `deposon_volcengine_minimax_m3_30cells_2026_09_10.json:6` 顶层 temperature 0.0、max_tokens 1024；minimax 30 cell latency、usage、correctness 在 `9E1CCBDCEACC` L26–48。 [`re-fit hypothesis, not calibrated`]
...
- 参与方点名：
  ...
  - `豆包工作` 主件 S-11 段 `assets/11-12-20-747-asset...19C97B5CFF3C:42` 同向；`an invited reader team (multi-agent)` 主件 S-11 段 `assets/11-12-20-756-asset...C78CFB42E33D:41` S-11 asset's "long-tail-fact consistency" is unsupported by content。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` addendum "Non-stationary decision lines" `assets/...8295C3201F20:63–67` P-E 阈值漂移 0.10 → 0.5 → 0.30 (`063AC8D00542:375`)。[`re-fit hypothesis, not calibrated`]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` addendum "Birthmarks, not watermarks" `assets/...8295C3201F20:69–73` "22-caption fingerprint is a birthmark: re-use is a resilience question, distillation one obfuscation transform, 'stealth watermark' the wrong noun"。[`re-fit hypothesis, not calibrated`]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-13+S-16+S-18 `assets/11-12-20-756-asset...C78CFB42E33D:47` "30 个 0–7 字符 strings 不足以支撑 distributional claim". [critique]
  - `豆包工作 brainstorm 想法 21` `assets/...A18E58C8E2A3:101` "信息论基线指纹"，重用假设但声明未获资产。[`re-fit hypothesis`]
  - `an invited reader team (multi-agent)` addendum "Difficulty-band shape" `assets/...8295C3201F20:51–55` "在已 pin retrieval 的新-30 跑批中，29/30 top-1 retrieval 返回 S2_n35（相似度 0.9999999984–1.0000000092）"。[`re-fit hypothesis, not calibrated`]
...
- 参与方点名：
  ...
  - `豆包工作` 主件 `assets/...19C97B5CFF3C:27` 同向；`codex brainstorm` `assets/...0FF6C042B845:31–33` "Mixture ancestry" — 5 corpus artifacts 不能建立该子问。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-13+S-16+S-18 `assets/...C78CFB42E33D:47` "the actual blank is a distillation-specific null model — a statistic 'almost impossible to produce by independent training' (the seed's words) — restating §4 item 1"。[`pure speculation`]
...
- 参与方点名：
  ...
  - `codex brainstorm` `assets/...0FF6C042B845:31` "Watermark survivability" — re-fit, no calibrated watermark finding claimed。
  - `an invited reader team (multi-agent)` 主件 S-19+S-26 `assets/...C78CFB42E33D:55` "Watermark–quality tension is a studied axis of the Kirchenbauer line; distillation-surviving watermarks exist directly" [memory-recalled]。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-25+S-28 段 `assets/...C78CFB42E33D:61` "S-25's adversary has already escalated once" — Krishna 2023, Sadasivan 2023, Moulin–O'Sullivan 2003。 [memory-recalled]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-19+S-26 `assets/...C78CFB42E33D:57` paraphrase / distortion-free / undetectable 三层擦除。[memory-recalled]
...
- 参与方点名：
  ...
  - `codex brainstorm` `assets/...0FF6C042B845:29` "Query-budget economics" — pure speculation。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-25+S-28 `assets/...C78CFB42E33D:61` Moulin–O'Sullivan 信息隐藏博弈。[memory-recalled]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-31+S-32+S-35 `assets/...C78CFB42E33D:79` "single API calls are stateless — a probe does not change weights"。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-31+S-32+S-35 段 同向。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-31+S-32+S-35 段同向。
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-36 段 `assets/...C78CFB42E33D:71` "Carlini 2022 'Membership Inference Attacks From First Principles'"。[memory-recalled]
...
- 参与方点名：
  ...
  - `an invited reader team (multi-agent)` 主件 S-13+S-16+S-18 `assets/...C78CFB42E33D:47` 自我陈述不构成 GT；与"相信被疑蒸馏模型的自述"同构。[`pure speculation`]
...
- 参与方点名：
  ...
  - `codex brainstorm` `assets/...0FF6C042B845:33–35` "Signature half-life" — re-fit, no calibrated half-life。
  - `an invited reader team (multi-agent)` addendum "Non-stationary decision lines" `assets/...8295C3201F20:63–67` P-E 阈值漂移 0.10 → 0.5 → 0.30 (`063AC8D00542:375`)。[`re-fit hypothesis, not calibrated`]
...
### §4.10 codex（D1 review，778）

`assets/11-12-20-778-asset...48387412B109`：
...
### §4.11 codex（brainstorm，14 条，782）

`assets/11-12-20-782-asset...0FF6C042B845`：
...
### §4.12 an invited reader team (multi-agent)（主件，756）
...
### §4.13 an invited reader team (multi-agent)（addendum，761）
...
| codex（D1 review） | 未给 (a)（自陈 incomplete for execution） | — | `assets/11-12-20-778-asset...48387412B109:9` |
| codex brainstorm | S-03 + S-36 | "what historical claims survive when full responses become extracted answers and then aggregate counts" | `assets/11-12-20-782-asset...0FF6C042B845:38` |
| an invited reader team (multi-agent) 主件 | S-36 / S-17 / S-37 瓶颈分散 | "three faces of one question ... our three analysts each picked one face; we read that as convergence" | `assets/11-12-20-756-asset...C78CFB42E33D:87` |
| an invited reader team (multi-agent) addendum | null-and-controls + measured false-positive structure | "supplying the three pieces our v1.0 Closing said the common-cause separability candidate lacks" | `assets/11-12-20-761-asset...8295C3201F20:89` |
...
| codex brainstorm | S-32 | "an unrestricted digital 'no-cloning' analogy ignores copying of accessible weights and code" | `assets/11-12-20-782-asset...0FF6C042B845:38` |
| an invited reader team (multi-agent) 主件 | S-32 | "two analysts independently found the analogy fails at the mechanism level" | `assets/11-12-20-756-asset...C78CFB42E33D:87` |
| an invited reader team (multi-agent) addendum | — | — | — |
...
| codex brainstorm | "in-context imitation vs parameter transfer" 显式分离 | seed field 当前未区分 | `assets/11-12-20-782-asset...0FF6C042B845:38` |
| an invited reader team (multi-agent) 主件 | "common-cause vs. direct-cause separability" | 三种材料 triangulate：S-35 reduction + S-18 null-model gap + GLM_1 attribution structure | `assets/11-12-20-756-asset...C78CFB42E33D:87` |
| an invited reader team (multi-agent) addendum | "claim form — and its unit (output, datum, artifact byte, population) — before a common-cause explanation is excluded" | — | `assets/11-12-20-761-asset...8295C3201F20:89` |
...
| codex brainstorm | "Which different learning histories remain observationally indistinguishable after the stack's surviving records are fixed?" | — | `assets/11-12-20-782-asset...0FF6C042B845:38` |
| an invited reader team (multi-agent) 主件 | "the referent of 'output-distribution baseline' — four of five `by_model/` slots store in-repo artifact hashes or indices rather than model-output text" | gates reading of §2.1 for S-03/S-07/S-15/S-17 | `assets/11-12-20-756-asset...C78CFB42E33D:87` |
| an invited reader team (multi-agent) addendum | "claim form — and its unit (output, datum, artifact byte, population) — before a common-cause explanation is excluded" | — | `assets/11-12-20-761-asset...8295C3201F20:89` |
...
按派工单 §3 第 7 条禁止擅自归并厂商，§0.3 两件 SHA-12 `C78CFB42E33D` 与 `8295C3201F20` 在磁盘上的署名如下：

- `assets/11-12-20-756-asset...C78CFB42E33D:3` 自陈 `From: an invited reader team (multi-agent), on a new channel per §5.4`。
- `assets/11-12-20-761-asset...8295C3201F20:3` 自陈 `From: the same invited reader team (multi-agent), same channel per §5.4`。

**两条署名的 on-disk 实测如上**；本稿将其并称"an invited reader team (multi-agent)"，在 §1、§3、§4、§5 中均按此署名引用，不并入已知 vendor / brand 链。

### §6.2 各行数冲突

本稿全部 16 个文件的双口径行数核验（`ReadAllLines` vs `Measure-Object -Line`）一律 `ReadAllLines > Measure-Object`（差值约为每文件末尾缺省换行符一行）。本稿引用行号时一律按 `ReadAllLines` 口径，未发现不可调和的冲突。
...
- 无。§0.3 列出的全部 6 件 assets 文件本会话均实读；results/ 下全部 11 件本会话均实读。
- 附件侧 747 / 756 / 761 / 766 / 778 / 782 全部读到 line 91/89/83/130/46/39。
- 附件侧 766 文件末尾 self-truncated line 130 (`assets/11-12-20-766-asset...A18E58C8E2A3:130`) 本会话实读为 line 130；附件 756 line 87 self-truncated (`...C78CFB42E33D:87`) 本会话实读到 line 89 末；本稿如引用 truncated 后内容，已在引文中明指 line 87 / line 130 之前与之后均可核到。
...
- `_v4_distillation_invitation_2026_09_20_v1.0.md` — `3D9F73519F6C` — 411 行 — 53,539 B
  - 代表引用：L26 / L66 / L83 / L96 / L120–125 / L218 / L383 等
...
- `_v4_experiment_invitation_2026_09_20.md` — `4E8C0EA57028` — 436 行 — 30,820 B
  - 仅 §0.1 出现一次（派工单内提供的另一份邀请函，非本文主体引用对象）
...
- `11-12-20-778-asset_20260921-111220-778_48387412b109_7ca46e76-v4_d1_review_codex_2026_09_20.md` — `48387412B109` — 46 行 — 4,506 B
  - 代表引用：L9, L13 (Track 1), L19 (Track 2), L25 (Track 3), L33 (cross-discipline), L39 (D1 action items)
- `11-12-20-782-asset_20260921-111220-782_0ff6c042b845_6d74a672-v4_distillation_brainstorm_reply_codex_2026_09_21.md` — `0FF6C042B845` — 39 行 — 9,048 B
...
### §7.4 其他被引用的非上述文件（仅出现在 §1.8 / §2.10 / §6.4）

- `docs/V3X/KT_B1_SPEC_V0.1.md` — `0410CA0FBDAE` — 629 行 — 35,688 B（仅 §6.4 L267–L274, L157–161 L254–256 引）
- `verifier/handoff/KT_ABC1_anchors_sha256_12.json` — `03C6C01F3697` — 6,680 B（§1.4 / §3.3 S-14 / §6.4）
...
- `docs/V3X/KT_B1_REWORK_REPORT_2026_09_10.md` — `baef94e393de` — 215 行 — 11,478 B（仅 §6.4 L78–86, L179 引）
...
- `results/boss_pa_1_rbr_rm_result_2026_09_15.json` — `C7C59E0D2F6C` — 8,753 B（§1.1 / §1.7 / §2.8 / §6.4）
- `results/d7_5anchor_60cells_9model_verdict_2026_09_18.json` — `4505CCA79C15` — 2,456 B（§3.3 S-14 / §3.6 S-35 / §4.12）
- `results/deposon_v3_v7_summary_2026_09_11.json` — `063AC8D00542` — 25,049 B（§1.4 / §1.5 / §1.13 / §3.5 S-25 / §3.6 S-30 / §3.7 S-38 / §3.7 S-37 / §4.12 / §4.13 / §6.4）
...
- `results/deposon_d_fix2_metric_verify_9m60c_2026_09_15.json` — `9F88A212A77C`（§2.8）
...
- 本任务 0 LLM / 0 API / 0 其他文件改动：本稿是新写一个文件，未触动 §0.1–§0.3 列出的任何 16 个输入文件之外的资产；`D:\私人资料\deposon-repo\` 工作区下其他任何文件均未读取 / 写入 / 修改 / 删除。
- 全部 16 个输入文件的字节数 / SHA-12 / 行数均由本会话内 `[System.IO.File]::ReadAllBytes` + `[System.Security.Cryptography.SHA256]` + `[System.IO.File]::ReadAllLines` 实算；与派工单第 2 节已知锚的差异见 §0.5。
- 全部 line 字段由本会话内 `[System.IO.File]::ReadAllLines(path, [System.Text.Encoding]::UTF8)[line-1]` 实读确认；引用格式 `path:SHA-12:line`。
- §3 中"无参与方点名"的检索：本稿对全部 40 条种子逐条点名核验，未发现完全无点名的种子（见 §3.8）。
- §6.3 列出本稿未在本会话实读 / 实测的引用：无；派工单第 2.3 节列出但本稿未引用的 `773` 附件按派工单指引不重复计。
- 派工单第 5 节所列验收点全部满足（见 §0 + §3 + §7）；派工单第 6 节回报格式见本会话最终回报。
- 中途无 abort / 未完成段落。
