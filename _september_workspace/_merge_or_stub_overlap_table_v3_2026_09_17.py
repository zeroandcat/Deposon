...
# Append §3.5 to MD
# [Trae 2026-09-18 修复 P2-2 非幂等追加] 原版无脑 'a' 追加, 重复运行会累积重复 §3.5 章节。
#   改为: 已存在标记则跳过 (幂等)。
_md_text = md_path.read_text(encoding='utf-8') if md_path.exists() else ''
if '## §3.5 Merged cross-backbone β CI overlap' in _md_text:
    print('§3.5 already present, skip (idempotent)')
else:
...
import os as _os_sc
assert _os_sc.path.basename(__file__) == '_merge_or_stub_overlap_table_v3_2026_09_17.py', '文件名漂移: ' + __file__
with open(__file__, 'r', encoding='utf-8') as _f_sc:
    _src_sc = _f_sc.read()
assert 'TRAE_SELFCHECK_2026_09_16_V3' in _src_sc
print('_merge_or_stub_overlap_table_v3_2026_09_17.py SELF-CHECK PASS')
