#!/usr/bin/env python3
"""verifier/v3 — Deposon v1.5.1 扩散原型验收。
...
# 3. 结果 JSON
d = json.load(open(os.path.join(ROOT, "results/deposon_v15_diffusion.json")))
...
check("A r=0.2 aggregate gold_top3 > random (正增益记录)",
      agg.get("gold_path_top3_hit",{}).get("mean",0) > a.get("arms",{}).get("random",{}).get("gold_path_top3_hit",{}).get("mean",1))
check("honesty 字段非空且含负面声明", len(d.get("honesty", [])) >= 3)
check("逐边明细 49 条", len(b.get("per_edge", [])) == 49)
