# -*- coding: utf-8 -*-
# GT-8c 新真实语义域图摄入（docs/SPEC_GT8C.md §3，Ark 后端）：
#   读 results/gt8c_cache/{domain}.json → parse_familyL_response 校验 →
#   图 JSON 写入 results/gt8c_cache/graphs/L_{domain}.json（独立目录，
#   不污染 corpus/v20，不动 results/gt8b_cache，不进既有语料索引）。
...
from run_v20_gt8c_fetch import (CACHE_DIR, GT8C_DOMAINS,
                                gt8c_prompt_manifest)

HERE = os.path.dirname(os.path.abspath(__file__))
RESULTS = os.path.join(HERE, "results")
GRAPH_DIR = os.path.join(CACHE_DIR, "graphs")
OUT_PATH = os.path.join(RESULTS, "deposon_v20_gt8c_ingest.json")
...
def ingest_domain(domain: str, cache_dir: str = CACHE_DIR,
    ...
    if domain not in GT8C_DOMAINS:
        raise ValueError(f"unknown GT-8c domain: {domain}")
    path = os.path.join(cache_dir, f"{domain}.json")
    if not os.path.exists(path):
        raise CacheMissingError(
            f"GT-8c cache missing: {path} — API 调用由主代理另行执行"
            "（run_v20_gt8c_fetch.py）；no LLM API calls issued by this "
            "script, 绝不伪造数据。")
    ...
    expected_sha = gt8c_prompt_manifest()[domain]["prompt_sha256"]
    ...
    rec = {"graph_id": f"L_{domain}", "family": "L", "real_semantics": 1,
           ...
           "generator_version": "v2.0-gt8c-ingest",
           "provenance": {
               "cache_path": path,
               ...
               "same_source_contamination": (
                   "先验臂与生成臂同后端同模型（Ark/doubao-seed-evolving）"
                   "⇒ 同源污染风险（SPEC_GT8C §6 声明）"),
...
def main():
    ...
    for domain in GT8C_DOMAINS:
        try:
            ...
            errors[domain] = str(e)
    out = {"experiment": "deposon_v20_gt8c_ingest",
           "spec": "docs/SPEC_GT8C.md §3",
           "ingested": ingested, "cache_errors": errors,
           "graph_dir": GRAPH_DIR,
           "honesty": [
               "no LLM API calls issued: 本脚本只读 results/gt8c_cache/；"
               "缓存缺失域显式报错（CacheMissingError），绝不伪造数据。",
               "named/filler 按族 L 冻结口径（DAG 最长路径族）划分；"
               "prompt_sha256 与预登记 manifest 不一致时如实标记，不阻断摄入。",
               "图写入 results/gt8c_cache/graphs/ 独立目录，不污染 corpus/v20，"
               "不动 results/gt8b_cache。"]}
    ...
    if errors and not ingested:
        raise CacheMissingError(
            f"no GT-8c caches ingested: {sorted(errors)}")
